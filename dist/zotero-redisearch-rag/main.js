"use strict";var qe=Object.create;var X=Object.defineProperty;var Me=Object.getOwnPropertyDescriptor;var je=Object.getOwnPropertyNames;var $e=Object.getPrototypeOf,Be=Object.prototype.hasOwnProperty;var Ge=(C,u)=>{for(var e in u)X(C,e,{get:u[e],enumerable:!0})},xe=(C,u,e,n)=>{if(u&&typeof u=="object"||typeof u=="function")for(let t of je(u))!Be.call(C,t)&&t!==e&&X(C,t,{get:()=>u[t],enumerable:!(n=Me(u,t))||n.enumerable});return C};var ee=(C,u,e)=>(e=C!=null?qe($e(C)):{},xe(u||!C||!C.__esModule?X(e,"default",{value:C,enumerable:!0}):e,C)),Ze=C=>xe(X({},"__esModule",{value:!0}),C);var tn={};Ge(tn,{default:()=>se});module.exports=Ze(tn);var p=require("obsidian"),Y=require("@codemirror/state"),F=require("@codemirror/view"),Z=require("child_process"),I=require("fs"),Ee=ee(require("http")),Le=ee(require("https")),pe=ee(require("net")),x=ee(require("path")),H=require("url"),te=require("crypto");var A=require("obsidian"),G=".zotero-redisearch-rag",U=`${G}/items`,$=`${G}/chunks`,we={zoteroBaseUrl:"http://127.0.0.1:23119/api",zoteroUserId:"0",webApiBaseUrl:"https://api.zotero.org",webApiLibraryType:"user",webApiLibraryId:"",webApiKey:"",pythonPath:"python3",dockerPath:"docker",autoStartRedis:!1,copyPdfToVault:!0,frontmatterTemplate:`doc_id: {{doc_id}}
...
item_json: {{item_json_yaml}}`,noteBodyTemplate:"{{pdf_block}}{{docling_markdown}}",tagSanitizeMode:"replace",tagSanitizeReplacement:"-",outputPdfDir:"Zotero/PDFs",outputNoteDir:"Zotero/Notes",chatOutputDir:"Zotero/Chats",chatPaneLocation:"right",redisUrl:"redis://127.0.0.1:6379",autoAssignRedisPort:!0,redisIndex:"idx:zotero",redisPrefix:"zotero:chunk:",embedBaseUrl:"http://localhost:1234/v1",embedApiKey:"lm-studio",embedModel:"google/embedding-gemma-300m",embedIncludeMetadata:!0,enableChunkTagging:!1,chatBaseUrl:"http://127.0.0.1:1234/v1",chatApiKey:"",chatModel:"openai/gpt-oss-20b",chatTemperature:.2,chatHistoryMessages:6,ocrMode:"auto",chunkingMode:"page",maxChunkChars:4e3,chunkOverlapChars:250,ocrQualityThreshold:.5,enableLlmCleanup:!1,llmCleanupBaseUrl:"http://127.0.0.1:1234/v1",llmCleanupApiKey:"",llmCleanupModel:"openai/gpt-oss-20b",llmCleanupTemperature:0,llmCleanupMinQuality:.35,llmCleanupMaxChars:2e3,enableFileLogging:!1,logFilePath:`${G}/logs/docling_extract.log`,createOcrLayeredPdf:!1,preferObsidianNoteForCitations:!0},ne=class extends A.PluginSettingTab{constructor(u,e){super(u,e),this.plugin=e}display(){let{containerEl:u}=this;u.empty(),u.createEl("h2",{text:"Prerequisites"}),new A.Setting(u).setName("Python path").setDesc("Optional path to the Python interpreter used to create or run the plugin env.").addText(d=>d.setPlaceholder("python3").setValue(this.plugin.settings.pythonPath).onChange(async g=>{this.plugin.settings.pythonPath=g.trim(),await this.plugin.saveSettings()})),new A.Setting(u).setName("Python environment").setDesc("Create or update the plugin's Python env (.venv in the plugin folder).").addButton(d=>{d.setButtonText("Create/Update").setCta(),d.onClick(async()=>{d.setDisabled(!0);try{await this.plugin.setupPythonEnv()}finally{d.setDisabled(!1)}})}),new A.Setting(u).setName("Docker/Podman path").setDesc("CLI path for Docker or Podman (used to start Redis Stack).").addText(d=>d.setPlaceholder("docker").setValue(this.plugin.settings.dockerPath).onChange(async g=>{this.plugin.settings.dockerPath=g.trim()||"docker",await this.plugin.saveSettings()})),new A.Setting(u).setName("Redis URL").addText(d=>d.setPlaceholder("redis://127.0.0.1:6379").setValue(this.plugin.settings.redisUrl).onChange(async g=>{this.plugin.settings.redisUrl=g.trim(),await this.plugin.saveSettings()})),new A.Setting(u).setName("Auto-assign Redis port").setDesc("When starting Redis stack, pick a free local port and update the Redis URL.").addToggle(d=>d.setValue(this.plugin.settings.autoAssignRedisPort).onChange(async g=>{this.plugin.settings.autoAssignRedisPort=g,await this.plugin.saveSettings()})),new A.Setting(u).setName("Auto-start Redis stack (Docker/Podman Compose)").setDesc("Requires Docker Desktop running and your vault path shared with Docker.").addToggle(d=>d.setValue(this.plugin.settings.autoStartRedis).onChange(async g=>{this.plugin.settings.autoStartRedis=g,await this.plugin.saveSettings()})),new A.Setting(u).setName("Start Redis stack now").setDesc("Restarts Docker/Podman Compose with the vault data directory.").addButton(d=>d.setButtonText("Start").onClick(async()=>{await this.plugin.startRedisStack()})),u.createEl("h2",{text:"Zotero Local API"}),new A.Setting(u).setName("Zotero base URL").setDesc("Local Zotero API base URL, e.g. http://127.0.0.1:23119/api").addText(d=>d.setPlaceholder("http://127.0.0.1:23119/api").setValue(this.plugin.settings.zoteroBaseUrl).onChange(async g=>{this.plugin.settings.zoteroBaseUrl=g.trim(),await this.plugin.saveSettings()}));let e=new A.Setting(u).setName("Zotero library").setDesc("Select your local library or a Zotero group library."),n=null,t=d=>{if(!n)return;let g=(this.plugin.settings.zoteroUserId||"0").trim()||"0";new Set(d.map(R=>R.value)).has(g)||(d=d.concat([{value:g,label:`Custom (${g})`}])),n.selectEl.options.length=0;for(let R of d)n.addOption(R.value,R.label);n.setValue(g)},i=async()=>{if(n){n.setDisabled(!0);try{let d=await this.plugin.fetchZoteroLibraryOptions();t(d)}finally{n.setDisabled(!1)}}};e.addDropdown(d=>{n=d;let g=(this.plugin.settings.zoteroUserId||"0").trim()||"0";d.addOption(g,"Loading..."),d.setValue(g),d.onChange(async P=>{this.plugin.settings.zoteroUserId=P.trim(),await this.plugin.saveSettings()})}),e.addButton(d=>{d.setButtonText("Refresh").onClick(async()=>{await i()})}),i(),u.createEl("h2",{text:"Zotero Web API"}),new A.Setting(u).setName("Web API base URL").setDesc("Zotero Web API base URL for write fallback, e.g. https://api.zotero.org").addText(d=>d.setPlaceholder("https://api.zotero.org").setValue(this.plugin.settings.webApiBaseUrl).onChange(async g=>{this.plugin.settings.webApiBaseUrl=g.trim(),await this.plugin.saveSettings()})),new A.Setting(u).setName("Web API library type").setDesc("Library type for Web API writes.").addDropdown(d=>d.addOption("user","user").addOption("group","group").setValue(this.plugin.settings.webApiLibraryType).onChange(async g=>{this.plugin.settings.webApiLibraryType=g,await this.plugin.saveSettings()})),new A.Setting(u).setName("Web API library ID").setDesc("Numeric Zotero user/group ID for Web API writes.").addText(d=>d.setPlaceholder("15218").setValue(this.plugin.settings.webApiLibraryId).onChange(async g=>{this.plugin.settings.webApiLibraryId=g.trim(),await this.plugin.saveSettings()})),new A.Setting(u).setName("Web API key").setDesc("Zotero API key for write fallback (from zotero.org).").addText(d=>d.setPlaceholder("your-api-key").setValue(this.plugin.settings.webApiKey).onChange(async g=>{this.plugin.settings.webApiKey=g.trim(),await this.plugin.saveSettings()})),u.createEl("h2",{text:"Output"}),new A.Setting(u).setName("PDF folder").addText(d=>d.setPlaceholder("zotero/pdfs").setValue(this.plugin.settings.outputPdfDir).onChange(async g=>{this.plugin.settings.outputPdfDir=g.trim(),await this.plugin.saveSettings()})),new A.Setting(u).setName("Notes folder").addText(d=>d.setPlaceholder("zotero/notes").setValue(this.plugin.settings.outputNoteDir).onChange(async g=>{this.plugin.settings.outputNoteDir=g.trim(),await this.plugin.saveSettings()})),new A.Setting(u).setName("Frontmatter template").setDesc("Template for note YAML frontmatter. Use {{var}} placeholders; leave blank to omit.").addTextArea(d=>{d.setValue(this.plugin.settings.frontmatterTemplate).onChange(async g=>{this.plugin.settings.frontmatterTemplate=g,await this.plugin.saveSettings()}),d.inputEl.rows=10,d.inputEl.style.width="100%"}),new A.Setting(u).setName("Tag sanitization").setDesc("Normalize Zotero tags for Obsidian (no spaces).").addDropdown(d=>d.addOption("none","No change").addOption("replace","Replace spaces").addOption("camel","camelCase").setValue(this.plugin.settings.tagSanitizeMode).onChange(async g=>{this.plugin.settings.tagSanitizeMode=g,await this.plugin.saveSettings()})).addText(d=>d.setPlaceholder("-").setValue(this.plugin.settings.tagSanitizeReplacement).onChange(async g=>{this.plugin.settings.tagSanitizeReplacement=g.trim()||"-",await this.plugin.saveSettings()})),new A.Setting(u).setName("Note body template").setDesc("Template for the note body after frontmatter. Use {{pdf_block}} and {{docling_markdown}} placeholders.").addTextArea(d=>{d.setValue(this.plugin.settings.noteBodyTemplate).onChange(async g=>{this.plugin.settings.noteBodyTemplate=g,await this.plugin.saveSettings()}),d.inputEl.rows=8,d.inputEl.style.width="100%"}),new A.Setting(u).setName("Saved chats folder").setDesc("Where exported chat notes are stored (vault-relative).").addText(d=>d.setPlaceholder("zotero/chats").setValue(this.plugin.settings.chatOutputDir).onChange(async g=>{this.plugin.settings.chatOutputDir=g.trim()||"zotero/chats",await this.plugin.saveSettings()})),new A.Setting(u).setName("Copy PDFs into vault").setDesc("Disable to use Zotero storage paths directly. If a local file path is unavailable, the plugin temporarily copies the PDF into the vault for processing.").addToggle(d=>d.setValue(this.plugin.settings.copyPdfToVault).onChange(async g=>{this.plugin.settings.copyPdfToVault=g,!g&&this.plugin.settings.createOcrLayeredPdf&&(this.plugin.settings.createOcrLayeredPdf=!1),await this.plugin.saveSettings(),this.display()})),new A.Setting(u).setName("Create OCR-layered PDF copy").setDesc("When OCR is used, replace the vault PDF with a Tesseract text layer (requires Copy PDFs into vault).").addToggle(d=>{let g=this.plugin.settings.copyPdfToVault;d.setValue(g?this.plugin.settings.createOcrLayeredPdf:!1).setDisabled(!g).onChange(async P=>{if(!this.plugin.settings.copyPdfToVault){this.plugin.settings.createOcrLayeredPdf=!1,await this.plugin.saveSettings();return}this.plugin.settings.createOcrLayeredPdf=P,await this.plugin.saveSettings()})}),new A.Setting(u).setName("Prefer Obsidian note for citations").setDesc("Link citations to the Obsidian note when available; otherwise use Zotero deep links.").addToggle(d=>d.setValue(this.plugin.settings.preferObsidianNoteForCitations).onChange(async g=>{this.plugin.settings.preferObsidianNoteForCitations=g,await this.plugin.saveSettings()})),u.createEl("h2",{text:"Docling"}),new A.Setting(u).setName("OCR mode").setDesc("auto: skip OCR when text is readable; force if bad: OCR only when text looks poor; force: always OCR.").addDropdown(d=>d.addOption("auto","auto").addOption("force_low_quality","force if quality is bad").addOption("force","force").setValue(this.plugin.settings.ocrMode).onChange(async g=>{this.plugin.settings.ocrMode=g,await this.plugin.saveSettings()})),new A.Setting(u).setName("Text quality threshold").setDesc("Lower values are stricter; below this threshold the text is treated as low quality.").addSlider(d=>{d.setLimits(0,1,.05).setValue(this.plugin.settings.ocrQualityThreshold).setDynamicTooltip().onChange(async g=>{this.plugin.settings.ocrQualityThreshold=g,await this.plugin.saveSettings()})}),new A.Setting(u).setName("Chunking").setDesc("page or section").addDropdown(d=>d.addOption("page","page").addOption("section","section").setValue(this.plugin.settings.chunkingMode).onChange(async g=>{this.plugin.settings.chunkingMode=g,await this.plugin.saveSettings()})),new A.Setting(u).setName("Section chunk max chars").setDesc("Split large section chunks into smaller pieces (section mode only).").addText(d=>d.setPlaceholder("3000").setValue(String(this.plugin.settings.maxChunkChars)).onChange(async g=>{let P=Number.parseInt(g,10);this.plugin.settings.maxChunkChars=Number.isFinite(P)?P:3e3,await this.plugin.saveSettings()})),new A.Setting(u).setName("Section chunk overlap chars").setDesc("Number of characters to overlap when splitting section chunks.").addText(d=>d.setPlaceholder("250").setValue(String(this.plugin.settings.chunkOverlapChars)).onChange(async g=>{let P=Number.parseInt(g,10);this.plugin.settings.chunkOverlapChars=Number.isFinite(P)?P:250,await this.plugin.saveSettings()})),u.createEl("h2",{text:"Automatic OCR cleanup"}),new A.Setting(u).setName("LLM cleanup for low-quality chunks").setDesc("Optional OpenAI-compatible cleanup for poor OCR. Can be slow/costly.").addToggle(d=>d.setValue(this.plugin.settings.enableLlmCleanup).onChange(async g=>{this.plugin.settings.enableLlmCleanup=g,await this.plugin.saveSettings()}));let r=[{value:"http://localhost:1234/v1",label:"LM Studio (http://localhost:1234/v1)"},{value:"http://localhost:11434/v1",label:"Ollama (http://localhost:11434/v1)"},{value:"https://openrouter.ai/api/v1",label:"OpenRouter (https://openrouter.ai/api/v1)"},{value:"https://api.openai.com/v1",label:"OpenAI (https://api.openai.com/v1)"}],s=null,a=async d=>{let g=d.trim();this.plugin.settings.llmCleanupBaseUrl=g,s&&s.setValue(g),await this.plugin.saveSettings()};new A.Setting(u).setName("LLM cleanup provider preset").setDesc("Pick a provider URL or keep a custom value.").addDropdown(d=>{let g=(this.plugin.settings.llmCleanupBaseUrl||"").trim(),P=[...r];g&&!P.some(R=>R.value===g)&&P.push({value:g,label:`Custom (${g})`});for(let R of P)d.addOption(R.value,R.label);d.setValue(g||r[0].value),d.onChange(async R=>{await a(R)})}),new A.Setting(u).setName("LLM cleanup base URL").setDesc("OpenAI-compatible endpoint, e.g. http://127.0.0.1:1234/v1").addText(d=>{s=d,d.setPlaceholder("http://127.0.0.1:1234/v1").setValue(this.plugin.settings.llmCleanupBaseUrl).onChange(async g=>{await a(g)})}),new A.Setting(u).setName("LLM cleanup API key").setDesc("Optional API key for the cleanup endpoint.").addText(d=>d.setPlaceholder("sk-...").setValue(this.plugin.settings.llmCleanupApiKey).onChange(async g=>{this.plugin.settings.llmCleanupApiKey=g.trim(),await this.plugin.saveSettings()}));let l=new A.Setting(u).setName("LLM cleanup model").setDesc("Select a cleanup-capable model from the provider."),c=null,h=d=>{if(!c)return;let g=(this.plugin.settings.llmCleanupModel||"").trim(),P=new Set(d.map(R=>R.value));g&&!P.has(g)&&(d=d.concat([{value:g,label:`Custom (${g})`}])),c.selectEl.options.length=0;for(let R of d)c.addOption(R.value,R.label);g&&c.setValue(g)},_=async()=>{if(c){c.setDisabled(!0);try{let d=await this.plugin.fetchCleanupModelOptions();h(d)}finally{c.setDisabled(!1)}}};l.addDropdown(d=>{c=d;let g=(this.plugin.settings.llmCleanupModel||"").trim();d.addOption(g||"loading","Loading..."),d.setValue(g||"loading"),d.onChange(async P=>{this.plugin.settings.llmCleanupModel=P.trim(),await this.plugin.saveSettings()})}),l.addButton(d=>{d.setButtonText("Refresh").onClick(async()=>{await _()})}),_(),new A.Setting(u).setName("LLM cleanup temperature").setDesc("Lower is more conservative.").addText(d=>d.setPlaceholder("0.0").setValue(String(this.plugin.settings.llmCleanupTemperature)).onChange(async g=>{let P=Number.parseFloat(g);this.plugin.settings.llmCleanupTemperature=Number.isFinite(P)?P:0,await this.plugin.saveSettings()})),new A.Setting(u).setName("LLM cleanup min quality").setDesc("Only run cleanup when chunk quality is below this threshold (0-1).").addSlider(d=>d.setLimits(0,1,.05).setValue(this.plugin.settings.llmCleanupMinQuality).setDynamicTooltip().onChange(async g=>{this.plugin.settings.llmCleanupMinQuality=g,await this.plugin.saveSettings()})),new A.Setting(u).setName("LLM cleanup max chars").setDesc("Skip cleanup for chunks longer than this limit.").addText(d=>d.setPlaceholder("2000").setValue(String(this.plugin.settings.llmCleanupMaxChars)).onChange(async g=>{let P=Number.parseInt(g,10);this.plugin.settings.llmCleanupMaxChars=Number.isFinite(P)?P:2e3,await this.plugin.saveSettings()})),u.createEl("h2",{text:"Text Embedding"});let f=[{value:"http://localhost:1234/v1",label:"LM Studio (http://localhost:1234/v1)"},{value:"http://localhost:11434/v1",label:"Ollama (http://localhost:11434/v1)"},{value:"https://openrouter.ai/api/v1",label:"OpenRouter (https://openrouter.ai/api/v1)"},{value:"https://api.openai.com/v1",label:"OpenAI (https://api.openai.com/v1)"}],y=null,m=async d=>{let g=d.trim();this.plugin.settings.embedBaseUrl=g,y&&y.setValue(g),await this.plugin.saveSettings()};new A.Setting(u).setName("Embeddings provider preset").setDesc("Pick a provider URL or keep a custom value.").addDropdown(d=>{let g=(this.plugin.settings.embedBaseUrl||"").trim(),P=[...f];g&&!P.some(R=>R.value===g)&&P.push({value:g,label:`Custom (${g})`});for(let R of P)d.addOption(R.value,R.label);d.setValue(g||f[0].value),d.onChange(async R=>{await m(R)})}),new A.Setting(u).setName("Embeddings base URL").addText(d=>{y=d,d.setPlaceholder("http://localhost:1234/v1").setValue(this.plugin.settings.embedBaseUrl).onChange(async g=>{await m(g)})}),new A.Setting(u).setName("Embeddings API key").addText(d=>d.setPlaceholder("lm-studio").setValue(this.plugin.settings.embedApiKey).onChange(async g=>{this.plugin.settings.embedApiKey=g.trim(),await this.plugin.saveSettings()}));let w=new A.Setting(u).setName("Embeddings model").setDesc("Select an embeddings model from the provider."),k=null,S=d=>{if(!k)return;let g=(this.plugin.settings.embedModel||"").trim(),P=new Set(d.map(R=>R.value));g&&!P.has(g)&&(d=d.concat([{value:g,label:`Custom (${g})`}])),k.selectEl.options.length=0;for(let R of d)k.addOption(R.value,R.label);g&&k.setValue(g)},v=async()=>{if(k){k.setDisabled(!0);try{let d=await this.plugin.fetchEmbeddingModelOptions();S(d)}finally{k.setDisabled(!1)}}};w.addDropdown(d=>{k=d;let g=(this.plugin.settings.embedModel||"").trim();d.addOption(g||"loading","Loading..."),d.setValue(g||"loading"),d.onChange(async P=>{this.plugin.settings.embedModel=P.trim(),await this.plugin.saveSettings()})}),w.addButton(d=>{d.setButtonText("Refresh").onClick(async()=>{await v()})}),v(),new A.Setting(u).setName("Include metadata in embeddings").setDesc("Prepend title/authors/tags/section info before embedding chunks.").addToggle(d=>d.setValue(this.plugin.settings.embedIncludeMetadata).onChange(async g=>{this.plugin.settings.embedIncludeMetadata=g,await this.plugin.saveSettings()})),new A.Setting(u).setName("Generate LLM tags for chunks").setDesc("Use the OCR cleanup model to tag chunks before indexing.").addToggle(d=>d.setValue(this.plugin.settings.enableChunkTagging).onChange(async g=>{this.plugin.settings.enableChunkTagging=g,await this.plugin.saveSettings()})),u.createEl("h2",{text:"Chat LLM"});let L=[{value:"http://localhost:1234/v1",label:"LM Studio (http://localhost:1234/v1)"},{value:"http://localhost:11434/v1",label:"Ollama (http://localhost:11434/v1)"},{value:"https://openrouter.ai/api/v1",label:"OpenRouter (https://openrouter.ai/api/v1)"},{value:"https://api.openai.com/v1",label:"OpenAI (https://api.openai.com/v1)"}],D=null,T=async d=>{let g=d.trim();this.plugin.settings.chatBaseUrl=g,D&&D.setValue(g),await this.plugin.saveSettings()};new A.Setting(u).setName("Chat provider preset").setDesc("Pick a provider URL or keep a custom value.").addDropdown(d=>{let g=(this.plugin.settings.chatBaseUrl||"").trim(),P=[...L];g&&!P.some(R=>R.value===g)&&P.push({value:g,label:`Custom (${g})`});for(let R of P)d.addOption(R.value,R.label);d.setValue(g||L[0].value),d.onChange(async R=>{await T(R)})}),new A.Setting(u).setName("Chat base URL").setDesc("OpenAI-compatible base URL for chat requests.").addText(d=>{D=d,d.setPlaceholder("http://localhost:1234/v1").setValue(this.plugin.settings.chatBaseUrl).onChange(async g=>{await T(g)})}),new A.Setting(u).setName("Chat API key").addText(d=>d.setPlaceholder("lm-studio").setValue(this.plugin.settings.chatApiKey).onChange(async g=>{this.plugin.settings.chatApiKey=g.trim(),await this.plugin.saveSettings()}));let E=new A.Setting(u).setName("Chat model").setDesc("Select a chat-capable model from the provider."),N=null,M=d=>{if(!N)return;let g=(this.plugin.settings.chatModel||"").trim(),P=new Set(d.map(R=>R.value));g&&!P.has(g)&&(d=d.concat([{value:g,label:`Custom (${g})`}])),N.selectEl.options.length=0;for(let R of d)N.addOption(R.value,R.label);g&&N.setValue(g)},j=async()=>{if(N){N.setDisabled(!0);try{let d=await this.plugin.fetchChatModelOptions();M(d)}finally{N.setDisabled(!1)}}};E.addDropdown(d=>{N=d;let g=(this.plugin.settings.chatModel||"").trim();d.addOption(g||"loading","Loading..."),d.setValue(g||"loading"),d.onChange(async P=>{this.plugin.settings.chatModel=P.trim(),await this.plugin.saveSettings()})}),E.addButton(d=>{d.setButtonText("Refresh").onClick(async()=>{await j()})}),j(),new A.Setting(u).setName("Temperature").addText(d=>d.setPlaceholder("0.2").setValue(String(this.plugin.settings.chatTemperature)).onChange(async g=>{let P=Number.parseFloat(g);this.plugin.settings.chatTemperature=Number.isFinite(P)?P:.2,await this.plugin.saveSettings()})),new A.Setting(u).setName("Chat history messages").setDesc("Number of recent messages to include for conversational continuity (0 disables).").addText(d=>d.setPlaceholder("6").setValue(String(this.plugin.settings.chatHistoryMessages)).onChange(async g=>{let P=Number.parseInt(g,10);this.plugin.settings.chatHistoryMessages=Number.isFinite(P)?Math.max(0,P):6,await this.plugin.saveSettings()})),new A.Setting(u).setName("Chat panel location").setDesc("Where to open the chat view by default.").addDropdown(d=>d.addOption("right","Right sidebar").addOption("main","Main window").setValue(this.plugin.settings.chatPaneLocation).onChange(async g=>{this.plugin.settings.chatPaneLocation=g,await this.plugin.saveSettings()})),u.createEl("h2",{text:"Logging"}),new A.Setting(u).setName("Enable logging to file").setDesc("Write Docling/Python logs to a file during extraction.").addToggle(d=>d.setValue(this.plugin.settings.enableFileLogging).onChange(async g=>{this.plugin.settings.enableFileLogging=g,await this.plugin.saveSettings()})),new A.Setting(u).setName("Log file path (vault-relative)").setDesc("Where to write logs. Keep inside the vault.").addText(d=>d.setPlaceholder(`${G}/logs/docling_extract.log`).setValue(this.plugin.settings.logFilePath).onChange(async g=>{this.plugin.settings.logFilePath=g.trim()||`${G}/logs/docling_extract.log`,await this.plugin.saveSettings()})),new A.Setting(u).setName("View or clear log").setDesc("Open the log contents or clear the file.").addButton(d=>d.setButtonText("Open log").onClick(async()=>{var g,P;await((P=(g=this.plugin).openLogFile)==null?void 0:P.call(g))})).addButton(d=>d.setButtonText("Clear log").onClick(async()=>{var g,P;await((P=(g=this.plugin).clearLogFile)==null?void 0:P.call(g))})),u.createEl("h2",{text:"Maintenance"}),new A.Setting(u).setName("Reindex Redis from cached chunks").setDesc("Rebuild the Redis index from cached chunk JSON files.").addButton(d=>d.setButtonText("Reindex").onClick(async()=>{await this.plugin.reindexRedisFromCache()})),new A.Setting(u).setName("Recreate missing notes from cache").setDesc("Rebuild missing notes using cached Zotero items and chunks.").addButton(d=>d.setButtonText("Recreate").onClick(async()=>{await this.plugin.recreateMissingNotesFromCache()})).addButton(d=>d.setButtonText("Cancel").onClick(()=>{this.plugin.cancelRecreateMissingNotesFromCache()}))}};var le={"zrr-picker":`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
...
`).replace(/\s+/g," ").trim()}scheduleNoteSync(e){let n=this.noteSyncTimers.get(e.path);n!==void 0&&window.clearTimeout(n);let t=window.setTimeout(()=>{this.noteSyncTimers.delete(e.path),this.syncNoteToRedis(e)},1200);this.noteSyncTimers.set(e.path,t)}escapeRegExp(e){return e.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")}formatCitationPageLabel(e){let n=e.page_start?String(e.page_start):"",t=e.page_end?String(e.page_end):"";if(n&&(!t||n===t))return n;if(n&&t)return`${n} - ${t}`;let i=(e.pages||"").trim();if(!i)return"?";let r=i.match(/^(\d+)\s*-\s*(\d+)$/);return r?r[1]===r[2]?r[1]:`${r[1]} - ${r[2]}`:i.replace("-"," - ")}normalizeChunkIdForNote(e,n){if(!e)return null;let t=String(e);if(n&&t.startsWith(`${n}:`))return t.slice(n.length+1);if(t.includes(":")){let i=t.split(":");if(i.length>1&&n&&i[0]===n)return i.slice(1).join(":")}return t}async syncNoteToRedis(e){var n,t,i,r;if(!this.noteSyncInFlight.has(e.path)&&!this.noteSyncSuppressed.has(e.path)){this.noteSyncInFlight.add(e.path);try{let s=await this.app.vault.read(e),a=this.extractSyncSection(s);if(!a)return;let o=(n=this.extractDocIdFromFrontmatter(s))!=null?n:this.extractDocIdFromSyncMarker(s);if(!o)return;let l=this.parseSyncedChunkBlocks(a);if(!l.length)return;let c=(0,p.normalizePath)(`${$}/${o}.json`),h=this.app.vault.adapter;if(!await h.exists(c))return;let _=await this.readChunkPayload(c);if(!_)return;let f=Array.isArray(_.chunks)?_.chunks:[],y=new Map;for(let v of f){let L=typeof(v==null?void 0:v.chunk_id)=="string"?v.chunk_id:String((t=v==null?void 0:v.chunk_id)!=null?t:"");L&&y.set(L,v)}let m=new Set,b=new Set,w=new Set,k=new Set,S=!1;for(let v of l){let L=v.chunkId;if(!L)continue;m.add(L);let D=y.get(L);if(!D){console.warn(`Sync note: chunk id not found in cache (${L})`);continue}if(v.excludeFlag){D.excluded!==!0&&(D.excluded=!0,S=!0);let E=this.normalizeChunkText(v.text);E&&E!==String((i=D.text)!=null?i:"")&&(D.text=E,D.char_count=E.length,S=!0),w.add(L);continue}if(D.excluded&&(D.excluded=!1,S=!0,b.add(L)),!v.text.trim()){w.add(L),k.add(L);continue}let T=this.normalizeChunkText(v.text);if(!T){w.add(L),k.add(L);continue}let z=typeof D.text=="string"?D.text:String((r=D.text)!=null?r:"");T!==z&&(D.text=T,D.char_count=T.length,b.add(L),S=!0)}for(let v of y.keys())m.has(v)||(w.add(v),k.add(v));if(!b.size&&!w.size&&!k.size&&!S)return;k.size&&(_.chunks=f.filter(v=>{var D;let L=typeof(v==null?void 0:v.chunk_id)=="string"?v.chunk_id:String((D=v==null?void 0:v.chunk_id)!=null?D:"");return L&&!k.has(L)}),S=!0),(S||k.size)&&await h.write(c,JSON.stringify(_,null,2)),await this.reindexChunkUpdates(o,c,Array.from(b),Array.from(w))}catch(s){console.warn("Failed to sync note edits to Redis",s)}finally{this.noteSyncInFlight.delete(e.path)}}}extractSyncSection(e){let n=K.exec(e);if(!n)return null;let t=e.slice(n.index+n[0].length),i=J.exec(t);return i?t.slice(0,i.index):null}extractDocIdFromSyncMarker(e){var r;let n=K.exec(e);if(!n)return null;let i=((r=n[0])!=null?r:"").match(/doc_id=([\"']?)([^\"'\s]+)\1/i);return i?i[2].trim():null}parseSyncedChunkBlocks(e){var o;let n=e.split(/\r?\n/),t=[],i="",r=!1,s=[],a=()=>{i&&(t.push({chunkId:i,text:s.join(`
`).trim(),excludeFlag:r}),i="",r=!1,s=[])};for(let l of n){let c=l.match(Q);if(c){a();let h=(o=c[1])!=null?o:"",_=h.match(/id=([\"']?)([^\"'\s]+)\1/i),f=_?_[2].trim():"";if(!f)continue;i=f,r=/\bexclude\b/i.test(h)||/\bdelete\b/i.test(h),s=[];continue}if(ue.test(l)){a();continue}if(i){if(ie.test(l)){r=!0;continue}s.push(l)}}return a(),t}normalizeChunkText(e){return e.split(/\r?\n/).map(n=>n.replace(/\s+/g," ").trim()).filter((n,t,i)=>!(n===""&&i[t-1]==="")).join(`

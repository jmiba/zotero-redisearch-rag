...
`);for(let g of d){let y=String(g.doc_id||"").trim(),k=String(g.chunk_id||"").trim(),x=String(g.page_start||"").trim(),h=String(g.page_end||"").trim(),b=String(g.title||"").trim(),w=String(g.section||"").trim(),v=String(g.score||"").trim(),R=String(g.authors||"").trim(),P=String(g.item_type||"").trim(),E=String(g.year||"").trim(),D=String(g.tags||"").trim(),L=String(g.chunk_tags||"").trim(),q=String(g.attachment_key||"").trim(),F=String(g.source_pdf||"").trim(),W=String(g.text||"").replace(/\s+/g," ").trim(),M=W.length>220?`${W.slice(0,220)}\u2026`:W,G=[y];k&&G.push(k),(x||h)&&G.push(`p.${x||"?"}-${h||"?"}`),p.push(G.filter(Boolean).join(" \u2022 ")),v&&p.push(`  score: ${v}`),b&&p.push(`  title: ${b}`),R&&p.push(`  authors: ${R}`),E&&p.push(`  year: ${E}`),P&&p.push(`  item_type: ${P}`),D&&p.push(`  tags: ${D}`),L&&p.push(`  chunk_tags: ${L}`),q&&p.push(`  attachment_key: ${q}`),w&&p.push(`  section: ${w}`),F&&p.push(`  source_pdf: ${F}`),M&&p.push(`  ${M}`),p.push("")}return p.join(`

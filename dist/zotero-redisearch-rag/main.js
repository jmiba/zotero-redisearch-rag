"use strict";var ln=Object.create;var be=Object.defineProperty;var dn=Object.getOwnPropertyDescriptor;var cn=Object.getOwnPropertyNames;var pn=Object.getPrototypeOf,un=Object.prototype.hasOwnProperty;var _n=(k,m)=>{for(var e in m)be(k,e,{get:m[e],enumerable:!0})},Ze=(k,m,e,n)=>{if(m&&typeof m=="object"||typeof m=="function")for(let t of cn(m))!un.call(k,t)&&t!==e&&be(k,t,{get:()=>m[t],enumerable:!(n=dn(m,t))||n.enumerable});return k};var le=(k,m,e)=>(e=k!=null?ln(pn(k)):{},Ze(m||!k||!k.__esModule?be(e,"default",{value:k,enumerable:!0}):e,k)),gn=k=>Ze(be({},"__esModule",{value:!0}),k);var Sn={};_n(Sn,{default:()=>Le});module.exports=gn(Sn);var u=require("obsidian"),ee=require("@codemirror/state"),M=require("@codemirror/view"),X=require("child_process"),G=require("fs"),en=le(require("http")),nn=le(require("https")),ge=le(require("net")),tn=le(require("os")),rn=le(require("tls")),L=le(require("path")),ue=require("url"),ve=require("crypto");var N=require("obsidian"),K=".zotero-redisearch-rag",J=`${K}/items`,V=`${K}/chunks`,He={pythonPath:"",pythonEnvLocation:"shared",dockerPath:"docker",redisUrl:"redis://127.0.0.1:6379",autoAssignRedisPort:!1,autoStartRedis:!0,zoteroBaseUrl:"http://127.0.0.1:23119/api",zoteroUserId:"0",webApiBaseUrl:"https://api.zotero.org",webApiLibraryType:"user",webApiLibraryId:"",webApiKey:"",outputPdfDir:"Zotero/PDFs",outputNoteDir:"Zotero/Notes",frontmatterTemplate:`doc_id: {{doc_id}}
...
`);for(let p of d){let g=String(p.doc_id||"").trim(),y=String(p.chunk_id||"").trim(),h=String(p.page_start||"").trim(),C=String(p.page_end||"").trim(),_=String(p.title||"").trim(),f=String(p.section||"").trim(),x=String(p.score||"").trim(),P=String(p.authors||"").trim(),S=String(p.item_type||"").trim(),E=String(p.year||"").trim(),w=String(p.tags||"").trim(),R=String(p.chunk_tags||"").trim(),I=String(p.attachment_key||"").trim(),j=String(p.source_pdf||"").trim(),D=String(p.text||"").replace(/\s+/g," ").trim(),q=D.length>220?`${D.slice(0,220)}\u2026`:D,T=[g];y&&T.push(y),(h||C)&&T.push(`p.${h||"?"}-${C||"?"}`),c.push(T.filter(Boolean).join(" \u2022 ")),x&&c.push(`  score: ${x}`),_&&c.push(`  title: ${_}`),P&&c.push(`  authors: ${P}`),E&&c.push(`  year: ${E}`),S&&c.push(`  item_type: ${S}`),w&&c.push(`  tags: ${w}`),R&&c.push(`  chunk_tags: ${R}`),I&&c.push(`  attachment_key: ${I}`),f&&c.push(`  section: ${f}`),j&&c.push(`  source_pdf: ${j}`),q&&c.push(`  ${q}`),c.push("")}return c.join(`

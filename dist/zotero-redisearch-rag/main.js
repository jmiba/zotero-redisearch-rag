...
item_json: {{item_json_yaml}}`,tagSanitizeMode:"kebab",noteBodyTemplate:"{{pdf_block}}{{docling_markdown}}",annotationPageLabel:"Seite",annotationColorMap:{yellow:{heading:"Fragen",callout:"question"},red:{heading:"Kritikw\xFCrdiges",callout:"bug"},green:{heading:"Hauptgedanken",callout:"idea"},blue:{heading:"Fakten",callout:"fact"},purple:{heading:"Argumente/L\xF6sungen",callout:"argument"},magenta:{heading:"Meinungen",callout:"opinion"},orange:{heading:"Weiterverfolgen",callout:"pursue"},gray:{heading:"Zitierbare Stellen",callout:"cite"}},includeAnnotationImages:!0,zoteroCompanionEnabled:!1,zoteroCompanionBaseUrl:"http://127.0.0.1:23120",zoteroCompanionToken:"",llmProviderProfiles:[{id:"lm-studio",name:"LM Studio",baseUrl:"http://localhost:1234/v1",apiKey:"lm-studio"},{id:"ollama",name:"Ollama",baseUrl:"http://localhost:11434/v1",apiKey:""},{id:"openrouter",name:"OpenRouter",baseUrl:"https://openrouter.ai/api/v1",apiKey:""},{id:"openai",name:"OpenAI",baseUrl:"https://api.openai.com/v1",apiKey:""}],chatOutputDir:"Zotero/Chats",copyPdfToVault:!0,createOcrLayeredPdf:!1,preferObsidianNoteForCitations:!0,ocrMode:"auto",ocrQualityThreshold:.5,chunkingMode:"page",ocrEngine:"auto",forcePerPageOcr:!1,paddleApiKey:"",paddleVlApiUrl:"",paddleStructureApiUrl:"",enableLlmCleanup:!1,llmCleanupProviderProfileId:"lm-studio",llmCleanupBaseUrl:"http://127.0.0.1:1234/v1",llmCleanupApiKey:"",llmCleanupModel:"openai/gpt-oss-20b",llmCleanupTemperature:0,llmCleanupMinQuality:.35,llmCleanupMaxChars:2e3,embedProviderProfileId:"lm-studio",embedBaseUrl:"http://localhost:1234/v1",embedApiKey:"lm-studio",embedModel:"google/embedding-gemma-300m",embedIncludeMetadata:!0,embedSubchunkChars:3500,embedSubchunkOverlap:200,embedContextWindow:1,embedContextChars:220,enableChunkTagging:!1,chatProviderProfileId:"lm-studio",chatBaseUrl:"http://127.0.0.1:1234/v1",chatApiKey:"",chatModel:"openai/gpt-oss-20b",chatTemperature:.2,chatHistoryMessages:6,chatPaneLocation:"right",enableQueryExpansion:!1,queryExpansionCount:3,enableCrossEncoderRerank:!1,rerankModel:"BAAI/bge-reranker-v2-m3",rerankCandidateMultiplier:4,rrfK:60,rrfLogTop:0,maxChunksPerDoc:0,enableFileLogging:!1,logFilePath:`${ee}/logs/docling_extract.log`,redisIndex:"idx:zotero",redisPrefix:"zotero:chunk:"},Oe=class extends R.PluginSettingTab{constructor(e,n){super(e,n);this.activeTab="prerequisites";this.companionTokenInput=null;this.plugin=n}display(){let{containerEl:e}=this;e.empty();let n=()=>Array.isArray(this.plugin.settings.llmProviderProfiles)?this.plugin.settings.llmProviderProfiles:[],t=async f=>{this.plugin.settings.llmProviderProfiles=f,await this.plugin.saveSettings()},i=f=>{f.inputEl.type="password",f.inputEl.autocomplete="off",f.inputEl.spellcheck=!1},d=[{id:"prerequisites",label:"Prerequisites",render:f=>{f.createEl("h2",{text:"Prerequisites"}),new R.Setting(f).setName("Python path").setDesc("Optional path to the Python interpreter used to create or run the plugin env. Leave blank to auto-detect (python3.13/3.12/3.11/3.10/python3/python, or py on Windows). Supports ~, $HOME, and %USERPROFILE%. Relative paths with separators resolve from your home dir.").addText(x=>x.setPlaceholder("auto-detect").setValue(this.plugin.settings.pythonPath).onChange(async b=>{this.plugin.settings.pythonPath=b.trim(),await this.plugin.saveSettings()})),new R.Setting(f).setName("Python environment").setDesc("Create or update the plugin's Python env (location configured below).").addButton(x=>{x.setButtonText("Create/Update").setCta(),x.onClick(async()=>{x.setDisabled(!0);try{await this.plugin.setupPythonEnv()}finally{x.setDisabled(!1)}})}),new R.Setting(f).setName("Python env location").setDesc("Shared user cache can be reused across vaults; plugin folder keeps a per-vault env.").addDropdown(x=>{x.addOption("shared","Shared user cache"),x.addOption("plugin","Plugin folder (.venv)"),x.setValue(this.plugin.settings.pythonEnvLocation).onChange(async b=>{b!=="shared"&&b!=="plugin"||(this.plugin.settings.pythonEnvLocation=b,await this.plugin.saveSettings())})}),new R.Setting(f).setName("Docker/Podman path").setDesc("CLI path for Docker or Podman (used to start Redis Stack). Leave as 'docker'/'podman' to auto-detect via PATH and common locations without saving an absolute path (keeps cross-OS sync). Supports ~, $HOME, and %USERPROFILE%. Relative paths with separators resolve from your home dir.").addText(x=>x.setPlaceholder("docker").setValue(this.plugin.settings.dockerPath).onChange(async b=>{this.plugin.settings.dockerPath=b.trim()||"docker",await this.plugin.saveSettings()})),new R.Setting(f).setName("Redis URL").addText(x=>x.setPlaceholder("redis://127.0.0.1:6379").setValue(this.plugin.settings.redisUrl).onChange(async b=>{this.plugin.settings.redisUrl=b.trim(),await this.plugin.saveSettings()})),new R.Setting(f).setName("Redis data directory override").setDesc("Optional path to store Redis persistence when auto-assign is off. Env var ZRR_DATA_DIR overrides this setting. Supports ~, $HOME, and %USERPROFILE%. Relative paths with separators resolve from your home dir; use ./ to keep it vault-relative.").addText(x=>x.setPlaceholder("~/Redis/zrr-data").setValue(this.plugin.settings.redisDataDirOverride).onChange(async b=>{this.plugin.settings.redisDataDirOverride=b.trim(),await this.plugin.saveSettings()})),new R.Setting(f).setName("Redis project name override").setDesc("Optional Docker/Podman Compose project name when auto-assign is off. Env var ZRR_PROJECT_NAME overrides this setting.").addText(x=>x.setPlaceholder("zrr-shared").setValue(this.plugin.settings.redisProjectName).onChange(async b=>{this.plugin.settings.redisProjectName=b.trim(),await this.plugin.saveSettings()})),new R.Setting(f).setName("Auto-assign Redis port").setDesc("When starting Redis stack, pick a free local port and update the Redis URL.").addToggle(x=>x.setValue(this.plugin.settings.autoAssignRedisPort).onChange(async b=>{this.plugin.settings.autoAssignRedisPort=b,await this.plugin.saveSettings()})),new R.Setting(f).setName("Auto-start Redis stack (Docker/Podman Compose)").setDesc("Requires Docker Desktop running and your vault path shared with Docker. Uses a vault-specific data dir at .obsidian/zotero-redisearch-rag/redis-data unless overridden.").addToggle(x=>x.setValue(this.plugin.settings.autoStartRedis).onChange(async b=>{this.plugin.settings.autoStartRedis=b,await this.plugin.saveSettings()})),new R.Setting(f).setName("Start Redis stack now").setDesc("Restarts Docker/Podman Compose with the vault data directory.").addButton(x=>x.setButtonText("Start").onClick(async()=>{await this.plugin.startRedisStack()}))}},{id:"zotero-import",label:"Zotero import",render:f=>{f.createEl("h2",{text:"Zotero Local API"}),new R.Setting(f).setName("Zotero base URL").setDesc("Local Zotero API base URL, e.g. http://127.0.0.1:23119/api").addText(P=>P.setPlaceholder("http://127.0.0.1:23119/api").setValue(this.plugin.settings.zoteroBaseUrl).onChange(async S=>{this.plugin.settings.zoteroBaseUrl=S.trim(),await this.plugin.saveSettings()}));let x=new R.Setting(f).setName("Zotero library").setDesc("Select your local library or a Zotero group library."),b=null,C=P=>{if(!b)return;let S=(this.plugin.settings.zoteroUserId||"0").trim()||"0";new Set(P.map(z=>z.value)).has(S)||(P=P.concat([{value:S,label:`Custom (${S})`}])),b.selectEl.options.length=0;for(let z of P)b.addOption(z.value,z.label);b.setValue(S)},L=async()=>{if(b){b.setDisabled(!0);try{let P=await this.plugin.fetchZoteroLibraryOptions();C(P)}finally{b.setDisabled(!1)}}};x.addDropdown(P=>{b=P;let S=(this.plugin.settings.zoteroUserId||"0").trim()||"0";P.addOption(S,"Loading..."),P.setValue(S),P.onChange(async D=>{this.plugin.settings.zoteroUserId=D.trim(),await this.plugin.saveSettings()})}),x.addButton(P=>{P.setButtonText("Refresh").onClick(async()=>{await L()})}),L(),f.createEl("h2",{text:"Zotero Web API"}),new R.Setting(f).setName("Web API base URL").setDesc("Zotero Web API base URL for write fallback, e.g. https://api.zotero.org").addText(P=>P.setPlaceholder("https://api.zotero.org").setValue(this.plugin.settings.webApiBaseUrl).onChange(async S=>{this.plugin.settings.webApiBaseUrl=S.trim(),await this.plugin.saveSettings()})),new R.Setting(f).setName("Web API library type").setDesc("Library type for Web API writes.").addDropdown(P=>P.addOption("user","user").addOption("group","group").setValue(this.plugin.settings.webApiLibraryType).onChange(async S=>{this.plugin.settings.webApiLibraryType=S,await this.plugin.saveSettings()})),new R.Setting(f).setName("Web API library ID").setDesc("Numeric Zotero user/group ID for Web API writes.").addText(P=>P.setPlaceholder("15218").setValue(this.plugin.settings.webApiLibraryId).onChange(async S=>{this.plugin.settings.webApiLibraryId=S.trim(),await this.plugin.saveSettings()})),new R.Setting(f).setName("Web API key").setDesc("Zotero API key for write fallback (from zotero.org).").addText(P=>{i(P),P.setPlaceholder("your-api-key").setValue(this.plugin.settings.webApiKey).onChange(async S=>{this.plugin.settings.webApiKey=S.trim(),await this.plugin.saveSettings()})}),f.createEl("h2",{text:"Output"}),new R.Setting(f).setName("PDF folder").addText(P=>P.setPlaceholder("zotero/pdfs").setValue(this.plugin.settings.outputPdfDir).onChange(async S=>{this.plugin.settings.outputPdfDir=S.trim(),await this.plugin.saveSettings()})),new R.Setting(f).setName("Notes folder").addText(P=>P.setPlaceholder("zotero/notes").setValue(this.plugin.settings.outputNoteDir).onChange(async S=>{this.plugin.settings.outputNoteDir=S.trim(),await this.plugin.saveSettings()})),new R.Setting(f).setName("Frontmatter template").setDesc("Template for note YAML frontmatter. Use {{var}} placeholders; leave blank to omit.").addTextArea(P=>{P.setValue(this.plugin.settings.frontmatterTemplate).onChange(async S=>{this.plugin.settings.frontmatterTemplate=S,await this.plugin.saveSettings()}),P.inputEl.rows=10,P.inputEl.style.width="100%"}),new R.Setting(f).setName("Tag sanitization").setDesc("Normalize Zotero tags for Obsidian (no spaces, punctuation trimmed).").addDropdown(P=>P.addOption("none","No change").addOption("camel","camelCase").addOption("pascal","PascalCase").addOption("snake","snake_case").addOption("kebab","kebab-case").setValue(this.plugin.settings.tagSanitizeMode==="replace"?"kebab":this.plugin.settings.tagSanitizeMode).onChange(async S=>{this.plugin.settings.tagSanitizeMode=S,await this.plugin.saveSettings()})),new R.Setting(f).setName("Note body template").setDesc("Template for the note body after frontmatter. Use {{pdf_block}}, {{annotation_block}}, and {{docling_markdown}} placeholders.").addTextArea(P=>{P.setValue(this.plugin.settings.noteBodyTemplate).onChange(async S=>{this.plugin.settings.noteBodyTemplate=S,await this.plugin.saveSettings()}),P.inputEl.rows=8,P.inputEl.style.width="100%"}),new R.Setting(f).setName("Saved chats folder").setDesc("Where exported chat notes are stored (vault-relative).").addText(P=>P.setPlaceholder("zotero/chats").setValue(this.plugin.settings.chatOutputDir).onChange(async S=>{this.plugin.settings.chatOutputDir=S.trim()||"zotero/chats",await this.plugin.saveSettings()})),new R.Setting(f).setName("Copy PDFs into vault").setDesc("Disable to use Zotero storage paths directly. If a local file path is unavailable, the plugin temporarily copies the PDF into the vault for processing.").addToggle(P=>P.setValue(this.plugin.settings.copyPdfToVault).onChange(async S=>{this.plugin.settings.copyPdfToVault=S,!S&&this.plugin.settings.createOcrLayeredPdf&&(this.plugin.settings.createOcrLayeredPdf=!1),await this.plugin.saveSettings(),this.display()})),new R.Setting(f).setName("Create OCR-layered PDF copy").setDesc("When OCR is used, replace the vault PDF with a Tesseract text layer (requires Copy PDFs into vault).").addToggle(P=>{let S=this.plugin.settings.copyPdfToVault;P.setValue(S?this.plugin.settings.createOcrLayeredPdf:!1).setDisabled(!S).onChange(async D=>{if(!this.plugin.settings.copyPdfToVault){this.plugin.settings.createOcrLayeredPdf=!1,await this.plugin.saveSettings();return}this.plugin.settings.createOcrLayeredPdf=D,await this.plugin.saveSettings()})}),new R.Setting(f).setName("Prefer Obsidian note for citations").setDesc("Link citations to the Obsidian note when available; otherwise use Zotero deep links.").addToggle(P=>P.setValue(this.plugin.settings.preferObsidianNoteForCitations).onChange(async S=>{this.plugin.settings.preferObsidianNoteForCitations=S,await this.plugin.saveSettings()}))}},{id:"annotations",label:"Annotations",render:f=>{f.createEl("h2",{text:"Annotations"}),new R.Setting(f).setName("Annotation page label").setDesc("Label shown before the page link in annotation callouts.").addText(L=>L.setPlaceholder("Page").setValue(this.plugin.settings.annotationPageLabel).onChange(async P=>{this.plugin.settings.annotationPageLabel=P.trim()||"Page",await this.plugin.saveSettings()})),new R.Setting(f).setName("Include annotation images").setDesc("Embed image/rect annotations as images in callouts when available.").addToggle(L=>L.setValue(this.plugin.settings.includeAnnotationImages).onChange(async P=>{this.plugin.settings.includeAnnotationImages=P,await this.plugin.saveSettings()})),f.createEl("h3",{text:"Zotero companion"}),f.createEl("p",{text:"Install the Zotero companion add-on to enable cached image/rect annotations. See tab Maintenance for instructions."}),new R.Setting(f).setName("Use Zotero companion for annotation images").setDesc("Fetch cached annotation images from a local Zotero companion plugin.").addToggle(L=>L.setValue(this.plugin.settings.zoteroCompanionEnabled).onChange(async P=>{this.plugin.settings.zoteroCompanionEnabled=P,await this.plugin.saveSettings()})),new R.Setting(f).setName("Zotero companion base URL").setDesc("Local URL for the Zotero companion plugin (loopback only).").addText(L=>L.setPlaceholder("http://127.0.0.1:23120").setValue(this.plugin.settings.zoteroCompanionBaseUrl).onChange(async P=>{this.plugin.settings.zoteroCompanionBaseUrl=P.trim(),await this.plugin.saveSettings()})),new R.Setting(f).setName("Zotero companion token").setDesc("Optional shared token for the companion endpoint.").addText(L=>{i(L),this.companionTokenInput=L,L.setPlaceholder("optional-token").setValue(this.plugin.settings.zoteroCompanionToken).onChange(async P=>{this.plugin.settings.zoteroCompanionToken=P.trim(),await this.plugin.saveSettings()})}),new R.Setting(f).setName("Generate companion token").setDesc("Create and paste a secure token. The generated token is copied to your clipboard. In Zotero, set your Zotero Companion Plugin token to this value.").addButton(L=>L.setButtonText("Generate token").onClick(async()=>{let P=(0,wn.randomBytes)(32).toString("base64url");this.plugin.settings.zoteroCompanionToken=P,await this.plugin.saveSettings(),this.companionTokenInput&&this.companionTokenInput.setValue(P);try{await navigator.clipboard.writeText(P),new R.Notice("Generated and copied companion token.")}catch(S){new R.Notice("Generated token, but failed to copy to clipboard."),console.warn("Failed to copy generated companion token",S)}})),f.createEl("h3",{text:"Annotation color map"}),f.createEl("p",{text:"Map Zotero highlight colors to section headings and callout types."});let x=f.createDiv({cls:"zrr-annotation-map"}),b=async L=>{this.plugin.settings.annotationColorMap=L,await this.plugin.saveSettings()},C=()=>{var T,M;x.empty();let L=this.plugin.settings.annotationColorMap||{},P=Object.entries(L);if(P.length||x.createEl("p",{text:"No color mappings configured."}),P.length){let I=x.createDiv({cls:"zrr-annotation-map-row-header"});I.createEl("span",{text:"Color"}),I.createEl("span",{text:"Heading"}),I.createEl("span",{text:"Callout"}),I.createEl("span")}for(let[I,F]of P){let O=x.createDiv({cls:"zrr-annotation-map-row"}).createDiv({cls:"zrr-annotation-map-row-body"}),q=O.createEl("input",{type:"text",value:I,cls:"zrr-annotation-map-input"}),$=O.createEl("input",{type:"text",value:(T=F.heading)!=null?T:"",cls:"zrr-annotation-map-input"}),B=O.createEl("input",{type:"text",value:(M=F.callout)!=null?M:"",cls:"zrr-annotation-map-input"}),J=O.createEl("button",{text:"Delete",cls:"zrr-annotation-map-delete"});J.type="button",q.addEventListener("change",async()=>{let V=q.value.trim().toLowerCase();if(!V){q.value=I;return}let Y={...this.plugin.settings.annotationColorMap||{}};if(V!==I&&Y[V]){new R.Notice(`Annotation color '${V}' already exists.`),q.value=I;return}let X=Y[I];delete Y[I],Y[V]=X!=null?X:{heading:"",callout:""},await b(Y),C()}),$.addEventListener("change",async()=>{var X;let V={...this.plugin.settings.annotationColorMap||{}},Y=(X=V[I])!=null?X:{heading:"",callout:""};Y.heading=$.value.trim(),V[I]=Y,await b(V)}),B.addEventListener("change",async()=>{var X;let V={...this.plugin.settings.annotationColorMap||{}},Y=(X=V[I])!=null?X:{heading:"",callout:""};Y.callout=B.value.trim(),V[I]=Y,await b(V)}),J.addEventListener("click",async()=>{let V={...this.plugin.settings.annotationColorMap||{}};delete V[I],await b(V),C()})}let S=x.createDiv({cls:"zrr-annotation-map-actions"}),D=S.createEl("button",{text:"Add mapping"});D.type="button",D.addEventListener("click",async()=>{let I={...this.plugin.settings.annotationColorMap||{}},F="new-color",j=1;for(;I[F];)F=`new-color-${j}`,j+=1;I[F]={heading:"",callout:""},await b(I),C()});let z=S.createEl("button",{text:"Reset to defaults"});z.type="button",z.addEventListener("click",async()=>{await b({...Ie.annotationColorMap}),C()})};C()}},{id:"ocr",label:"OCR",render:f=>{f.createEl("h2",{text:"Docling"});let x=null,b=S=>{if(!x)return;let D=this.plugin.settings.ocrEngine;new Set(S.map(T=>T.value)).has(D)||(S=S.concat([{value:D,label:`Current (unavailable): ${D}`}])),x.selectEl.options.length=0;for(let T of S)x.addOption(T.value,T.label);x.setValue(D)},C=async()=>{if(!x)return;x.setDisabled(!0);let S={tesseract:!1,paddleStructureLocal:!1,paddleVlLocal:!1};if(this.plugin.detectOcrEngines)try{S=await this.plugin.detectOcrEngines()}catch(T){S={tesseract:!1,paddleStructureLocal:!1,paddleVlLocal:!1}}let D=[{value:"auto",label:"Auto (default)"}];S.tesseract&&D.push({value:"tesseract",label:"Tesseract (local)"}),S.paddleStructureLocal&&D.push({value:"paddle_structure_local",label:"Paddle PP-StructureV3 (local)"}),S.paddleVlLocal&&D.push({value:"paddle_vl_local",label:"PaddleOCR-VL (local)"}),(this.plugin.settings.paddleApiKey||"").trim()&&(D.push({value:"paddle_structure_api",label:"PP-StructureV3 API"}),D.push({value:"paddle_vl_api",label:"PaddleOCR-VL API"})),b(D),x.setDisabled(!1)},L=new R.Setting(f).setName("Paddle OCR API key").setDesc("API token for PaddleOCR-VL / PP-StructureV3 endpoints. Get a free API key at "),P=document.createElement("a");P.href="https://aistudio.baidu.com/paddleocr",P.textContent="https://aistudio.baidu.com/paddleocr",P.target="_blank",P.rel="noopener noreferrer",L.descEl.appendChild(P),L.descEl.append("."),L.addText(S=>{i(S),S.setPlaceholder("your-api-token").setValue(this.plugin.settings.paddleApiKey).onChange(async D=>{this.plugin.settings.paddleApiKey=D.trim(),await this.plugin.saveSettings(),await C()})}),new R.Setting(f).setName("PaddleOCR-VL API URL").setDesc("Optional override for the PaddleOCR-VL API endpoint.").addText(S=>S.setPlaceholder("https://.../layout-parsing").setValue(this.plugin.settings.paddleVlApiUrl).onChange(async D=>{this.plugin.settings.paddleVlApiUrl=D.trim(),await this.plugin.saveSettings()})),new R.Setting(f).setName("PP-StructureV3 API URL").setDesc("API endpoint for PP-StructureV3 (see Baidu AI Studio docs).").addText(S=>S.setPlaceholder("https://.../pp-structure").setValue(this.plugin.settings.paddleStructureApiUrl).onChange(async D=>{this.plugin.settings.paddleStructureApiUrl=D.trim(),await this.plugin.saveSettings(),await C()})),new R.Setting(f).setName("OCR engine").setDesc("Select the OCR engine to use when OCR is required.").addDropdown(S=>{x=S,S.addOption("auto","Auto (default)"),S.setValue(this.plugin.settings.ocrEngine),S.onChange(async D=>{this.plugin.settings.ocrEngine=D,await this.plugin.saveSettings()})}),new R.Setting(f).setName("OCR decision (when to OCR)").setDesc("Controls when OCR runs; per-page behavior is configured separately below.").addDropdown(S=>S.addOption("auto","Auto: use text layer when reliable").addOption("force_low_quality","OCR only if text is poor").addOption("force","Prefer OCR for full document").setValue(this.plugin.settings.ocrMode).onChange(async D=>{this.plugin.settings.ocrMode=D,await this.plugin.saveSettings()})),new R.Setting(f).setName("OCR layout override (per-page)").setDesc("Force per-page OCR when OCR runs, bypassing layout heuristics; can be slower for multi-column PDFs.").addToggle(S=>S.setValue(this.plugin.settings.forcePerPageOcr).onChange(async D=>{this.plugin.settings.forcePerPageOcr=D,await this.plugin.saveSettings()})),new R.Setting(f).setName("Text quality threshold").setDesc("Lower values are stricter; below this threshold the text is treated as low quality.").addSlider(S=>{S.setLimits(0,1,.05).setValue(this.plugin.settings.ocrQualityThreshold).setDynamicTooltip().onChange(async D=>{this.plugin.settings.ocrQualityThreshold=D,await this.plugin.saveSettings()})}),new R.Setting(f).setName("Chunking").setDesc("page or section").addDropdown(S=>S.addOption("page","page").addOption("section","section").setValue(this.plugin.settings.chunkingMode).onChange(async D=>{this.plugin.settings.chunkingMode=D,await this.plugin.saveSettings()})),C()}},{id:"llms",label:"LLMs",render:f=>{f.createEl("h2",{text:"LLM Provider Profiles"});let x=f.createDiv({cls:"zrr-llm-profiles"}),b=()=>{x.empty();let v=n();v.length||x.createEl("p",{text:"No profiles yet. Add one below."});for(let w of v){let N=x.createEl("details",{cls:"zrr-profile"});v.length===1&&(N.open=!0);let G=N.createEl("summary",{text:w.name||w.id||"Profile"});G.addClass("zrr-profile-title");let Le=N.createDiv({cls:"zrr-profile-body"});new R.Setting(Le).setName("Profile name").addText(le=>le.setPlaceholder("My provider").setValue(w.name||"").onChange(async de=>{w.name=de.trim(),G.textContent=w.name||w.id||"Profile",await t(n())})),new R.Setting(Le).setName("Base URL").addText(le=>le.setPlaceholder("http://localhost:1234/v1").setValue(w.baseUrl||"").onChange(async de=>{w.baseUrl=de.trim(),await t(n())})),new R.Setting(Le).setName("API key").setDesc("Stored in settings (not encrypted).").addText(le=>{i(le),le.setPlaceholder("sk-...").setValue(w.apiKey||"").onChange(async de=>{w.apiKey=de.trim(),await t(n())})}),new R.Setting(Le).setName("Remove profile").setDesc("Deletes this saved profile.").addButton(le=>le.setButtonText("Delete profile").onClick(async()=>{let de=n().filter(Yn=>Yn.id!==w.id);this.plugin.settings.embedProviderProfileId=this.plugin.settings.embedProviderProfileId===w.id?"":this.plugin.settings.embedProviderProfileId,this.plugin.settings.chatProviderProfileId=this.plugin.settings.chatProviderProfileId===w.id?"":this.plugin.settings.chatProviderProfileId,this.plugin.settings.llmCleanupProviderProfileId=this.plugin.settings.llmCleanupProviderProfileId===w.id?"":this.plugin.settings.llmCleanupProviderProfileId,await t(de),b()}))}new R.Setting(x).addButton(w=>w.setButtonText("Add profile").onClick(async()=>{let N=`profile-${Date.now().toString(36)}`,G=n().concat([{id:N,name:"Custom",baseUrl:"",apiKey:""}]);await t(G),b()}))};b(),f.createEl("h2",{text:"OCR cleanup"}),new R.Setting(f).setName("LLM cleanup for low-quality chunks").setDesc("Automatic AI cleanup for poor OCR at import. Can be slow/costly.").addToggle(v=>v.setValue(this.plugin.settings.enableLlmCleanup).onChange(async w=>{this.plugin.settings.enableLlmCleanup=w,await this.plugin.saveSettings()}));let C=null,L=null,P=null,S=async()=>{},D=async(v,w=!0)=>{let N=v.trim();this.plugin.settings.llmCleanupBaseUrl=N,w&&(this.plugin.settings.llmCleanupProviderProfileId="",C&&C.setValue("custom")),L&&L.setValue(N),await this.plugin.saveSettings()},z=async v=>{let w=n().find(N=>N.id===v);this.plugin.settings.llmCleanupProviderProfileId=v,w&&(this.plugin.settings.llmCleanupBaseUrl=w.baseUrl,this.plugin.settings.llmCleanupApiKey=w.apiKey,L==null||L.setValue(w.baseUrl),P==null||P.setValue(w.apiKey)),await this.plugin.saveSettings(),await S()};new R.Setting(f).setName("LLM cleanup provider profile").setDesc("Select a profile to populate base URL and API key.").addDropdown(v=>{C=v,v.addOption("custom","Custom (manual)");for(let N of n())v.addOption(N.id,N.name||N.id);let w=this.plugin.settings.llmCleanupProviderProfileId;v.setValue(w&&n().some(N=>N.id===w)?w:"custom"),v.onChange(async N=>{if(N==="custom"){this.plugin.settings.llmCleanupProviderProfileId="",await this.plugin.saveSettings();return}await z(N)})}),new R.Setting(f).setName("LLM cleanup base URL").setDesc("OpenAI-compatible endpoint, e.g. http://127.0.0.1:1234/v1").addText(v=>{L=v,v.setPlaceholder("http://127.0.0.1:1234/v1").setValue(this.plugin.settings.llmCleanupBaseUrl).onChange(async w=>{await D(w)})}),new R.Setting(f).setName("LLM cleanup API key").setDesc("Optional API key for the cleanup endpoint.").addText(v=>{P=v,i(v),v.setPlaceholder("sk-...").setValue(this.plugin.settings.llmCleanupApiKey).onChange(async w=>{this.plugin.settings.llmCleanupApiKey=w.trim(),this.plugin.settings.llmCleanupProviderProfileId="",C&&C.setValue("custom"),await this.plugin.saveSettings()})});let T=new R.Setting(f).setName("LLM cleanup model").setDesc("Select a cleanup-capable model from the provider."),M=null,I=v=>{if(!M)return;let w=(this.plugin.settings.llmCleanupModel||"").trim(),N=new Set(v.map(G=>G.value));w&&!N.has(w)&&(v=v.concat([{value:w,label:`Custom (${w})`}])),M.selectEl.options.length=0;for(let G of v)M.addOption(G.value,G.label);w&&M.setValue(w)};S=async()=>{if(M){M.setDisabled(!0);try{let v=await this.plugin.fetchCleanupModelOptions();I(v)}finally{M.setDisabled(!1)}}},T.addDropdown(v=>{M=v;let w=(this.plugin.settings.llmCleanupModel||"").trim();v.addOption(w||"loading","Loading..."),v.setValue(w||"loading"),v.onChange(async N=>{this.plugin.settings.llmCleanupModel=N.trim(),await this.plugin.saveSettings()})}),T.addButton(v=>{v.setButtonText("Refresh").onClick(async()=>{await S()})}),S(),new R.Setting(f).setName("LLM cleanup temperature").setDesc("Lower is more conservative.").addText(v=>v.setPlaceholder("0.0").setValue(String(this.plugin.settings.llmCleanupTemperature)).onChange(async w=>{let N=Number.parseFloat(w);this.plugin.settings.llmCleanupTemperature=Number.isFinite(N)?N:0,await this.plugin.saveSettings()})),new R.Setting(f).setName("LLM cleanup min quality").setDesc("Only run cleanup when chunk quality is below this threshold (0-1).").addSlider(v=>v.setLimits(0,1,.05).setValue(this.plugin.settings.llmCleanupMinQuality).setDynamicTooltip().onChange(async w=>{this.plugin.settings.llmCleanupMinQuality=w,await this.plugin.saveSettings()})),new R.Setting(f).setName("LLM cleanup max chars").setDesc("Skip cleanup for chunks longer than this limit.").addText(v=>v.setPlaceholder("2000").setValue(String(this.plugin.settings.llmCleanupMaxChars)).onChange(async w=>{let N=Number.parseInt(w,10);this.plugin.settings.llmCleanupMaxChars=Number.isFinite(N)?N:2e3,await this.plugin.saveSettings()})),f.createEl("h2",{text:"Text Embedding"});let F=null,j=null,O=null,q=async()=>{},$=async(v,w=!0)=>{let N=v.trim();this.plugin.settings.embedBaseUrl=N,w&&(this.plugin.settings.embedProviderProfileId="",F&&F.setValue("custom")),j&&j.setValue(N),await this.plugin.saveSettings()},B=async v=>{let w=n().find(N=>N.id===v);this.plugin.settings.embedProviderProfileId=v,w&&(this.plugin.settings.embedBaseUrl=w.baseUrl,this.plugin.settings.embedApiKey=w.apiKey,j==null||j.setValue(w.baseUrl),O==null||O.setValue(w.apiKey)),await this.plugin.saveSettings(),await q()};new R.Setting(f).setName("Embeddings provider profile").setDesc("Select a profile to populate base URL and API key.").addDropdown(v=>{F=v,v.addOption("custom","Custom (manual)");for(let N of n())v.addOption(N.id,N.name||N.id);let w=this.plugin.settings.embedProviderProfileId;v.setValue(w&&n().some(N=>N.id===w)?w:"custom"),v.onChange(async N=>{if(N==="custom"){this.plugin.settings.embedProviderProfileId="",await this.plugin.saveSettings();return}await B(N)})}),new R.Setting(f).setName("Embeddings base URL").addText(v=>{j=v,v.setPlaceholder("http://localhost:1234/v1").setValue(this.plugin.settings.embedBaseUrl).onChange(async w=>{await $(w)})}),new R.Setting(f).setName("Embeddings API key").addText(v=>{O=v,i(v),v.setPlaceholder("lm-studio").setValue(this.plugin.settings.embedApiKey).onChange(async w=>{this.plugin.settings.embedApiKey=w.trim(),this.plugin.settings.embedProviderProfileId="",F&&F.setValue("custom"),await this.plugin.saveSettings()})});let J=new R.Setting(f).setName("Embeddings model").setDesc("Select an embeddings model from the provider."),V=null,Y=v=>{if(!V)return;let w=(this.plugin.settings.embedModel||"").trim(),N=new Set(v.map(G=>G.value));w&&!N.has(w)&&(v=v.concat([{value:w,label:`Custom (${w})`}])),V.selectEl.options.length=0;for(let G of v)V.addOption(G.value,G.label);w&&V.setValue(w)};q=async()=>{if(V){V.setDisabled(!0);try{let v=await this.plugin.fetchEmbeddingModelOptions();Y(v)}finally{V.setDisabled(!1)}}},J.addDropdown(v=>{V=v;let w=(this.plugin.settings.embedModel||"").trim();v.addOption(w||"loading","Loading..."),v.setValue(w||"loading"),v.onChange(async N=>{this.plugin.settings.embedModel=N.trim(),await this.plugin.saveSettings()})}),J.addButton(v=>{v.setButtonText("Refresh").onClick(async()=>{await q()})}),q(),new R.Setting(f).setName("Include metadata in embeddings").setDesc("Prepend title/authors/tags/section info before embedding chunks.").addToggle(v=>v.setValue(this.plugin.settings.embedIncludeMetadata).onChange(async w=>{this.plugin.settings.embedIncludeMetadata=w,await this.plugin.saveSettings()})),new R.Setting(f).setName("Embedding context window (chunks)").setDesc("Include neighboring chunk text around each chunk when embedding (0 disables).").addText(v=>v.setPlaceholder("1").setValue(String(this.plugin.settings.embedContextWindow)).onChange(async w=>{let N=Number.parseInt(w,10);this.plugin.settings.embedContextWindow=Number.isFinite(N)?Math.max(0,N):1,await this.plugin.saveSettings()})),new R.Setting(f).setName("Embedding context snippet size (chars)").setDesc("Max chars per neighboring chunk included in embeddings.").addText(v=>v.setPlaceholder("220").setValue(String(this.plugin.settings.embedContextChars)).onChange(async w=>{let N=Number.parseInt(w,10);this.plugin.settings.embedContextChars=Number.isFinite(N)?Math.max(0,N):220,await this.plugin.saveSettings()})),new R.Setting(f).setName("Embedding subchunk size (chars)").setDesc("Split long chunks into smaller subchunks for embedding only (0 disables).").addText(v=>v.setPlaceholder("1800").setValue(String(this.plugin.settings.embedSubchunkChars)).onChange(async w=>{let N=Number.parseInt(w,10);this.plugin.settings.embedSubchunkChars=Number.isFinite(N)?Math.max(0,N):3500,await this.plugin.saveSettings()})),new R.Setting(f).setName("Embedding subchunk overlap (chars)").setDesc("Overlap between embedding subchunks to keep context intact.").addText(v=>v.setPlaceholder("200").setValue(String(this.plugin.settings.embedSubchunkOverlap)).onChange(async w=>{let N=Number.parseInt(w,10);this.plugin.settings.embedSubchunkOverlap=Number.isFinite(N)?Math.max(0,N):200,await this.plugin.saveSettings()})),new R.Setting(f).setName("Generate LLM tags for chunks").setDesc("Use the OCR cleanup model to tag chunks before indexing.").addToggle(v=>v.setValue(this.plugin.settings.enableChunkTagging).onChange(async w=>{this.plugin.settings.enableChunkTagging=w,await this.plugin.saveSettings()})),f.createEl("h2",{text:"Chat LLM"});let X=null,ie=null,se=null,ge=async()=>{},Ye=async(v,w=!0)=>{let N=v.trim();this.plugin.settings.chatBaseUrl=N,w&&(this.plugin.settings.chatProviderProfileId="",X&&X.setValue("custom")),ie&&ie.setValue(N),await this.plugin.saveSettings()},Qe=async v=>{let w=n().find(N=>N.id===v);this.plugin.settings.chatProviderProfileId=v,w&&(this.plugin.settings.chatBaseUrl=w.baseUrl,this.plugin.settings.chatApiKey=w.apiKey,ie==null||ie.setValue(w.baseUrl),se==null||se.setValue(w.apiKey)),await this.plugin.saveSettings(),await ge()};new R.Setting(f).setName("Chat provider profile").setDesc("Select a profile to populate base URL and API key.").addDropdown(v=>{X=v,v.addOption("custom","Custom (manual)");for(let N of n())v.addOption(N.id,N.name||N.id);let w=this.plugin.settings.chatProviderProfileId;v.setValue(w&&n().some(N=>N.id===w)?w:"custom"),v.onChange(async N=>{if(N==="custom"){this.plugin.settings.chatProviderProfileId="",await this.plugin.saveSettings();return}await Qe(N)})}),new R.Setting(f).setName("Chat base URL").setDesc("OpenAI-compatible base URL for chat requests.").addText(v=>{ie=v,v.setPlaceholder("http://localhost:1234/v1").setValue(this.plugin.settings.chatBaseUrl).onChange(async w=>{await Ye(w)})}),new R.Setting(f).setName("Chat API key").addText(v=>{se=v,i(v),v.setPlaceholder("lm-studio").setValue(this.plugin.settings.chatApiKey).onChange(async w=>{this.plugin.settings.chatApiKey=w.trim(),this.plugin.settings.chatProviderProfileId="",X&&X.setValue("custom"),await this.plugin.saveSettings()})});let Se=new R.Setting(f).setName("Chat model").setDesc("Select a chat-capable model from the provider."),re=null,en=v=>{if(!re)return;let w=(this.plugin.settings.chatModel||"").trim(),N=new Set(v.map(G=>G.value));w&&!N.has(w)&&(v=v.concat([{value:w,label:`Custom (${w})`}])),re.selectEl.options.length=0;for(let G of v)re.addOption(G.value,G.label);w&&re.setValue(w)};ge=async()=>{if(re){re.setDisabled(!0);try{let v=await this.plugin.fetchChatModelOptions();en(v)}finally{re.setDisabled(!1)}}},Se.addDropdown(v=>{re=v;let w=(this.plugin.settings.chatModel||"").trim();v.addOption(w||"loading","Loading..."),v.setValue(w||"loading"),v.onChange(async N=>{this.plugin.settings.chatModel=N.trim(),await this.plugin.saveSettings()})}),Se.addButton(v=>{v.setButtonText("Refresh").onClick(async()=>{await ge()})}),ge(),new R.Setting(f).setName("Temperature").addText(v=>v.setPlaceholder("0.2").setValue(String(this.plugin.settings.chatTemperature)).onChange(async w=>{let N=Number.parseFloat(w);this.plugin.settings.chatTemperature=Number.isFinite(N)?N:.2,await this.plugin.saveSettings()})),new R.Setting(f).setName("Chat history messages").setDesc("Number of recent messages to include for conversational continuity (0 disables).").addText(v=>v.setPlaceholder("6").setValue(String(this.plugin.settings.chatHistoryMessages)).onChange(async w=>{let N=Number.parseInt(w,10);this.plugin.settings.chatHistoryMessages=Number.isFinite(N)?Math.max(0,N):6,await this.plugin.saveSettings()})),new R.Setting(f).setName("Chat panel location").setDesc("Where to open the chat view by default.").addDropdown(v=>v.addOption("right","Right sidebar").addOption("main","Main window").setValue(this.plugin.settings.chatPaneLocation).onChange(async w=>{this.plugin.settings.chatPaneLocation=w,await this.plugin.saveSettings()})),f.createEl("h2",{text:"Retrieval"}),new R.Setting(f).setName("Enable query expansion").setDesc("Use the chat model to expand queries before retrieval.").addToggle(v=>v.setValue(this.plugin.settings.enableQueryExpansion).onChange(async w=>{this.plugin.settings.enableQueryExpansion=w,await this.plugin.saveSettings()})),new R.Setting(f).setName("Query expansion count").setDesc("Number of expansion variants to request.").addText(v=>v.setPlaceholder("3").setValue(String(this.plugin.settings.queryExpansionCount)).onChange(async w=>{let N=Number.parseInt(w,10);this.plugin.settings.queryExpansionCount=Number.isFinite(N)?Math.max(1,N):3,await this.plugin.saveSettings()})),new R.Setting(f).setName("Enable cross-encoder reranking").setDesc("Rerank candidates locally with sentence-transformers (downloads model on first use).").addToggle(v=>v.setValue(this.plugin.settings.enableCrossEncoderRerank).onChange(async w=>{this.plugin.settings.enableCrossEncoderRerank=w,await this.plugin.saveSettings()})),new R.Setting(f).setName("Cross-encoder model").setDesc("Local reranker model name or path.").addText(v=>v.setPlaceholder("BAAI/bge-reranker-v2-m3").setValue(this.plugin.settings.rerankModel).onChange(async w=>{this.plugin.settings.rerankModel=w.trim()||"BAAI/bge-reranker-v2-m3",await this.plugin.saveSettings()})),new R.Setting(f).setName("Rerank candidate multiplier").setDesc("Retrieve k \xD7 N candidates before reranking.").addText(v=>v.setPlaceholder("4").setValue(String(this.plugin.settings.rerankCandidateMultiplier)).onChange(async w=>{let N=Number.parseInt(w,10);this.plugin.settings.rerankCandidateMultiplier=Number.isFinite(N)?Math.max(1,N):4,await this.plugin.saveSettings()})),new R.Setting(f).setName("RRF k").setDesc("Rank fusion constant for blending lexical and vector results.").addText(v=>v.setPlaceholder("60").setValue(String(this.plugin.settings.rrfK)).onChange(async w=>{let N=Number.parseInt(w,10);this.plugin.settings.rrfK=Number.isFinite(N)?Math.max(1,N):60,await this.plugin.saveSettings()})),new R.Setting(f).setName("RRF log top N").setDesc("Log the top N RRF-ranked chunks to stderr (0 disables).").addText(v=>v.setPlaceholder("0").setValue(String(this.plugin.settings.rrfLogTop)).onChange(async w=>{let N=Number.parseInt(w,10);this.plugin.settings.rrfLogTop=Number.isFinite(N)?Math.max(0,N):0,await this.plugin.saveSettings()})),new R.Setting(f).setName("Max chunks per document").setDesc("Limit how many chunks from a single document can appear in retrieval (0 disables).").addText(v=>v.setPlaceholder("0").setValue(String(this.plugin.settings.maxChunksPerDoc)).onChange(async w=>{let N=Number.parseInt(w,10);this.plugin.settings.maxChunksPerDoc=Number.isFinite(N)?Math.max(0,N):0,await this.plugin.saveSettings()}))}},{id:"maintenance",label:"Maintenance",render:f=>{f.createEl("h2",{text:"Logging"}),new R.Setting(f).setName("Enable logging to file").setDesc("Write plugin logs to a file.").addToggle(x=>x.setValue(this.plugin.settings.enableFileLogging).onChange(async b=>{this.plugin.settings.enableFileLogging=b,await this.plugin.saveSettings()})),new R.Setting(f).setName("Log file path (vault-relative)").setDesc("Where to write the log file. Keep inside the vault.").addText(x=>x.setPlaceholder(`${ee}/logs/docling_extract.log`).setValue(this.plugin.settings.logFilePath).onChange(async b=>{this.plugin.settings.logFilePath=b.trim()||`${ee}/logs/docling_extract.log`,await this.plugin.saveSettings()})),new R.Setting(f).setName("View or clear log").setDesc("Open the log file or clear it.").addButton(x=>x.setButtonText("Open log").onClick(async()=>{var b,C;await((C=(b=this.plugin).openLogFile)==null?void 0:C.call(b))})).addButton(x=>x.setButtonText("Clear log").onClick(async()=>{var b,C;await((C=(b=this.plugin).clearLogFile)==null?void 0:C.call(b))})),f.createEl("h2",{text:"Zotero Companion Plugin"}),f.createEl("p",{text:"Download the XPI, then in Zotero go to Tools \u2192 Add-ons \u2192 Install from File and restart."}),new R.Setting(f).setName("Download companion add-on").setDesc("Downloads the companion XPI to your system Downloads folder.").addButton(x=>x.setButtonText("Download XPI").onClick(async()=>{let b=this.getDefaultDownloadDir();if(!b){new R.Notice("Unable to resolve the system Downloads folder.");return}let C=Te.default.join(b,"zrr-companion.xpi");if(await this.companionXpiExists(C)){new R.Notice(`Companion XPI already exists: ${C}`);return}new R.Notice("Downloading companion XPI..."),await this.downloadCompanionXpi(C)})),new R.Setting(f).setName("Open Zotero").setDesc("Launch Zotero and open the Add-ons window (Tools \u2192 Add-ons).").addButton(x=>x.setButtonText("Open Zotero").onClick(async()=>{await this.plugin.openZoteroAddons()})),new R.Setting(f).setName("Check companion status").setDesc("Ping the companion /health endpoint.").addButton(x=>x.setButtonText("Check status").onClick(async()=>{await this.plugin.checkZoteroCompanionHealth()})),f.createEl("h2",{text:"Redis Indexing"}),new R.Setting(f).setName("Reindex Redis from cached chunks").setDesc("Rebuild the Redis index from cached chunk JSON files.").addButton(x=>x.setButtonText("Reindex").onClick(async()=>{await this.plugin.reindexRedisFromCache()})),new R.Setting(f).setName("Recreate missing notes from cache").setDesc("Rebuild missing notes using cached Zotero items and chunks.").addButton(x=>x.setButtonText("Recreate").onClick(async()=>{await this.plugin.recreateMissingNotesFromCache()})).addButton(x=>x.setButtonText("Cancel").onClick(()=>{this.plugin.cancelRecreateMissingNotesFromCache()}))}}],p=new Map,u=new Map,m=f=>{this.activeTab=f;for(let[x,b]of p){let C=x===f;b.classList.toggle("is-active",C),b.setAttribute("aria-selected",C?"true":"false"),b.setAttribute("tabindex",C?"0":"-1")}for(let[x,b]of u){let C=x===f;b.classList.toggle("is-active",C),b.hidden=!C}},y=e.createDiv({cls:"zrr-settings-tabs"});y.setAttribute("role","tablist");let k=e.createDiv({cls:"zrr-settings-tabs-panels"});for(let f of d){let x=y.createEl("button",{text:f.label,cls:"zrr-settings-tab-button"}),b=`zrr-settings-tab-${f.id}`,C=`zrr-settings-panel-${f.id}`;x.type="button",x.id=b,x.setAttribute("role","tab"),x.setAttribute("aria-controls",C),x.setAttribute("aria-selected","false"),x.addEventListener("click",()=>m(f.id));let L=k.createDiv({cls:"zrr-settings-tab-panel"});L.id=C,L.setAttribute("role","tabpanel"),L.setAttribute("aria-labelledby",b),L.hidden=!0,f.render(L),p.set(f.id,x),u.set(f.id,L)}let A=d.some(f=>f.id===this.activeTab)?this.activeTab:d[0].id;m(A)}getDefaultDownloadDir(){let e=process.env.HOME||process.env.USERPROFILE;return e?Te.default.join(e,"Downloads"):null}async companionXpiExists(e){try{return await De.promises.access(e),!0}catch(n){return!1}}async downloadCompanionXpi(e){try{await De.promises.mkdir(Te.default.dirname(e),{recursive:!0});let n=await(0,R.requestUrl)({url:ct,method:"GET"});if(n.status!==200){new R.Notice(`Failed to download companion XPI (HTTP ${n.status}).`);return}let t=Buffer.from(n.arrayBuffer);await De.promises.writeFile(e,t),new R.Notice(`Downloaded companion XPI: ${e}`)}catch(n){new R.Notice("Failed to download companion XPI. See console for details."),console.warn("Failed to download companion XPI",n)}}};var Cn=require("@codemirror/state"),Sn=require("@codemirror/view"),oe=require("obsidian");var dt=g=>{let _=g.scrollDOM.getBoundingClientRect(),e=g.posAtCoords({x:_.left+8,y:+_.height*.25});return e===null?null:g.state.doc.lineAt(e).number},Fe=class{constructor(_,e){this.pdfSidebarLeaf=null;this.pdfSidebarDocId=null;this.pdfSidebarPdfPath=null;this.pdfSidebarPage=null;this.pendingPdfSync=null;this.chunkPageCache=new Map;this.previewScrollEl=null;this.previewScrollHandler=null;this.previewScrollFrame=null;this.deps=_,this.helpers=e}createSyncExtension(){let _=this,e=this.helpers;return Sn.ViewPlugin.fromClass(class{constructor(n){this.docId=null;this.lastPage=null;this.lastChunkId=null;this.scrollFrame=null;this.view=n,this.docId=e.extractDocIdFromDoc(n.state.doc),this.onScroll=()=>this.scheduleSync(!1),n.scrollDOM.addEventListener("scroll",this.onScroll,{passive:!0}),this.scheduleSync(!0)}update(n){n.docChanged&&(this.docId=e.extractDocIdFromDoc(n.view.state.doc),this.lastPage=null,this.lastChunkId=null),(n.docChanged||n.viewportChanged)&&this.scheduleSync(!1)}destroy(){this.view.scrollDOM.removeEventListener("scroll",this.onScroll),this.scrollFrame!==null&&(window.cancelAnimationFrame(this.scrollFrame),this.scrollFrame=null)}scheduleSync(n){this.scrollFrame===null&&(this.scrollFrame=window.requestAnimationFrame(()=>{this.scrollFrame=null,this.syncPdfSidebar(this.view,n)}))}syncPdfSidebar(n,t){var p,u;let i=this.docId;if(!i)return;let r=dt(n);if(r===null)return;let a=e.findChunkStartLineInDoc(n.state.doc,r);if(!a)return;let s=e.parseChunkMarkerLine(a.text),o=(p=s==null?void 0:s.pageNumber)!=null?p:null,l=(u=s==null?void 0:s.chunkId)!=null?u:null;if(!o&&!l)return;let c=o!==null&&this.lastPage===o,d=o===null&&l!==null&&this.lastChunkId===l;!t&&(c||d)||(this.lastPage=o,this.lastChunkId=l,_.syncPdfSidebarForDoc(i,o!=null?o:void 0,l!=null?l:void 0))}})}async maybeSyncPendingPdf(){var e,n;if(!this.pendingPdfSync||(!this.pdfSidebarLeaf||((n=(e=this.pdfSidebarLeaf.view)==null?void 0:e.getViewType)==null?void 0:n.call(e))!=="pdf")&&!await this.getPdfSidebarLeaf()||!this.pdfSidebarLeaf||!this.isLeafTabActive(this.pdfSidebarLeaf))return;let _=this.pendingPdfSync;this.pendingPdfSync=null,await this.syncPdfSidebarForDoc(_.docId,_.pageNumber,_.chunkId)}updatePreviewScrollHandler(){var t;let _=this.deps.app.workspace.getActiveViewOfType(oe.MarkdownView);if(!_||_.getMode()!=="preview"){this.detachPreviewScrollHandler();return}let e=(t=_.previewMode)==null?void 0:t.containerEl;if(!e){this.detachPreviewScrollHandler();return}if(this.previewScrollEl===e)return;this.detachPreviewScrollHandler();let n=()=>this.schedulePreviewSync(_);e.addEventListener("scroll",n,{passive:!0}),this.previewScrollEl=e,this.previewScrollHandler=n,this.schedulePreviewSync(_)}async syncPdfSidebarForFile(_){try{let e=await this.deps.app.vault.read(_),n=await this.deps.resolveDocIdForNote(_,e);if(!n)return;let t=this.helpers.extractFirstChunkMarkerFromContent(e);await this.syncPdfSidebarForDoc(n,t==null?void 0:t.pageNumber,t==null?void 0:t.chunkId)}catch(e){console.warn("Failed to sync PDF sidebar for opened note",e)}}async syncPdfSidebarForDoc(_,e,n){if(!_)return;let t=await this.deps.getDocIndexEntry(_);t||(t=await this.deps.hydrateDocIndexFromCache(_));let i=t!=null&&t.pdf_path?String(t.pdf_path):"";if(!i)return;let r=this.deps.toVaultRelativePath(i);if(!r){let u=(0,oe.normalizePath)(i);this.deps.app.vault.getAbstractFileByPath(u)instanceof oe.TFile&&(r=u)}if(!r)return;let a=this.deps.app.vault.getAbstractFileByPath(r);if(!(a instanceof oe.TFile)||a.extension.toLowerCase()!=="pdf")return;let s=await this.getPdfSidebarLeaf();if(!s){this.pendingPdfSync={docId:_,pageNumber:e,chunkId:n};return}if(this.updatePdfSidebarIcon(s),!this.isLeafTabActive(s)){this.pendingPdfSync={docId:_,pageNumber:e,chunkId:n};return}let o=Number.isFinite(e!=null?e:NaN)?Number(e):null;if(o===null&&n&&(o=await this.resolvePageNumberForChunk(_,n)),o===null||this.pdfSidebarDocId===_&&this.pdfSidebarPdfPath===a.path&&this.pdfSidebarPage===o)return;this.pdfSidebarDocId=_,this.pdfSidebarPdfPath=a.path,this.pdfSidebarPage=o;let l=r,c=o!==null?`${l}#page=${o}`:l,d=this.deps.app.workspace,p=d.getMostRecentLeaf();d.setActiveLeaf(s,{focus:!1});try{await this.openPdfLinkInLeaf(s,c),this.updatePdfSidebarIcon(s)}finally{p&&d.setActiveLeaf(p,{focus:!1})}}detachPreviewScrollHandler(){this.previewScrollEl&&this.previewScrollHandler&&this.previewScrollEl.removeEventListener("scroll",this.previewScrollHandler),this.previewScrollEl=null,this.previewScrollHandler=null,this.previewScrollFrame!==null&&(window.cancelAnimationFrame(this.previewScrollFrame),this.previewScrollFrame=null)}schedulePreviewSync(_){this.previewScrollFrame===null&&(this.previewScrollFrame=window.requestAnimationFrame(()=>{this.previewScrollFrame=null,this.syncPdfSidebarForPreview(_)}))}syncPdfSidebarForPreview(_){let e=this.previewScrollEl;if(!e)return;let n=_.getViewData();if(!n)return;let t=Cn.Text.of(n.split(/\r?\n/)),i=this.helpers.extractDocIdFromDoc(t);if(!i)return;let r=this.getPreviewTopLineNumber(e);if(r===null)return;let a=this.helpers.findChunkStartLineInDoc(t,r+1);if(!a)return;let s=this.helpers.parseChunkMarkerLine(a.text);!(s!=null&&s.pageNumber)&&!(s!=null&&s.chunkId)||this.syncPdfSidebarForDoc(i,s==null?void 0:s.pageNumber,s==null?void 0:s.chunkId)}getPreviewTopLineNumber(_){var t;let e=_.getBoundingClientRect().top,n=_.querySelectorAll("[data-line]");for(let i of Array.from(n))if(i.getBoundingClientRect().bottom>=e+2){let a=(t=i.getAttribute("data-line"))!=null?t:"",s=Number.parseInt(a,10);if(Number.isFinite(s))return s}return null}async getPdfSidebarLeaf(){var t,i;if(this.pdfSidebarLeaf&&((i=(t=this.pdfSidebarLeaf.view)==null?void 0:t.getViewType)==null?void 0:i.call(t))==="pdf")return this.pdfSidebarLeaf;let _=this.deps.app.workspace;if(typeof _.ensureSideLeaf=="function")try{let r=await _.ensureSideLeaf("pdf","right",{active:!1,split:!1,reveal:!0});return this.isRightSidebarLeaf(r)?(this.pdfSidebarLeaf=r,this.updatePdfSidebarIcon(r),r):null}catch(r){console.warn("Failed to ensure PDF side leaf",r)}let e=this.deps.app.workspace.getRightLeaf(!1);if(e&&this.isRightSidebarLeaf(e))return this.pdfSidebarLeaf=e,this.updatePdfSidebarIcon(e),e;let n=this.deps.app.workspace.getRightLeaf(!0);return n&&this.isRightSidebarLeaf(n)?(this.pdfSidebarLeaf=n,this.updatePdfSidebarIcon(n),n):null}isRightSidebarLeaf(_){if(!_)return!1;let n=_.containerEl;return!!(n!=null&&n.closest(".workspace-split.mod-right-split, .mod-right-split"))}updatePdfSidebarIcon(_){if(!_||!this.isRightSidebarLeaf(_))return;let n=_.containerEl,t=[];n&&t.push(...Array.from(n.querySelectorAll(".view-header-icon")));let i=_;if(i.tabHeaderEl&&t.push(...Array.from(i.tabHeaderEl.querySelectorAll(".workspace-tab-header-inner-icon, .view-header-icon"))),i.tabHeaderInnerIconEl&&t.push(i.tabHeaderInnerIconEl),t.length===0)return;let r=new Set;for(let s of t)r.has(s)||(r.add(s),s.innerHTML="",(0,oe.setIcon)(s,"zrr-pdf"),!s.querySelector("svg")&&this.deps.iconSvg&&(s.innerHTML=this.deps.iconSvg),s.dataset&&(s.dataset.zrrIcon="zrr-pdf"));let a=_.view;a.icon="zrr-pdf",typeof a.getIcon=="function"&&(a.getIcon=()=>"zrr-pdf")}async openPdfLinkInLeaf(_,e){var r,a,s;let n=(r=this.getPluginsRegistry())==null?void 0:r["pdf-plus"],t=(s=(a=n==null?void 0:n.lib)==null?void 0:a.workspace)==null?void 0:s.openPDFLinkTextInLeaf;if(typeof t=="function"){await t.call(n.lib.workspace,_,e,"",{active:!1});return}let i=_;if(typeof i.openLinkText=="function"){await i.openLinkText(e,"",{active:!1});return}await this.deps.app.workspace.openLinkText(e,"",!1)}getPluginsRegistry(){var _;return(_=this.deps.app.plugins)==null?void 0:_.plugins}isLeafTabActive(_){var i;let e=_;if((i=e.parent)!=null&&i.activeLeaf)return e.parent.activeLeaf===_;let n=e.containerEl;if(n!=null&&n.classList.contains("is-active")||n!=null&&n.classList.contains("mod-active"))return!0;let t=e.tabHeaderEl;return!!(t!=null&&t.classList.contains("is-active")||t!=null&&t.classList.contains("mod-active"))}async resolvePageNumberForChunk(_,e){var l,c,d,p,u;if(!_||!e)return null;let n=this.deps.normalizeChunkIdForNote(e,_)||e,t=this.chunkPageCache.get(_);if(t&&t.has(n))return(l=t.get(n))!=null?l:null;let i=(0,oe.normalizePath)(`${H}/${_}.json`);if(!await this.deps.app.vault.adapter.exists(i))return null;let a=await this.deps.readChunkPayload(i),s=Array.isArray(a==null?void 0:a.chunks)?a==null?void 0:a.chunks:[],o=new Map;for(let m of s){let y=typeof(m==null?void 0:m.chunk_id)=="string"?m.chunk_id.trim():"";if(!y)continue;let k=Number.isFinite((c=m==null?void 0:m.page_start)!=null?c:NaN)?Number(m.page_start):null,A=Number.isFinite((d=m==null?void 0:m.page_end)!=null?d:NaN)?Number(m.page_end):null,f=k!=null?k:A;f!==null&&(o.set(y,f),o.set(`${_}:${y}`,f))}return this.chunkPageCache.set(_,o),(u=(p=o.get(n))!=null?p:o.get(e))!=null?u:null}};var ze={"zrr-picker":`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
...
`,"ocr_wordlist.txt":`# zotero-redisearch-rag tool version: 0.6.2
aai
aam
abb
abge
...
\xF6pnv
\xFCberarb
`,"requirements.txt":`# zotero-redisearch-rag tool version: 0.6.2
docling
langcodes[data]
markdown
markdown-it-py
...
spylls
`,"docker-compose.yml":`# zotero-redisearch-rag tool version: 0.6.2
services:
  redis-stack:
...
`)}async renderFrontmatter(e,n,t,i,r,a){var d;let s=(d=this.settings.frontmatterTemplate)!=null?d:"";if(!s.trim())return"";let o=a!=null?a:await this.buildTemplateVars(e,n,t,i,r),l=this.renderTemplate(s,o,"",{appendDocling:!1}).trim(),c=this.stripEmptyFrontmatterFields(l);return this.normalizeFrontmatterKeySpacing(c)}stripEmptyFrontmatterFields(e){if(!e.trim())return"";let n=e.split(/\r?\n/),t=[],i=/^([A-Za-z0-9][A-Za-z0-9 _-]*)\s*:\s*(.*)$/,r=/^[ \t]+-\s*(.*)$/,a=new Set(["abstract"]),s=l=>l===""||l==='""'||l==="''",o=0;for(;o<n.length;){let l=n[o],c=l.match(i);if(!c){t.push(l),o+=1;continue}let d=c[1].trim(),p=c[2].trim();if(a.has(d)){t.push(l),o+=1;continue}if(!s(p)){t.push(l),o+=1;continue}let u=o+1,m=[];for(;u<n.length&&n[u].match(r);)m.push(n[u]),u+=1;if(m.length===0){for(o=u;o<n.length&&n[o].trim()==="";)o+=1;continue}let y=m.filter(k=>{let A=k.match(r);if(!A)return!1;let f=A[1].trim();return!s(f)});if(y.length>0&&t.push(l,...y),o=u,y.length===0)for(;o<n.length&&n[o].trim()==="";)o+=1}for(;t.length>0&&t[t.length-1].trim()==="";)t.pop();return t.join(`

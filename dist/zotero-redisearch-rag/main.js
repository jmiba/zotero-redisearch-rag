...
`,"ocr_wordlist.txt":`# zotero-redisearch-rag tool version: 0.8.2
ackley
afton
alonso
altmetric
...
zaiser
zenodo
`,"requirements.txt":`# zotero-redisearch-rag tool version: 0.8.2
docling
langcodes[data]
markdown
markdown-it-py
...
spylls
`,"docker-compose.yml":`# zotero-redisearch-rag tool version: 0.8.2
services:
  redis-stack:
...
`,"python-worker.Dockerfile":`# zotero-redisearch-rag tool version: 0.8.2
...
`).trim(),excludeFlag:r}),i="",r=!1,a=[])};for(let l of t){let c=l.match(ce);if(c){s();let d=(o=c[1])!=null?o:"",_=d.match(/id=([\"']?)([^\"'\s]+)\1/i),u=_?_[2].trim():"";if(!u)continue;i=u,r=/\bexclude\b/i.test(d)||/\bdelete\b/i.test(d),a=[];continue}if(Ne.test(l)){s();continue}if(i){if(xe.test(l)){r=!0;continue}a.push(l)}}return s(),n}normalizeChunkText(e){return e.split(/\r?\n/).map(t=>t.replace(/\s+/g," ").trim()).filter((t,n,i)=>!(t===""&&i[n-1]==="")).join(`
...
${r}`;let a=this.getAnnotationColorMap(),s=Object.keys(a),o=new Map;for(let u of n){let m=u.colorKey||this.normalizeAnnotationColorKey(u.colorKey),y=(_=o.get(m))!=null?_:[];y.push(u),o.set(m,y)}let l=[i],c=new Set,d=[...s,...Array.from(o.keys()).filter(u=>!s.includes(u))];for(let u of d){let m=o.get(u);if(!m||!m.length)continue;c.add(u);let{heading:y}=this.resolveAnnotationColorMeta(u);l.push("",`## ${y}`),m.sort((b,C)=>{var x,v;let f=(x=b.pageIndex)!=null?x:0,w=(v=C.pageIndex)!=null?v:0;return f!==w?f-w:b.sortIndex!==C.sortIndex?b.sortIndex-C.sortIndex:b.key.localeCompare(C.key)});for(let b of m)l.push(...this.formatAnnotationCallout(b,t,e))}return l.push("",r),l.join(`
`).trim()}formatAnnotationCallout(e,t,n){let i=this.settings.annotationPageLabel||"Page",r=e.pageLabel||(e.pageIndex?String(e.pageIndex):"?"),a=e.pageIndex?String(e.pageIndex):"",s=this.buildZoteroDeepLink(n,t,a,e.key),l=[`> [!${e.callout}] ${i} [${r}](${s})`];e.imagePath&&l.push(`> ![[${e.imagePath}]]`);let c=_=>{if(_)for(let u of _.split(/\r?\n/))l.push(u.trim()?`> ${u}`:">")};c(e.text),e.comment&&(l.push(">","> ---"),c(e.comment)),e.tags.length&&l.push(`> **Tags:** ${e.tags.map(_=>`#${_}`).join(" ")}`);let d=e.pageIndex?String(e.pageIndex):"0";return l.push(`> ^${e.key}a${t}p${d}`),l.push(""),l}findAnnotationBlockRange(e){let t=An.exec(e);if(!t)return null;let n=e.slice(t.index+t[0].length),i=Cn.exec(n);if(!i)return null;let r=t.index,a=t.index+t[0].length+i.index+i[0].length,s=n.slice(0,i.index);return{start:r,end:a,block:s,startMarker:t[0]}}parseAnnotationBlockMarker(e){let t=e.match(/doc_id=([\"']?)([^\"'\s]+)\1/i),n=e.match(/attachment_key=([\"']?)([^\"'\s]+)\1/i);return{docId:t?t[2].trim():void 0,attachmentKey:n?n[2].trim():void 0}}parseAnnotationBlock(e,t){let n=e.split(/\r?\n/),i=[],r=0;for(;r<n.length;){if(!n[r].trim().startsWith("> [!")){r+=1;continue}let s=[];for(;r<n.length&&n[r].trim().startsWith(">");)s.push(n[r]),r+=1;let o=this.parseAnnotationCallout(s,t);o&&i.push(o)}return i}parseAnnotationImageLine(e){var o,l;let t=e.trim();if(!t)return null;let n="",i=t.match(/!\[\[([^\]]+)\]\]/);i&&(n=i[1].trim());let r=n?null:t.match(/!\[[^\]]*\]\(([^)]+)\)/);r&&(n=r[1].trim());let a=n?null:t.match(/<img[^>]+src=[\"']([^\"']+)[\"']/i);if(a&&(n=a[1].trim()),!n)return null;let s=(l=(o=n.split("|")[0])==null?void 0:o.trim())!=null?l:"";return s?{path:s,hash:this.extractAnnotationImageHashFromPath(s)}:null}parseAnnotationCallout(e,t){if(!e.length)return null;let n=e[0].replace(/^>\s?/,"").trim(),i=n.match(/\[!([^\]]+)\]/),r=i?i[1].trim():"note",a=n.match(/\((zotero:\/\/open-pdf\/library\/items\/[^)]+)\)/i),s="",o=t,l="",c=null;if(a){let w=a[1],x=w.match(/items\/([A-Z0-9]{8})/i);x&&(o=x[1]);let v=w.match(/annotation=([A-Z0-9]{8})/i);v&&(s=v[1]);let R=w.match(/page=(\d+)/i);R&&(c=Number(R[1]));let A=n.match(/\[([^\]]+)\]\(zotero:\/\//i);A&&(l=A[1].trim())}let d=[],_=[],u=[],m=!1,y="",b="";for(let w=1;w<e.length;w+=1){let x=e[w].replace(/^>\s?/,""),v=x.trim();if(!v){m?u.push(""):_.push("");continue}let R=this.parseAnnotationImageLine(v);if(R){y=R.path,b=R.hash;continue}if(v.startsWith("^")){let A=v.match(/^\^([A-Z0-9]{8})a([A-Z0-9]{8})p(\d+)/i);A&&(s=A[1],o=A[2],c=Number(A[3]));continue}if(/^(\*\*tags:\*\*|tags:)/i.test(v)){d.push(v);continue}if(v==="---"){m=!0;continue}m?u.push(x):_.push(x)}let C=d.join(" "),f=C?C.replace(/\*\*tags:\*\*/i,"").replace(/tags:/i,"").split(/[\s,]+/).map(w=>w.trim().replace(/^#+/,"")).filter(Boolean):[];return s?{key:s,attachmentKey:o,pageLabel:l,pageIndex:c,callout:r,text:this.normalizeAnnotationText(_.join(`
...
`)}async renderFrontmatter(e,t,n,i,r,a){var d;let s=(d=this.settings.frontmatterTemplate)!=null?d:"";if(!s.trim())return"";let o=a!=null?a:await this.buildTemplateVars(e,t,n,i,r),l=this.renderTemplate(s,o,"",{appendDocling:!1}).trim(),c=this.stripEmptyFrontmatterFields(l);return this.normalizeFrontmatterKeySpacing(c)}stripEmptyFrontmatterFields(e){if(!e.trim())return"";let t=e.split(/\r?\n/),n=[],i=/^([A-Za-z0-9][A-Za-z0-9 _-]*)\s*:\s*(.*)$/,r=/^[ \t]+-\s*(.*)$/,a=new Set(["abstract"]),s=l=>l===""||l==='""'||l==="''",o=0;for(;o<t.length;){let l=t[o],c=l.match(i);if(!c){n.push(l),o+=1;continue}let d=c[1].trim(),_=c[2].trim();if(a.has(d)){n.push(l),o+=1;continue}if(!s(_)){n.push(l),o+=1;continue}let u=o+1,m=[];for(;u<t.length&&t[u].match(r);)m.push(t[u]),u+=1;if(m.length===0){for(o=u;o<t.length&&t[o].trim()==="";)o+=1;continue}let y=m.filter(b=>{let C=b.match(r);if(!C)return!1;let f=C[1].trim();return!s(f)});if(y.length>0&&n.push(l,...y),o=u,y.length===0)for(;o<t.length&&t[o].trim()==="";)o+=1}for(;n.length>0&&n[n.length-1].trim()==="";)n.pop();return n.join(`

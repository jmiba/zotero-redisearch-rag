"use strict";var Re=Object.create;var X=Object.defineProperty;var Ne=Object.getOwnPropertyDescriptor;var Te=Object.getOwnPropertyNames;var Oe=Object.getPrototypeOf,ze=Object.prototype.hasOwnProperty;var Ie=(v,p)=>{for(var e in p)X(v,e,{get:p[e],enumerable:!0})},be=(v,p,e,n)=>{if(p&&typeof p=="object"||typeof p=="function")for(let t of Te(p))!ze.call(v,t)&&t!==e&&X(v,t,{get:()=>p[t],enumerable:!(n=Ne(p,t))||n.enumerable});return v};var ee=(v,p,e)=>(e=v!=null?Re(Oe(v)):{},be(p||!v||!v.__esModule?X(e,"default",{value:v,enumerable:!0}):e,v)),Fe=v=>be(X({},"__esModule",{value:!0}),v);var Ke={};Ie(Ke,{default:()=>se});module.exports=Fe(Ke);var g=require("obsidian"),Y=require("@codemirror/state"),I=require("@codemirror/view"),G=require("child_process"),z=require("fs"),Se=ee(require("http")),Ee=ee(require("https")),de=ee(require("net")),b=ee(require("path")),H=require("url"),te=require("crypto");var E=require("obsidian"),B=".zotero-redisearch-rag",U=`${B}/items`,j=`${B}/chunks`,xe={zoteroBaseUrl:"http://127.0.0.1:23119/api",zoteroUserId:"0",webApiBaseUrl:"https://api.zotero.org",webApiLibraryType:"user",webApiLibraryId:"",webApiKey:"",pythonPath:"python3",dockerPath:"docker",autoStartRedis:!1,copyPdfToVault:!0,frontmatterTemplate:`doc_id: {{doc_id}}
...
# zotero-redisearch-rag tool version: 0.3.1
import argparse
import json
import math
import logging
...
`,"ocr_wordlist.txt":`# zotero-redisearch-rag tool version: 0.3.1
aai
aam
abb
abge
...
\xF6pnv
\xFCberarb
`,"requirements.txt":`# zotero-redisearch-rag tool version: 0.3.1
docling
paddleocr
paddlex[ocr]
paddlepaddle==3.2.2
...
spylls
`,"docker-compose.yml":`# zotero-redisearch-rag tool version: 0.3.1
services:
  redis-stack:
...
`).replace(/\s+/g," ").trim()}scheduleNoteSync(e){let n=this.noteSyncTimers.get(e.path);n!==void 0&&window.clearTimeout(n);let t=window.setTimeout(()=>{this.noteSyncTimers.delete(e.path),this.syncNoteToRedis(e)},1200);this.noteSyncTimers.set(e.path,t)}escapeRegExp(e){return e.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")}formatCitationPageLabel(e){let n=e.page_start?String(e.page_start):"",t=e.page_end?String(e.page_end):"";if(n&&(!t||n===t))return n;if(n&&t)return`${n} - ${t}`;let i=(e.pages||"").trim();if(!i)return"?";let r=i.match(/^(\d+)\s*-\s*(\d+)$/);return r?r[1]===r[2]?r[1]:`${r[1]} - ${r[2]}`:i.replace("-"," - ")}normalizeChunkIdForNote(e,n){if(!e)return null;let t=String(e);if(n&&t.startsWith(`${n}:`))return t.slice(n.length+1);if(t.includes(":")){let i=t.split(":");if(i.length>1&&n&&i[0]===n)return i.slice(1).join(":")}return t}async syncNoteToRedis(e){var n,t,i,r;if(!this.noteSyncInFlight.has(e.path)&&!this.noteSyncSuppressed.has(e.path)){this.noteSyncInFlight.add(e.path);try{let s=await this.app.vault.read(e),a=this.extractSyncSection(s);if(!a)return;let o=(n=this.extractDocIdFromFrontmatter(s))!=null?n:this.extractDocIdFromSyncMarker(s);if(!o)return;let l=this.parseSyncedChunkBlocks(a);if(!l.length)return;let d=(0,g.normalizePath)(`${j}/${o}.json`),h=this.app.vault.adapter;if(!await h.exists(d))return;let _=await this.readChunkPayload(d);if(!_)return;let f=Array.isArray(_.chunks)?_.chunks:[],y=new Map;for(let S of f){let D=typeof(S==null?void 0:S.chunk_id)=="string"?S.chunk_id:String((t=S==null?void 0:S.chunk_id)!=null?t:"");D&&y.set(D,S)}let m=new Set,x=new Set,k=new Set,C=new Set,A=!1;for(let S of l){let D=S.chunkId;if(!D)continue;m.add(D);let N=y.get(D);if(!N){console.warn(`Sync note: chunk id not found in cache (${D})`);continue}if(S.excludeFlag){N.excluded!==!0&&(N.excluded=!0,A=!0);let P=this.normalizeChunkText(S.text);P&&P!==String((i=N.text)!=null?i:"")&&(N.text=P,N.char_count=P.length,A=!0),k.add(D);continue}if(N.excluded&&(N.excluded=!1,A=!0,x.add(D)),!S.text.trim()){k.add(D),C.add(D);continue}let R=this.normalizeChunkText(S.text);if(!R){k.add(D),C.add(D);continue}let O=typeof N.text=="string"?N.text:String((r=N.text)!=null?r:"");R!==O&&(N.text=R,N.char_count=R.length,x.add(D),A=!0)}for(let S of y.keys())m.has(S)||(k.add(S),C.add(S));if(!x.size&&!k.size&&!C.size&&!A)return;C.size&&(_.chunks=f.filter(S=>{var N;let D=typeof(S==null?void 0:S.chunk_id)=="string"?S.chunk_id:String((N=S==null?void 0:S.chunk_id)!=null?N:"");return D&&!C.has(D)}),A=!0),(A||C.size)&&await h.write(d,JSON.stringify(_,null,2)),await this.reindexChunkUpdates(o,d,Array.from(x),Array.from(k))}catch(s){console.warn("Failed to sync note edits to Redis",s)}finally{this.noteSyncInFlight.delete(e.path)}}}extractSyncSection(e){let n=K.exec(e);if(!n)return null;let t=e.slice(n.index+n[0].length),i=J.exec(t);return i?t.slice(0,i.index):null}extractDocIdFromSyncMarker(e){var r;let n=K.exec(e);if(!n)return null;let i=((r=n[0])!=null?r:"").match(/doc_id=([\"']?)([^\"'\s]+)\1/i);return i?i[2].trim():null}parseSyncedChunkBlocks(e){var o;let n=e.split(/\r?\n/),t=[],i="",r=!1,s=[],a=()=>{i&&(t.push({chunkId:i,text:s.join(`

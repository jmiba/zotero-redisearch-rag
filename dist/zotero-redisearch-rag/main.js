...
`);for(let p of d){let g=String(p.doc_id||"").trim(),y=String(p.chunk_id||"").trim(),h=String(p.page_start||"").trim(),C=String(p.page_end||"").trim(),_=String(p.title||"").trim(),f=String(p.section||"").trim(),x=String(p.score||"").trim(),P=String(p.authors||"").trim(),S=String(p.item_type||"").trim(),L=String(p.year||"").trim(),w=String(p.tags||"").trim(),R=String(p.chunk_tags||"").trim(),I=String(p.attachment_key||"").trim(),j=String(p.source_pdf||"").trim(),D=String(p.text||"").replace(/\s+/g," ").trim(),q=D.length>220?`${D.slice(0,220)}\u2026`:D,T=[g];y&&T.push(y),(h||C)&&T.push(`p.${h||"?"}-${C||"?"}`),c.push(T.filter(Boolean).join(" \u2022 ")),x&&c.push(`  score: ${x}`),_&&c.push(`  title: ${_}`),P&&c.push(`  authors: ${P}`),L&&c.push(`  year: ${L}`),S&&c.push(`  item_type: ${S}`),w&&c.push(`  tags: ${w}`),R&&c.push(`  chunk_tags: ${R}`),I&&c.push(`  attachment_key: ${I}`),f&&c.push(`  section: ${f}`),j&&c.push(`  source_pdf: ${j}`),q&&c.push(`  ${q}`),c.push("")}return c.join(`

...
`);for(let _ of d){let g=String(_.doc_id||"").trim(),y=String(_.chunk_id||"").trim(),b=String(_.page_start||"").trim(),L=String(_.page_end||"").trim(),f=String(_.title||"").trim(),h=String(_.section||"").trim(),C=String(_.score||"").trim(),v=String(_.authors||"").trim(),E=String(_.item_type||"").trim(),P=String(_.year||"").trim(),w=String(_.tags||"").trim(),D=String(_.chunk_tags||"").trim(),I=String(_.attachment_key||"").trim(),z=String(_.source_pdf||"").trim(),N=String(_.text||"").replace(/\s+/g," ").trim(),F=N.length>220?`${N.slice(0,220)}\u2026`:N,O=[g];y&&O.push(y),(b||L)&&O.push(`p.${b||"?"}-${L||"?"}`),c.push(O.filter(Boolean).join(" \u2022 ")),C&&c.push(`  score: ${C}`),f&&c.push(`  title: ${f}`),v&&c.push(`  authors: ${v}`),P&&c.push(`  year: ${P}`),E&&c.push(`  item_type: ${E}`),w&&c.push(`  tags: ${w}`),D&&c.push(`  chunk_tags: ${D}`),I&&c.push(`  attachment_key: ${I}`),h&&c.push(`  section: ${h}`),z&&c.push(`  source_pdf: ${z}`),F&&c.push(`  ${F}`),c.push("")}return c.join(`

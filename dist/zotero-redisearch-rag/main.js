"use strict";var De=Object.create;var X=Object.defineProperty;var Ne=Object.getOwnPropertyDescriptor;var Te=Object.getOwnPropertyNames;var Oe=Object.getPrototypeOf,ze=Object.prototype.hasOwnProperty;var Ie=(P,u)=>{for(var e in u)X(P,e,{get:u[e],enumerable:!0})},be=(P,u,e,n)=>{if(u&&typeof u=="object"||typeof u=="function")for(let t of Te(u))!ze.call(P,t)&&t!==e&&X(P,t,{get:()=>u[t],enumerable:!(n=Ne(u,t))||n.enumerable});return P};var ee=(P,u,e)=>(e=P!=null?De(Oe(P)):{},be(u||!P||!P.__esModule?X(e,"default",{value:P,enumerable:!0}):e,P)),Fe=P=>be(X({},"__esModule",{value:!0}),P);var Ke={};Ie(Ke,{default:()=>se});module.exports=Fe(Ke);var p=require("obsidian"),Y=require("@codemirror/state"),I=require("@codemirror/view"),G=require("child_process"),z=require("fs"),Se=ee(require("http")),Ee=ee(require("https")),de=ee(require("net")),b=ee(require("path")),H=require("url"),te=require("crypto");var L=require("obsidian"),B=".zotero-redisearch-rag",Z=`${B}/items`,j=`${B}/chunks`,xe={zoteroBaseUrl:"http://127.0.0.1:23119/api",zoteroUserId:"0",webApiBaseUrl:"https://api.zotero.org",webApiLibraryType:"user",webApiLibraryId:"",webApiKey:"",pythonPath:"python3",dockerPath:"docker",autoStartRedis:!1,copyPdfToVault:!0,frontmatterTemplate:`doc_id: {{doc_id}}
...
# zotero-redisearch-rag tool version: 0.3.2
import argparse
import json
import math
import logging
...
`,"ocr_wordlist.txt":`# zotero-redisearch-rag tool version: 0.3.2
aai
aam
abb
abge
...
\xF6pnv
\xFCberarb
`,"requirements.txt":`# zotero-redisearch-rag tool version: 0.3.2
docling
paddleocr
paddlex[ocr]
paddlepaddle==3.2.2
...
spylls
`,"docker-compose.yml":`# zotero-redisearch-rag tool version: 0.3.2
services:
  redis-stack:
...
`).replace(/\s+/g," ").trim()}scheduleNoteSync(e){let n=this.noteSyncTimers.get(e.path);n!==void 0&&window.clearTimeout(n);let t=window.setTimeout(()=>{this.noteSyncTimers.delete(e.path),this.syncNoteToRedis(e)},1200);this.noteSyncTimers.set(e.path,t)}escapeRegExp(e){return e.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")}formatCitationPageLabel(e){let n=e.page_start?String(e.page_start):"",t=e.page_end?String(e.page_end):"";if(n&&(!t||n===t))return n;if(n&&t)return`${n} - ${t}`;let i=(e.pages||"").trim();if(!i)return"?";let r=i.match(/^(\d+)\s*-\s*(\d+)$/);return r?r[1]===r[2]?r[1]:`${r[1]} - ${r[2]}`:i.replace("-"," - ")}normalizeChunkIdForNote(e,n){if(!e)return null;let t=String(e);if(n&&t.startsWith(`${n}:`))return t.slice(n.length+1);if(t.includes(":")){let i=t.split(":");if(i.length>1&&n&&i[0]===n)return i.slice(1).join(":")}return t}async syncNoteToRedis(e){var n,t,i,r;if(!this.noteSyncInFlight.has(e.path)&&!this.noteSyncSuppressed.has(e.path)){this.noteSyncInFlight.add(e.path);try{let s=await this.app.vault.read(e),a=this.extractSyncSection(s);if(!a)return;let o=(n=this.extractDocIdFromFrontmatter(s))!=null?n:this.extractDocIdFromSyncMarker(s);if(!o)return;let l=this.parseSyncedChunkBlocks(a);if(!l.length)return;let c=(0,p.normalizePath)(`${j}/${o}.json`),h=this.app.vault.adapter;if(!await h.exists(c))return;let _=await this.readChunkPayload(c);if(!_)return;let f=Array.isArray(_.chunks)?_.chunks:[],y=new Map;for(let S of f){let R=typeof(S==null?void 0:S.chunk_id)=="string"?S.chunk_id:String((t=S==null?void 0:S.chunk_id)!=null?t:"");R&&y.set(R,S)}let m=new Set,x=new Set,w=new Set,k=new Set,E=!1;for(let S of l){let R=S.chunkId;if(!R)continue;m.add(R);let D=y.get(R);if(!D){console.warn(`Sync note: chunk id not found in cache (${R})`);continue}if(S.excludeFlag){D.excluded!==!0&&(D.excluded=!0,E=!0);let C=this.normalizeChunkText(S.text);C&&C!==String((i=D.text)!=null?i:"")&&(D.text=C,D.char_count=C.length,E=!0),w.add(R);continue}if(D.excluded&&(D.excluded=!1,E=!0,x.add(R)),!S.text.trim()){w.add(R),k.add(R);continue}let N=this.normalizeChunkText(S.text);if(!N){w.add(R),k.add(R);continue}let O=typeof D.text=="string"?D.text:String((r=D.text)!=null?r:"");N!==O&&(D.text=N,D.char_count=N.length,x.add(R),E=!0)}for(let S of y.keys())m.has(S)||(w.add(S),k.add(S));if(!x.size&&!w.size&&!k.size&&!E)return;k.size&&(_.chunks=f.filter(S=>{var D;let R=typeof(S==null?void 0:S.chunk_id)=="string"?S.chunk_id:String((D=S==null?void 0:S.chunk_id)!=null?D:"");return R&&!k.has(R)}),E=!0),(E||k.size)&&await h.write(c,JSON.stringify(_,null,2)),await this.reindexChunkUpdates(o,c,Array.from(x),Array.from(w))}catch(s){console.warn("Failed to sync note edits to Redis",s)}finally{this.noteSyncInFlight.delete(e.path)}}}extractSyncSection(e){let n=K.exec(e);if(!n)return null;let t=e.slice(n.index+n[0].length),i=J.exec(t);return i?t.slice(0,i.index):null}extractDocIdFromSyncMarker(e){var r;let n=K.exec(e);if(!n)return null;let i=((r=n[0])!=null?r:"").match(/doc_id=([\"']?)([^\"'\s]+)\1/i);return i?i[2].trim():null}parseSyncedChunkBlocks(e){var o;let n=e.split(/\r?\n/),t=[],i="",r=!1,s=[],a=()=>{i&&(t.push({chunkId:i,text:s.join(`
`).trim(),excludeFlag:r}),i="",r=!1,s=[])};for(let l of n){let c=l.match(Q);if(c){a();let h=(o=c[1])!=null?o:"",_=h.match(/id=([\"']?)([^\"'\s]+)\1/i),f=_?_[2].trim():"";if(!f)continue;i=f,r=/\bexclude\b/i.test(h)||/\bdelete\b/i.test(h),s=[];continue}if(pe.test(l)){a();continue}if(i){if(ie.test(l)){r=!0;continue}s.push(l)}}return a(),t}normalizeChunkText(e){return e.split(/\r?\n/).map(n=>n.replace(/\s+/g," ").trim()).filter((n,t,i)=>!(n===""&&i[t-1]==="")).join(`

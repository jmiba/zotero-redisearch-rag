...
item_json: {{item_json_yaml}}`,tagSanitizeMode:"kebab",noteBodyTemplate:"{{annotation_block}}{{docling_markdown}}",annotationPageLabel:"Seite",annotationColorMap:{yellow:{heading:"Questions",callout:"question"},red:{heading:"Problems/Critique",callout:"bug"},green:{heading:"Main Ideas",callout:"idea"},blue:{heading:"Facts",callout:"fact"},purple:{heading:"Arguments/Solutions",callout:"argument"},magenta:{heading:"Opinions",callout:"opinion"},orange:{heading:"Follow Up",callout:"pursue"},gray:{heading:"Citable Passages",callout:"cite"}},includeAnnotationImages:!0,zoteroCompanionEnabled:!1,zoteroCompanionBaseUrl:"http://127.0.0.1:23120",zoteroCompanionToken:"",llmProviderProfiles:[{id:"lm-studio",name:"LM Studio",baseUrl:"http://localhost:1234/v1",apiKey:"lm-studio"},{id:"ollama",name:"Ollama",baseUrl:"http://localhost:11434/v1",apiKey:""},{id:"openrouter",name:"OpenRouter",baseUrl:"https://openrouter.ai/api/v1",apiKey:""},{id:"openai",name:"OpenAI",baseUrl:"https://api.openai.com/v1",apiKey:""}],chatOutputDir:"Zotero/Chats",chatExportTemplatePath:"",chatExportPostCreateCommandId:"",copyPdfToVault:!0,createOcrLayeredPdf:!1,preferObsidianNoteForCitations:!0,ocrMode:"auto",ocrQualityThreshold:.5,chunkingMode:"page",ocrEngine:"auto",forcePerPageOcr:!1,paddleApiKey:"",paddleVlApiUrl:"",paddleStructureApiUrl:"",enableLlmCleanup:!1,llmCleanupProviderProfileId:"lm-studio",llmCleanupBaseUrl:"http://127.0.0.1:1234/v1",llmCleanupApiKey:"",llmCleanupModel:"openai/gpt-oss-20b",llmCleanupTemperature:0,llmCleanupMinQuality:.35,llmCleanupMaxChars:2e3,embedProviderProfileId:"lm-studio",embedBaseUrl:"http://localhost:1234/v1",embedApiKey:"lm-studio",embedModel:"google/embedding-gemma-300m",embedIncludeMetadata:!0,embedSubchunkChars:3500,embedSubchunkOverlap:200,embedContextWindow:1,embedContextChars:220,enableChunkTagging:!1,chatProviderProfileId:"lm-studio",chatBaseUrl:"http://127.0.0.1:1234/v1",chatApiKey:"",chatModel:"openai/gpt-oss-20b",chatTemperature:.2,chatHistoryMessages:6,lastSeenReleaseNotesVersion:"",chatPaneLocation:"right",enableAgenticRag:!1,agenticMaxIters:2,enableQueryExpansion:!1,queryExpansionCount:3,enableCrossEncoderRerank:!1,rerankModel:Ae,rerankCandidateMultiplier:4,rrfK:60,rrfLogTop:0,maxChunksPerDoc:0,enableFileLogging:!1,logFilePath:`${ae}/logs/docling_extract.log`,redisIndex:"idx:zotero",redisPrefix:"zotero:chunk:"},He=class extends L.PluginSettingTab{constructor(e,t){super(e,t);this.activeTab="prerequisites";this.companionTokenInput=null;this.plugin=t}display(){let{containerEl:e}=this;e.empty();let t=()=>Array.isArray(this.plugin.settings.llmProviderProfiles)?this.plugin.settings.llmProviderProfiles:[],n=async h=>{this.plugin.settings.llmProviderProfiles=h,await this.plugin.saveSettings()},i=h=>{h.inputEl.type="password",h.inputEl.autocomplete="off",h.inputEl.spellcheck=!1},d=[{id:"prerequisites",label:"Prerequisites",render:h=>{new L.Setting(h).setName("Prerequisites").setHeading();let b=null,w=null,P=null,S=null,v=null,C=null,N=null,D=null,F=null,V=(I,z)=>{I&&(I.settingEl.classList.toggle("is-disabled",z),I.settingEl.classList.toggle("zrr-setting-disabled",z))},U=(I,z)=>{I&&I.settingEl.classList.toggle("zrr-setting-hidden",!z)},j=()=>{let I=!!this.plugin.settings.showAdvancedPythonRuntimeOptions,z=this.plugin.settings.pythonRuntime==="local",q=I&&z;w==null||w.setDisabled(!I),V(b,!I),U(P,I),U(S,I),U(v,I),C==null||C.setDisabled(!q),N==null||N.setDisabled(!q),F==null||F.setDisabled(!q),D&&(D.disabled=!q),V(P,!q),V(S,!q),V(v,!q)};new L.Setting(h).setName("Docker/podman path").setDesc("CLI path for Docker or Podman (used to start Redis Stack and the Python worker). Leave as 'docker'/'podman' to auto-detect via PATH and common locations without saving an absolute path (keeps cross-OS sync). Supports ~, $HOME, and %USERPROFILE%. Relative paths with separators resolve from your home dir.").addText(I=>I.setPlaceholder("Docker").setValue(this.plugin.settings.dockerPath).onChange(async z=>{this.plugin.settings.dockerPath=z.trim()||"docker",await this.plugin.saveSettings()})),new L.Setting(h).setName("Advanced python runtime options").setDesc("Show legacy local interpreter/venv controls. Recommended to keep this disabled.").addToggle(I=>I.setValue(this.plugin.settings.showAdvancedPythonRuntimeOptions).onChange(async z=>{this.plugin.settings.showAdvancedPythonRuntimeOptions=z,!z&&this.plugin.settings.pythonRuntime==="local"&&(this.plugin.settings.pythonRuntime="worker",new L.Notice("Switched python runtime to worker mode.")),await this.plugin.saveSettings(),j()})),b=new L.Setting(h).setName("Python runtime").setDesc("Worker runtime is recommended. Enable advanced options to expose local interpreter/venv mode.").addDropdown(I=>{w=I,I.addOption("worker","Python worker container (recommended)"),I.addOption("local","Local interpreter/venv"),I.setValue(this.plugin.settings.pythonRuntime).onChange(async z=>{z!=="worker"&&z!=="local"||(this.plugin.settings.pythonRuntime=z,await this.plugin.saveSettings(),j())})}),P=new L.Setting(h).setName("Python path").setDesc("Local mode only: optional path to the Python interpreter used to create or run the plugin env. Leave blank to auto-detect (python3.13/3.12/3.11/3.10/python3/python, or py on Windows). Supports ~, $HOME, and %USERPROFILE%. Relative paths with separators resolve from your home dir.").addText(I=>(C=I,I.setPlaceholder("Auto-detect").setValue(this.plugin.settings.pythonPath).onChange(async z=>{this.plugin.settings.pythonPath=z.trim(),await this.plugin.saveSettings()}))),S=new L.Setting(h).setName("Python environment").setDesc("Local mode: create or update the plugin's python env. Worker mode: managed by docker startup.").addButton(I=>{F=I,D=I.buttonEl,I.setButtonText("Create/update").setCta(),I.onClick(async()=>{I.setDisabled(!0);try{await this.plugin.setupPythonEnv()}finally{I.setDisabled(this.plugin.settings.pythonRuntime!=="local")}})}),v=new L.Setting(h).setName("Python env location").setDesc("Local mode only: shared user cache can be reused across vaults; plugin folder keeps a per-vault env.").addDropdown(I=>{N=I,I.addOption("shared","Shared user cache"),I.addOption("plugin","Plugin folder (.venv)"),I.setValue(this.plugin.settings.pythonEnvLocation).onChange(async z=>{z!=="shared"&&z!=="plugin"||(this.plugin.settings.pythonEnvLocation=z,await this.plugin.saveSettings())})}),j(),new L.Setting(h).setName("Redis URL").addText(I=>I.setPlaceholder("Redis://127.0.0.1:6379").setValue(this.plugin.settings.redisUrl).onChange(async z=>{this.plugin.settings.redisUrl=z.trim(),await this.plugin.saveSettings()}));let B=null,T=null,G=null,Z=null,W=()=>{let I=this.plugin.settings.autoAssignRedisPort;G==null||G.setDisabled(I),Z==null||Z.setDisabled(I),B==null||B.settingEl.classList.toggle("is-disabled",I),T==null||T.settingEl.classList.toggle("is-disabled",I)};new L.Setting(h).setName("Auto-assign redis port").setDesc("When starting redis stack, pick a free local port and update the redis URL.").addToggle(I=>I.setValue(this.plugin.settings.autoAssignRedisPort).onChange(async z=>{this.plugin.settings.autoAssignRedisPort=z,await this.plugin.saveSettings(),W()})),B=new L.Setting(h).setName("Redis data directory override").setDesc("Optional path to store Redis persistence when auto-assign is off. Env var ZRR_DATA_DIR overrides this setting. Supports ~, $HOME, and %USERPROFILE%. Relative paths with separators resolve from your home dir; use ./ to keep it vault-relative.").addText(I=>(G=I,I.setPlaceholder("~/redis/zrr-data").setValue(this.plugin.settings.redisDataDirOverride).onChange(async z=>{this.plugin.settings.redisDataDirOverride=z.trim(),await this.plugin.saveSettings()}))),B.settingEl.addClass("zrr-redis-override-setting"),T=new L.Setting(h).setName("Redis project name override").setDesc("Optional Docker/Podman Compose project name when auto-assign is off. Env var ZRR_PROJECT_NAME overrides this setting.").addText(I=>(Z=I,I.setPlaceholder("Zrr-shared").setValue(this.plugin.settings.redisProjectName).onChange(async z=>{this.plugin.settings.redisProjectName=z.trim(),await this.plugin.saveSettings()}))),T.settingEl.addClass("zrr-redis-override-setting"),W(),new L.Setting(h).setName("Auto-start redis stack (docker/podman compose)").setDesc("Requires Docker Desktop running and your vault path shared with Docker. Starts Redis and, in worker mode, the Python worker container.").addToggle(I=>I.setValue(this.plugin.settings.autoStartRedis).onChange(async z=>{this.plugin.settings.autoStartRedis=z,await this.plugin.saveSettings()})),new L.Setting(h).setName("Start redis stack now").setDesc("Starts or restarts redis stack (and python worker in worker mode).").addButton(I=>I.setButtonText("Start").onClick(async()=>{await this.plugin.startRedisStack()}))}},{id:"zotero-import",label:"Zotero import",render:h=>{new L.Setting(h).setName("Zotero local API").setHeading(),new L.Setting(h).setName("Zotero base URL").setDesc("Local Zotero API base URL, e.g. HTTP://127.0.0.1:23119/API").addText(v=>v.setPlaceholder("HTTP://127.0.0.1:23119/API").setValue(this.plugin.settings.zoteroBaseUrl).onChange(async C=>{this.plugin.settings.zoteroBaseUrl=C.trim(),await this.plugin.saveSettings()}));let b=new L.Setting(h).setName("Zotero library").setDesc("Select your local library or a Zotero group library."),w=null,P=v=>{if(!w)return;let C=(this.plugin.settings.zoteroUserId||"0").trim()||"0";new Set(v.map(D=>D.value)).has(C)||(v=v.concat([{value:C,label:`Custom (${C})`}])),w.selectEl.options.length=0;for(let D of v)w.addOption(D.value,D.label);w.setValue(C)},S=async()=>{if(w){w.setDisabled(!0);try{let v=await this.plugin.fetchZoteroLibraryOptions();P(v)}finally{w.setDisabled(!1)}}};b.addDropdown(v=>{w=v;let C=(this.plugin.settings.zoteroUserId||"0").trim()||"0";v.addOption(C,"Loading..."),v.setValue(C),v.onChange(async N=>{this.plugin.settings.zoteroUserId=N.trim(),await this.plugin.saveSettings()})}),b.addButton(v=>{v.setButtonText("Refresh").onClick(async()=>{await S()})}),S(),new L.Setting(h).setName("Zotero web API").setHeading(),new L.Setting(h).setName("Web API base URL").setDesc("Zotero Web API base URL for write fallback, e.g. https://api.zotero.org").addText(v=>v.setPlaceholder("https://api.zotero.org").setValue(this.plugin.settings.webApiBaseUrl).onChange(async C=>{this.plugin.settings.webApiBaseUrl=C.trim(),await this.plugin.saveSettings()})),new L.Setting(h).setName("Web API library type").setDesc("Library type for web API writes.").addDropdown(v=>v.addOption("user","User").addOption("group","Group").setValue(this.plugin.settings.webApiLibraryType).onChange(async C=>{this.plugin.settings.webApiLibraryType=C,await this.plugin.saveSettings()})),new L.Setting(h).setName("Web API library ID").setDesc("Numeric Zotero user/group ID for web API writes.").addText(v=>v.setPlaceholder("15218").setValue(this.plugin.settings.webApiLibraryId).onChange(async C=>{this.plugin.settings.webApiLibraryId=C.trim(),await this.plugin.saveSettings()})),new L.Setting(h).setName("Web API key").setDesc("Zotero API key for write fallback (from Zotero.org).").addText(v=>{i(v),v.setPlaceholder("Your-api-key").setValue(this.plugin.settings.webApiKey).onChange(async C=>{this.plugin.settings.webApiKey=C.trim(),await this.plugin.saveSettings()})}),new L.Setting(h).setName("Output").setHeading(),new L.Setting(h).setName("PDF folder").addText(v=>v.setPlaceholder("Zotero/pdfs").setValue(this.plugin.settings.outputPdfDir).onChange(async C=>{this.plugin.settings.outputPdfDir=C.trim(),await this.plugin.saveSettings()})),new L.Setting(h).setName("Notes folder").addText(v=>v.setPlaceholder("Zotero/notes").setValue(this.plugin.settings.outputNoteDir).onChange(async C=>{this.plugin.settings.outputNoteDir=C.trim(),await this.plugin.saveSettings()})),new L.Setting(h).setName("Frontmatter template").setDesc("Template for note YAML frontmatter. Use {{var}} placeholders; leave blank to omit.").addTextArea(v=>{v.setValue(this.plugin.settings.frontmatterTemplate).onChange(async C=>{this.plugin.settings.frontmatterTemplate=C,await this.plugin.saveSettings()}),v.inputEl.rows=10,v.inputEl.addClass("zrr-u-width-100")}),new L.Setting(h).setName("Tag sanitization").setDesc("Normalize Zotero tags for Obsidian (no spaces, punctuation trimmed).").addDropdown(v=>v.addOption("none","No change").addOption("camel","Camelcase").addOption("pascal","Pascalcase").addOption("snake","Snake_case").addOption("kebab","Kebab-case").setValue(this.plugin.settings.tagSanitizeMode==="replace"?"kebab":this.plugin.settings.tagSanitizeMode).onChange(async C=>{this.plugin.settings.tagSanitizeMode=C,await this.plugin.saveSettings()})),new L.Setting(h).setName("Note body template").setDesc("Template for the note body after frontmatter. Use {{pdf_block}}, {{annotation_block}}, and {{docling_markdown}} placeholders.").addTextArea(v=>{v.setValue(this.plugin.settings.noteBodyTemplate).onChange(async C=>{this.plugin.settings.noteBodyTemplate=C,await this.plugin.saveSettings()}),v.inputEl.rows=8,v.inputEl.addClass("zrr-u-width-100")}),new L.Setting(h).setName("Saved chats folder").setDesc("Where exported chat notes are stored (vault-relative).").addText(v=>v.setPlaceholder("Zotero/chats").setValue(this.plugin.settings.chatOutputDir).onChange(async C=>{this.plugin.settings.chatOutputDir=C.trim()||"zotero/chats",await this.plugin.saveSettings()})),new L.Setting(h).setName("Chat export template").setDesc("Optional vault-relative template file for copied chat notes. Use {{chat_body}} to control where the transcript is inserted (otherwise template text is appended after the transcript).").addText(v=>v.setPlaceholder("Path to chat export template note").setValue(this.plugin.settings.chatExportTemplatePath).onChange(async C=>{this.plugin.settings.chatExportTemplatePath=C.trim(),await this.plugin.saveSettings()})),new L.Setting(h).setName("Chat export post-create command").setDesc("Optional command ID to run after the chat note is created and opened. Useful for templater workflows that need the full note body already present.").addText(v=>v.setPlaceholder("Command ID to run after note creation").setValue(this.plugin.settings.chatExportPostCreateCommandId).onChange(async C=>{this.plugin.settings.chatExportPostCreateCommandId=C.trim(),await this.plugin.saveSettings()})),new L.Setting(h).setName("Copy pdfs into vault").setDesc("Disable to use Zotero storage paths directly. If a local file path is unavailable, the plugin temporarily copies the PDF into the vault for processing.").addToggle(v=>v.setValue(this.plugin.settings.copyPdfToVault).onChange(async C=>{this.plugin.settings.copyPdfToVault=C,!C&&this.plugin.settings.createOcrLayeredPdf&&(this.plugin.settings.createOcrLayeredPdf=!1),await this.plugin.saveSettings(),this.display()})),new L.Setting(h).setName("Create ocr-layered PDF copy").setDesc("When ocr is used, replace the vault PDF with a tesseract text layer (requires copy pdfs into vault).").addToggle(v=>{let C=this.plugin.settings.copyPdfToVault;v.setValue(C?this.plugin.settings.createOcrLayeredPdf:!1).setDisabled(!C).onChange(async N=>{if(!this.plugin.settings.copyPdfToVault){this.plugin.settings.createOcrLayeredPdf=!1,await this.plugin.saveSettings();return}this.plugin.settings.createOcrLayeredPdf=N,await this.plugin.saveSettings()})}),new L.Setting(h).setName("Prefer Obsidian note for citations").setDesc("Link citations to the Obsidian note when available; otherwise use Zotero deep links.").addToggle(v=>v.setValue(this.plugin.settings.preferObsidianNoteForCitations).onChange(async C=>{this.plugin.settings.preferObsidianNoteForCitations=C,await this.plugin.saveSettings()}))}},{id:"annotations",label:"Annotations",render:h=>{new L.Setting(h).setName("Annotations").setHeading(),new L.Setting(h).setName("Annotation page label").setDesc("Label shown before the page link in annotation callouts.").addText(S=>S.setPlaceholder("Page").setValue(this.plugin.settings.annotationPageLabel).onChange(async v=>{this.plugin.settings.annotationPageLabel=v.trim()||"Page",await this.plugin.saveSettings()})),new L.Setting(h).setName("Include annotation images").setDesc("Embed image/rect annotations as images in callouts when available.").addToggle(S=>S.setValue(this.plugin.settings.includeAnnotationImages).onChange(async v=>{this.plugin.settings.includeAnnotationImages=v,await this.plugin.saveSettings()})),new L.Setting(h).setName("Zotero companion").setHeading(),h.createEl("p",{text:"Install the Zotero companion add-on to enable cached image/rect annotations. See tab Maintenance for instructions."}),new L.Setting(h).setName("Use Zotero companion for annotation images").setDesc("Fetch cached annotation images from a local Zotero companion plugin.").addToggle(S=>S.setValue(this.plugin.settings.zoteroCompanionEnabled).onChange(async v=>{this.plugin.settings.zoteroCompanionEnabled=v,await this.plugin.saveSettings()})),new L.Setting(h).setName("Zotero companion base URL").setDesc("Local URL for the Zotero companion plugin (loopback only).").addText(S=>S.setPlaceholder("HTTP://127.0.0.1:23120").setValue(this.plugin.settings.zoteroCompanionBaseUrl).onChange(async v=>{this.plugin.settings.zoteroCompanionBaseUrl=v.trim(),await this.plugin.saveSettings()})),new L.Setting(h).setName("Zotero companion token").setDesc("Optional shared token for the companion endpoint.").addText(S=>{i(S),this.companionTokenInput=S,S.setPlaceholder("Optional-token").setValue(this.plugin.settings.zoteroCompanionToken).onChange(async v=>{this.plugin.settings.zoteroCompanionToken=v.trim(),await this.plugin.saveSettings()})}),new L.Setting(h).setName("Generate companion token").setDesc("Create and paste a secure token. The generated token is copied to your clipboard. In Zotero, set your Zotero companion plugin token to this value.").addButton(S=>S.setButtonText("Generate token").onClick(async()=>{let v=(0,Vt.randomBytes)(32).toString("base64url");this.plugin.settings.zoteroCompanionToken=v,await this.plugin.saveSettings(),this.companionTokenInput&&this.companionTokenInput.setValue(v);try{await navigator.clipboard.writeText(v),new L.Notice("Generated and copied companion token.")}catch(C){new L.Notice("Generated token, but failed to copy to clipboard."),console.warn("Failed to copy generated companion token",C)}})),new L.Setting(h).setName("Annotation color map").setHeading(),h.createEl("p",{text:"Map Zotero highlight colors to section headings and callout types."});let b=h.createDiv({cls:"zrr-annotation-map"}),w=async S=>{this.plugin.settings.annotationColorMap=S,await this.plugin.saveSettings()},P=()=>{var F,V;b.empty();let S=this.plugin.settings.annotationColorMap||{},v=Object.entries(S);if(v.length||b.createEl("p",{text:"No color mappings configured."}),v.length){let U=b.createDiv({cls:"zrr-annotation-map-row-header"});U.createEl("span",{text:"Color"}),U.createEl("span",{text:"Heading"}),U.createEl("span",{text:"Callout"}),U.createEl("span")}for(let[U,j]of v){let T=b.createDiv({cls:"zrr-annotation-map-row"}).createDiv({cls:"zrr-annotation-map-row-body"}),G=T.createEl("input",{type:"text",value:U,cls:"zrr-annotation-map-input"}),Z=T.createEl("input",{type:"text",value:(F=j.heading)!=null?F:"",cls:"zrr-annotation-map-input"}),W=T.createEl("input",{type:"text",value:(V=j.callout)!=null?V:"",cls:"zrr-annotation-map-input"}),I=T.createEl("button",{text:"Delete",cls:"zrr-annotation-map-delete"});I.type="button",G.addEventListener("change",()=>{(async()=>{let z=G.value.trim().toLowerCase();if(!z){G.value=U;return}let q={...this.plugin.settings.annotationColorMap||{}};if(z!==U&&q[z]){new L.Notice(`Annotation color '${z}' already exists.`),G.value=U;return}let M=q[U];delete q[U],q[z]=M!=null?M:{heading:"",callout:""},await w(q),P()})()}),Z.addEventListener("change",()=>{(async()=>{var M;let z={...this.plugin.settings.annotationColorMap||{}},q=(M=z[U])!=null?M:{heading:"",callout:""};q.heading=Z.value.trim(),z[U]=q,await w(z)})()}),W.addEventListener("change",()=>{(async()=>{var M;let z={...this.plugin.settings.annotationColorMap||{}},q=(M=z[U])!=null?M:{heading:"",callout:""};q.callout=W.value.trim(),z[U]=q,await w(z)})()}),I.addEventListener("click",()=>{(async()=>{let z={...this.plugin.settings.annotationColorMap||{}};delete z[U],await w(z),P()})()})}let C=b.createDiv({cls:"zrr-annotation-map-actions"}),N=C.createEl("button",{text:"Add mapping"});N.type="button",N.addEventListener("click",()=>{(async()=>{let U={...this.plugin.settings.annotationColorMap||{}},j="new-color",B=1;for(;U[j];)j=`new-color-${B}`,B+=1;U[j]={heading:"",callout:""},await w(U),P()})()});let D=C.createEl("button",{text:"Reset to defaults"});D.type="button",D.addEventListener("click",()=>{(async()=>(await w({...Ke.annotationColorMap}),P()))()})};P()}},{id:"ocr",label:"OCR",render:h=>{new L.Setting(h).setName("Docling").setHeading();let b=null,w=C=>{if(!b)return;let N=this.plugin.settings.ocrEngine;new Set(C.map(F=>F.value)).has(N)||(C=C.concat([{value:N,label:`Current (unavailable): ${N}`}])),b.selectEl.options.length=0;for(let F of C)b.addOption(F.value,F.label);b.setValue(N)},P=async()=>{if(!b)return;b.setDisabled(!0);let C={tesseract:!1,paddleStructureLocal:!1,paddleVlLocal:!1};if(this.plugin.detectOcrEngines)try{C=await this.plugin.detectOcrEngines()}catch(F){C={tesseract:!1,paddleStructureLocal:!1,paddleVlLocal:!1}}let N=[{value:"auto",label:"Auto (default)"}];C.tesseract&&N.push({value:"tesseract",label:"Tesseract (local)"}),C.paddleStructureLocal&&N.push({value:"paddle_structure_local",label:"Paddle PP-StructureV3 (local)"}),C.paddleVlLocal&&N.push({value:"paddle_vl_local",label:"PaddleOCR-VL (local)"}),(this.plugin.settings.paddleApiKey||"").trim()&&(N.push({value:"paddle_structure_api",label:"PP-StructureV3 API"}),N.push({value:"paddle_vl_api",label:"PaddleOCR-VL API"})),w(N),b.setDisabled(!1)},S=new L.Setting(h).setName("Paddle ocr API key").setDesc("API token for paddleocr-vl / pp-structurev3 endpoints. Get a free API key at "),v=document.createElement("a");v.href="https://aistudio.baidu.com/paddleocr",v.textContent="HTTPS://aistudio.baidu.com/paddleocr",v.target="_blank",v.rel="noopener noreferrer",S.descEl.appendChild(v),S.descEl.append("."),S.addText(C=>{i(C),C.setPlaceholder("Your-api-token").setValue(this.plugin.settings.paddleApiKey).onChange(async N=>{this.plugin.settings.paddleApiKey=N.trim(),await this.plugin.saveSettings(),await P()})}),new L.Setting(h).setName("Paddleocr-vl API URL").setDesc("Optional override for the paddleocr-vl API endpoint.").addText(C=>C.setPlaceholder("HTTPS://.../layout-parsing").setValue(this.plugin.settings.paddleVlApiUrl).onChange(async N=>{this.plugin.settings.paddleVlApiUrl=N.trim(),await this.plugin.saveSettings()})),new L.Setting(h).setName("Pp-structurev3 API URL").setDesc("API endpoint for pp-structurev3 (see baidu AI studio docs).").addText(C=>C.setPlaceholder("HTTPS://.../pp-structure").setValue(this.plugin.settings.paddleStructureApiUrl).onChange(async N=>{this.plugin.settings.paddleStructureApiUrl=N.trim(),await this.plugin.saveSettings(),await P()})),new L.Setting(h).setName("Ocr engine").setDesc("Select the ocr engine to use when ocr is required.").addDropdown(C=>{b=C,C.addOption("auto","Auto (default)"),C.setValue(this.plugin.settings.ocrEngine),C.onChange(async N=>{this.plugin.settings.ocrEngine=N,await this.plugin.saveSettings()})}),new L.Setting(h).setName("Ocr decision (when to ocr)").setDesc("Controls when ocr runs; per-page behavior is configured separately below.").addDropdown(C=>C.addOption("auto","Auto: use text layer when reliable").addOption("force_low_quality","Ocr only if text is poor").addOption("force","Prefer ocr for full document").setValue(this.plugin.settings.ocrMode).onChange(async N=>{this.plugin.settings.ocrMode=N,await this.plugin.saveSettings()})),new L.Setting(h).setName("Ocr layout override (per-page)").setDesc("Force per-page ocr when ocr runs, bypassing layout heuristics; can be slower for multi-column pdfs.").addToggle(C=>C.setValue(this.plugin.settings.forcePerPageOcr).onChange(async N=>{this.plugin.settings.forcePerPageOcr=N,await this.plugin.saveSettings()})),new L.Setting(h).setName("Text quality threshold").setDesc("Lower values are stricter; below this threshold the text is treated as low quality.").addSlider(C=>{C.setLimits(0,1,.05).setValue(this.plugin.settings.ocrQualityThreshold).setDynamicTooltip().onChange(async N=>{this.plugin.settings.ocrQualityThreshold=N,await this.plugin.saveSettings()})}),new L.Setting(h).setName("Chunking").setDesc("Page or section").addDropdown(C=>C.addOption("page","Page").addOption("section","Section").setValue(this.plugin.settings.chunkingMode).onChange(async N=>{this.plugin.settings.chunkingMode=N,await this.plugin.saveSettings()})),P()}},{id:"llms",label:"LLMs",render:h=>{new L.Setting(h).setName("Model provider profiles").setHeading();let b=h.createDiv({cls:"zrr-llm-profiles"}),w=()=>{b.empty();let A=t();A.length||b.createEl("p",{text:"No profiles yet. Add one below."});for(let E of A){let O=b.createEl("details",{cls:"zrr-profile"});A.length===1&&(O.open=!0);let ie=O.createEl("summary",{text:E.name||E.id||"Profile"});ie.addClass("zrr-profile-title");let $e=O.createDiv({cls:"zrr-profile-body"});new L.Setting($e).setName("Profile name").addText(fe=>fe.setPlaceholder("My provider").setValue(E.name||"").onChange(async ye=>{E.name=ye.trim(),ie.textContent=E.name||E.id||"Profile",await n(t())})),new L.Setting($e).setName("Base URL").addText(fe=>fe.setPlaceholder("HTTP://localhost:1234/v1").setValue(E.baseUrl||"").onChange(async ye=>{E.baseUrl=ye.trim(),await n(t())})),new L.Setting($e).setName("API key").setDesc("Stored in settings (not encrypted).").addText(fe=>{i(fe),fe.setPlaceholder("Sk-...").setValue(E.apiKey||"").onChange(async ye=>{E.apiKey=ye.trim(),await n(t())})}),new L.Setting($e).setName("Remove profile").setDesc("Deletes this saved profile.").addButton(fe=>fe.setButtonText("Delete profile").onClick(async()=>{let ye=t().filter(yn=>yn.id!==E.id);this.plugin.settings.embedProviderProfileId=this.plugin.settings.embedProviderProfileId===E.id?"":this.plugin.settings.embedProviderProfileId,this.plugin.settings.chatProviderProfileId=this.plugin.settings.chatProviderProfileId===E.id?"":this.plugin.settings.chatProviderProfileId,this.plugin.settings.llmCleanupProviderProfileId=this.plugin.settings.llmCleanupProviderProfileId===E.id?"":this.plugin.settings.llmCleanupProviderProfileId,await n(ye),w()}))}new L.Setting(b).addButton(E=>E.setButtonText("Add profile").onClick(async()=>{let O=`profile-${Date.now().toString(36)}`,ie=t().concat([{id:O,name:"Custom",baseUrl:"",apiKey:""}]);await n(ie),w()}))};w(),new L.Setting(h).setName("Ocr cleanup").setHeading(),new L.Setting(h).setName("Model cleanup for low-quality chunks").setDesc("Automatic AI cleanup for poor ocr at import. Can be slow/costly.").addToggle(A=>A.setValue(this.plugin.settings.enableLlmCleanup).onChange(async E=>{this.plugin.settings.enableLlmCleanup=E,await this.plugin.saveSettings()}));let P=null,S=null,v=null,C=()=>Promise.resolve(),N=async(A,E=!0)=>{let O=A.trim();this.plugin.settings.llmCleanupBaseUrl=O,E&&(this.plugin.settings.llmCleanupProviderProfileId="",P&&P.setValue("custom")),S&&S.setValue(O),await this.plugin.saveSettings()},D=async A=>{let E=t().find(O=>O.id===A);this.plugin.settings.llmCleanupProviderProfileId=A,E&&(this.plugin.settings.llmCleanupBaseUrl=E.baseUrl,this.plugin.settings.llmCleanupApiKey=E.apiKey,S==null||S.setValue(E.baseUrl),v==null||v.setValue(E.apiKey)),await this.plugin.saveSettings(),await C()};new L.Setting(h).setName("Cleanup provider profile").setDesc("Select a profile to populate base URL and API key.").addDropdown(A=>{P=A,A.addOption("custom","Custom (manual)");for(let O of t())A.addOption(O.id,O.name||O.id);let E=this.plugin.settings.llmCleanupProviderProfileId;A.setValue(E&&t().some(O=>O.id===E)?E:"custom"),A.onChange(async O=>{if(O==="custom"){this.plugin.settings.llmCleanupProviderProfileId="",await this.plugin.saveSettings();return}await D(O)})}),new L.Setting(h).setName("Cleanup base URL").setDesc("OpenAI-compatible endpoint, e.g. HTTP://127.0.0.1:1234/v1").addText(A=>{S=A,A.setPlaceholder("HTTP://127.0.0.1:1234/v1").setValue(this.plugin.settings.llmCleanupBaseUrl).onChange(async E=>{await N(E)})}),new L.Setting(h).setName("Cleanup API key").setDesc("Optional API key for the cleanup endpoint.").addText(A=>{v=A,i(A),A.setPlaceholder("Sk-...").setValue(this.plugin.settings.llmCleanupApiKey).onChange(async E=>{this.plugin.settings.llmCleanupApiKey=E.trim(),this.plugin.settings.llmCleanupProviderProfileId="",P&&P.setValue("custom"),await this.plugin.saveSettings()})});let F=new L.Setting(h).setName("Cleanup model").setDesc("Select a cleanup-capable model from the provider."),V=null,U=A=>{if(!V)return;let E=(this.plugin.settings.llmCleanupModel||"").trim(),O=new Set(A.map(ie=>ie.value));E&&!O.has(E)&&(A=A.concat([{value:E,label:`Custom (${E})`}])),V.selectEl.options.length=0;for(let ie of A)V.addOption(ie.value,ie.label);E&&V.setValue(E)};C=async()=>{if(V){V.setDisabled(!0);try{let A=await this.plugin.fetchCleanupModelOptions();U(A)}finally{V.setDisabled(!1)}}},F.addDropdown(A=>{V=A;let E=(this.plugin.settings.llmCleanupModel||"").trim();A.addOption(E||"loading","Loading..."),A.setValue(E||"loading"),A.onChange(async O=>{this.plugin.settings.llmCleanupModel=O.trim(),await this.plugin.saveSettings()})}),F.addButton(A=>{A.setButtonText("Refresh").onClick(async()=>{await C()})}),C(),new L.Setting(h).setName("Cleanup temperature").setDesc("Lower is more conservative.").addText(A=>A.setPlaceholder("0.0").setValue(String(this.plugin.settings.llmCleanupTemperature)).onChange(async E=>{let O=Number.parseFloat(E);this.plugin.settings.llmCleanupTemperature=Number.isFinite(O)?O:0,await this.plugin.saveSettings()})),new L.Setting(h).setName("Cleanup minimum quality").setDesc("Only run cleanup when chunk quality is below this threshold (0-1).").addSlider(A=>A.setLimits(0,1,.05).setValue(this.plugin.settings.llmCleanupMinQuality).setDynamicTooltip().onChange(async E=>{this.plugin.settings.llmCleanupMinQuality=E,await this.plugin.saveSettings()})),new L.Setting(h).setName("Cleanup maximum chars").setDesc("Skip cleanup for chunks longer than this limit.").addText(A=>A.setPlaceholder("2000").setValue(String(this.plugin.settings.llmCleanupMaxChars)).onChange(async E=>{let O=Number.parseInt(E,10);this.plugin.settings.llmCleanupMaxChars=Number.isFinite(O)?O:2e3,await this.plugin.saveSettings()})),new L.Setting(h).setName("Text embedding").setHeading();let j=null,B=null,T=null,G=()=>Promise.resolve(),Z=async(A,E=!0)=>{let O=A.trim();this.plugin.settings.embedBaseUrl=O,E&&(this.plugin.settings.embedProviderProfileId="",j&&j.setValue("custom")),B&&B.setValue(O),await this.plugin.saveSettings()},W=async A=>{let E=t().find(O=>O.id===A);this.plugin.settings.embedProviderProfileId=A,E&&(this.plugin.settings.embedBaseUrl=E.baseUrl,this.plugin.settings.embedApiKey=E.apiKey,B==null||B.setValue(E.baseUrl),T==null||T.setValue(E.apiKey)),await this.plugin.saveSettings(),await G()};new L.Setting(h).setName("Embeddings provider profile").setDesc("Select a profile to populate base URL and API key.").addDropdown(A=>{j=A,A.addOption("custom","Custom (manual)");for(let O of t())A.addOption(O.id,O.name||O.id);let E=this.plugin.settings.embedProviderProfileId;A.setValue(E&&t().some(O=>O.id===E)?E:"custom"),A.onChange(async O=>{if(O==="custom"){this.plugin.settings.embedProviderProfileId="",await this.plugin.saveSettings();return}await W(O)})}),new L.Setting(h).setName("Embeddings base URL").addText(A=>{B=A,A.setPlaceholder("HTTP://localhost:1234/v1").setValue(this.plugin.settings.embedBaseUrl).onChange(async E=>{await Z(E)})}),new L.Setting(h).setName("Embeddings API key").addText(A=>{T=A,i(A),A.setPlaceholder("Lm-studio").setValue(this.plugin.settings.embedApiKey).onChange(async E=>{this.plugin.settings.embedApiKey=E.trim(),this.plugin.settings.embedProviderProfileId="",j&&j.setValue("custom"),await this.plugin.saveSettings()})});let I=new L.Setting(h).setName("Embeddings model").setDesc("Select an embeddings model from the provider."),z=null,q=A=>{if(!z)return;let E=(this.plugin.settings.embedModel||"").trim(),O=new Set(A.map(ie=>ie.value));E&&!O.has(E)&&(A=A.concat([{value:E,label:`Custom (${E})`}])),z.selectEl.options.length=0;for(let ie of A)z.addOption(ie.value,ie.label);E&&z.setValue(E)};G=async()=>{if(z){z.setDisabled(!0);try{let A=await this.plugin.fetchEmbeddingModelOptions();q(A)}finally{z.setDisabled(!1)}}},I.addDropdown(A=>{z=A;let E=(this.plugin.settings.embedModel||"").trim();A.addOption(E||"loading","Loading..."),A.setValue(E||"loading"),A.onChange(async O=>{this.plugin.settings.embedModel=O.trim(),await this.plugin.saveSettings()})}),I.addButton(A=>{A.setButtonText("Refresh").onClick(async()=>{await G()})}),G(),new L.Setting(h).setName("Include metadata in embeddings").setDesc("Prepend title/authors/tags/section info before embedding chunks.").addToggle(A=>A.setValue(this.plugin.settings.embedIncludeMetadata).onChange(async E=>{this.plugin.settings.embedIncludeMetadata=E,await this.plugin.saveSettings()})),new L.Setting(h).setName("Embedding context window (chunks)").setDesc("Include neighboring chunk text around each chunk when embedding (0 disables).").addText(A=>A.setPlaceholder("1").setValue(String(this.plugin.settings.embedContextWindow)).onChange(async E=>{let O=Number.parseInt(E,10);this.plugin.settings.embedContextWindow=Number.isFinite(O)?Math.max(0,O):1,await this.plugin.saveSettings()})),new L.Setting(h).setName("Embedding context snippet size (chars)").setDesc("Max chars per neighboring chunk included in embeddings.").addText(A=>A.setPlaceholder("220").setValue(String(this.plugin.settings.embedContextChars)).onChange(async E=>{let O=Number.parseInt(E,10);this.plugin.settings.embedContextChars=Number.isFinite(O)?Math.max(0,O):220,await this.plugin.saveSettings()})),new L.Setting(h).setName("Embedding subchunk size (chars)").setDesc("Split long chunks into smaller subchunks for embedding only (0 disables).").addText(A=>A.setPlaceholder("1800").setValue(String(this.plugin.settings.embedSubchunkChars)).onChange(async E=>{let O=Number.parseInt(E,10);this.plugin.settings.embedSubchunkChars=Number.isFinite(O)?Math.max(0,O):3500,await this.plugin.saveSettings()})),new L.Setting(h).setName("Embedding subchunk overlap (chars)").setDesc("Overlap between embedding subchunks to keep context intact.").addText(A=>A.setPlaceholder("200").setValue(String(this.plugin.settings.embedSubchunkOverlap)).onChange(async E=>{let O=Number.parseInt(E,10);this.plugin.settings.embedSubchunkOverlap=Number.isFinite(O)?Math.max(0,O):200,await this.plugin.saveSettings()})),new L.Setting(h).setName("Generate model tags for chunks").setDesc("Use the ocr cleanup model to tag chunks before indexing.").addToggle(A=>A.setValue(this.plugin.settings.enableChunkTagging).onChange(async E=>{this.plugin.settings.enableChunkTagging=E,await this.plugin.saveSettings()})),new L.Setting(h).setName("Chat model").setHeading();let M=null,$=null,K=null,ne=()=>Promise.resolve(),pe=async(A,E=!0)=>{let O=A.trim();this.plugin.settings.chatBaseUrl=O,E&&(this.plugin.settings.chatProviderProfileId="",M&&M.setValue("custom")),$&&$.setValue(O),await this.plugin.saveSettings()},he=async A=>{let E=t().find(O=>O.id===A);this.plugin.settings.chatProviderProfileId=A,E&&(this.plugin.settings.chatBaseUrl=E.baseUrl,this.plugin.settings.chatApiKey=E.apiKey,$==null||$.setValue(E.baseUrl),K==null||K.setValue(E.apiKey)),await this.plugin.saveSettings(),await ne()};new L.Setting(h).setName("Chat provider profile").setDesc("Select a profile to populate base URL and API key.").addDropdown(A=>{M=A,A.addOption("custom","Custom (manual)");for(let O of t())A.addOption(O.id,O.name||O.id);let E=this.plugin.settings.chatProviderProfileId;A.setValue(E&&t().some(O=>O.id===E)?E:"custom"),A.onChange(async O=>{if(O==="custom"){this.plugin.settings.chatProviderProfileId="",await this.plugin.saveSettings();return}await he(O)})}),new L.Setting(h).setName("Chat base URL").setDesc("Compatible endpoint for chat requests.").addText(A=>{$=A,A.setPlaceholder("HTTP://localhost:1234/v1").setValue(this.plugin.settings.chatBaseUrl).onChange(async E=>{await pe(E)})}),new L.Setting(h).setName("Chat API key").addText(A=>{K=A,i(A),A.setPlaceholder("Lm-studio").setValue(this.plugin.settings.chatApiKey).onChange(async E=>{this.plugin.settings.chatApiKey=E.trim(),this.plugin.settings.chatProviderProfileId="",M&&M.setValue("custom"),await this.plugin.saveSettings()})});let Pe=new L.Setting(h).setName("Chat model").setDesc("Select a chat-capable model from the provider."),ce=null,_t=A=>{if(!ce)return;let E=(this.plugin.settings.chatModel||"").trim(),O=new Set(A.map(ie=>ie.value));E&&!O.has(E)&&(A=A.concat([{value:E,label:`Custom (${E})`}])),ce.selectEl.options.length=0;for(let ie of A)ce.addOption(ie.value,ie.label);E&&ce.setValue(E)};ne=async()=>{if(ce){ce.setDisabled(!0);try{let A=await this.plugin.fetchChatModelOptions();_t(A)}finally{ce.setDisabled(!1)}}},Pe.addDropdown(A=>{ce=A;let E=(this.plugin.settings.chatModel||"").trim();A.addOption(E||"loading","Loading..."),A.setValue(E||"loading"),A.onChange(async O=>{this.plugin.settings.chatModel=O.trim(),await this.plugin.saveSettings()})}),Pe.addButton(A=>{A.setButtonText("Refresh").onClick(async()=>{await ne()})}),ne(),new L.Setting(h).setName("Temperature").addText(A=>A.setPlaceholder("0.2").setValue(String(this.plugin.settings.chatTemperature)).onChange(async E=>{let O=Number.parseFloat(E);this.plugin.settings.chatTemperature=Number.isFinite(O)?O:.2,await this.plugin.saveSettings()})),new L.Setting(h).setName("Chat history messages").setDesc("Number of recent messages to include for conversational continuity (0 disables).").addText(A=>A.setPlaceholder("6").setValue(String(this.plugin.settings.chatHistoryMessages)).onChange(async E=>{let O=Number.parseInt(E,10);this.plugin.settings.chatHistoryMessages=Number.isFinite(O)?Math.max(0,O):6,await this.plugin.saveSettings()})),new L.Setting(h).setName("Chat panel location").setDesc("Where to open the chat view by default.").addDropdown(A=>A.addOption("right","Right sidebar").addOption("main","Main window").setValue(this.plugin.settings.chatPaneLocation).onChange(async E=>{this.plugin.settings.chatPaneLocation=E,await this.plugin.saveSettings()})),new L.Setting(h).setName("Retrieval").setHeading(),new L.Setting(h).setName("Enable agentic retrieval").setDesc("Use a small planner step that can trigger expansion retry or full-document retrieval before answering.").addToggle(A=>A.setValue(this.plugin.settings.enableAgenticRag).onChange(async E=>{this.plugin.settings.enableAgenticRag=E,await this.plugin.saveSettings()})),new L.Setting(h).setName("Agentic max iterations").setDesc("Maximum number of planner steps per query.").addText(A=>A.setPlaceholder("2").setValue(String(this.plugin.settings.agenticMaxIters)).onChange(async E=>{let O=Number.parseInt(E,10);this.plugin.settings.agenticMaxIters=Number.isFinite(O)?Math.max(1,O):2,await this.plugin.saveSettings()})),new L.Setting(h).setName("Enable query expansion").setDesc("Use the chat model to expand queries before retrieval.").addToggle(A=>A.setValue(this.plugin.settings.enableQueryExpansion).onChange(async E=>{this.plugin.settings.enableQueryExpansion=E,await this.plugin.saveSettings()})),new L.Setting(h).setName("Query expansion count").setDesc("Number of expansion variants to request.").addText(A=>A.setPlaceholder("3").setValue(String(this.plugin.settings.queryExpansionCount)).onChange(async E=>{let O=Number.parseInt(E,10);this.plugin.settings.queryExpansionCount=Number.isFinite(O)?Math.max(1,O):3,await this.plugin.saveSettings()})),new L.Setting(h).setName("Enable cross-encoder reranking").setDesc("Rerank candidates locally with sentence-transformers (downloads model on first use).").addToggle(A=>A.setValue(this.plugin.settings.enableCrossEncoderRerank).onChange(async E=>{this.plugin.settings.enableCrossEncoderRerank=E,await this.plugin.saveSettings()}));let X=new Set($t.map(A=>A.value)),Q=(this.plugin.settings.rerankModel||"").trim()||Ae,_e=X.has(Q)?Q:Ne,se=null,Et=()=>{if(!se)return;let E=!(_e===Ne);se.setDisabled(E),se.inputEl.disabled=E,se.inputEl.readOnly=E,se.inputEl.classList.toggle("is-disabled",E),se.inputEl.setAttribute("aria-disabled",String(E))},St=new L.Setting(h).setName("Cross-encoder model").setDesc("Choose a multilingual preset, or select custom to edit the model ID.");St.addDropdown(A=>{for(let E of $t)A.addOption(E.value,E.label);A.addOption(Ne,"Custom"),A.setValue(_e),A.onChange(async E=>{_e=E,E!==Ne?this.plugin.settings.rerankModel=E:(this.plugin.settings.rerankModel||"").trim()||(this.plugin.settings.rerankModel=Ae),await this.plugin.saveSettings(),se==null||se.setValue(this.plugin.settings.rerankModel||Ae),Et()})}),St.addText(A=>{se=A,A.setPlaceholder(Ae).setValue(Q).onChange(async E=>{_e===Ne&&(this.plugin.settings.rerankModel=E.trim()||Ae,await this.plugin.saveSettings())}),Et()}),new L.Setting(h).setName("Rerank candidate multiplier").setDesc("Retrieve k \xD7 n candidates before reranking.").addText(A=>A.setPlaceholder("4").setValue(String(this.plugin.settings.rerankCandidateMultiplier)).onChange(async E=>{let O=Number.parseInt(E,10);this.plugin.settings.rerankCandidateMultiplier=Number.isFinite(O)?Math.max(1,O):4,await this.plugin.saveSettings()})),new L.Setting(h).setName("Rrf k").setDesc("Rank fusion constant for blending lexical and vector results.").addText(A=>A.setPlaceholder("60").setValue(String(this.plugin.settings.rrfK)).onChange(async E=>{let O=Number.parseInt(E,10);this.plugin.settings.rrfK=Number.isFinite(O)?Math.max(1,O):60,await this.plugin.saveSettings()})),new L.Setting(h).setName("Rrf log top n").setDesc("Log the top n rrf-ranked chunks to stderr (0 disables).").addText(A=>A.setPlaceholder("0").setValue(String(this.plugin.settings.rrfLogTop)).onChange(async E=>{let O=Number.parseInt(E,10);this.plugin.settings.rrfLogTop=Number.isFinite(O)?Math.max(0,O):0,await this.plugin.saveSettings()})),new L.Setting(h).setName("Max chunks per document").setDesc("Limit how many chunks from a single document can appear in retrieval (0 disables).").addText(A=>A.setPlaceholder("0").setValue(String(this.plugin.settings.maxChunksPerDoc)).onChange(async E=>{let O=Number.parseInt(E,10);this.plugin.settings.maxChunksPerDoc=Number.isFinite(O)?Math.max(0,O):0,await this.plugin.saveSettings()}))}},{id:"maintenance",label:"Maintenance",render:h=>{new L.Setting(h).setName("Python runtime").setHeading(),new L.Setting(h).setName("Use local runtime (legacy)").setDesc("Switch to local interpreter/venv mode and enable advanced runtime settings.").addButton(b=>b.setButtonText("Switch").onClick(async()=>{await this.plugin.switchPythonRuntimeToLocalLegacy()})),new L.Setting(h).setName("Logging").setHeading(),new L.Setting(h).setName("Enable logging to file").setDesc("Write plugin logs to a file.").addToggle(b=>b.setValue(this.plugin.settings.enableFileLogging).onChange(async w=>{this.plugin.settings.enableFileLogging=w,await this.plugin.saveSettings()})),new L.Setting(h).setName("Log file path (vault-relative)").setDesc("Where to write the log file. Keep inside the vault.").addText(b=>b.setPlaceholder(`${ae}/logs/docling_extract.log`).setValue(this.plugin.settings.logFilePath).onChange(async w=>{this.plugin.settings.logFilePath=w.trim()||`${ae}/logs/docling_extract.log`,await this.plugin.saveSettings()})),new L.Setting(h).setName("View or clear log").setDesc("Open the log file or clear it.").addButton(b=>b.setButtonText("Open log").onClick(async()=>{var w,P;await((P=(w=this.plugin).openLogFile)==null?void 0:P.call(w))})).addButton(b=>b.setButtonText("Clear log").onClick(async()=>{var w,P;await((P=(w=this.plugin).clearLogFile)==null?void 0:P.call(w))})),new L.Setting(h).setName("Zotero companion plugin").setHeading(),h.createEl("p",{text:"Download the xpi, then in Zotero go to tools \u2192 add-ons \u2192 install from file and restart."}),new L.Setting(h).setName("Download companion add-on").setDesc("Downloads the companion xpi to your system downloads folder.").addButton(b=>b.setButtonText("Download xpi").onClick(async()=>{let w=this.getDefaultDownloadDir();if(!w){new L.Notice("Unable to resolve the system downloads folder.");return}let P=We.default.join(w,"zrr-companion.xpi");if(await this.companionXpiExists(P)){new L.Notice(`Companion XPI already exists: ${P}`);return}new L.Notice("Downloading companion xpi..."),await this.downloadCompanionXpi(P)})),new L.Setting(h).setName("Open Zotero").setDesc("Launch Zotero and open the add-ons window (tools \u2192 add-ons).").addButton(b=>b.setButtonText("Open Zotero").onClick(async()=>{await this.plugin.openZoteroAddons()})),new L.Setting(h).setName("Check companion status").setDesc("Ping the companion /health endpoint.").addButton(b=>b.setButtonText("Check status").onClick(async()=>{await this.plugin.checkZoteroCompanionHealth()})),new L.Setting(h).setName("Redis indexing").setHeading(),new L.Setting(h).setName("Reindex redis from cached chunks").setDesc("Rebuild the redis index from cached chunk JSON files.").addButton(b=>b.setButtonText("Reindex").onClick(async()=>{await this.plugin.reindexRedisFromCache()})),new L.Setting(h).setName("Recreate redis stack").setDesc("Pulls the configured redis image and force-recreates the redis-stack service.").addButton(b=>b.setButtonText("Recreate").onClick(async()=>{await this.plugin.recreateRedisStack()})),new L.Setting(h).setName("Recreate missing notes from cache").setDesc("Rebuild missing notes using cached Zotero items and chunks.").addButton(b=>b.setButtonText("Recreate").onClick(async()=>{await this.plugin.recreateMissingNotesFromCache()})).addButton(b=>b.setButtonText("Cancel").onClick(()=>{this.plugin.cancelRecreateMissingNotesFromCache()})),new L.Setting(h).setName("Release notes").setHeading(),new L.Setting(h).setName("Show release notes").setDesc("Open the current version splash screen.").addButton(b=>b.setButtonText("Show").onClick(()=>{this.plugin.openReleaseNotesModal()}))}}],p=new Map,u=new Map,g=h=>{this.activeTab=h;for(let[b,w]of p){let P=b===h;w.classList.toggle("is-active",P),w.setAttribute("aria-selected",P?"true":"false"),w.setAttribute("tabindex",P?"0":"-1")}for(let[b,w]of u){let P=b===h;w.classList.toggle("is-active",P),w.hidden=!P}},y=e.createDiv({cls:"zrr-settings-tabs"});y.setAttribute("role","tablist");let x=e.createDiv({cls:"zrr-settings-tabs-panels"});for(let h of d){let b=y.createEl("button",{text:h.label,cls:"zrr-settings-tab-button"}),w=`zrr-settings-tab-${h.id}`,P=`zrr-settings-panel-${h.id}`;b.type="button",b.id=w,b.setAttribute("role","tab"),b.setAttribute("aria-controls",P),b.setAttribute("aria-selected","false"),b.addEventListener("click",()=>g(h.id));let S=x.createDiv({cls:"zrr-settings-tab-panel"});S.id=P,S.setAttribute("role","tabpanel"),S.setAttribute("aria-labelledby",w),S.hidden=!0,h.render(S),p.set(h.id,b),u.set(h.id,S)}let k=d.some(h=>h.id===this.activeTab)?this.activeTab:d[0].id;g(k)}getDefaultDownloadDir(){let e=process.env.HOME||process.env.USERPROFILE;return e?We.default.join(e,"Downloads"):null}async companionXpiExists(e){try{return await Ge.promises.access(e),!0}catch(t){return!1}}async downloadCompanionXpi(e){try{await Ge.promises.mkdir(We.default.dirname(e),{recursive:!0});let t=await(0,L.requestUrl)({url:Rn,method:"GET"});if(t.status!==200){new L.Notice(`Failed to download companion XPI (HTTP ${t.status}).`);return}let n=Buffer.from(t.arrayBuffer);await Ge.promises.writeFile(e,n),new L.Notice(`Downloaded companion XPI: ${e}`)}catch(t){new L.Notice("Failed to download companion xpi. See console for details."),console.warn("Failed to download companion XPI",t)}}};var Bt=require("@codemirror/state"),Gt=require("@codemirror/view"),ue=require("obsidian");var Ln=f=>{let _=f.scrollDOM.getBoundingClientRect(),e=f.posAtCoords({x:_.left+8,y:+_.height*.25});return e===null?null:f.state.doc.lineAt(e).number},Te=f=>!f||typeof f!="object"?null:f,Je=class{constructor(_,e){this.pdfSidebarLeaf=null;this.pdfSidebarDocId=null;this.pdfSidebarPdfPath=null;this.pdfSidebarPage=null;this.pendingPdfSync=null;this.chunkPageCache=new Map;this.previewScrollEl=null;this.previewScrollHandler=null;this.previewScrollFrame=null;this.pdfSyncInFlight=!1;this.queuedPdfSync=null;this.deps=_,this.helpers=e}createSyncExtension(){let _=this.helpers,e=this.syncPdfSidebarForDoc.bind(this);return Gt.ViewPlugin.fromClass(class{constructor(t){this.docId=null;this.lastPage=null;this.lastChunkId=null;this.scrollFrame=null;this.view=t,this.docId=_.extractDocIdFromDoc(t.state.doc),this.onScroll=()=>this.scheduleSync(!1),t.scrollDOM.addEventListener("scroll",this.onScroll,{passive:!0}),this.scheduleSync(!0)}update(t){t.docChanged&&(this.docId=_.extractDocIdFromDoc(t.view.state.doc),this.lastPage=null,this.lastChunkId=null),(t.docChanged||t.viewportChanged)&&this.scheduleSync(!1)}destroy(){this.view.scrollDOM.removeEventListener("scroll",this.onScroll),this.scrollFrame!==null&&(window.cancelAnimationFrame(this.scrollFrame),this.scrollFrame=null)}scheduleSync(t){this.scrollFrame===null&&(this.scrollFrame=window.requestAnimationFrame(()=>{this.scrollFrame=null,this.syncPdfSidebar(this.view,t)}))}syncPdfSidebar(t,n){var p,u;let i=this.docId;if(!i)return;let r=Ln(t);if(r===null)return;let a=_.findChunkStartLineInDoc(t.state.doc,r);if(!a)return;let s=_.parseChunkMarkerLine(a.text),o=(p=s==null?void 0:s.pageNumber)!=null?p:null,l=(u=s==null?void 0:s.chunkId)!=null?u:null;if(!o&&!l)return;let c=o!==null&&this.lastPage===o,d=o===null&&l!==null&&this.lastChunkId===l;!n&&(c||d)||(this.lastPage=o,this.lastChunkId=l,e(i,o!=null?o:void 0,l!=null?l:void 0))}})}async maybeSyncPendingPdf(_={}){if(!this.pendingPdfSync||!this.isPdfSidebarLeaf(this.pdfSidebarLeaf)&&!await this.getPdfSidebarLeaf(!!_.allowCreateLeaf)||!this.pdfSidebarLeaf||!this.isLeafTabActive(this.pdfSidebarLeaf))return;let e=this.pendingPdfSync;this.pendingPdfSync=null,await this.syncPdfSidebarForDoc(e.docId,e.pageNumber,e.chunkId)}async revealPdfSidebar(_={}){let e=await this.getPdfSidebarLeaf(!!_.allowCreateLeaf);if(e)try{await this.deps.app.workspace.revealLeaf(e)}catch(t){console.warn("Failed to reveal PDF sidebar leaf",t)}}updatePreviewScrollHandler(){var n;let _=this.deps.app.workspace.getActiveViewOfType(ue.MarkdownView);if(!_||_.getMode()!=="preview"){this.detachPreviewScrollHandler();return}let e=(n=_.previewMode)==null?void 0:n.containerEl;if(!e){this.detachPreviewScrollHandler();return}if(this.previewScrollEl===e)return;this.detachPreviewScrollHandler();let t=()=>this.schedulePreviewSync(_);e.addEventListener("scroll",t,{passive:!0}),this.previewScrollEl=e,this.previewScrollHandler=t,this.schedulePreviewSync(_)}async syncPdfSidebarForFile(_,e={}){try{let t=await this.deps.app.vault.read(_),n=await this.deps.resolveDocIdForNote(_,t);if(!n)return;let i=this.helpers.extractFirstChunkMarkerFromContent(t);await this.syncPdfSidebarForDoc(n,i==null?void 0:i.pageNumber,i==null?void 0:i.chunkId,e)}catch(t){console.warn("Failed to sync PDF sidebar for opened note",t)}}async syncPdfSidebarForDoc(_,e,t,n={}){if(_){if(this.pdfSyncInFlight){this.queuedPdfSync={docId:_,pageNumber:e,chunkId:t};return}this.pdfSyncInFlight=!0;try{let i=await this.deps.getDocIndexEntry(_);i||(i=await this.deps.hydrateDocIndexFromCache(_));let r=i!=null&&i.pdf_path?String(i.pdf_path):"";if(!r)return;let a=this.deps.toVaultRelativePath(r);if(!a){let p=(0,ue.normalizePath)(r);this.deps.app.vault.getAbstractFileByPath(p)instanceof ue.TFile&&(a=p)}if(!a)return;let s=this.deps.app.vault.getAbstractFileByPath(a);if(!(s instanceof ue.TFile)||s.extension.toLowerCase()!=="pdf")return;let o=await this.getPdfSidebarLeaf(!!n.allowCreateLeaf);if(!o){this.pendingPdfSync={docId:_,pageNumber:e,chunkId:t};return}if(!this.isLeafTabActive(o)&&!n.allowCreateLeaf){this.pendingPdfSync={docId:_,pageNumber:e,chunkId:t};return}let l=Number.isFinite(e!=null?e:NaN)?Number(e):null;if(l===null&&t&&(l=await this.resolvePageNumberForChunk(_,t)),l===null||this.isPdfSidebarLeaf(o)&&this.pdfSidebarLeaf===o&&this.pdfSidebarDocId===_&&this.pdfSidebarPdfPath===s.path&&this.pdfSidebarPage===l)return;let c=a,d=l!==null?`${c}#page=${l}`:c;if(await this.openPdfLinkInLeaf(o,s,d),this.pdfSidebarLeaf=o,this.pdfSidebarDocId=_,this.pdfSidebarPdfPath=s.path,this.pdfSidebarPage=l,this.updatePdfSidebarIcon(o),n.allowCreateLeaf&&!this.isLeafTabActive(o))try{await this.deps.app.workspace.revealLeaf(o)}catch(p){console.warn("Failed to reveal PDF sidebar leaf",p)}}finally{this.pdfSyncInFlight=!1;let i=this.queuedPdfSync;this.queuedPdfSync=null,i&&this.syncPdfSidebarForDoc(i.docId,i.pageNumber,i.chunkId)}}}detachPreviewScrollHandler(){this.previewScrollEl&&this.previewScrollHandler&&this.previewScrollEl.removeEventListener("scroll",this.previewScrollHandler),this.previewScrollEl=null,this.previewScrollHandler=null,this.previewScrollFrame!==null&&(window.cancelAnimationFrame(this.previewScrollFrame),this.previewScrollFrame=null)}schedulePreviewSync(_){this.previewScrollFrame===null&&(this.previewScrollFrame=window.requestAnimationFrame(()=>{this.previewScrollFrame=null,this.syncPdfSidebarForPreview(_)}))}syncPdfSidebarForPreview(_){let e=this.previewScrollEl;if(!e)return;let t=_.getViewData();if(!t)return;let n=Bt.Text.of(t.split(/\r?\n/)),i=this.helpers.extractDocIdFromDoc(n);if(!i)return;let r=this.getPreviewTopLineNumber(e);if(r===null)return;let a=this.helpers.findChunkStartLineInDoc(n,r+1);if(!a)return;let s=this.helpers.parseChunkMarkerLine(a.text);!(s!=null&&s.pageNumber)&&!(s!=null&&s.chunkId)||this.syncPdfSidebarForDoc(i,s==null?void 0:s.pageNumber,s==null?void 0:s.chunkId)}getPreviewTopLineNumber(_){var n;let e=_.getBoundingClientRect().top,t=_.querySelectorAll("[data-line]");for(let i of Array.from(t))if(i.getBoundingClientRect().bottom>=e+2){let a=(n=i.getAttribute("data-line"))!=null?n:"",s=Number.parseInt(a,10);if(Number.isFinite(s))return s}return null}async getPdfSidebarLeaf(_=!1){if(this.pdfSidebarLeaf&&this.isUsableRightSidebarLeaf(this.pdfSidebarLeaf)&&this.isPdfSidebarLeaf(this.pdfSidebarLeaf))return this.pdfSidebarLeaf;this.resetPdfSidebarLeaf();let e=this.findRightSidebarPdfLeaf();if(e)return this.pdfSidebarLeaf=e,this.updatePdfSidebarIcon(e),e;if(_){let t=this.deps.app.workspace;if(typeof t.ensureSideLeaf=="function")try{let i=await t.ensureSideLeaf("pdf","right",{active:!1,split:!1,reveal:!1});if(this.isUsableRightSidebarLeaf(i))return this.pdfSidebarLeaf=i,i.isDeferred&&await i.loadIfDeferred(),this.updatePdfSidebarIcon(i),i}catch(i){console.warn("Failed to ensure PDF side leaf",i)}let n=this.deps.app.workspace.getRightLeaf(!1);if(this.isUsableRightSidebarLeaf(n))return this.pdfSidebarLeaf=n,n}return null}findRightSidebarPdfLeaf(){let _=this.deps.app.workspace;if(typeof _.iterateAllLeaves!="function")return null;let e=null;return _.iterateAllLeaves(t=>{if(!(!this.isUsableRightSidebarLeaf(t)||!this.isPdfSidebarLeaf(t))){if(this.isLeafTabActive(t)){e=t;return}e||(e=t)}}),e}resetPdfSidebarLeaf(){this.pdfSidebarLeaf=null,this.pdfSidebarDocId=null,this.pdfSidebarPdfPath=null,this.pdfSidebarPage=null}isUsableRightSidebarLeaf(_){var t;return this.isRightSidebarLeaf(_)?!!((t=_.containerEl)!=null&&t.isConnected):!1}getLeafViewType(_){var t,n;if(!_)return"";try{let i=_.getViewState();if(typeof(i==null?void 0:i.type)=="string"&&i.type)return i.type}catch(i){console.debug("Failed to read leaf view state",i)}let e=(n=(t=_.view)==null?void 0:t.getViewType)==null?void 0:n.call(t);return typeof e=="string"?e:""}isPdfSidebarLeaf(_){return this.getLeafViewType(_)==="pdf"}isRightSidebarLeaf(_){if(!_)return!1;let t=_.containerEl;return t?!!t.closest(".workspace-split.mod-right-split, .mod-right-split"):!1}updatePdfSidebarIcon(_){if(!_||!this.isRightSidebarLeaf(_)||!this.isPdfSidebarLeaf(_))return;let t=_.containerEl,n=[];t&&n.push(...Array.from(t.querySelectorAll(".view-header-icon")));let i=_;if(i.tabHeaderEl&&n.push(...Array.from(i.tabHeaderEl.querySelectorAll(".workspace-tab-header-inner-icon, .view-header-icon"))),i.tabHeaderInnerIconEl&&n.push(i.tabHeaderInnerIconEl),n.length===0)return;let r=new Set;for(let s of n)if(!r.has(s)){if(r.add(s),s.replaceChildren(),(0,ue.setIcon)(s,"zrr-pdf"),!s.querySelector("svg")&&this.deps.iconSvg){let o=document.createRange().createContextualFragment(this.deps.iconSvg);s.replaceChildren(o)}s.dataset&&(s.dataset.zrrIcon="zrr-pdf")}let a=_.view;a.icon="zrr-pdf",typeof a.getIcon=="function"&&(a.getIcon=()=>"zrr-pdf")}async openPdfLinkInLeaf(_,e,t){var l;if(!await this.ensurePdfLeafReady(_,e)){await _.openFile(e,{active:!1});return}let i=Te((l=this.getPluginsRegistry())==null?void 0:l["pdf-plus"]),r=Te(i==null?void 0:i.lib),a=Te(r==null?void 0:r.workspace),s=typeof(a==null?void 0:a.openPDFLinkTextInLeaf)=="function"?a.openPDFLinkTextInLeaf:null;if(typeof s=="function")try{await s.call(a,_,t,"",{active:!1});return}catch(c){if(!this.isPdfAnnotationRaceError(c))throw c;await this.delay(150)}let o=_;if(typeof o.openLinkText=="function"){try{await o.openLinkText(t,"",{active:!1})}catch(c){if(!this.isPdfAnnotationRaceError(c))throw c;await this.delay(150),await o.openLinkText(t,"",{active:!1})}return}await _.openFile(e,{active:!1})}async ensurePdfLeafReady(_,e){if(this.isPdfSidebarLeaf(_))return!0;try{await _.setViewState({type:"pdf",active:!1})}catch(t){console.debug("Failed to set PDF view state on sidebar leaf",t)}if(_.isDeferred)try{await _.loadIfDeferred()}catch(t){console.debug("Failed to load deferred PDF sidebar leaf",t)}if(this.isPdfSidebarLeaf(_))return!0;try{await _.openFile(e,{active:!1})}catch(t){return console.warn("Failed to open PDF file in sidebar leaf",t),!1}return this.isPdfSidebarLeaf(_)}isPdfAnnotationRaceError(_){let e=_ instanceof Error?_.message:typeof _=="string"?_:"";return e?/injectLinkAnnotations/i.test(e)||/render method must be called before/i.test(e):!1}delay(_){return new Promise(e=>setTimeout(e,_))}getPluginsRegistry(){var e;return(e=this.deps.app.plugins)==null?void 0:e.plugins}isLeafTabActive(_){var i;let e=_;if((i=e.parent)!=null&&i.activeLeaf)return e.parent.activeLeaf===_;let t=e.containerEl;if(t!=null&&t.classList.contains("is-active")||t!=null&&t.classList.contains("mod-active"))return!0;let n=e.tabHeaderEl;return!!(n!=null&&n.classList.contains("is-active")||n!=null&&n.classList.contains("mod-active"))}async resolvePageNumberForChunk(_,e){var l,c,d,p,u;if(!_||!e)return null;let t=this.deps.normalizeChunkIdForNote(e,_)||e,n=this.chunkPageCache.get(_);if(n&&n.has(t))return(l=n.get(t))!=null?l:null;let i=(0,ue.normalizePath)(`${Y}/${_}.json`);if(!await this.deps.app.vault.adapter.exists(i))return null;let a=Te(await this.deps.readChunkPayload(i)),s=Array.isArray(a==null?void 0:a.chunks)?a.chunks:[],o=new Map;for(let g of s){let y=Te(g),x=typeof(y==null?void 0:y.chunk_id)=="string"?y.chunk_id.trim():"";if(!x)continue;let k=Number.isFinite((c=y==null?void 0:y.page_start)!=null?c:NaN)?Number(y==null?void 0:y.page_start):null,h=Number.isFinite((d=y==null?void 0:y.page_end)!=null?d:NaN)?Number(y==null?void 0:y.page_end):null,b=k!=null?k:h;b!==null&&(o.set(x,b),o.set(`${_}:${x}`,b))}return this.chunkPageCache.set(_,o),(u=(p=o.get(t))!=null?p:o.get(e))!=null?u:null}};var Xe={"zrr-picker":`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
...
`);for(let g of d){let y=String(g.doc_id||"").trim(),x=String(g.chunk_id||"").trim(),k=String(g.page_start||"").trim(),h=String(g.page_end||"").trim(),b=String(g.title||"").trim(),w=String(g.section||"").trim(),P=String(g.score||"").trim(),S=String(g.authors||"").trim(),v=String(g.item_type||"").trim(),C=String(g.year||"").trim(),N=String(g.tags||"").trim(),D=String(g.chunk_tags||"").trim(),F=String(g.attachment_key||"").trim(),V=String(g.source_pdf||"").trim(),U=String(g.text||"").replace(/\s+/g," ").trim(),j=U.length>220?`${U.slice(0,220)}\u2026`:U,B=[y];x&&B.push(x),(k||h)&&B.push(`p.${k||"?"}-${h||"?"}`),p.push(B.filter(Boolean).join(" \u2022 ")),P&&p.push(`  score: ${P}`),b&&p.push(`  title: ${b}`),S&&p.push(`  authors: ${S}`),C&&p.push(`  year: ${C}`),v&&p.push(`  item_type: ${v}`),N&&p.push(`  tags: ${N}`),D&&p.push(`  chunk_tags: ${D}`),F&&p.push(`  attachment_key: ${F}`),w&&p.push(`  section: ${w}`),V&&p.push(`  source_pdf: ${V}`),j&&p.push(`  ${j}`),p.push("")}return p.join(`

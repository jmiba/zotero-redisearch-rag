"use strict";var Ye=Object.create;var ge=Object.defineProperty;var Qe=Object.getOwnPropertyDescriptor;var en=Object.getOwnPropertyNames;var nn=Object.getPrototypeOf,tn=Object.prototype.hasOwnProperty;var rn=(x,g)=>{for(var e in g)ge(x,e,{get:g[e],enumerable:!0})},ze=(x,g,e,n)=>{if(g&&typeof g=="object"||typeof g=="function")for(let t of en(g))!tn.call(x,t)&&t!==e&&ge(x,t,{get:()=>g[t],enumerable:!(n=Qe(g,t))||n.enumerable});return x};var ie=(x,g,e)=>(e=x!=null?Ye(nn(x)):{},ze(g||!x||!x.__esModule?ge(e,"default",{value:x,enumerable:!0}):e,x)),sn=x=>ze(ge({},"__esModule",{value:!0}),x);var yn={};rn(yn,{default:()=>be});module.exports=sn(yn);var p=require("obsidian"),ne=require("@codemirror/state"),$=require("@codemirror/view"),Y=require("child_process"),B=require("fs"),Ue=ie(require("http")),Ze=ie(require("https")),de=ie(require("net")),Ve=ie(require("os")),He=ie(require("tls")),C=ie(require("path")),ae=require("url"),he=require("crypto");var L=require("obsidian"),J=".zotero-redisearch-rag",X=`${J}/items`,H=`${J}/chunks`,Fe={pythonPath:"",pythonEnvLocation:"shared",dockerPath:"docker",redisUrl:"redis://127.0.0.1:6379",autoAssignRedisPort:!1,autoStartRedis:!0,zoteroBaseUrl:"http://127.0.0.1:23119/api",zoteroUserId:"0",webApiBaseUrl:"https://api.zotero.org",webApiLibraryType:"user",webApiLibraryId:"",webApiKey:"",outputPdfDir:"Zotero/PDFs",outputNoteDir:"Zotero/Notes",frontmatterTemplate:`doc_id: {{doc_id}}
...
item_json: {{item_json_yaml}}`,tagSanitizeMode:"kebab",noteBodyTemplate:"{{pdf_block}}{{docling_markdown}}",llmProviderProfiles:[{id:"lm-studio",name:"LM Studio",baseUrl:"http://localhost:1234/v1",apiKey:"lm-studio"},{id:"ollama",name:"Ollama",baseUrl:"http://localhost:11434/v1",apiKey:""},{id:"openrouter",name:"OpenRouter",baseUrl:"https://openrouter.ai/api/v1",apiKey:""},{id:"openai",name:"OpenAI",baseUrl:"https://api.openai.com/v1",apiKey:""}],chatOutputDir:"Zotero/Chats",copyPdfToVault:!0,createOcrLayeredPdf:!1,preferObsidianNoteForCitations:!0,ocrMode:"auto",ocrQualityThreshold:.5,chunkingMode:"page",enableLlmCleanup:!1,llmCleanupProviderProfileId:"lm-studio",llmCleanupBaseUrl:"http://127.0.0.1:1234/v1",llmCleanupApiKey:"",llmCleanupModel:"openai/gpt-oss-20b",llmCleanupTemperature:0,llmCleanupMinQuality:.35,llmCleanupMaxChars:2e3,embedProviderProfileId:"lm-studio",embedBaseUrl:"http://localhost:1234/v1",embedApiKey:"lm-studio",embedModel:"google/embedding-gemma-300m",embedIncludeMetadata:!0,embedSubchunkChars:3500,embedSubchunkOverlap:200,enableChunkTagging:!1,chatProviderProfileId:"lm-studio",chatBaseUrl:"http://127.0.0.1:1234/v1",chatApiKey:"",chatModel:"openai/gpt-oss-20b",chatTemperature:.2,chatHistoryMessages:6,chatPaneLocation:"right",enableFileLogging:!1,logFilePath:`${J}/logs/docling_extract.log`,redisIndex:"idx:zotero",redisPrefix:"zotero:chunk:"},fe=class extends L.PluginSettingTab{constructor(g,e){super(g,e),this.plugin=e}display(){let{containerEl:g}=this;g.empty();let e=()=>Array.isArray(this.plugin.settings.llmProviderProfiles)?this.plugin.settings.llmProviderProfiles:[],n=async _=>{this.plugin.settings.llmProviderProfiles=_,await this.plugin.saveSettings()},t=_=>{_.inputEl.type="password",_.inputEl.autocomplete="off",_.inputEl.spellcheck=!1};g.createEl("h2",{text:"Prerequisites"}),new L.Setting(g).setName("Python path").setDesc("Optional path to the Python interpreter used to create or run the plugin env. Leave blank to auto-detect (python3.13/3.12/3.11/3.10/python3/python, or py on Windows).").addText(_=>_.setPlaceholder("auto-detect").setValue(this.plugin.settings.pythonPath).onChange(async f=>{this.plugin.settings.pythonPath=f.trim(),await this.plugin.saveSettings()})),new L.Setting(g).setName("Python environment").setDesc("Create or update the plugin's Python env (location configured below).").addButton(_=>{_.setButtonText("Create/Update").setCta(),_.onClick(async()=>{_.setDisabled(!0);try{await this.plugin.setupPythonEnv()}finally{_.setDisabled(!1)}})}),new L.Setting(g).setName("Python env location").setDesc("Shared user cache can be reused across vaults; plugin folder keeps a per-vault env.").addDropdown(_=>{_.addOption("shared","Shared user cache"),_.addOption("plugin","Plugin folder (.venv)"),_.setValue(this.plugin.settings.pythonEnvLocation).onChange(async f=>{f!=="shared"&&f!=="plugin"||(this.plugin.settings.pythonEnvLocation=f,await this.plugin.saveSettings())})}),new L.Setting(g).setName("Docker/Podman path").setDesc("CLI path for Docker or Podman (used to start Redis Stack).").addText(_=>_.setPlaceholder("docker").setValue(this.plugin.settings.dockerPath).onChange(async f=>{this.plugin.settings.dockerPath=f.trim()||"docker",await this.plugin.saveSettings()})),new L.Setting(g).setName("Redis URL").addText(_=>_.setPlaceholder("redis://127.0.0.1:6379").setValue(this.plugin.settings.redisUrl).onChange(async f=>{this.plugin.settings.redisUrl=f.trim(),await this.plugin.saveSettings()})),new L.Setting(g).setName("Auto-assign Redis port").setDesc("When starting Redis stack, pick a free local port and update the Redis URL.").addToggle(_=>_.setValue(this.plugin.settings.autoAssignRedisPort).onChange(async f=>{this.plugin.settings.autoAssignRedisPort=f,await this.plugin.saveSettings()})),new L.Setting(g).setName("Auto-start Redis stack (Docker/Podman Compose)").setDesc("Requires Docker Desktop running and your vault path shared with Docker. Uses a vault-specific data dir at .obsidian/zotero-redisearch-rag/redis-data.").addToggle(_=>_.setValue(this.plugin.settings.autoStartRedis).onChange(async f=>{this.plugin.settings.autoStartRedis=f,await this.plugin.saveSettings()})),new L.Setting(g).setName("Start Redis stack now").setDesc("Restarts Docker/Podman Compose with the vault data directory.").addButton(_=>_.setButtonText("Start").onClick(async()=>{await this.plugin.startRedisStack()})),g.createEl("h2",{text:"Zotero Local API"}),new L.Setting(g).setName("Zotero base URL").setDesc("Local Zotero API base URL, e.g. http://127.0.0.1:23119/api").addText(_=>_.setPlaceholder("http://127.0.0.1:23119/api").setValue(this.plugin.settings.zoteroBaseUrl).onChange(async f=>{this.plugin.settings.zoteroBaseUrl=f.trim(),await this.plugin.saveSettings()}));let i=new L.Setting(g).setName("Zotero library").setDesc("Select your local library or a Zotero group library."),r=null,s=_=>{if(!r)return;let f=(this.plugin.settings.zoteroUserId||"0").trim()||"0";new Set(_.map(M=>M.value)).has(f)||(_=_.concat([{value:f,label:`Custom (${f})`}])),r.selectEl.options.length=0;for(let M of _)r.addOption(M.value,M.label);r.setValue(f)},a=async()=>{if(r){r.setDisabled(!0);try{let _=await this.plugin.fetchZoteroLibraryOptions();s(_)}finally{r.setDisabled(!1)}}};i.addDropdown(_=>{r=_;let f=(this.plugin.settings.zoteroUserId||"0").trim()||"0";_.addOption(f,"Loading..."),_.setValue(f),_.onChange(async P=>{this.plugin.settings.zoteroUserId=P.trim(),await this.plugin.saveSettings()})}),i.addButton(_=>{_.setButtonText("Refresh").onClick(async()=>{await a()})}),a(),g.createEl("h2",{text:"Zotero Web API"}),new L.Setting(g).setName("Web API base URL").setDesc("Zotero Web API base URL for write fallback, e.g. https://api.zotero.org").addText(_=>_.setPlaceholder("https://api.zotero.org").setValue(this.plugin.settings.webApiBaseUrl).onChange(async f=>{this.plugin.settings.webApiBaseUrl=f.trim(),await this.plugin.saveSettings()})),new L.Setting(g).setName("Web API library type").setDesc("Library type for Web API writes.").addDropdown(_=>_.addOption("user","user").addOption("group","group").setValue(this.plugin.settings.webApiLibraryType).onChange(async f=>{this.plugin.settings.webApiLibraryType=f,await this.plugin.saveSettings()})),new L.Setting(g).setName("Web API library ID").setDesc("Numeric Zotero user/group ID for Web API writes.").addText(_=>_.setPlaceholder("15218").setValue(this.plugin.settings.webApiLibraryId).onChange(async f=>{this.plugin.settings.webApiLibraryId=f.trim(),await this.plugin.saveSettings()})),new L.Setting(g).setName("Web API key").setDesc("Zotero API key for write fallback (from zotero.org).").addText(_=>{t(_),_.setPlaceholder("your-api-key").setValue(this.plugin.settings.webApiKey).onChange(async f=>{this.plugin.settings.webApiKey=f.trim(),await this.plugin.saveSettings()})}),g.createEl("h2",{text:"Output"}),new L.Setting(g).setName("PDF folder").addText(_=>_.setPlaceholder("zotero/pdfs").setValue(this.plugin.settings.outputPdfDir).onChange(async f=>{this.plugin.settings.outputPdfDir=f.trim(),await this.plugin.saveSettings()})),new L.Setting(g).setName("Notes folder").addText(_=>_.setPlaceholder("zotero/notes").setValue(this.plugin.settings.outputNoteDir).onChange(async f=>{this.plugin.settings.outputNoteDir=f.trim(),await this.plugin.saveSettings()})),new L.Setting(g).setName("Frontmatter template").setDesc("Template for note YAML frontmatter. Use {{var}} placeholders; leave blank to omit.").addTextArea(_=>{_.setValue(this.plugin.settings.frontmatterTemplate).onChange(async f=>{this.plugin.settings.frontmatterTemplate=f,await this.plugin.saveSettings()}),_.inputEl.rows=10,_.inputEl.style.width="100%"}),new L.Setting(g).setName("Tag sanitization").setDesc("Normalize Zotero tags for Obsidian (no spaces, punctuation trimmed).").addDropdown(_=>_.addOption("none","No change").addOption("camel","camelCase").addOption("pascal","PascalCase").addOption("snake","snake_case").addOption("kebab","kebab-case").setValue(this.plugin.settings.tagSanitizeMode==="replace"?"kebab":this.plugin.settings.tagSanitizeMode).onChange(async f=>{this.plugin.settings.tagSanitizeMode=f,await this.plugin.saveSettings()})),new L.Setting(g).setName("Note body template").setDesc("Template for the note body after frontmatter. Use {{pdf_block}} and {{docling_markdown}} placeholders.").addTextArea(_=>{_.setValue(this.plugin.settings.noteBodyTemplate).onChange(async f=>{this.plugin.settings.noteBodyTemplate=f,await this.plugin.saveSettings()}),_.inputEl.rows=8,_.inputEl.style.width="100%"}),g.createEl("h2",{text:"LLM Provider Profiles"});let o=g.createDiv({cls:"zrr-llm-profiles"}),l=()=>{o.empty();let _=e();_.length||o.createEl("p",{text:"No profiles yet. Add one below."});for(let f of _){let P=o.createEl("details",{cls:"zrr-profile"});_.length===1&&(P.open=!0);let M=P.createEl("summary",{text:f.name||f.id||"Profile"});M.addClass("zrr-profile-title");let te=P.createDiv({cls:"zrr-profile-body"});new L.Setting(te).setName("Profile name").addText(Q=>Q.setPlaceholder("My provider").setValue(f.name||"").onChange(async ee=>{f.name=ee.trim(),M.textContent=f.name||f.id||"Profile",await n(e())})),new L.Setting(te).setName("Base URL").addText(Q=>Q.setPlaceholder("http://localhost:1234/v1").setValue(f.baseUrl||"").onChange(async ee=>{f.baseUrl=ee.trim(),await n(e())})),new L.Setting(te).setName("API key").setDesc("Stored in settings (not encrypted).").addText(Q=>{t(Q),Q.setPlaceholder("sk-...").setValue(f.apiKey||"").onChange(async ee=>{f.apiKey=ee.trim(),await n(e())})}),new L.Setting(te).setName("Remove profile").setDesc("Deletes this saved profile.").addButton(Q=>Q.setButtonText("Delete profile").onClick(async()=>{let ee=e().filter(le=>le.id!==f.id);this.plugin.settings.embedProviderProfileId=this.plugin.settings.embedProviderProfileId===f.id?"":this.plugin.settings.embedProviderProfileId,this.plugin.settings.chatProviderProfileId=this.plugin.settings.chatProviderProfileId===f.id?"":this.plugin.settings.chatProviderProfileId,this.plugin.settings.llmCleanupProviderProfileId=this.plugin.settings.llmCleanupProviderProfileId===f.id?"":this.plugin.settings.llmCleanupProviderProfileId,await n(ee),l()}))}new L.Setting(o).addButton(f=>f.setButtonText("Add profile").onClick(async()=>{let P=`profile-${Date.now().toString(36)}`,M=e().concat([{id:P,name:"Custom",baseUrl:"",apiKey:""}]);await n(M),l()}))};l(),new L.Setting(g).setName("Saved chats folder").setDesc("Where exported chat notes are stored (vault-relative).").addText(_=>_.setPlaceholder("zotero/chats").setValue(this.plugin.settings.chatOutputDir).onChange(async f=>{this.plugin.settings.chatOutputDir=f.trim()||"zotero/chats",await this.plugin.saveSettings()})),new L.Setting(g).setName("Copy PDFs into vault").setDesc("Disable to use Zotero storage paths directly. If a local file path is unavailable, the plugin temporarily copies the PDF into the vault for processing.").addToggle(_=>_.setValue(this.plugin.settings.copyPdfToVault).onChange(async f=>{this.plugin.settings.copyPdfToVault=f,!f&&this.plugin.settings.createOcrLayeredPdf&&(this.plugin.settings.createOcrLayeredPdf=!1),await this.plugin.saveSettings(),this.display()})),new L.Setting(g).setName("Create OCR-layered PDF copy").setDesc("When OCR is used, replace the vault PDF with a Tesseract text layer (requires Copy PDFs into vault).").addToggle(_=>{let f=this.plugin.settings.copyPdfToVault;_.setValue(f?this.plugin.settings.createOcrLayeredPdf:!1).setDisabled(!f).onChange(async P=>{if(!this.plugin.settings.copyPdfToVault){this.plugin.settings.createOcrLayeredPdf=!1,await this.plugin.saveSettings();return}this.plugin.settings.createOcrLayeredPdf=P,await this.plugin.saveSettings()})}),new L.Setting(g).setName("Prefer Obsidian note for citations").setDesc("Link citations to the Obsidian note when available; otherwise use Zotero deep links.").addToggle(_=>_.setValue(this.plugin.settings.preferObsidianNoteForCitations).onChange(async f=>{this.plugin.settings.preferObsidianNoteForCitations=f,await this.plugin.saveSettings()})),g.createEl("h2",{text:"Docling"}),new L.Setting(g).setName("OCR mode").setDesc("auto: skip OCR when text is readable; force if bad: OCR only when text looks poor; force: always OCR.").addDropdown(_=>_.addOption("auto","auto").addOption("force_low_quality","force if quality is bad").addOption("force","force").setValue(this.plugin.settings.ocrMode).onChange(async f=>{this.plugin.settings.ocrMode=f,await this.plugin.saveSettings()})),new L.Setting(g).setName("Text quality threshold").setDesc("Lower values are stricter; below this threshold the text is treated as low quality.").addSlider(_=>{_.setLimits(0,1,.05).setValue(this.plugin.settings.ocrQualityThreshold).setDynamicTooltip().onChange(async f=>{this.plugin.settings.ocrQualityThreshold=f,await this.plugin.saveSettings()})}),new L.Setting(g).setName("Chunking").setDesc("page or section").addDropdown(_=>_.addOption("page","page").addOption("section","section").setValue(this.plugin.settings.chunkingMode).onChange(async f=>{this.plugin.settings.chunkingMode=f,await this.plugin.saveSettings()})),g.createEl("h2",{text:"OCR cleanup"}),new L.Setting(g).setName("LLM cleanup for low-quality chunks").setDesc("Automatic AI cleanup for poor OCR at import. Can be slow/costly.").addToggle(_=>_.setValue(this.plugin.settings.enableLlmCleanup).onChange(async f=>{this.plugin.settings.enableLlmCleanup=f,await this.plugin.saveSettings()}));let c=null,d=null,u=null,h=async()=>{},m=async(_,f=!0)=>{let P=_.trim();this.plugin.settings.llmCleanupBaseUrl=P,f&&(this.plugin.settings.llmCleanupProviderProfileId="",c&&c.setValue("custom")),d&&d.setValue(P),await this.plugin.saveSettings()},y=async _=>{let f=e().find(P=>P.id===_);this.plugin.settings.llmCleanupProviderProfileId=_,f&&(this.plugin.settings.llmCleanupBaseUrl=f.baseUrl,this.plugin.settings.llmCleanupApiKey=f.apiKey,d==null||d.setValue(f.baseUrl),u==null||u.setValue(f.apiKey)),await this.plugin.saveSettings(),await h()};new L.Setting(g).setName("LLM cleanup provider profile").setDesc("Select a profile to populate base URL and API key.").addDropdown(_=>{c=_,_.addOption("custom","Custom (manual)");for(let P of e())_.addOption(P.id,P.name||P.id);let f=this.plugin.settings.llmCleanupProviderProfileId;_.setValue(f&&e().some(P=>P.id===f)?f:"custom"),_.onChange(async P=>{if(P==="custom"){this.plugin.settings.llmCleanupProviderProfileId="",await this.plugin.saveSettings();return}await y(P)})}),new L.Setting(g).setName("LLM cleanup base URL").setDesc("OpenAI-compatible endpoint, e.g. http://127.0.0.1:1234/v1").addText(_=>{d=_,_.setPlaceholder("http://127.0.0.1:1234/v1").setValue(this.plugin.settings.llmCleanupBaseUrl).onChange(async f=>{await m(f)})}),new L.Setting(g).setName("LLM cleanup API key").setDesc("Optional API key for the cleanup endpoint.").addText(_=>{u=_,t(_),_.setPlaceholder("sk-...").setValue(this.plugin.settings.llmCleanupApiKey).onChange(async f=>{this.plugin.settings.llmCleanupApiKey=f.trim(),this.plugin.settings.llmCleanupProviderProfileId="",c&&c.setValue("custom"),await this.plugin.saveSettings()})});let w=new L.Setting(g).setName("LLM cleanup model").setDesc("Select a cleanup-capable model from the provider."),b=null,k=_=>{if(!b)return;let f=(this.plugin.settings.llmCleanupModel||"").trim(),P=new Set(_.map(M=>M.value));f&&!P.has(f)&&(_=_.concat([{value:f,label:`Custom (${f})`}])),b.selectEl.options.length=0;for(let M of _)b.addOption(M.value,M.label);f&&b.setValue(f)};h=async()=>{if(b){b.setDisabled(!0);try{let _=await this.plugin.fetchCleanupModelOptions();k(_)}finally{b.setDisabled(!1)}}},w.addDropdown(_=>{b=_;let f=(this.plugin.settings.llmCleanupModel||"").trim();_.addOption(f||"loading","Loading..."),_.setValue(f||"loading"),_.onChange(async P=>{this.plugin.settings.llmCleanupModel=P.trim(),await this.plugin.saveSettings()})}),w.addButton(_=>{_.setButtonText("Refresh").onClick(async()=>{await h()})}),h(),new L.Setting(g).setName("LLM cleanup temperature").setDesc("Lower is more conservative.").addText(_=>_.setPlaceholder("0.0").setValue(String(this.plugin.settings.llmCleanupTemperature)).onChange(async f=>{let P=Number.parseFloat(f);this.plugin.settings.llmCleanupTemperature=Number.isFinite(P)?P:0,await this.plugin.saveSettings()})),new L.Setting(g).setName("LLM cleanup min quality").setDesc("Only run cleanup when chunk quality is below this threshold (0-1).").addSlider(_=>_.setLimits(0,1,.05).setValue(this.plugin.settings.llmCleanupMinQuality).setDynamicTooltip().onChange(async f=>{this.plugin.settings.llmCleanupMinQuality=f,await this.plugin.saveSettings()})),new L.Setting(g).setName("LLM cleanup max chars").setDesc("Skip cleanup for chunks longer than this limit.").addText(_=>_.setPlaceholder("2000").setValue(String(this.plugin.settings.llmCleanupMaxChars)).onChange(async f=>{let P=Number.parseInt(f,10);this.plugin.settings.llmCleanupMaxChars=Number.isFinite(P)?P:2e3,await this.plugin.saveSettings()})),g.createEl("h2",{text:"Text Embedding"});let v=null,E=null,R=null,D=async()=>{},N=async(_,f=!0)=>{let P=_.trim();this.plugin.settings.embedBaseUrl=P,f&&(this.plugin.settings.embedProviderProfileId="",v&&v.setValue("custom")),E&&E.setValue(P),await this.plugin.saveSettings()},F=async _=>{let f=e().find(P=>P.id===_);this.plugin.settings.embedProviderProfileId=_,f&&(this.plugin.settings.embedBaseUrl=f.baseUrl,this.plugin.settings.embedApiKey=f.apiKey,E==null||E.setValue(f.baseUrl),R==null||R.setValue(f.apiKey)),await this.plugin.saveSettings(),await D()};new L.Setting(g).setName("Embeddings provider profile").setDesc("Select a profile to populate base URL and API key.").addDropdown(_=>{v=_,_.addOption("custom","Custom (manual)");for(let P of e())_.addOption(P.id,P.name||P.id);let f=this.plugin.settings.embedProviderProfileId;_.setValue(f&&e().some(P=>P.id===f)?f:"custom"),_.onChange(async P=>{if(P==="custom"){this.plugin.settings.embedProviderProfileId="",await this.plugin.saveSettings();return}await F(P)})}),new L.Setting(g).setName("Embeddings base URL").addText(_=>{E=_,_.setPlaceholder("http://localhost:1234/v1").setValue(this.plugin.settings.embedBaseUrl).onChange(async f=>{await N(f)})}),new L.Setting(g).setName("Embeddings API key").addText(_=>{R=_,t(_),_.setPlaceholder("lm-studio").setValue(this.plugin.settings.embedApiKey).onChange(async f=>{this.plugin.settings.embedApiKey=f.trim(),this.plugin.settings.embedProviderProfileId="",v&&v.setValue("custom"),await this.plugin.saveSettings()})});let I=new L.Setting(g).setName("Embeddings model").setDesc("Select an embeddings model from the provider."),q=null,S=_=>{if(!q)return;let f=(this.plugin.settings.embedModel||"").trim(),P=new Set(_.map(M=>M.value));f&&!P.has(f)&&(_=_.concat([{value:f,label:`Custom (${f})`}])),q.selectEl.options.length=0;for(let M of _)q.addOption(M.value,M.label);f&&q.setValue(f)};D=async()=>{if(q){q.setDisabled(!0);try{let _=await this.plugin.fetchEmbeddingModelOptions();S(_)}finally{q.setDisabled(!1)}}},I.addDropdown(_=>{q=_;let f=(this.plugin.settings.embedModel||"").trim();_.addOption(f||"loading","Loading..."),_.setValue(f||"loading"),_.onChange(async P=>{this.plugin.settings.embedModel=P.trim(),await this.plugin.saveSettings()})}),I.addButton(_=>{_.setButtonText("Refresh").onClick(async()=>{await D()})}),D(),new L.Setting(g).setName("Include metadata in embeddings").setDesc("Prepend title/authors/tags/section info before embedding chunks.").addToggle(_=>_.setValue(this.plugin.settings.embedIncludeMetadata).onChange(async f=>{this.plugin.settings.embedIncludeMetadata=f,await this.plugin.saveSettings()})),new L.Setting(g).setName("Embedding subchunk size (chars)").setDesc("Split long chunks into smaller subchunks for embedding only (0 disables).").addText(_=>_.setPlaceholder("1800").setValue(String(this.plugin.settings.embedSubchunkChars)).onChange(async f=>{let P=Number.parseInt(f,10);this.plugin.settings.embedSubchunkChars=Number.isFinite(P)?Math.max(0,P):3500,await this.plugin.saveSettings()})),new L.Setting(g).setName("Embedding subchunk overlap (chars)").setDesc("Overlap between embedding subchunks to keep context intact.").addText(_=>_.setPlaceholder("200").setValue(String(this.plugin.settings.embedSubchunkOverlap)).onChange(async f=>{let P=Number.parseInt(f,10);this.plugin.settings.embedSubchunkOverlap=Number.isFinite(P)?Math.max(0,P):200,await this.plugin.saveSettings()})),new L.Setting(g).setName("Generate LLM tags for chunks").setDesc("Use the OCR cleanup model to tag chunks before indexing.").addToggle(_=>_.setValue(this.plugin.settings.enableChunkTagging).onChange(async f=>{this.plugin.settings.enableChunkTagging=f,await this.plugin.saveSettings()})),g.createEl("h2",{text:"Chat LLM"});let T=null,O=null,U=null,W=async()=>{},A=async(_,f=!0)=>{let P=_.trim();this.plugin.settings.chatBaseUrl=P,f&&(this.plugin.settings.chatProviderProfileId="",T&&T.setValue("custom")),O&&O.setValue(P),await this.plugin.saveSettings()},z=async _=>{let f=e().find(P=>P.id===_);this.plugin.settings.chatProviderProfileId=_,f&&(this.plugin.settings.chatBaseUrl=f.baseUrl,this.plugin.settings.chatApiKey=f.apiKey,O==null||O.setValue(f.baseUrl),U==null||U.setValue(f.apiKey)),await this.plugin.saveSettings(),await W()};new L.Setting(g).setName("Chat provider profile").setDesc("Select a profile to populate base URL and API key.").addDropdown(_=>{T=_,_.addOption("custom","Custom (manual)");for(let P of e())_.addOption(P.id,P.name||P.id);let f=this.plugin.settings.chatProviderProfileId;_.setValue(f&&e().some(P=>P.id===f)?f:"custom"),_.onChange(async P=>{if(P==="custom"){this.plugin.settings.chatProviderProfileId="",await this.plugin.saveSettings();return}await z(P)})}),new L.Setting(g).setName("Chat base URL").setDesc("OpenAI-compatible base URL for chat requests.").addText(_=>{O=_,_.setPlaceholder("http://localhost:1234/v1").setValue(this.plugin.settings.chatBaseUrl).onChange(async f=>{await A(f)})}),new L.Setting(g).setName("Chat API key").addText(_=>{U=_,t(_),_.setPlaceholder("lm-studio").setValue(this.plugin.settings.chatApiKey).onChange(async f=>{this.plugin.settings.chatApiKey=f.trim(),this.plugin.settings.chatProviderProfileId="",T&&T.setValue("custom"),await this.plugin.saveSettings()})});let Z=new L.Setting(g).setName("Chat model").setDesc("Select a chat-capable model from the provider."),V=null,oe=_=>{if(!V)return;let f=(this.plugin.settings.chatModel||"").trim(),P=new Set(_.map(M=>M.value));f&&!P.has(f)&&(_=_.concat([{value:f,label:`Custom (${f})`}])),V.selectEl.options.length=0;for(let M of _)V.addOption(M.value,M.label);f&&V.setValue(f)};W=async()=>{if(V){V.setDisabled(!0);try{let _=await this.plugin.fetchChatModelOptions();oe(_)}finally{V.setDisabled(!1)}}},Z.addDropdown(_=>{V=_;let f=(this.plugin.settings.chatModel||"").trim();_.addOption(f||"loading","Loading..."),_.setValue(f||"loading"),_.onChange(async P=>{this.plugin.settings.chatModel=P.trim(),await this.plugin.saveSettings()})}),Z.addButton(_=>{_.setButtonText("Refresh").onClick(async()=>{await W()})}),W(),new L.Setting(g).setName("Temperature").addText(_=>_.setPlaceholder("0.2").setValue(String(this.plugin.settings.chatTemperature)).onChange(async f=>{let P=Number.parseFloat(f);this.plugin.settings.chatTemperature=Number.isFinite(P)?P:.2,await this.plugin.saveSettings()})),new L.Setting(g).setName("Chat history messages").setDesc("Number of recent messages to include for conversational continuity (0 disables).").addText(_=>_.setPlaceholder("6").setValue(String(this.plugin.settings.chatHistoryMessages)).onChange(async f=>{let P=Number.parseInt(f,10);this.plugin.settings.chatHistoryMessages=Number.isFinite(P)?Math.max(0,P):6,await this.plugin.saveSettings()})),new L.Setting(g).setName("Chat panel location").setDesc("Where to open the chat view by default.").addDropdown(_=>_.addOption("right","Right sidebar").addOption("main","Main window").setValue(this.plugin.settings.chatPaneLocation).onChange(async f=>{this.plugin.settings.chatPaneLocation=f,await this.plugin.saveSettings()})),g.createEl("h2",{text:"Logging"}),new L.Setting(g).setName("Enable logging to file").setDesc("Write plugin logs to a file.").addToggle(_=>_.setValue(this.plugin.settings.enableFileLogging).onChange(async f=>{this.plugin.settings.enableFileLogging=f,await this.plugin.saveSettings()})),new L.Setting(g).setName("Log file path (vault-relative)").setDesc("Where to write the log file. Keep inside the vault.").addText(_=>_.setPlaceholder(`${J}/logs/docling_extract.log`).setValue(this.plugin.settings.logFilePath).onChange(async f=>{this.plugin.settings.logFilePath=f.trim()||`${J}/logs/docling_extract.log`,await this.plugin.saveSettings()})),new L.Setting(g).setName("View or clear log").setDesc("Open the log file or clear it.").addButton(_=>_.setButtonText("Open log").onClick(async()=>{var f,P;await((P=(f=this.plugin).openLogFile)==null?void 0:P.call(f))})).addButton(_=>_.setButtonText("Clear log").onClick(async()=>{var f,P;await((P=(f=this.plugin).clearLogFile)==null?void 0:P.call(f))})),g.createEl("h2",{text:"Maintenance"}),new L.Setting(g).setName("Reindex Redis from cached chunks").setDesc("Rebuild the Redis index from cached chunk JSON files.").addButton(_=>_.setButtonText("Reindex").onClick(async()=>{await this.plugin.reindexRedisFromCache()})),new L.Setting(g).setName("Recreate missing notes from cache").setDesc("Rebuild missing notes using cached Zotero items and chunks.").addButton(_=>_.setButtonText("Recreate").onClick(async()=>{await this.plugin.recreateMissingNotesFromCache()})).addButton(_=>_.setButtonText("Cancel").onClick(()=>{this.plugin.cancelRecreateMissingNotesFromCache()}))}};var ke={"zrr-picker":`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
...
`).replace(/[ \t]*\n[ \t]*/g,`
`).trim()}scheduleNoteSync(e){let n=this.noteSyncTimers.get(e.path);n!==void 0&&window.clearTimeout(n);let t=window.setTimeout(()=>{this.noteSyncTimers.delete(e.path),this.syncNoteToRedis(e)},1200);this.noteSyncTimers.set(e.path,t)}escapeRegExp(e){return e.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")}formatCitationPageLabel(e){let n=e.page_start?String(e.page_start):"",t=e.page_end?String(e.page_end):"";if(n&&(!t||n===t))return n;if(n&&t)return`${n} - ${t}`;let i=(e.pages||"").trim();if(!i)return"?";let r=i.match(/^(\d+)\s*-\s*(\d+)$/);return r?r[1]===r[2]?r[1]:`${r[1]} - ${r[2]}`:i.replace("-"," - ")}normalizeChunkIdForNote(e,n){if(!e)return null;let t=String(e);if(n&&t.startsWith(`${n}:`))return t.slice(n.length+1);if(t.includes(":")){let i=t.split(":");if(i.length>1&&n&&i[0]===n)return i.slice(1).join(":")}return t}async syncNoteToRedis(e){var n,t,i;if(!this.noteSyncInFlight.has(e.path)&&!this.noteSyncSuppressed.has(e.path)){this.noteSyncInFlight.add(e.path);try{let r=await this.app.vault.read(e),s=this.extractSyncSection(r);if(!s)return;let a=await this.resolveDocIdForNote(e,r);if(!a)return;let o=this.parseSyncedChunkBlocks(s);if(!o.length)return;let l=(0,p.normalizePath)(`${H}/${a}.json`),c=this.app.vault.adapter;if(!await c.exists(l))return;let d=await this.readChunkPayload(l);if(!d)return;let u=Array.isArray(d.chunks)?d.chunks:[],h=new Map;for(let v of u){let E=typeof(v==null?void 0:v.chunk_id)=="string"?v.chunk_id:String((n=v==null?void 0:v.chunk_id)!=null?n:"");E&&h.set(E,v)}let m=new Set,y=new Set,w=new Set,b=new Set,k=!1;for(let v of o){let E=v.chunkId;if(!E)continue;m.add(E);let R=h.get(E);if(!R){console.warn(`Sync note: chunk id not found in cache (${E})`);continue}if(v.excludeFlag){R.excluded!==!0&&(R.excluded=!0,k=!0);let F=this.normalizeChunkText(v.text);F&&F!==String((t=R.text)!=null?t:"")&&(R.text=F,R.char_count=F.length,k=!0),w.add(E);continue}if(R.excluded&&(R.excluded=!1,k=!0,y.add(E)),!v.text.trim()){w.add(E),b.add(E);continue}let D=this.normalizeChunkText(v.text);if(!D){w.add(E),b.add(E);continue}let N=typeof R.text=="string"?R.text:String((i=R.text)!=null?i:"");D!==N&&(R.text=D,R.char_count=D.length,y.add(E),k=!0)}for(let v of h.keys())m.has(v)||(w.add(v),b.add(v));if(!y.size&&!w.size&&!b.size&&!k)return;b.size&&(d.chunks=u.filter(v=>{var R;let E=typeof(v==null?void 0:v.chunk_id)=="string"?v.chunk_id:String((R=v==null?void 0:v.chunk_id)!=null?R:"");return E&&!b.has(E)}),k=!0),(k||b.size)&&await c.write(l,JSON.stringify(d,null,2)),await this.reindexChunkUpdates(a,l,Array.from(y),Array.from(w))}catch(r){console.warn("Failed to sync note edits to Redis",r)}finally{this.noteSyncInFlight.delete(e.path)}}}extractSyncSection(e){let n=se.exec(e);if(!n)return null;let t=e.slice(n.index+n[0].length),i=pe.exec(t);return i?t.slice(0,i.index):null}extractDocIdFromSyncMarker(e){var r;let n=se.exec(e);if(!n)return null;let i=((r=n[0])!=null?r:"").match(/doc_id=([\"']?)([^\"'\s]+)\1/i);return i?i[2].trim():null}parseSyncedChunkBlocks(e){var o;let n=e.split(/\r?\n/),t=[],i="",r=!1,s=[],a=()=>{i&&(t.push({chunkId:i,text:s.join(`
`).trim(),excludeFlag:r}),i="",r=!1,s=[])};for(let l of n){let c=l.match(ue);if(c){a();let d=(o=c[1])!=null?o:"",u=d.match(/id=([\"']?)([^\"'\s]+)\1/i),h=u?u[2].trim():"";if(!h)continue;i=h,r=/\bexclude\b/i.test(d)||/\bdelete\b/i.test(d),s=[];continue}if(Pe.test(l)){a();continue}if(i){if(me.test(l)){r=!0;continue}s.push(l)}}return a(),t}normalizeChunkText(e){return e.split(/\r?\n/).map(n=>n.replace(/\s+/g," ").trim()).filter((n,t,i)=>!(n===""&&i[t-1]==="")).join(`

"use strict";var xe=Object.create;var H=Object.defineProperty;var we=Object.getOwnPropertyDescriptor;var ve=Object.getOwnPropertyNames;var ke=Object.getPrototypeOf,Pe=Object.prototype.hasOwnProperty;var Ce=(C,u)=>{for(var e in u)H(C,e,{get:u[e],enumerable:!0})},de=(C,u,e,n)=>{if(u&&typeof u=="object"||typeof u=="function")for(let t of ve(u))!Pe.call(C,t)&&t!==e&&H(C,t,{get:()=>u[t],enumerable:!(n=we(u,t))||n.enumerable});return C};var W=(C,u,e)=>(e=C!=null?xe(ke(C)):{},de(u||!C||!C.__esModule?H(e,"default",{value:C,enumerable:!0}):e,C)),Se=C=>de(H({},"__esModule",{value:!0}),C);var Ne={};Ce(Ne,{default:()=>Y});module.exports=Se(Ne);var d=require("obsidian"),V=require("@codemirror/state"),M=require("@codemirror/view"),q=require("child_process"),D=require("fs"),ye=W(require("http")),be=W(require("https")),se=W(require("net")),f=W(require("path")),Z=require("url"),J=require("crypto");var P=require("obsidian"),z=".zotero-redisearch-rag",$=`${z}/items`,F=`${z}/chunks`,pe={zoteroBaseUrl:"http://127.0.0.1:23119/api",zoteroUserId:"0",webApiBaseUrl:"https://api.zotero.org",webApiLibraryType:"user",webApiLibraryId:"",webApiKey:"",pythonPath:"python3",dockerPath:"docker",autoStartRedis:!1,copyPdfToVault:!0,frontmatterTemplate:`doc_id: {{doc_id}}
...
item_type: {{item_type}}
pdf_link: {{pdf_link}}
item_json: {{item_json}}`,outputPdfDir:"zotero/pdfs",outputNoteDir:"zotero/notes",chatOutputDir:"zotero/chats",chatPaneLocation:"right",redisUrl:"redis://127.0.0.1:6379",autoAssignRedisPort:!0,redisIndex:"idx:zotero",redisPrefix:"zotero:chunk:",embedBaseUrl:"http://localhost:1234/v1",embedApiKey:"lm-studio",embedModel:"google/embedding-gemma-300m",chatBaseUrl:"http://127.0.0.1:1234/v1",chatApiKey:"",chatModel:"openai/gpt-oss-20b",chatTemperature:.2,chatHistoryMessages:6,ocrMode:"auto",chunkingMode:"page",maxChunkChars:4e3,chunkOverlapChars:250,removeImagePlaceholders:!0,ocrQualityThreshold:.5,enableLlmCleanup:!1,llmCleanupBaseUrl:"http://127.0.0.1:1234/v1",llmCleanupApiKey:"",llmCleanupModel:"openai/gpt-oss-20b",llmCleanupTemperature:0,llmCleanupMinQuality:.35,llmCleanupMaxChars:2e3,enableFileLogging:!1,logFilePath:`${z}/logs/docling_extract.log`,createOcrLayeredPdf:!1,preferVaultPdfForCitations:!1},K=class extends P.PluginSettingTab{constructor(u,e){super(u,e),this.plugin=e}display(){let{containerEl:u}=this;u.empty(),u.createEl("h2",{text:"Zotero RAG Settings"}),new P.Setting(u).setName("Zotero base URL").setDesc("Local Zotero API base URL, e.g. http://127.0.0.1:23119/api").addText(e=>e.setPlaceholder("http://127.0.0.1:23119/api").setValue(this.plugin.settings.zoteroBaseUrl).onChange(async n=>{this.plugin.settings.zoteroBaseUrl=n.trim(),await this.plugin.saveSettings()})),new P.Setting(u).setName("Zotero user ID").setDesc("Use 0 for local library. You can also enter users/<id> or groups/<id>.").addText(e=>e.setPlaceholder("123456").setValue(this.plugin.settings.zoteroUserId).onChange(async n=>{this.plugin.settings.zoteroUserId=n.trim(),await this.plugin.saveSettings()})),u.createEl("h3",{text:"Zotero Web API (optional fallback)"}),new P.Setting(u).setName("Web API base URL").setDesc("Zotero Web API base URL for write fallback, e.g. https://api.zotero.org").addText(e=>e.setPlaceholder("https://api.zotero.org").setValue(this.plugin.settings.webApiBaseUrl).onChange(async n=>{this.plugin.settings.webApiBaseUrl=n.trim(),await this.plugin.saveSettings()})),new P.Setting(u).setName("Web API library type").setDesc("Library type for Web API writes.").addDropdown(e=>e.addOption("user","user").addOption("group","group").setValue(this.plugin.settings.webApiLibraryType).onChange(async n=>{this.plugin.settings.webApiLibraryType=n,await this.plugin.saveSettings()})),new P.Setting(u).setName("Web API library ID").setDesc("Numeric Zotero user/group ID for Web API writes.").addText(e=>e.setPlaceholder("15218").setValue(this.plugin.settings.webApiLibraryId).onChange(async n=>{this.plugin.settings.webApiLibraryId=n.trim(),await this.plugin.saveSettings()})),new P.Setting(u).setName("Web API key").setDesc("Zotero API key for write fallback (from zotero.org).").addText(e=>e.setPlaceholder("your-api-key").setValue(this.plugin.settings.webApiKey).onChange(async n=>{this.plugin.settings.webApiKey=n.trim(),await this.plugin.saveSettings()})),u.createEl("h3",{text:"Prerequisites"}),new P.Setting(u).setName("Python path").setDesc("Optional path to the Python interpreter used to create or run the plugin env.").addText(e=>e.setPlaceholder("python3").setValue(this.plugin.settings.pythonPath).onChange(async n=>{this.plugin.settings.pythonPath=n.trim(),await this.plugin.saveSettings()})),new P.Setting(u).setName("Python environment").setDesc("Create or update the plugin's Python env (.venv in the plugin folder).").addButton(e=>{e.setButtonText("Create/Update").setCta(),e.onClick(async()=>{e.setDisabled(!0);try{await this.plugin.setupPythonEnv()}finally{e.setDisabled(!1)}})}),new P.Setting(u).setName("Docker/Podman path").setDesc("CLI path for Docker or Podman (used to start Redis Stack).").addText(e=>e.setPlaceholder("docker").setValue(this.plugin.settings.dockerPath).onChange(async n=>{this.plugin.settings.dockerPath=n.trim()||"docker",await this.plugin.saveSettings()})),new P.Setting(u).setName("Redis URL").addText(e=>e.setPlaceholder("redis://127.0.0.1:6379").setValue(this.plugin.settings.redisUrl).onChange(async n=>{this.plugin.settings.redisUrl=n.trim(),await this.plugin.saveSettings()})),new P.Setting(u).setName("Auto-assign Redis port").setDesc("When starting Redis Stack, pick a free local port and update the Redis URL.").addToggle(e=>e.setValue(this.plugin.settings.autoAssignRedisPort).onChange(async n=>{this.plugin.settings.autoAssignRedisPort=n,await this.plugin.saveSettings()})),new P.Setting(u).setName("Auto-start Redis Stack (Docker/Podman Compose)").setDesc("Requires Docker Desktop running and your vault path shared with Docker.").addToggle(e=>e.setValue(this.plugin.settings.autoStartRedis).onChange(async n=>{this.plugin.settings.autoStartRedis=n,await this.plugin.saveSettings()})),new P.Setting(u).setName("Start Redis Stack now").setDesc("Restarts Docker/Podman Compose with the vault data directory.").addButton(e=>e.setButtonText("Start").onClick(async()=>{await this.plugin.startRedisStack()})),new P.Setting(u).setName("Copy PDFs into vault").setDesc("Disable to use Zotero storage paths directly. If a local file path is unavailable, the plugin temporarily copies the PDF into the vault for processing.").addToggle(e=>e.setValue(this.plugin.settings.copyPdfToVault).onChange(async n=>{this.plugin.settings.copyPdfToVault=n,!n&&this.plugin.settings.createOcrLayeredPdf&&(this.plugin.settings.createOcrLayeredPdf=!1),await this.plugin.saveSettings(),this.display()})),new P.Setting(u).setName("Create OCR-layered PDF copy").setDesc("When OCR is used, replace the vault PDF with a Tesseract text layer (requires Copy PDFs into vault).").addToggle(e=>{let n=this.plugin.settings.copyPdfToVault;e.setValue(n?this.plugin.settings.createOcrLayeredPdf:!1).setDisabled(!n).onChange(async t=>{if(!this.plugin.settings.copyPdfToVault){this.plugin.settings.createOcrLayeredPdf=!1,await this.plugin.saveSettings();return}this.plugin.settings.createOcrLayeredPdf=t,await this.plugin.saveSettings()})}),new P.Setting(u).setName("Frontmatter template").setDesc("Use {{doc_id}}, {{zotero_key}}, {{title}}, {{title_yaml}}, {{year}}, {{date}}, {{authors}}, {{authors_yaml}}, {{tags}}, {{item_type}}, {{pdf_link}}").addTextArea(e=>{e.inputEl.rows=8,e.setValue(this.plugin.settings.frontmatterTemplate).onChange(async n=>{this.plugin.settings.frontmatterTemplate=n,await this.plugin.saveSettings()})}),u.createEl("h3",{text:"Output folders (vault-relative)"}),new P.Setting(u).setName("PDF folder").addText(e=>e.setPlaceholder("zotero/pdfs").setValue(this.plugin.settings.outputPdfDir).onChange(async n=>{this.plugin.settings.outputPdfDir=n.trim(),await this.plugin.saveSettings()})),new P.Setting(u).setName("Notes folder").addText(e=>e.setPlaceholder("zotero/notes").setValue(this.plugin.settings.outputNoteDir).onChange(async n=>{this.plugin.settings.outputNoteDir=n.trim(),await this.plugin.saveSettings()})),new P.Setting(u).setName("Saved chats folder").setDesc("Where exported chat notes are stored (vault-relative).").addText(e=>e.setPlaceholder("zotero/chats").setValue(this.plugin.settings.chatOutputDir).onChange(async n=>{this.plugin.settings.chatOutputDir=n.trim()||"zotero/chats",await this.plugin.saveSettings()})),new P.Setting(u).setName("Prefer vault PDF for citations").setDesc("Use vault PDFs for citation links when available instead of Zotero deep links.").addToggle(e=>e.setValue(this.plugin.settings.preferVaultPdfForCitations).onChange(async n=>{this.plugin.settings.preferVaultPdfForCitations=n,await this.plugin.saveSettings()})),u.createEl("h3",{text:"Embeddings (LM Studio)"}),new P.Setting(u).setName("Embeddings base URL").addText(e=>e.setPlaceholder("http://localhost:1234/v1").setValue(this.plugin.settings.embedBaseUrl).onChange(async n=>{this.plugin.settings.embedBaseUrl=n.trim(),await this.plugin.saveSettings()})),new P.Setting(u).setName("Embeddings API key").addText(e=>e.setPlaceholder("lm-studio").setValue(this.plugin.settings.embedApiKey).onChange(async n=>{this.plugin.settings.embedApiKey=n.trim(),await this.plugin.saveSettings()})),new P.Setting(u).setName("Embeddings model").addText(e=>e.setPlaceholder("google/embedding-gemma-300m").setValue(this.plugin.settings.embedModel).onChange(async n=>{this.plugin.settings.embedModel=n.trim(),await this.plugin.saveSettings()})),u.createEl("h3",{text:"Chat LLM"}),new P.Setting(u).setName("Chat base URL").setDesc("OpenAI-compatible chat endpoint base URL").addText(e=>e.setPlaceholder("http://localhost:1234/v1").setValue(this.plugin.settings.chatBaseUrl).onChange(async n=>{this.plugin.settings.chatBaseUrl=n.trim(),await this.plugin.saveSettings()})),new P.Setting(u).setName("Chat API key").addText(e=>e.setPlaceholder("lm-studio").setValue(this.plugin.settings.chatApiKey).onChange(async n=>{this.plugin.settings.chatApiKey=n.trim(),await this.plugin.saveSettings()})),new P.Setting(u).setName("Chat model").addText(e=>e.setPlaceholder("meta-llama/llama-3.1-405b-instruct").setValue(this.plugin.settings.chatModel).onChange(async n=>{this.plugin.settings.chatModel=n.trim(),await this.plugin.saveSettings()})),new P.Setting(u).setName("Temperature").addText(e=>e.setPlaceholder("0.2").setValue(String(this.plugin.settings.chatTemperature)).onChange(async n=>{let t=Number.parseFloat(n);this.plugin.settings.chatTemperature=Number.isFinite(t)?t:.2,await this.plugin.saveSettings()})),new P.Setting(u).setName("Chat history messages").setDesc("Number of recent messages to include for conversational continuity (0 disables).").addText(e=>e.setPlaceholder("6").setValue(String(this.plugin.settings.chatHistoryMessages)).onChange(async n=>{let t=Number.parseInt(n,10);this.plugin.settings.chatHistoryMessages=Number.isFinite(t)?Math.max(0,t):6,await this.plugin.saveSettings()})),new P.Setting(u).setName("Chat panel location").setDesc("Where to open the chat view by default.").addDropdown(e=>e.addOption("right","Right sidebar").addOption("main","Main window").setValue(this.plugin.settings.chatPaneLocation).onChange(async n=>{this.plugin.settings.chatPaneLocation=n,await this.plugin.saveSettings()})),u.createEl("h3",{text:"Docling"}),new P.Setting(u).setName("OCR mode").setDesc("auto: skip OCR when text is readable; force if bad: OCR only when text looks poor; force: always OCR.").addDropdown(e=>e.addOption("auto","auto").addOption("force_low_quality","force if quality is bad").addOption("force","force").setValue(this.plugin.settings.ocrMode).onChange(async n=>{this.plugin.settings.ocrMode=n,await this.plugin.saveSettings()})),new P.Setting(u).setName("Text quality threshold").setDesc("Lower values are stricter; below this threshold the text is treated as low quality.").addSlider(e=>{e.setLimits(0,1,.05).setValue(this.plugin.settings.ocrQualityThreshold).setDynamicTooltip().onChange(async n=>{this.plugin.settings.ocrQualityThreshold=n,await this.plugin.saveSettings()})}),new P.Setting(u).setName("Chunking").setDesc("page or section").addDropdown(e=>e.addOption("page","page").addOption("section","section").setValue(this.plugin.settings.chunkingMode).onChange(async n=>{this.plugin.settings.chunkingMode=n,await this.plugin.saveSettings()})),new P.Setting(u).setName("Section chunk max chars").setDesc("Split large section chunks into smaller pieces (section mode only).").addText(e=>e.setPlaceholder("3000").setValue(String(this.plugin.settings.maxChunkChars)).onChange(async n=>{let t=Number.parseInt(n,10);this.plugin.settings.maxChunkChars=Number.isFinite(t)?t:3e3,await this.plugin.saveSettings()})),new P.Setting(u).setName("Section chunk overlap chars").setDesc("Number of characters to overlap when splitting section chunks.").addText(e=>e.setPlaceholder("250").setValue(String(this.plugin.settings.chunkOverlapChars)).onChange(async n=>{let t=Number.parseInt(n,10);this.plugin.settings.chunkOverlapChars=Number.isFinite(t)?t:250,await this.plugin.saveSettings()})),new P.Setting(u).setName("Remove image placeholders").setDesc("Strip '<!-- image -->' tags before chunking.").addToggle(e=>e.setValue(this.plugin.settings.removeImagePlaceholders).onChange(async n=>{this.plugin.settings.removeImagePlaceholders=n,await this.plugin.saveSettings()})),u.createEl("h4",{text:"OCR cleanup (optional)"}),new P.Setting(u).setName("LLM cleanup for low-quality chunks").setDesc("Optional OpenAI-compatible cleanup for poor OCR. Can be slow/costly.").addToggle(e=>e.setValue(this.plugin.settings.enableLlmCleanup).onChange(async n=>{this.plugin.settings.enableLlmCleanup=n,await this.plugin.saveSettings()})),new P.Setting(u).setName("LLM cleanup base URL").setDesc("OpenAI-compatible endpoint, e.g. http://127.0.0.1:1234/v1").addText(e=>e.setPlaceholder("http://127.0.0.1:1234/v1").setValue(this.plugin.settings.llmCleanupBaseUrl).onChange(async n=>{this.plugin.settings.llmCleanupBaseUrl=n.trim(),await this.plugin.saveSettings()})),new P.Setting(u).setName("LLM cleanup API key").setDesc("Optional API key for the cleanup endpoint.").addText(e=>e.setPlaceholder("sk-...").setValue(this.plugin.settings.llmCleanupApiKey).onChange(async n=>{this.plugin.settings.llmCleanupApiKey=n.trim(),await this.plugin.saveSettings()})),new P.Setting(u).setName("LLM cleanup model").setDesc("Model to use for cleanup.").addText(e=>e.setPlaceholder("openai/gpt-oss-20b").setValue(this.plugin.settings.llmCleanupModel).onChange(async n=>{this.plugin.settings.llmCleanupModel=n.trim(),await this.plugin.saveSettings()})),new P.Setting(u).setName("LLM cleanup temperature").setDesc("Lower is more conservative.").addText(e=>e.setPlaceholder("0.0").setValue(String(this.plugin.settings.llmCleanupTemperature)).onChange(async n=>{let t=Number.parseFloat(n);this.plugin.settings.llmCleanupTemperature=Number.isFinite(t)?t:0,await this.plugin.saveSettings()})),new P.Setting(u).setName("LLM cleanup min quality").setDesc("Only run cleanup when chunk quality is below this threshold (0-1).").addSlider(e=>e.setLimits(0,1,.05).setValue(this.plugin.settings.llmCleanupMinQuality).setDynamicTooltip().onChange(async n=>{this.plugin.settings.llmCleanupMinQuality=n,await this.plugin.saveSettings()})),new P.Setting(u).setName("LLM cleanup max chars").setDesc("Skip cleanup for chunks longer than this limit.").addText(e=>e.setPlaceholder("2000").setValue(String(this.plugin.settings.llmCleanupMaxChars)).onChange(async n=>{let t=Number.parseInt(n,10);this.plugin.settings.llmCleanupMaxChars=Number.isFinite(t)?t:2e3,await this.plugin.saveSettings()})),u.createEl("h3",{text:"Logging"}),new P.Setting(u).setName("Enable logging to file").setDesc("Write Docling/Python logs to a file during extraction.").addToggle(e=>e.setValue(this.plugin.settings.enableFileLogging).onChange(async n=>{this.plugin.settings.enableFileLogging=n,await this.plugin.saveSettings()})),new P.Setting(u).setName("Log file path (vault-relative)").setDesc("Where to write logs. Keep inside the vault.").addText(e=>e.setPlaceholder(`${z}/logs/docling_extract.log`).setValue(this.plugin.settings.logFilePath).onChange(async n=>{this.plugin.settings.logFilePath=n.trim()||`${z}/logs/docling_extract.log`,await this.plugin.saveSettings()})),new P.Setting(u).setName("View or clear log").setDesc("Open the log contents or clear the file.").addButton(e=>e.setButtonText("Open log").onClick(async()=>{var n,t;await((t=(n=this.plugin).openLogFile)==null?void 0:t.call(n))})).addButton(e=>e.setButtonText("Clear log").onClick(async()=>{var n,t;await((t=(n=this.plugin).clearLogFile)==null?void 0:t.call(n))})),u.createEl("h3",{text:"Maintenance"}),new P.Setting(u).setName("Reindex Redis from cached chunks").setDesc("Rebuild the Redis index from cached chunk JSON files.").addButton(e=>e.setButtonText("Reindex").onClick(async()=>{await this.plugin.reindexRedisFromCache()})),new P.Setting(u).setName("Recreate missing notes from cache").setDesc("Rebuild missing notes using cached Zotero items and chunks.").addButton(e=>e.setButtonText("Recreate").onClick(async()=>{await this.plugin.recreateMissingNotesFromCache()}))}};var X={"zrr-picker":`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
...
def normalize_chunk_whitespace(text: str) -> str:
    ...
    footnote_re = re.compile(r"^(?:\\[\\^\\d{1,3}\\]:|\\[\\s*\\d+\\s*\\]|\\(\\s*\\d+\\s*\\)|\\d+[\\.\\)])\\s*\\S")
    for line in lines:
        stripped = line.strip()
        if not stripped:
            flush()
            if out_lines and out_lines[-1] != "":
                out_lines.append("")
            continue
        if heading_re.match(stripped) or list_re.match(stripped) or footnote_re.match(stripped):
            flush()
            if footnote_re.match(stripped) and out_lines and out_lines[-1] != "":
                out_lines.append("")
...
_FOOTNOTE_HEADING_RE = re.compile(
    r"^#{1,6}\\s+(references|bibliography|footnotes?|endnotes?|notes)\\b",
    re.IGNORECASE,
)
...
def _is_footnote_heading(line: str) -> bool:
    return bool(_FOOTNOTE_HEADING_RE.match(line.strip()))


def _match_footnote_definition(line: str) -> Optional[Tuple[str, str]]:
    stripped = line.strip()
    if not stripped:
        return None
    patterns = (
...
def _clustered_definition_indices(lines: Sequence[str]) -> Set[int]:
    indices = [idx for idx, line in enumerate(lines) if _match_footnote_definition(line)]
...
def extract_footnote_definitions(text: str) -> Dict[str, str]:
    if not text:
        return {}
    lines = text.splitlines()
    clustered = _clustered_definition_indices(lines)
    ...
    for idx, line in enumerate(lines):
        ...
        if _is_heading_line(stripped):
            in_heading_block = _is_footnote_heading(stripped)
            current_id = None
            continue
        matched = _match_footnote_definition(line)
        if matched and (in_heading_block or idx in clustered):
            fid, content = matched
        ...
        if current_id:
            if not stripped:
                current_id = None
                continue
            footnotes[current_id] = f"{footnotes[current_id]} {stripped}".strip()
    return footnotes


def _replace_inline_footnote_refs(text: str, footnote_ids: Set[str]) -> str:
    if not text or not footnote_ids:
        return text

    def _replace_list(match: re.Match) -> str:
        nums = re.findall(r"\\d{1,3}", match.group(1))
        if not nums or not all(num in footnote_ids for num in nums):
...
def apply_footnotes_to_text(text: str, footnotes: Dict[str, str]) -> str:
    ...
    for idx, line in enumerate(lines):
        ...
        if matched and (in_heading_block or idx in clustered):
            ...
            if content:
                out_lines.append(f"[^{fid}]: {content}")
                current_id = fid
            else:
                out_lines.append(line)
                current_id = None
    ...
    normalized = "\\n".join(out_lines)
    return _replace_inline_footnote_refs(normalized, set(footnotes.keys()))
...
def main() -> int:
    ...
    try:
        ...
        footnote_source = markdown if markdown.strip() else "\\n\\n".join(str(page.get("text", "")) for page in pages)
        footnotes = extract_footnote_definitions(footnote_source)
        if footnotes:
            markdown = apply_footnotes_to_text(markdown, footnotes)
            updated_pages: List[Dict[str, Any]] = []
            for page in pages:
                updated_pages.append({
                    "page_num": page.get("page_num", 0),
                    "text": apply_footnotes_to_text(str(page.get("text", "")), footnotes),
                })
            pages = updated_pages

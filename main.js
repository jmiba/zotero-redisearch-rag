"use strict";var Q=Object.create;var I=Object.defineProperty;var K=Object.getOwnPropertyDescriptor;var W=Object.getOwnPropertyNames;var Y=Object.getPrototypeOf,X=Object.prototype.hasOwnProperty;var ee=(k,d)=>{for(var e in d)I(k,e,{get:d[e],enumerable:!0})},U=(k,d,e,t)=>{if(d&&typeof d=="object"||typeof d=="function")for(let n of W(d))!X.call(k,n)&&n!==e&&I(k,n,{get:()=>d[n],enumerable:!(t=K(d,n))||t.enumerable});return k};var B=(k,d,e)=>(e=k!=null?Q(Y(k)):{},U(d||!k||!k.__esModule?I(e,"default",{value:k,enumerable:!0}):e,k)),te=k=>U(I({},"__esModule",{value:!0}),k);var ne={};ee(ne,{default:()=>z});module.exports=te(ne);var l=require("obsidian"),A=require("child_process"),R=require("fs"),H=B(require("http")),J=B(require("https")),g=B(require("path")),q=require("url");var m=require("obsidian"),S=".zotero-redisearch-rag",D=`${S}/items`,E=`${S}/chunks`,V={zoteroBaseUrl:"http://127.0.0.1:23119/api",zoteroUserId:"0",pythonPath:"python3",dockerPath:"docker",autoStartRedis:!1,copyPdfToVault:!0,frontmatterTemplate:`doc_id: {{doc_id}}
...
item_type: {{item_type}}
pdf_link: {{pdf_link}}
item_json: {{item_json}}`,outputPdfDir:"zotero/pdfs",outputNoteDir:"zotero/notes",chatPaneLocation:"right",redisUrl:"redis://127.0.0.1:6379",redisIndex:"idx:zotero",redisPrefix:"zotero:chunk:",embedBaseUrl:"http://localhost:1234/v1",embedApiKey:"lm-studio",embedModel:"google/embedding-gemma-300m",chatBaseUrl:"http://127.0.0.1:1234/v1",chatApiKey:"",chatModel:"openai/gpt-oss-20b",chatTemperature:.2,ocrMode:"auto",chunkingMode:"page",ocrQualityThreshold:.5,enableLlmCleanup:!1,llmCleanupBaseUrl:"http://127.0.0.1:1234/v1",llmCleanupApiKey:"",llmCleanupModel:"openai/gpt-oss-20b",llmCleanupTemperature:0,llmCleanupMinQuality:.35,llmCleanupMaxChars:2e3},F=class extends m.PluginSettingTab{constructor(d,e){super(d,e),this.plugin=e}display(){let{containerEl:d}=this;d.empty(),d.createEl("h2",{text:"Zotero RAG Settings"}),new m.Setting(d).setName("Zotero base URL").setDesc("Local Zotero API base URL, e.g. http://127.0.0.1:23119/api").addText(e=>e.setPlaceholder("http://127.0.0.1:23119/api").setValue(this.plugin.settings.zoteroBaseUrl).onChange(async t=>{this.plugin.settings.zoteroBaseUrl=t.trim(),await this.plugin.saveSettings()})),new m.Setting(d).setName("Zotero user ID").setDesc("Use 0 for local library. You can also enter users/<id> or groups/<id>.").addText(e=>e.setPlaceholder("123456").setValue(this.plugin.settings.zoteroUserId).onChange(async t=>{this.plugin.settings.zoteroUserId=t.trim(),await this.plugin.saveSettings()})),new m.Setting(d).setName("Python path").setDesc("Path to python3").addText(e=>e.setPlaceholder("python3").setValue(this.plugin.settings.pythonPath).onChange(async t=>{this.plugin.settings.pythonPath=t.trim()||"python3",await this.plugin.saveSettings()})),new m.Setting(d).setName("Copy PDFs into vault").setDesc("Disable to use Zotero storage paths directly.").addToggle(e=>e.setValue(this.plugin.settings.copyPdfToVault).onChange(async t=>{this.plugin.settings.copyPdfToVault=t,await this.plugin.saveSettings()})),new m.Setting(d).setName("Frontmatter template").setDesc("Use {{doc_id}}, {{zotero_key}}, {{title}}, {{title_yaml}}, {{year}}, {{date}}, {{authors}}, {{authors_yaml}}, {{tags}}, {{item_type}}, {{pdf_link}}, {{item_json}}").addTextArea(e=>{e.inputEl.rows=8,e.setValue(this.plugin.settings.frontmatterTemplate).onChange(async t=>{this.plugin.settings.frontmatterTemplate=t,await this.plugin.saveSettings()})}),new m.Setting(d).setName("Docker path").setDesc("CLI path for Docker (used to start Redis Stack).").addText(e=>e.setPlaceholder("docker").setValue(this.plugin.settings.dockerPath).onChange(async t=>{this.plugin.settings.dockerPath=t.trim()||"docker",await this.plugin.saveSettings()})),d.createEl("h3",{text:"Output folders (vault-relative)"}),new m.Setting(d).setName("PDF folder").addText(e=>e.setPlaceholder("zotero/pdfs").setValue(this.plugin.settings.outputPdfDir).onChange(async t=>{this.plugin.settings.outputPdfDir=t.trim(),await this.plugin.saveSettings()})),new m.Setting(d).setName("Notes folder").addText(e=>e.setPlaceholder("zotero/notes").setValue(this.plugin.settings.outputNoteDir).onChange(async t=>{this.plugin.settings.outputNoteDir=t.trim(),await this.plugin.saveSettings()})),d.createEl("h3",{text:"Redis Stack"}),new m.Setting(d).setName("Redis URL").addText(e=>e.setPlaceholder("redis://127.0.0.1:6379").setValue(this.plugin.settings.redisUrl).onChange(async t=>{this.plugin.settings.redisUrl=t.trim(),await this.plugin.saveSettings()})),new m.Setting(d).setName("Index name").addText(e=>e.setPlaceholder("idx:zotero").setValue(this.plugin.settings.redisIndex).onChange(async t=>{this.plugin.settings.redisIndex=t.trim(),await this.plugin.saveSettings()})),new m.Setting(d).setName("Key prefix").addText(e=>e.setPlaceholder("zotero:chunk:").setValue(this.plugin.settings.redisPrefix).onChange(async t=>{this.plugin.settings.redisPrefix=t.trim(),await this.plugin.saveSettings()})),new m.Setting(d).setName("Auto-start Redis Stack (Docker Compose)").setDesc("Requires Docker Desktop running and your vault path shared with Docker.").addToggle(e=>e.setValue(this.plugin.settings.autoStartRedis).onChange(async t=>{this.plugin.settings.autoStartRedis=t,await this.plugin.saveSettings()})),new m.Setting(d).setName("Start Redis Stack now").setDesc("Runs Docker Compose with the vault data directory.").addButton(e=>e.setButtonText("Start").onClick(async()=>{await this.plugin.startRedisStack()})),new m.Setting(d).setName("Stop Redis Stack now").setDesc("Stops the Docker Compose Redis Stack container.").addButton(e=>e.setButtonText("Stop").onClick(async()=>{await this.plugin.stopRedisStack()})),new m.Setting(d).setName("Remove stale redis-stack container").setDesc("Use only if Docker reports a name conflict when starting.").addButton(e=>e.setButtonText("Remove").onClick(async()=>{window.confirm("Remove the existing redis-stack container? This does not delete your data volume.")&&await this.plugin.removeRedisStackContainer()})),d.createEl("h3",{text:"Embeddings (LM Studio)"}),new m.Setting(d).setName("Embeddings base URL").addText(e=>e.setPlaceholder("http://localhost:1234/v1").setValue(this.plugin.settings.embedBaseUrl).onChange(async t=>{this.plugin.settings.embedBaseUrl=t.trim(),await this.plugin.saveSettings()})),new m.Setting(d).setName("Embeddings API key").addText(e=>e.setPlaceholder("lm-studio").setValue(this.plugin.settings.embedApiKey).onChange(async t=>{this.plugin.settings.embedApiKey=t.trim(),await this.plugin.saveSettings()})),new m.Setting(d).setName("Embeddings model").addText(e=>e.setPlaceholder("google/embedding-gemma-300m").setValue(this.plugin.settings.embedModel).onChange(async t=>{this.plugin.settings.embedModel=t.trim(),await this.plugin.saveSettings()})),d.createEl("h3",{text:"Chat LLM"}),new m.Setting(d).setName("Chat base URL").setDesc("OpenAI-compatible chat endpoint base URL").addText(e=>e.setPlaceholder("http://localhost:1234/v1").setValue(this.plugin.settings.chatBaseUrl).onChange(async t=>{this.plugin.settings.chatBaseUrl=t.trim(),await this.plugin.saveSettings()})),new m.Setting(d).setName("Chat API key").addText(e=>e.setPlaceholder("lm-studio").setValue(this.plugin.settings.chatApiKey).onChange(async t=>{this.plugin.settings.chatApiKey=t.trim(),await this.plugin.saveSettings()})),new m.Setting(d).setName("Chat model").addText(e=>e.setPlaceholder("meta-llama/llama-3.1-405b-instruct").setValue(this.plugin.settings.chatModel).onChange(async t=>{this.plugin.settings.chatModel=t.trim(),await this.plugin.saveSettings()})),new m.Setting(d).setName("Temperature").addText(e=>e.setPlaceholder("0.2").setValue(String(this.plugin.settings.chatTemperature)).onChange(async t=>{let n=Number.parseFloat(t);this.plugin.settings.chatTemperature=Number.isFinite(n)?n:.2,await this.plugin.saveSettings()})),new m.Setting(d).setName("Chat panel location").setDesc("Where to open the chat view by default.").addDropdown(e=>e.addOption("right","Right sidebar").addOption("main","Main window").setValue(this.plugin.settings.chatPaneLocation).onChange(async t=>{this.plugin.settings.chatPaneLocation=t,await this.plugin.saveSettings()})),d.createEl("h3",{text:"Docling"}),new m.Setting(d).setName("OCR mode").setDesc("auto: skip OCR when text is readable; force if bad: OCR only when text looks poor; force: always OCR.").addDropdown(e=>e.addOption("auto","auto").addOption("force_low_quality","force if quality is bad").addOption("force","force").setValue(this.plugin.settings.ocrMode).onChange(async t=>{this.plugin.settings.ocrMode=t,await this.plugin.saveSettings()})),new m.Setting(d).setName("Text quality threshold").setDesc("Lower values are stricter; below this threshold the text is treated as low quality.").addSlider(e=>{e.setLimits(0,1,.05).setValue(this.plugin.settings.ocrQualityThreshold).setDynamicTooltip().onChange(async t=>{this.plugin.settings.ocrQualityThreshold=t,await this.plugin.saveSettings()})}),new m.Setting(d).setName("Chunking").setDesc("page or section").addDropdown(e=>e.addOption("page","page").addOption("section","section").setValue(this.plugin.settings.chunkingMode).onChange(async t=>{this.plugin.settings.chunkingMode=t,await this.plugin.saveSettings()})),d.createEl("h4",{text:"OCR cleanup (optional)"}),new m.Setting(d).setName("LLM cleanup for low-quality chunks").setDesc("Optional OpenAI-compatible cleanup for poor OCR. Can be slow/costly.").addToggle(e=>e.setValue(this.plugin.settings.enableLlmCleanup).onChange(async t=>{this.plugin.settings.enableLlmCleanup=t,await this.plugin.saveSettings()})),new m.Setting(d).setName("LLM cleanup base URL").setDesc("OpenAI-compatible endpoint, e.g. http://127.0.0.1:1234/v1").addText(e=>e.setPlaceholder("http://127.0.0.1:1234/v1").setValue(this.plugin.settings.llmCleanupBaseUrl).onChange(async t=>{this.plugin.settings.llmCleanupBaseUrl=t.trim(),await this.plugin.saveSettings()})),new m.Setting(d).setName("LLM cleanup API key").setDesc("Optional API key for the cleanup endpoint.").addText(e=>e.setPlaceholder("sk-...").setValue(this.plugin.settings.llmCleanupApiKey).onChange(async t=>{this.plugin.settings.llmCleanupApiKey=t.trim(),await this.plugin.saveSettings()})),new m.Setting(d).setName("LLM cleanup model").setDesc("Model to use for cleanup.").addText(e=>e.setPlaceholder("openai/gpt-oss-20b").setValue(this.plugin.settings.llmCleanupModel).onChange(async t=>{this.plugin.settings.llmCleanupModel=t.trim(),await this.plugin.saveSettings()})),new m.Setting(d).setName("LLM cleanup temperature").setDesc("Lower is more conservative.").addText(e=>e.setPlaceholder("0.0").setValue(String(this.plugin.settings.llmCleanupTemperature)).onChange(async t=>{let n=Number.parseFloat(t);this.plugin.settings.llmCleanupTemperature=Number.isFinite(n)?n:0,await this.plugin.saveSettings()})),new m.Setting(d).setName("LLM cleanup min quality").setDesc("Only run cleanup when chunk quality is below this threshold (0-1).").addText(e=>e.setPlaceholder("0.35").setValue(String(this.plugin.settings.llmCleanupMinQuality)).onChange(async t=>{let n=Number.parseFloat(t);this.plugin.settings.llmCleanupMinQuality=Number.isFinite(n)?n:.35,await this.plugin.saveSettings()})),new m.Setting(d).setName("LLM cleanup max chars").setDesc("Skip cleanup for chunks longer than this limit.").addText(e=>e.setPlaceholder("2000").setValue(String(this.plugin.settings.llmCleanupMaxChars)).onChange(async t=>{let n=Number.parseInt(t,10);this.plugin.settings.llmCleanupMaxChars=Number.isFinite(n)?n:2e3,await this.plugin.saveSettings()}))}};var G={"docling_extract.py":`#!/usr/bin/env python3
...
if __name__ == "__main__":
    sys.exit(main())
`,"ocr_wordlist.txt":`# zotero-redisearch-rag tool version: 0.1.5
...
methode
analyse
`,"docker-compose.yml":`# zotero-redisearch-rag tool version: 0.1.5
services:
  redis-stack:
...
`,"redis-stack.conf":`# zotero-redisearch-rag tool version: 0.1.5
# Redis Stack persistence config for local RAG index
appendonly yes
appendfsync everysec

dir /data
`};var L=require("obsidian"),O="zotero-redisearch-rag-chat",N=class extends L.ItemView{constructor(e,t){super(e);this.messages=[];this.messageEls=new Map;this.pendingRender=new Map;this.busy=!1;this.plugin=t}getViewType(){return O}getDisplayText(){return"Zotero RAG Chat"}async onOpen(){let{containerEl:e}=this;e.empty(),e.addClass("zrr-chat-view");let t=e.createEl("div",{cls:"zrr-chat-header"});t.createEl("div",{cls:"zrr-chat-title",text:"Zotero RAG Chat"}),this.clearButton=t.createEl("button",{cls:"zrr-chat-clear",text:"Clear"}),this.clearButton.addEventListener("click",()=>this.clearChat()),this.messagesEl=e.createEl("div",{cls:"zrr-chat-messages"});let n=e.createEl("div",{cls:"zrr-chat-input"});this.inputEl=n.createEl("textarea",{cls:"zrr-chat-textarea",attr:{placeholder:"Ask your Zotero library..."}}),this.sendButton=n.createEl("button",{cls:"zrr-chat-send",text:"Send"}),this.sendButton.addEventListener("click",()=>this.handleSend()),this.inputEl.addEventListener("keydown",r=>{r.key==="Enter"&&!r.shiftKey&&(r.preventDefault(),this.handleSend())}),await this.loadHistory(),await this.renderAll()}focusInput(){var e;(e=this.inputEl)==null||e.focus()}async loadHistory(){try{this.messages=await this.plugin.loadChatHistory()}catch(e){console.error(e),this.messages=[]}}async saveHistory(){try{await this.plugin.saveChatHistory(this.messages)}catch(e){console.error(e)}}async renderAll(){this.messagesEl.empty(),this.messageEls.clear();for(let e of this.messages)await this.renderMessage(e);this.scrollToBottom()}async renderMessage(e){let t=this.messagesEl.createEl("div",{cls:`zrr-chat-message zrr-chat-${e.role}`});t.createEl("div",{cls:"zrr-chat-meta"}).setText(e.role==="user"?"You":"Assistant");let r=t.createEl("div",{cls:"zrr-chat-content"}),s=t.createEl("div",{cls:"zrr-chat-citations"});this.messageEls.set(e.id,{wrapper:t,content:r,citations:s}),await this.renderMessageContent(e)}scheduleRender(e){if(this.pendingRender.has(e.id))return;let t=window.setTimeout(async()=>{this.pendingRender.delete(e.id),await this.renderMessageContent(e),this.scrollToBottom()},80);this.pendingRender.set(e.id,t)}async renderMessageContent(e){var n;let t=this.messageEls.get(e.id);t&&(t.content.empty(),t.citations.empty(),await L.MarkdownRenderer.renderMarkdown(e.content||"",t.content,"",this.plugin),await this.renderCitations(t.citations,(n=e.citations)!=null?n:[]))}async renderCitations(e,t){if(e.empty(),!t.length)return;e.createEl("div",{cls:"zrr-chat-citations-label",text:"Citations"});let n=e.createEl("ul",{cls:"zrr-chat-citation-list"});for(let r of t){let s=await this.plugin.resolveCitationDisplay(r),i=n.createEl("li");i.createEl("a",{text:s.label,href:"#"}).addEventListener("click",o=>{o.preventDefault(),this.plugin.openCitationTarget(r,s)}),s.zoteroUrl&&(i.createEl("span",{text:" \xB7 "}),i.createEl("a",{text:"Zotero",href:"#"}).addEventListener("click",p=>{p.preventDefault(),this.plugin.openExternalUrl(s.zoteroUrl)}))}}scrollToBottom(){this.messagesEl.scrollTop=this.messagesEl.scrollHeight}async handleSend(){if(this.busy)return;let e=this.inputEl.value.trim();if(!e){new L.Notice("Query cannot be empty.");return}if(!this.plugin.settings.chatBaseUrl){new L.Notice("Chat base URL must be set in settings.");return}this.inputEl.value="",this.busy=!0,this.sendButton.disabled=!0;let t={id:this.generateId(),role:"user",content:e,createdAt:new Date().toISOString()};this.messages.push(t),await this.renderMessage(t),this.scrollToBottom(),await this.saveHistory();let n={id:this.generateId(),role:"assistant",content:"",citations:[],createdAt:new Date().toISOString()};this.messages.push(n),await this.renderMessage(n),this.scrollToBottom();let r=!1;try{await this.plugin.runRagQueryStreaming(e,s=>{r=!0,n.content+=s,this.scheduleRender(n)},s=>{(!r&&(s!=null&&s.answer)||s!=null&&s.answer)&&(n.content=s.answer),Array.isArray(s==null?void 0:s.citations)&&(n.citations=s.citations),this.scheduleRender(n)})}catch(s){console.error(s),n.content="Failed to fetch answer. See console for details.",this.scheduleRender(n)}finally{this.busy=!1,this.sendButton.disabled=!1,await this.saveHistory()}}async clearChat(){this.messages=[],await this.saveHistory(),await this.renderAll()}generateId(){return typeof crypto!="undefined"&&"randomUUID"in crypto?crypto.randomUUID():`${Date.now().toString(36)}-${Math.random().toString(36).slice(2,8)}`}};var j=class extends l.Modal{constructor(d,e,t,n,r="Value cannot be empty."){super(d),this.titleText=e,this.placeholder=t,this.onSubmit=n,this.emptyMessage=r}onOpen(){let{contentEl:d}=this;d.empty(),d.createEl("h3",{text:this.titleText});let e=d.createEl("input",{type:"text",placeholder:this.placeholder});e.style.width="100%",e.focus();let t=d.createEl("button",{text:"Submit"});t.style.marginTop="0.75rem";let n=()=>{let r=e.value.trim();if(!r){new l.Notice(this.emptyMessage);return}this.close(),this.onSubmit(r)};t.addEventListener("click",n),e.addEventListener("keydown",r=>{r.key==="Enter"&&n()})}};var z=class extends l.Plugin{constructor(){super(...arguments);this.docIndex=null}async onload(){await this.loadSettings(),await this.migrateCachePaths(),this.addSettingTab(new F(this.app,this)),this.registerView(O,e=>new N(e,this)),this.setupStatusBar();try{await this.ensureBundledTools()}catch(e){console.error("Failed to sync bundled tools",e)}this.addCommand({id:"import-zotero-item-index",name:"Import Zotero item and index (Docling -> RedisSearch)",callback:()=>this.importZoteroItem()}),this.addCommand({id:"ask-zotero-library",name:"Ask my Zotero library (RAG via RedisSearch)",callback:()=>this.askZoteroLibrary()}),this.addCommand({id:"open-zotero-chat",name:"Open Zotero RAG chat panel",callback:()=>this.openChatView(!0)}),this.addCommand({id:"rebuild-zotero-note-cache",name:"Rebuild Zotero note from cache (Docling + RedisSearch)",callback:()=>this.rebuildNoteFromCache()}),this.addCommand({id:"rebuild-doc-index-cache",name:"Rebuild doc index from cache",callback:()=>this.rebuildDocIndexFromCache()}),this.addCommand({id:"recreate-missing-notes-cache",name:"Recreate missing notes from cache (Docling + RedisSearch)",callback:()=>this.recreateMissingNotesFromCache()}),this.addCommand({id:"start-redis-stack",name:"Start Redis Stack (Docker Compose)",callback:()=>this.startRedisStack()}),this.addCommand({id:"stop-redis-stack",name:"Stop Redis Stack (Docker Compose)",callback:()=>this.stopRedisStack()}),this.settings.autoStartRedis&&this.startRedisStack(!0)}async loadSettings(){this.settings=Object.assign({},V,await this.loadData())}async saveSettings(){await this.saveData(this.settings)}async importZoteroItem(){var b,y;try{await this.ensureBundledTools()}catch(c){new l.Notice("Failed to sync bundled tools. See console for details."),console.error(c);return}let e;try{e=await this.promptZoteroItem()}catch(c){new l.Notice("Zotero search failed. See console for details."),console.error(c);return}if(!e){new l.Notice("No Zotero item selected.");return}let t=(b=e.data)!=null?b:e;!t.key&&e.key&&(t.key=e.key);let n=this.getDocId(t);if(!n){new l.Notice("Could not resolve a stable doc_id from Zotero item.");return}let r=await this.resolvePdfAttachment(t,n);if(!r){new l.Notice("No PDF attachment found for item.");return}this.showStatusProgress("Preparing...",5);let s=typeof t.title=="string"?t.title:"",i=this.sanitizeFileName(s)||n,a=await this.resolveUniqueBaseName(i,n),o=(0,l.normalizePath)(`${this.settings.outputPdfDir}/${a}.pdf`),p=(0,l.normalizePath)(`${D}/${n}.json`),u=(0,l.normalizePath)(`${E}/${n}.json`),_=(0,l.normalizePath)(`${this.settings.outputNoteDir}/${a}.md`);try{await this.ensureFolder(D),await this.ensureFolder(E),await this.ensureFolder(this.settings.outputNoteDir),this.settings.copyPdfToVault&&await this.ensureFolder(this.settings.outputPdfDir)}catch(c){new l.Notice("Failed to create output folders."),console.error(c),this.clearStatusProgress();return}let h="",f="";try{if(this.settings.copyPdfToVault){let c=r.filePath?await R.promises.readFile(r.filePath):await this.downloadZoteroPdf(r.key);await this.app.vault.adapter.writeBinary(o,this.bufferToArrayBuffer(c)),h=this.getAbsoluteVaultPath(o),f=`[[${o}]]`}else{if(!r.filePath){new l.Notice("PDF file path missing. Enable PDF copying or check Zotero storage."),this.clearStatusProgress();return}h=r.filePath,f=`[PDF](${(0,q.pathToFileURL)(r.filePath).toString()})`}}catch(c){new l.Notice("Failed to download PDF attachment."),console.error(c),this.clearStatusProgress();return}try{await this.app.vault.adapter.write(p,JSON.stringify(e,null,2))}catch(c){new l.Notice("Failed to write Zotero item JSON."),console.error(c),this.clearStatusProgress();return}let x=this.getPluginDir(),v=g.default.join(x,"tools","docling_extract.py"),w=g.default.join(x,"tools","index_redisearch.py"),C=null;try{C=await this.readDoclingQualityLabelFromPdf(h),this.showStatusProgress(this.formatStatusLabel("Docling extraction...",C),null),await this.runPython(v,this.buildDoclingArgs(h,n,u,_)),C=await this.readDoclingQualityLabel(u)}catch(c){new l.Notice("Docling extraction failed. See console for details."),console.error(c),this.clearStatusProgress();return}try{this.showStatusProgress(this.formatStatusLabel("Indexing chunks...",C),0),await this.runPythonStreaming(w,["--chunks-json",this.getAbsoluteVaultPath(u),"--redis-url",this.settings.redisUrl,"--index",this.settings.redisIndex,"--prefix",this.settings.redisPrefix,"--embed-base-url",this.settings.embedBaseUrl,"--embed-api-key",this.settings.embedApiKey,"--embed-model",this.settings.embedModel,"--progress"],c=>{if((c==null?void 0:c.type)==="progress"&&c.total){let P=Math.round(c.current/c.total*100),T=this.formatStatusLabel(`Indexing chunks ${c.current}/${c.total}`,C);this.showStatusProgress(T,P)}},()=>{})}catch(c){new l.Notice("RedisSearch indexing failed. See console for details."),console.error(c),this.clearStatusProgress();return}try{let c=await this.app.vault.adapter.read(_),P=this.buildNoteMarkdown(t,(y=e.meta)!=null?y:{},n,f,p,c);await this.app.vault.adapter.write(_,P)}catch(c){new l.Notice("Failed to finalize note markdown."),console.error(c),this.clearStatusProgress();return}try{await this.updateDocIndex({doc_id:n,note_path:_,note_title:a,zotero_title:s,pdf_path:h,attachment_key:r.key})}catch(c){console.error("Failed to update doc index",c)}this.showStatusProgress("Done",100),window.setTimeout(()=>this.clearStatusProgress(),1200),new l.Notice(`Indexed Zotero item ${n}.`)}async askZoteroLibrary(){await this.openChatView(!0)}getChatLeaf(){var e;return this.settings.chatPaneLocation==="right"?(e=this.app.workspace.getRightLeaf(!1))!=null?e:this.app.workspace.getLeaf("split"):this.app.workspace.getLeaf("tab")}async openChatView(e=!1){let t=this.getChatLeaf();await t.setViewState({type:O,active:!0}),this.app.workspace.revealLeaf(t);let n=t.view;return n instanceof N&&e&&n.focusInput(),n}async loadChatHistory(){let e=this.app.vault.adapter,t=(0,l.normalizePath)(`${S}/chat.json`);if(!await e.exists(t))return[];let n=await e.read(t),r;try{r=JSON.parse(n)}catch(i){return[]}let s=Array.isArray(r)?r:r==null?void 0:r.messages;return Array.isArray(s)?s.filter(i=>i&&typeof i.content=="string").map(i=>({id:i.id||this.generateChatId(),role:i.role==="assistant"?"assistant":"user",content:i.content,citations:Array.isArray(i.citations)?i.citations:[],createdAt:i.createdAt||new Date().toISOString()})):[]}async saveChatHistory(e){await this.ensureFolder(S);let t=this.app.vault.adapter,n=(0,l.normalizePath)(`${S}/chat.json`),r={version:1,messages:e};await t.write(n,JSON.stringify(r,null,2))}async runRagQueryStreaming(e,t,n){await this.ensureBundledTools();let r=this.getPluginDir(),s=g.default.join(r,"tools","rag_query_redisearch.py"),i=["--query",e,"--k","5","--redis-url",this.settings.redisUrl,"--index",this.settings.redisIndex,"--prefix",this.settings.redisPrefix,"--embed-base-url",this.settings.embedBaseUrl,"--embed-api-key",this.settings.embedApiKey,"--embed-model",this.settings.embedModel,"--chat-base-url",this.settings.chatBaseUrl,"--chat-api-key",this.settings.chatApiKey,"--chat-model",this.settings.chatModel,"--temperature",String(this.settings.chatTemperature),"--stream"];return this.runPythonStreaming(s,i,a=>{if((a==null?void 0:a.type)==="delta"&&typeof a.content=="string"){t(a.content);return}if((a==null?void 0:a.type)==="final"){n(a);return}a!=null&&a.answer&&n(a)},n)}async resolveCitationDisplay(e){var p,u;let t=await this.getDocIndexEntry(e.doc_id);(!t||!t.note_title||!t.zotero_title||!t.note_path||!t.pdf_path)&&(t=await this.hydrateDocIndexFromCache(e.doc_id));let n=(t==null?void 0:t.zotero_title)||(t==null?void 0:t.note_title)||e.doc_id||"?",r=e.pages||`${(p=e.page_start)!=null?p:"?"}-${(u=e.page_end)!=null?u:"?"}`,s=`${n} pages ${r}`,i=e.page_start?String(e.page_start):"",a=(t==null?void 0:t.pdf_path)||e.source_pdf||"",o=e.doc_id?this.buildZoteroDeepLink(e.doc_id,t==null?void 0:t.attachment_key,i):void 0;return{label:s,notePath:t==null?void 0:t.note_path,pdfPath:a||void 0,zoteroUrl:o,pageStart:i||void 0}}async openCitationTarget(e,t){let n=t!=null?t:await this.resolveCitationDisplay(e);if(n.notePath){await this.openNoteInMain(n.notePath);return}if(!(n.pdfPath&&await this.openPdfInMain(n.pdfPath,n.pageStart))){if(n.zoteroUrl){this.openExternalUrl(n.zoteroUrl);return}new l.Notice("Unable to open citation target.")}}async rebuildNoteFromCache(){let e=await this.promptDocId();if(!e){new l.Notice("No doc_id provided.");return}await this.rebuildNoteFromCacheForDocId(e,!0)&&new l.Notice(`Rebuilt Zotero note for ${e}.`)}async rebuildDocIndexFromCache(){var p,u,_;let e=this.app.vault.adapter,t=await this.listDocIds(D),n=await this.listDocIds(E),r=await this.scanNotesForDocIds(this.settings.outputNoteDir),s=Object.keys(r),i=Array.from(new Set([...t,...n,...s]));if(i.length===0){new l.Notice("No cached items found.");return}this.showStatusProgress("Rebuilding doc index...",0);let a=await this.getDocIndex(),o=0;for(let h of i){o+=1;let f={},x=r[h];x&&(f.note_path=x.note_path,f.note_title=x.note_title);let v=(0,l.normalizePath)(`${D}/${h}.json`);if(await e.exists(v))try{let b=await e.read(v),y=JSON.parse(b),c=(u=(p=y==null?void 0:y.data)!=null?p:y)!=null?u:{},P=typeof c.title=="string"?c.title:"";P&&(f.zotero_title=P);let T=this.sanitizeFileName(P)||h,M=(0,l.normalizePath)(`${this.settings.outputNoteDir}/${T}.md`),$=(0,l.normalizePath)(`${this.settings.outputNoteDir}/${T}-${h}.md`);await e.exists(M)?(f.note_path=M,f.note_title=g.default.basename(M,".md")):await e.exists($)&&(f.note_path=$,f.note_title=g.default.basename($,".md"))}catch(b){console.error("Failed to read cached item JSON",b)}let w=(0,l.normalizePath)(`${E}/${h}.json`);if(await e.exists(w))try{let b=await e.read(w),y=JSON.parse(b);typeof(y==null?void 0:y.source_pdf)=="string"&&(f.pdf_path=y.source_pdf)}catch(b){console.error("Failed to read cached chunks JSON",b)}if(Object.keys(f).length>0){let y={...(_=a[h])!=null?_:{doc_id:h},...f,doc_id:h,updated_at:new Date().toISOString()};!y.note_title&&y.note_path&&(y.note_title=g.default.basename(y.note_path,".md")),a[h]=y}let C=Math.round(o/i.length*100);this.showStatusProgress(`Rebuilding doc index ${o}/${i.length}`,C)}await this.saveDocIndex(a),this.showStatusProgress("Done",100),window.setTimeout(()=>this.clearStatusProgress(),1200),new l.Notice(`Rebuilt doc index for ${i.length} items.`)}async recreateMissingNotesFromCache(){let e=this.app.vault.adapter,t=await this.listDocIds(D),n=await this.listDocIds(E),r=await this.scanNotesForDocIds(this.settings.outputNoteDir),s=Object.keys(r),i=Array.from(new Set([...t,...n,...s]));if(i.length===0){new l.Notice("No cached items found.");return}let a=[];for(let p of i){if(r[p])continue;let u=await this.getDocIndexEntry(p);if(u!=null&&u.note_path&&await e.exists(u.note_path))continue;let _=await this.inferNotePathFromCache(p);_&&await e.exists(_)||a.push(p)}if(a.length===0){new l.Notice("No missing notes detected.");return}this.showStatusProgress("Recreating missing notes...",0);let o=0;for(let p=0;p<a.length;p+=1){let u=a[p],_=Math.round((p+1)/a.length*100);this.showStatusProgress(`Recreating ${p+1}/${a.length}`,_),await this.rebuildNoteFromCacheForDocId(u,!1)&&(o+=1)}this.showStatusProgress("Done",100),window.setTimeout(()=>this.clearStatusProgress(),1200),new l.Notice(`Recreated ${o}/${a.length} missing notes.`)}async promptZoteroItem(){return new Promise(e=>{new Z(this.app,this,e).open()})}async listDocIds(e){let t=this.app.vault.adapter,n=(0,l.normalizePath)(e);return await t.exists(n)?(await t.list(n)).files.filter(s=>s.endsWith(".json")).map(s=>g.default.basename(s,".json")):[]}async listMarkdownFiles(e){let t=this.app.vault.adapter,n=(0,l.normalizePath)(e);if(!await t.exists(n))return[];let r=[n],s=[];for(;r.length>0;){let i=r.pop();if(!i)continue;let a=await t.list(i);for(let o of a.files)o.endsWith(".md")&&s.push(o);for(let o of a.folders)r.push(o)}return s}extractDocIdFromFrontmatter(e){let t=e.match(/^---\s*\n([\s\S]*?)\n---/);if(!t)return null;let r=t[1].split(/\r?\n/);for(let s of r){let i=s.trim();if(!i||i.startsWith("#"))continue;let a=i.split(":");if(a.length<2)continue;let o=a[0].trim().toLowerCase();if(o!=="doc_id"&&o!=="zotero_key")continue;let u=i.slice(i.indexOf(":")+1).trim().replace(/^["']|["']$/g,"").trim();if(u)return u}return null}async scanNotesForDocIds(e){let t=this.app.vault.adapter,n=await this.listMarkdownFiles(e),r={};for(let s of n)try{let i=await t.read(s),a=this.extractDocIdFromFrontmatter(i);if(!a)continue;r[a]={doc_id:a,note_path:s,note_title:g.default.basename(s,".md"),updated_at:new Date().toISOString()}}catch(i){console.error("Failed to read note for doc_id scan",i)}return r}setupStatusBar(){let e=this.addStatusBarItem();e.addClass("zrr-status-progress"),e.addClass("status-bar-item-segment"),e.style.display="none";let t=e.createEl("span",{text:"Idle"});t.addClass("zrr-status-label");let r=e.createEl("div",{cls:"zrr-status-bar"}).createEl("div",{cls:"zrr-status-bar-inner"});this.statusBarEl=e,this.statusLabelEl=t,this.statusBarInnerEl=r}showStatusProgress(e,t){if(!(!this.statusBarEl||!this.statusLabelEl||!this.statusBarInnerEl))if(this.statusBarEl.style.display="flex",this.statusLabelEl.setText(e),t===null)this.statusBarInnerEl.addClass("indeterminate"),this.statusBarInnerEl.style.width="40%";else{this.statusBarInnerEl.removeClass("indeterminate");let n=Math.max(0,Math.min(100,t));this.statusBarInnerEl.style.width=`${n}%`}}clearStatusProgress(){!this.statusBarEl||!this.statusBarInnerEl||(this.statusBarEl.style.display="none",this.statusBarInnerEl.removeClass("indeterminate"),this.statusBarInnerEl.style.width="0%")}formatStatusLabel(e,t){return t?`${e} (Text layer quality ${t})`:e}async readDoclingQualityLabel(e){var t;try{let n=await this.app.vault.adapter.read(e),r=JSON.parse(n),s=(t=r==null?void 0:r.metadata)==null?void 0:t.confidence_proxy;if(typeof s=="number")return s.toFixed(2)}catch(n){console.warn("Failed to read Docling quality metadata",n)}return null}async readDoclingQualityLabelFromPdf(e){try{let t=this.getPluginDir(),n=g.default.join(t,"tools","docling_extract.py"),r=this.settings.ocrMode==="force_low_quality"?"auto":this.settings.ocrMode,s=["--quality-only","--pdf",e,"--ocr",r];this.settings.ocrMode==="force_low_quality"&&s.push("--force-ocr-low-quality"),s.push("--quality-threshold",String(this.settings.ocrQualityThreshold));let i=await this.runPythonWithOutput(n,s),a=JSON.parse(i),o=a==null?void 0:a.confidence_proxy;if(typeof o=="number")return o.toFixed(2)}catch(t){console.warn("Failed to read Docling quality from PDF",t)}return null}async promptDocId(){return new Promise(e=>{new j(this.app,"Rebuild Zotero note from cache","Enter Zotero doc_id (e.g., ABC123)",t=>e(t),"Doc ID cannot be empty.").open()})}getDocId(e){let t=[e.key,e.itemKey,e.id,e.citationKey];for(let n of t)if(typeof n=="string"&&n.trim())return n.trim();return null}sanitizeFileName(e){let t=e.replace(/[\\/:*?"<>|]/g,"").replace(/\s+/g," ").trim();return t?t.replace(/[.]+$/g,"").trim().replace(/ /g,"-").slice(0,120):""}async resolveUniqueBaseName(e,t){let n=this.app.vault.adapter,r=(0,l.normalizePath)(`${this.settings.outputNoteDir}/${e}.md`),s=(0,l.normalizePath)(`${this.settings.outputPdfDir}/${e}.pdf`),i=await n.exists(r),a=this.settings.copyPdfToVault?await n.exists(s):!1;return i||a?`${e}-${t}`:e}async searchZoteroItems(e){let t=new URLSearchParams;t.set("itemType","-attachment"),t.set("limit","25"),t.set("include","data,meta"),e.trim()&&t.set("q",e.trim());let n=this.buildZoteroUrl(`/${this.getZoteroLibraryPath()}/items?${t.toString()}`),r=await this.requestLocalApi(n,`Zotero search failed for ${n}`),s=JSON.parse(r.toString("utf8"));return Array.isArray(s)?s.map(i=>{var a,o,p,u;return{key:(o=i.key)!=null?o:(a=i.data)==null?void 0:a.key,data:(p=i.data)!=null?p:{},meta:(u=i.meta)!=null?u:{}}}).filter(i=>typeof i.key=="string"&&i.key.trim().length>0):[]}async resolvePdfAttachment(e,t){let n=this.pickPdfAttachment(e);if(n)return n;try{let r=await this.fetchZoteroChildren(t);for(let s of r){let i=this.toPdfAttachment(s);if(i)return i}}catch(r){console.error("Failed to fetch Zotero children",r)}return null}pickPdfAttachment(e){var n,r,s;let t=(s=(r=(n=e.attachments)!=null?n:e.children)!=null?r:e.items)!=null?s:[];if(!Array.isArray(t))return null;for(let i of t){let a=this.toPdfAttachment(i);if(a)return a}return null}toPdfAttachment(e){var s,i,a,o,p,u;if(((a=(s=e==null?void 0:e.contentType)!=null?s:e==null?void 0:e.mimeType)!=null?a:(i=e==null?void 0:e.data)==null?void 0:i.contentType)!=="application/pdf")return null;let n=(u=(o=e==null?void 0:e.key)!=null?o:e==null?void 0:e.attachmentKey)!=null?u:(p=e==null?void 0:e.data)==null?void 0:p.key;if(!n)return null;let r=this.extractAttachmentPath(e);return r?{key:n,filePath:r}:{key:n}}extractAttachmentPath(e){var n,r,s,i,a,o,p,u;let t=(u=(i=(r=(n=e==null?void 0:e.links)==null?void 0:n.enclosure)==null?void 0:r.href)!=null?i:(s=e==null?void 0:e.enclosure)==null?void 0:s.href)!=null?u:(p=(o=(a=e==null?void 0:e.data)==null?void 0:a.links)==null?void 0:o.enclosure)==null?void 0:p.href;if(typeof t=="string"&&t.startsWith("file://"))try{return(0,q.fileURLToPath)(t)}catch(_){return null}return null}async fetchZoteroChildren(e){let t=this.buildZoteroUrl(`/${this.getZoteroLibraryPath()}/items/${e}/children`),n=await this.requestLocalApi(t,`Zotero children request failed for ${t}`);return JSON.parse(n.toString("utf8"))}async downloadZoteroPdf(e){let t=this.buildZoteroUrl(`/${this.getZoteroLibraryPath()}/items/${e}/file`),n=await this.requestLocalApiRaw(t),r=await this.followFileRedirect(n);if(r)return r;if(n.statusCode>=300)throw new Error(`Request failed, status ${n.statusCode}`);return n.body}buildZoteroUrl(e){return`${this.settings.zoteroBaseUrl.replace(/\/$/,"")}${e}`}requestLocalApiRaw(e){return new Promise((t,n)=>{let r=new URL(e),i=(r.protocol==="https:"?J.default:H.default).request({method:"GET",hostname:r.hostname,port:r.port||void 0,path:`${r.pathname}${r.search}`,headers:{Accept:"*/*"}},a=>{let o=[];a.on("data",p=>o.push(Buffer.from(p))),a.on("end",()=>{var u;let p=Buffer.concat(o);t({statusCode:(u=a.statusCode)!=null?u:0,headers:a.headers,body:p})})});i.on("error",n),i.end()})}async requestLocalApi(e,t){let n=await this.requestLocalApiRaw(e);if(n.statusCode>=400){let r=n.body.toString("utf8");throw new Error(`${t!=null?t:"Request failed"}, status ${n.statusCode}: ${r||"no response body"}`)}if(n.statusCode>=300)throw new Error(`${t!=null?t:"Request failed"}, status ${n.statusCode}`);return n.body}async followFileRedirect(e){if(e.statusCode<300||e.statusCode>=400)return null;let t=e.headers.location,n=Array.isArray(t)?t[0]:t;if(!n||typeof n!="string")return null;if(n.startsWith("file://")){let r=(0,q.fileURLToPath)(n);return R.promises.readFile(r)}return n.startsWith("http://")||n.startsWith("https://")?this.requestLocalApi(n):null}bufferToArrayBuffer(e){return e.buffer.slice(e.byteOffset,e.byteOffset+e.byteLength)}buildPdfLinkFromSourcePath(e){if(!e)return"";let t=g.default.normalize(this.getVaultBasePath()),n=g.default.normalize(e),r=t.endsWith(g.default.sep)?t:`${t}${g.default.sep}`;return n.startsWith(r)?`[[${(0,l.normalizePath)(g.default.relative(t,n))}]]`:`[PDF](${(0,q.pathToFileURL)(e).toString()})`}async openNoteInMain(e){let t=(0,l.normalizePath)(e);await this.app.workspace.openLinkText(t,"","tab")}async openPdfInMain(e,t){if(!e)return!1;let n=g.default.normalize(this.getVaultBasePath()),r=g.default.normalize(e),s=n.endsWith(g.default.sep)?n:`${n}${g.default.sep}`;if(r.startsWith(s)){let i=(0,l.normalizePath)(g.default.relative(n,r)),a=t?`#page=${t}`:"";return await this.app.workspace.openLinkText(`${i}${a}`,"","tab"),!0}try{return window.open((0,q.pathToFileURL)(e).toString()),!0}catch(i){return!1}}openExternalUrl(e){e&&window.open(e)}buildZoteroDeepLink(e,t,n){if(t){let r=n?`?page=${encodeURIComponent(n)}`:"";return`zotero://open-pdf/library/items/${t}${r}`}return`zotero://select/library/items/${e}`}formatCitationsMarkdown(e){return e.length?e.map(n=>this.formatCitationMarkdown(n)).filter(Boolean).join(`
`):""}formatCitationMarkdown(e){var i,a;let t=e.doc_id||"?",n=e.pages||`${(i=e.page_start)!=null?i:"?"}-${(a=e.page_end)!=null?a:"?"}`,r=`${t} pages ${n}`,s=e.source_pdf||"";if(s){let o=this.buildPdfLinkFromSourcePath(s);if(o.startsWith("[["))return`- [[${o.slice(2,-2)}|${r}]]`;let p=o.match(/^\[PDF\]\((.+)\)$/);if(p)return`- [${r}](${p[1]})`}return`- ${r}`}generateChatId(){return typeof crypto!="undefined"&&"randomUUID"in crypto?crypto.randomUUID():`${Date.now().toString(36)}-${Math.random().toString(36).slice(2,8)}`}getDocIndexPath(){return(0,l.normalizePath)(`${S}/doc_index.json`)}async getDocIndex(){return this.docIndex?this.docIndex:(this.docIndex=await this.loadDocIndexFromDisk(),this.docIndex)}async loadDocIndexFromDisk(){var n;let e=this.app.vault.adapter,t=this.getDocIndexPath();if(!await e.exists(t))return{};try{let r=await e.read(t),s=JSON.parse(r);if(s&&typeof s=="object"){let i=(n=s.entries)!=null?n:s;if(Array.isArray(i)){let a={};for(let o of i)o!=null&&o.doc_id&&(a[String(o.doc_id)]=o);return a}if(i&&typeof i=="object")return i}}catch(r){console.error("Failed to read doc index",r)}return{}}async saveDocIndex(e){await this.ensureFolder(S);let t=this.app.vault.adapter,n=this.getDocIndexPath(),r={version:1,entries:e};await t.write(n,JSON.stringify(r,null,2)),this.docIndex=e}async updateDocIndex(e){var s;let t=await this.getDocIndex(),n=(s=t[e.doc_id])!=null?s:{doc_id:e.doc_id},r={...n,...e,doc_id:e.doc_id,updated_at:new Date().toISOString()};e.note_path===void 0&&n.note_path&&(r.note_path=n.note_path),e.note_title===void 0&&n.note_title&&(r.note_title=n.note_title),e.zotero_title===void 0&&n.zotero_title&&(r.zotero_title=n.zotero_title),e.pdf_path===void 0&&n.pdf_path&&(r.pdf_path=n.pdf_path),e.attachment_key===void 0&&n.attachment_key&&(r.attachment_key=n.attachment_key),t[e.doc_id]=r,await this.saveDocIndex(t)}async hydrateDocIndexFromCache(e){var a,o;if(!e)return null;let t=this.app.vault.adapter,n=await this.getDocIndexEntry(e),r={},s=(0,l.normalizePath)(`${D}/${e}.json`);if(await t.exists(s))try{let p=await t.read(s),u=JSON.parse(p),_=(o=(a=u==null?void 0:u.data)!=null?a:u)!=null?o:{},h=typeof _.title=="string"?_.title:"";if(h&&(r.zotero_title=h),!r.note_title||!r.note_path){let f=this.sanitizeFileName(h)||e,x=(0,l.normalizePath)(`${this.settings.outputNoteDir}/${f}.md`),v=(0,l.normalizePath)(`${this.settings.outputNoteDir}/${f}-${e}.md`),w="";await t.exists(x)?w=x:await t.exists(v)&&(w=v),w&&(r.note_path=w,r.note_title=g.default.basename(w,".md"))}}catch(p){console.error("Failed to read cached item JSON",p)}!r.note_title&&(n!=null&&n.note_path)&&(r.note_title=g.default.basename(n.note_path,".md"));let i=(0,l.normalizePath)(`${E}/${e}.json`);if(await t.exists(i))try{let p=await t.read(i),u=JSON.parse(p);typeof(u==null?void 0:u.source_pdf)=="string"&&(r.pdf_path=u.source_pdf)}catch(p){console.error("Failed to read cached chunks JSON",p)}return Object.keys(r).length>0&&await this.updateDocIndex({doc_id:e,...r}),this.getDocIndexEntry(e)}async getDocIndexEntry(e){var n;return e&&(n=(await this.getDocIndex())[e])!=null?n:null}async inferNotePathFromCache(e){var r,s;let t=this.app.vault.adapter,n=(0,l.normalizePath)(`${D}/${e}.json`);if(!await t.exists(n))return"";try{let i=await t.read(n),a=JSON.parse(i),o=(s=(r=a==null?void 0:a.data)!=null?r:a)!=null?s:{},p=typeof o.title=="string"?o.title:"",u=this.sanitizeFileName(p)||e,_=(0,l.normalizePath)(`${this.settings.outputNoteDir}/${u}.md`),h=(0,l.normalizePath)(`${this.settings.outputNoteDir}/${u}-${e}.md`);if(await t.exists(_))return _;if(await t.exists(h))return h}catch(i){console.error("Failed to infer note path from cache",i)}return""}async rebuildNoteFromCacheForDocId(e,t){var b,y;try{await this.ensureBundledTools()}catch(c){return t&&new l.Notice("Failed to sync bundled tools. See console for details."),console.error(c),!1}let n=this.app.vault.adapter,r=(0,l.normalizePath)(`${D}/${e}.json`),s=(0,l.normalizePath)(`${E}/${e}.json`);if(!await n.exists(r)||!await n.exists(s))return t&&new l.Notice("Cached item or chunks JSON not found."),!1;this.showStatusProgress("Preparing...",5);let i;try{let c=await n.read(r);i=JSON.parse(c)}catch(c){return t&&new l.Notice("Failed to read cached item JSON."),console.error(c),this.clearStatusProgress(),!1}let a;try{let c=await n.read(s);a=JSON.parse(c)}catch(c){return t&&new l.Notice("Failed to read cached chunks JSON."),console.error(c),this.clearStatusProgress(),!1}let o=typeof a.source_pdf=="string"?a.source_pdf:"";if(!o)return t&&new l.Notice("Cached chunk JSON is missing source_pdf."),this.clearStatusProgress(),!1;try{await R.promises.access(o)}catch(c){return t&&new l.Notice("Cached source PDF path is not accessible."),console.error(c),this.clearStatusProgress(),!1}let p=(b=i.data)!=null?b:i,u=typeof p.title=="string"?p.title:"",_="",h=await this.getDocIndexEntry(e);if(h!=null&&h.note_path&&await n.exists(h.note_path)&&(_=(0,l.normalizePath)(h.note_path)),!_){let c=this.sanitizeFileName(u)||e,P=(0,l.normalizePath)(`${this.settings.outputNoteDir}/${c}.md`),T=await n.exists(P)?c:await this.resolveUniqueBaseName(c,e);_=(0,l.normalizePath)(`${this.settings.outputNoteDir}/${T}.md`)}try{await this.ensureFolder(this.settings.outputNoteDir)}catch(c){return t&&new l.Notice("Failed to create notes folder."),console.error(c),this.clearStatusProgress(),!1}let f=this.getPluginDir(),x=g.default.join(f,"tools","docling_extract.py"),v=g.default.join(f,"tools","index_redisearch.py"),w=null;try{w=await this.readDoclingQualityLabelFromPdf(o),this.showStatusProgress(this.formatStatusLabel("Docling extraction...",w),null),await this.runPython(x,this.buildDoclingArgs(o,e,s,_)),w=await this.readDoclingQualityLabel(s)}catch(c){return t&&new l.Notice("Docling extraction failed. See console for details."),console.error(c),this.clearStatusProgress(),!1}try{this.showStatusProgress(this.formatStatusLabel("Indexing chunks...",w),0),await this.runPythonStreaming(v,["--chunks-json",this.getAbsoluteVaultPath(s),"--redis-url",this.settings.redisUrl,"--index",this.settings.redisIndex,"--prefix",this.settings.redisPrefix,"--embed-base-url",this.settings.embedBaseUrl,"--embed-api-key",this.settings.embedApiKey,"--embed-model",this.settings.embedModel,"--upsert","--progress"],c=>{if((c==null?void 0:c.type)==="progress"&&c.total){let P=Math.round(c.current/c.total*100),T=this.formatStatusLabel(`Indexing chunks ${c.current}/${c.total}`,w);this.showStatusProgress(T,P)}},()=>{})}catch(c){return t&&new l.Notice("RedisSearch indexing failed. See console for details."),console.error(c),this.clearStatusProgress(),!1}let C=this.buildPdfLinkFromSourcePath(o);try{let c=await this.app.vault.adapter.read(_),P=this.buildNoteMarkdown(p,(y=i.meta)!=null?y:{},e,C,r,c);await this.app.vault.adapter.write(_,P)}catch(c){return t&&new l.Notice("Failed to finalize note markdown."),console.error(c),this.clearStatusProgress(),!1}try{await this.updateDocIndex({doc_id:e,note_path:_,note_title:g.default.basename(_,".md"),zotero_title:u,pdf_path:o})}catch(c){console.error("Failed to update doc index",c)}return!0}getZoteroLibraryPath(){let e=(this.settings.zoteroUserId||"0").trim();return!e||e==="0"?"users/0":e.startsWith("users/")||e.startsWith("groups/")?e:`users/${e}`}async ensureFolder(e){let t=this.app.vault.adapter,n=(0,l.normalizePath)(e).split("/").filter(Boolean),r="";for(let s of n)r=r?`${r}/${s}`:s,await t.exists(r)||await t.mkdir(r)}buildNoteMarkdown(e,t,n,r,s,i){let a=`[[${s}]]`,o=this.renderFrontmatter(e,t,n,r,a);return`${o?`---
${o}
---
...
${i}`}renderFrontmatter(e,t,n,r,s){var o;let i=(o=this.settings.frontmatterTemplate)!=null?o:"";if(!i.trim())return"";let a=this.buildTemplateVars(e,t,n,r,s);return i.replace(/{{\s*([a-z0-9_]+)\s*}}/gi,(p,u)=>{var _;return(_=a[u])!=null?_:""}).trim()}buildTemplateVars(e,t,n,r,s){let i=typeof e.title=="string"?e.title:"",a=typeof e.shortTitle=="string"?e.shortTitle:"",o=typeof e.date=="string"?e.date:"",p=typeof(t==null?void 0:t.parsedDate)=="string"?t.parsedDate:"",u=this.extractYear(p||o),h=(Array.isArray(e.creators)?e.creators:[]).filter(y=>y.creatorType==="author").map(y=>this.formatCreatorName(y)),f=h.join("; "),x=Array.isArray(e.tags)?e.tags.map(y=>typeof y=="string"?y:y==null?void 0:y.tag).filter(Boolean):[],v=x.join("; "),w=typeof e.itemType=="string"?e.itemType:"",C=typeof(t==null?void 0:t.creatorSummary)=="string"?t.creatorSummary:"",b={doc_id:n,zotero_key:typeof e.key=="string"?e.key:n,title:i,short_title:a,date:o,year:u,authors:f,tags:v,item_type:w,creator_summary:C,pdf_link:this.escapeYamlString(r),item_json:this.escapeYamlString(s)};for(let[y,c]of Object.entries(b))b[`${y}_yaml`]=this.escapeYamlString(c);return b.authors_yaml=this.toYamlList(h),b.tags_yaml=this.toYamlList(x),b}extractYear(e){if(!e)return"";let t=e.match(/\b(\d{4})\b/);return t?t[1]:""}formatCreatorName(e){if(!e||typeof e!="object")return"";if(e.name)return String(e.name);let t=e.firstName?String(e.firstName):"",n=e.lastName?String(e.lastName):"";return[n,t].filter(Boolean).join(", ")||`${t} ${n}`.trim()}escapeYamlString(e){return`"${String(e).replace(/\\/g,"\\\\").replace(/"/g,'\\"')}"`}toYamlList(e){return e.length?e.map(t=>`  - ${this.escapeYamlString(t)}`).join(`
`):'  - ""'}getVaultBasePath(){var n;let e=this.app.vault.adapter;if(e instanceof l.FileSystemAdapter)return e.getBasePath();let t=(n=e.getBasePath)==null?void 0:n.call(e);if(t)return t;throw new Error("Vault base path is unavailable.")}getPluginDir(){var r;let e=this.getVaultBasePath(),t=(r=this.manifest.dir)!=null?r:this.manifest.id;if(!t)throw new Error("Plugin directory is unavailable.");let n=g.default.isAbsolute(t)?t:g.default.join(e,t);return g.default.normalize(n)}async ensureBundledTools(){let e=this.getPluginDir(),t=g.default.join(e,"tools");await R.promises.mkdir(t,{recursive:!0});for(let[n,r]of Object.entries(G)){let s=g.default.join(t,n),i=!0;try{await R.promises.readFile(s,"utf8")===r&&(i=!1)}catch(a){}i&&await R.promises.writeFile(s,r,"utf8")}}async migrateCachePaths(){let e="zotero/items",t="zotero/chunks",n=D,r=E,s=this.app.vault.adapter,i=(0,l.normalizePath)(e),a=(0,l.normalizePath)(t),o=(0,l.normalizePath)(n),p=(0,l.normalizePath)(r),u=o.split("/").slice(0,-1).join("/"),_=p.split("/").slice(0,-1).join("/");u&&await this.ensureFolder(u),_&&await this.ensureFolder(_);let h=await s.exists(i),f=await s.exists(a),x=await s.exists(o),v=await s.exists(p);h&&!x&&await s.rename(i,o),f&&!v&&await s.rename(a,p)}getAbsoluteVaultPath(e){let t=this.getVaultBasePath(),n=g.default.isAbsolute(e)?e:g.default.join(t,e);return g.default.normalize(n)}buildDoclingArgs(e,t,n,r){let s=this.settings.ocrMode==="force_low_quality"?"auto":this.settings.ocrMode,i=["--pdf",e,"--doc-id",t,"--out-json",this.getAbsoluteVaultPath(n),"--out-md",this.getAbsoluteVaultPath(r),"--chunking",this.settings.chunkingMode,"--ocr",s];return this.settings.ocrMode==="force_low_quality"&&i.push("--force-ocr-low-quality"),i.push("--quality-threshold",String(this.settings.ocrQualityThreshold)),this.settings.enableLlmCleanup&&(i.push("--enable-llm-cleanup"),this.settings.llmCleanupBaseUrl&&i.push("--llm-cleanup-base-url",this.settings.llmCleanupBaseUrl),this.settings.llmCleanupApiKey&&i.push("--llm-cleanup-api-key",this.settings.llmCleanupApiKey),this.settings.llmCleanupModel&&i.push("--llm-cleanup-model",this.settings.llmCleanupModel),i.push("--llm-cleanup-temperature",String(this.settings.llmCleanupTemperature)),i.push("--llm-cleanup-min-quality",String(this.settings.llmCleanupMinQuality)),i.push("--llm-cleanup-max-chars",String(this.settings.llmCleanupMaxChars))),i}getRedisDataDir(){return g.default.join(this.getVaultBasePath(),S,"redis-data")}getDockerComposePath(){let e=this.getPluginDir();return g.default.join(e,"tools","docker-compose.yml")}async startRedisStack(e){var t;try{await this.ensureBundledTools();let n=this.getDockerComposePath(),r=this.getRedisDataDir();await R.promises.mkdir(r,{recursive:!0});let s=((t=this.settings.dockerPath)==null?void 0:t.trim())||"docker";await this.runCommand(s,["compose","-f",n,"up","-d"],{cwd:g.default.dirname(n),env:{...process.env,ZRR_DATA_DIR:r}}),e||new l.Notice("Redis Stack started.")}catch(n){e||new l.Notice("Failed to start Redis Stack. Check Docker Desktop and File Sharing."),console.error("Failed to start Redis Stack",n)}}async stopRedisStack(e){var t;try{await this.ensureBundledTools();let n=this.getDockerComposePath(),r=((t=this.settings.dockerPath)==null?void 0:t.trim())||"docker";await this.runCommand(r,["compose","-f",n,"down"],{cwd:g.default.dirname(n)}),e||new l.Notice("Redis Stack stopped.")}catch(n){e||new l.Notice("Failed to stop Redis Stack. See console for details."),console.error("Failed to stop Redis Stack",n)}}async removeRedisStackContainer(e){var t;try{let n=((t=this.settings.dockerPath)==null?void 0:t.trim())||"docker";await this.runCommand(n,["rm","-f","redis-stack"]),e||new l.Notice("Removed redis-stack container.")}catch(n){if((n instanceof Error?n.message:String(n)).includes("No such container")){e||new l.Notice("redis-stack container not found.");return}e||new l.Notice("Failed to remove redis-stack container. See console for details."),console.error("Failed to remove redis-stack container",n)}}runPython(e,t){return new Promise((n,r)=>{let s=(0,A.spawn)(this.settings.pythonPath,[e,...t],{cwd:g.default.dirname(e)}),i="";s.stderr.on("data",a=>{i+=a.toString()}),s.on("close",a=>{a===0?n():r(new Error(i||`Process exited with code ${a}`))})})}runCommand(e,t,n){return new Promise((r,s)=>{let i=(0,A.spawn)(e,t,{cwd:n==null?void 0:n.cwd,env:n==null?void 0:n.env}),a="";i.stderr.on("data",o=>{a+=o.toString()}),i.on("close",o=>{o===0?r():s(new Error(a||`Process exited with code ${o}`))})})}runPythonStreaming(e,t,n,r){return new Promise((s,i)=>{let a=(0,A.spawn)(this.settings.pythonPath,[e,...t],{cwd:g.default.dirname(e)}),o="",p="",u=null,_=!1,h=f=>{if(f.trim())try{let x=JSON.parse(f);u=x,((x==null?void 0:x.type)==="final"||x!=null&&x.answer)&&(_=!0),n(x)}catch(x){}};a.stdout.on("data",f=>{var v;o+=f.toString();let x=o.split(/\r?\n/);o=(v=x.pop())!=null?v:"";for(let w of x)h(w)}),a.stderr.on("data",f=>{p+=f.toString()}),a.on("close",f=>{o.trim()&&h(o),!_&&u&&r(u),f===0?s():i(new Error(p||`Process exited with code ${f}`))})})}runPythonWithOutput(e,t){return new Promise((n,r)=>{let s=(0,A.spawn)(this.settings.pythonPath,[e,...t],{cwd:g.default.dirname(e)}),i="",a="";s.stdout.on("data",o=>{i+=o.toString()}),s.stderr.on("data",o=>{a+=o.toString()}),s.on("close",o=>{o===0?n(i.trim()):r(new Error(a||`Process exited with code ${o}`))})})}},Z=class extends l.SuggestModal{constructor(e,t,n){super(e);this.lastError=null;this.plugin=t,this.resolveSelection=n,this.setPlaceholder("Search Zotero items...")}async getSuggestions(e){try{return await this.plugin.searchZoteroItems(e)}catch(t){let n=t instanceof Error?t.message:String(t);return this.lastError!==n&&(this.lastError=n,new l.Notice(n)),console.error("Zotero search failed",t),[]}}renderSuggestion(e,t){var s,i;let n=(i=(s=e.data)==null?void 0:s.title)!=null?i:"[No title]",r=this.extractYear(e);t.createEl("div",{text:n}),t.createEl("small",{text:r?`${r}`:""}),t.addEventListener("click",()=>{this.resolveSelection&&(this.resolveSelection(e),this.resolveSelection=null),this.close()})}onChooseSuggestion(e,t){this.resolveSelection&&(this.resolveSelection(e),this.resolveSelection=null),this.close()}onClose(){this.resolveSelection&&(this.resolveSelection(null),this.resolveSelection=null)}extractYear(e){var r,s,i,a;let t=(a=(i=(r=e.meta)==null?void 0:r.parsedDate)!=null?i:(s=e.data)==null?void 0:s.date)!=null?a:"";if(typeof t!="string")return"";let n=t.match(/\b(\d{4})\b/);return n?n[1]:""}};

"use strict";var rn=Object.create;var be=Object.defineProperty;var sn=Object.getOwnPropertyDescriptor;var an=Object.getOwnPropertyNames;var on=Object.getPrototypeOf,ln=Object.prototype.hasOwnProperty;var dn=(v,h)=>{for(var e in h)be(v,e,{get:h[e],enumerable:!0})},Ge=(v,h,e,n)=>{if(h&&typeof h=="object"||typeof h=="function")for(let t of an(h))!ln.call(v,t)&&t!==e&&be(v,t,{get:()=>h[t],enumerable:!(n=sn(h,t))||n.enumerable});return v};var le=(v,h,e)=>(e=v!=null?rn(on(v)):{},Ge(h||!v||!v.__esModule?be(e,"default",{value:v,enumerable:!0}):e,v)),cn=v=>Ge(be({},"__esModule",{value:!0}),v);var vn={};dn(vn,{default:()=>Pe});module.exports=cn(vn);var u=require("obsidian"),te=require("@codemirror/state"),M=require("@codemirror/view"),X=require("child_process"),G=require("fs"),Je=le(require("http")),Xe=le(require("https")),_e=le(require("net")),Ye=le(require("os")),Qe=le(require("tls")),L=le(require("path")),pe=require("url"),ke=require("crypto");var N=require("obsidian"),K=".zotero-redisearch-rag",J=`${K}/items`,U=`${K}/chunks`,Be={pythonPath:"",pythonEnvLocation:"shared",dockerPath:"docker",redisUrl:"redis://127.0.0.1:6379",autoAssignRedisPort:!1,autoStartRedis:!0,zoteroBaseUrl:"http://127.0.0.1:23119/api",zoteroUserId:"0",webApiBaseUrl:"https://api.zotero.org",webApiLibraryType:"user",webApiLibraryId:"",webApiKey:"",outputPdfDir:"Zotero/PDFs",outputNoteDir:"Zotero/Notes",frontmatterTemplate:`doc_id: {{doc_id}}
...
`};var Ve={"docling_extract.py":`#!/usr/bin/env python3
# zotero-redisearch-rag tool version: 0.4.0
import argparse
import base64
import errno
import json
...
`,"ocr_wordlist.txt":`# zotero-redisearch-rag tool version: 0.4.0
aai
aam
abb
abge
...
\xF6pnv
\xFCberarb
`,"requirements.txt":`# zotero-redisearch-rag tool version: 0.4.0
docling
langcodes[data]
markdown
markdown-it-py
...
spylls
`,"docker-compose.yml":`# zotero-redisearch-rag tool version: 0.4.0
services:
  redis-stack:
...
`).replace(/[ \t]*\n[ \t]*/g,`
`).trim()}scheduleNoteSync(e){let n=this.noteSyncTimers.get(e.path);n!==void 0&&window.clearTimeout(n);let t=window.setTimeout(()=>{this.noteSyncTimers.delete(e.path),this.syncNoteToRedis(e)},1200);this.noteSyncTimers.set(e.path,t)}escapeRegExp(e){return e.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")}formatCitationPageLabel(e){let n=e.page_start?String(e.page_start):"",t=e.page_end?String(e.page_end):"";if(n&&(!t||n===t))return n;if(n&&t)return`${n} - ${t}`;let i=(e.pages||"").trim();if(!i)return"?";let r=i.match(/^(\d+)\s*-\s*(\d+)$/);return r?r[1]===r[2]?r[1]:`${r[1]} - ${r[2]}`:i.replace("-"," - ")}normalizeChunkIdForNote(e,n){if(!e)return null;let t=String(e);if(n&&t.startsWith(`${n}:`))return t.slice(n.length+1);if(t.includes(":")){let i=t.split(":");if(i.length>1&&n&&i[0]===n)return i.slice(1).join(":")}return t}async syncNoteToRedis(e){var n,t,i;if(!this.noteSyncInFlight.has(e.path)&&!this.noteSyncSuppressed.has(e.path)){this.noteSyncInFlight.add(e.path);try{let r=await this.app.vault.read(e),s=this.extractSyncSection(r);if(!s)return;let a=await this.resolveDocIdForNote(e,r);if(!a)return;let o=this.parseSyncedChunkBlocks(s);if(!o.length)return;let l=(0,u.normalizePath)(`${U}/${a}.json`),d=this.app.vault.adapter;if(!await d.exists(l))return;let c=await this.readChunkPayload(l);if(!c)return;let p=Array.isArray(c.chunks)?c.chunks:[],g=new Map;for(let x of p){let P=typeof(x==null?void 0:x.chunk_id)=="string"?x.chunk_id:String((n=x==null?void 0:x.chunk_id)!=null?n:"");P&&g.set(P,x)}let y=new Set,b=new Set,C=new Set,_=new Set,f=!1;for(let x of o){let P=x.chunkId;if(!P)continue;y.add(P);let R=g.get(P);if(!R){console.warn(`Sync note: chunk id not found in cache (${P})`);continue}if(x.excludeFlag){R.excluded!==!0&&(R.excluded=!0,f=!0);let S=this.normalizeChunkText(x.text);S&&S!==String((t=R.text)!=null?t:"")&&(R.text=S,R.char_count=S.length,f=!0),C.add(P);continue}if(R.excluded&&(R.excluded=!1,f=!0,b.add(P)),!x.text.trim()){C.add(P),_.add(P);continue}let E=this.normalizeChunkText(x.text);if(!E){C.add(P),_.add(P);continue}let w=typeof R.text=="string"?R.text:String((i=R.text)!=null?i:"");E!==w&&(R.text=E,R.char_count=E.length,b.add(P),f=!0)}for(let x of g.keys())y.has(x)||(C.add(x),_.add(x));if(!b.size&&!C.size&&!_.size&&!f)return;_.size&&(c.chunks=p.filter(x=>{var R;let P=typeof(x==null?void 0:x.chunk_id)=="string"?x.chunk_id:String((R=x==null?void 0:x.chunk_id)!=null?R:"");return P&&!_.has(P)}),f=!0),(f||_.size)&&await d.write(l,JSON.stringify(c,null,2)),await this.reindexChunkUpdates(a,l,Array.from(b),Array.from(C))}catch(r){console.warn("Failed to sync note edits to Redis",r)}finally{this.noteSyncInFlight.delete(e.path)}}}extractSyncSection(e){let n=ce.exec(e);if(!n)return null;let t=e.slice(n.index+n[0].length),i=ge.exec(t);return i?t.slice(0,i.index):null}extractDocIdFromSyncMarker(e){var r;let n=ce.exec(e);if(!n)return null;let i=((r=n[0])!=null?r:"").match(/doc_id=([\"']?)([^\"'\s]+)\1/i);return i?i[2].trim():null}parseSyncedChunkBlocks(e){var o;let n=e.split(/\r?\n/),t=[],i="",r=!1,s=[],a=()=>{i&&(t.push({chunkId:i,text:s.join(`
`).trim(),excludeFlag:r}),i="",r=!1,s=[])};for(let l of n){let d=l.match(fe);if(d){a();let c=(o=d[1])!=null?o:"",p=c.match(/id=([\"']?)([^\"'\s]+)\1/i),g=p?p[2].trim():"";if(!g)continue;i=g,r=/\bexclude\b/i.test(c)||/\bdelete\b/i.test(c),s=[];continue}if(Ne.test(l)){a();continue}if(i){if(we.test(l)){r=!0;continue}s.push(l)}}return a(),t}normalizeChunkText(e){return e.split(/\r?\n/).map(n=>n.replace(/\s+/g," ").trim()).filter((n,t,i)=>!(n===""&&i[t-1]==="")).join(`
`).trim()}buildSyncedDoclingContent(e,n,t){let i=Array.isArray(n==null?void 0:n.chunks)?n==null?void 0:n.chunks:[];if(!i.length)return`<!-- zrr:sync-start doc_id=${e} -->
${t}

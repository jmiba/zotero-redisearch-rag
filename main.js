"use strict";var en=Object.create;var he=Object.defineProperty;var nn=Object.getOwnPropertyDescriptor;var tn=Object.getOwnPropertyNames;var rn=Object.getPrototypeOf,sn=Object.prototype.hasOwnProperty;var an=(w,h)=>{for(var e in h)he(w,e,{get:h[e],enumerable:!0})},ze=(w,h,e,n)=>{if(h&&typeof h=="object"||typeof h=="function")for(let t of tn(h))!sn.call(w,t)&&t!==e&&he(w,t,{get:()=>h[t],enumerable:!(n=nn(h,t))||n.enumerable});return w};var ie=(w,h,e)=>(e=w!=null?en(rn(w)):{},ze(h||!w||!w.__esModule?he(e,"default",{value:w,enumerable:!0}):e,w)),on=w=>ze(he({},"__esModule",{value:!0}),w);var yn={};an(yn,{default:()=>be});module.exports=on(yn);var p=require("obsidian"),ne=require("@codemirror/state"),j=require("@codemirror/view"),Q=require("child_process"),U=require("fs"),Ge=ie(require("http")),Ze=ie(require("https")),de=ie(require("net")),Ve=ie(require("os")),He=ie(require("tls")),C=ie(require("path")),ae=require("url"),fe=require("crypto");var E=require("obsidian"),J=".zotero-redisearch-rag",Y=`${J}/items`,H=`${J}/chunks`,Fe={zoteroBaseUrl:"http://127.0.0.1:23119/api",zoteroUserId:"0",webApiBaseUrl:"https://api.zotero.org",webApiLibraryType:"user",webApiLibraryId:"",webApiKey:"",pythonPath:"",pythonEnvLocation:"shared",dockerPath:"docker",autoStartRedis:!1,copyPdfToVault:!0,frontmatterTemplate:`doc_id: {{doc_id}}
...
item_json: {{item_json_yaml}}`,noteBodyTemplate:"{{pdf_block}}{{docling_markdown}}",llmProviderProfiles:[{id:"lm-studio",name:"LM Studio",baseUrl:"http://localhost:1234/v1",apiKey:"lm-studio"},{id:"ollama",name:"Ollama",baseUrl:"http://localhost:11434/v1",apiKey:""},{id:"openrouter",name:"OpenRouter",baseUrl:"https://openrouter.ai/api/v1",apiKey:""},{id:"openai",name:"OpenAI",baseUrl:"https://api.openai.com/v1",apiKey:""}],embedProviderProfileId:"lm-studio",chatProviderProfileId:"lm-studio",llmCleanupProviderProfileId:"lm-studio",tagSanitizeMode:"kebab",outputPdfDir:"Zotero/PDFs",outputNoteDir:"Zotero/Notes",chatOutputDir:"Zotero/Chats",chatPaneLocation:"right",redisUrl:"redis://127.0.0.1:6379",autoAssignRedisPort:!0,redisIndex:"idx:zotero",redisPrefix:"zotero:chunk:",embedBaseUrl:"http://localhost:1234/v1",embedApiKey:"lm-studio",embedModel:"google/embedding-gemma-300m",embedIncludeMetadata:!0,embedSubchunkChars:3500,embedSubchunkOverlap:200,enableChunkTagging:!1,chatBaseUrl:"http://127.0.0.1:1234/v1",chatApiKey:"",chatModel:"openai/gpt-oss-20b",chatTemperature:.2,chatHistoryMessages:6,ocrMode:"auto",chunkingMode:"page",ocrQualityThreshold:.5,enableLlmCleanup:!1,llmCleanupBaseUrl:"http://127.0.0.1:1234/v1",llmCleanupApiKey:"",llmCleanupModel:"openai/gpt-oss-20b",llmCleanupTemperature:0,llmCleanupMinQuality:.35,llmCleanupMaxChars:2e3,enableFileLogging:!1,logFilePath:`${J}/logs/docling_extract.log`,createOcrLayeredPdf:!1,preferObsidianNoteForCitations:!0},_e=class extends E.PluginSettingTab{constructor(h,e){super(h,e),this.plugin=e}display(){let{containerEl:h}=this;h.empty();let e=()=>Array.isArray(this.plugin.settings.llmProviderProfiles)?this.plugin.settings.llmProviderProfiles:[],n=async g=>{this.plugin.settings.llmProviderProfiles=g,await this.plugin.saveSettings()},t=g=>{g.inputEl.type="password",g.inputEl.autocomplete="off",g.inputEl.spellcheck=!1};h.createEl("h2",{text:"Prerequisites"}),new E.Setting(h).setName("Python path").setDesc("Optional path to the Python interpreter used to create or run the plugin env. Leave blank to auto-detect (python3.12/3.11/3.10/python3/python, or py on Windows).").addText(g=>g.setPlaceholder("auto-detect").setValue(this.plugin.settings.pythonPath).onChange(async _=>{this.plugin.settings.pythonPath=_.trim(),await this.plugin.saveSettings()})),new E.Setting(h).setName("Python environment").setDesc("Create or update the plugin's Python env (location configured below).").addButton(g=>{g.setButtonText("Create/Update").setCta(),g.onClick(async()=>{g.setDisabled(!0);try{await this.plugin.setupPythonEnv()}finally{g.setDisabled(!1)}})}),new E.Setting(h).setName("Python env location").setDesc("Shared user cache can be reused across vaults; plugin folder keeps a per-vault env.").addDropdown(g=>{g.addOption("shared","Shared user cache"),g.addOption("plugin","Plugin folder (.venv)"),g.setValue(this.plugin.settings.pythonEnvLocation).onChange(async _=>{_!=="shared"&&_!=="plugin"||(this.plugin.settings.pythonEnvLocation=_,await this.plugin.saveSettings())})}),new E.Setting(h).setName("Docker/Podman path").setDesc("CLI path for Docker or Podman (used to start Redis Stack).").addText(g=>g.setPlaceholder("docker").setValue(this.plugin.settings.dockerPath).onChange(async _=>{this.plugin.settings.dockerPath=_.trim()||"docker",await this.plugin.saveSettings()})),new E.Setting(h).setName("Redis URL").addText(g=>g.setPlaceholder("redis://127.0.0.1:6379").setValue(this.plugin.settings.redisUrl).onChange(async _=>{this.plugin.settings.redisUrl=_.trim(),await this.plugin.saveSettings()})),new E.Setting(h).setName("Auto-assign Redis port").setDesc("When starting Redis stack, pick a free local port and update the Redis URL.").addToggle(g=>g.setValue(this.plugin.settings.autoAssignRedisPort).onChange(async _=>{this.plugin.settings.autoAssignRedisPort=_,await this.plugin.saveSettings()})),new E.Setting(h).setName("Auto-start Redis stack (Docker/Podman Compose)").setDesc("Requires Docker Desktop running and your vault path shared with Docker. Uses a vault-specific data dir at .obsidian/zotero-redisearch-rag/redis-data.").addToggle(g=>g.setValue(this.plugin.settings.autoStartRedis).onChange(async _=>{this.plugin.settings.autoStartRedis=_,await this.plugin.saveSettings()})),new E.Setting(h).setName("Start Redis stack now").setDesc("Restarts Docker/Podman Compose with the vault data directory.").addButton(g=>g.setButtonText("Start").onClick(async()=>{await this.plugin.startRedisStack()})),h.createEl("h2",{text:"Zotero Local API"}),new E.Setting(h).setName("Zotero base URL").setDesc("Local Zotero API base URL, e.g. http://127.0.0.1:23119/api").addText(g=>g.setPlaceholder("http://127.0.0.1:23119/api").setValue(this.plugin.settings.zoteroBaseUrl).onChange(async _=>{this.plugin.settings.zoteroBaseUrl=_.trim(),await this.plugin.saveSettings()}));let i=new E.Setting(h).setName("Zotero library").setDesc("Select your local library or a Zotero group library."),r=null,s=g=>{if(!r)return;let _=(this.plugin.settings.zoteroUserId||"0").trim()||"0";new Set(g.map(M=>M.value)).has(_)||(g=g.concat([{value:_,label:`Custom (${_})`}])),r.selectEl.options.length=0;for(let M of g)r.addOption(M.value,M.label);r.setValue(_)},a=async()=>{if(r){r.setDisabled(!0);try{let g=await this.plugin.fetchZoteroLibraryOptions();s(g)}finally{r.setDisabled(!1)}}};i.addDropdown(g=>{r=g;let _=(this.plugin.settings.zoteroUserId||"0").trim()||"0";g.addOption(_,"Loading..."),g.setValue(_),g.onChange(async P=>{this.plugin.settings.zoteroUserId=P.trim(),await this.plugin.saveSettings()})}),i.addButton(g=>{g.setButtonText("Refresh").onClick(async()=>{await a()})}),a(),h.createEl("h2",{text:"Zotero Web API"}),new E.Setting(h).setName("Web API base URL").setDesc("Zotero Web API base URL for write fallback, e.g. https://api.zotero.org").addText(g=>g.setPlaceholder("https://api.zotero.org").setValue(this.plugin.settings.webApiBaseUrl).onChange(async _=>{this.plugin.settings.webApiBaseUrl=_.trim(),await this.plugin.saveSettings()})),new E.Setting(h).setName("Web API library type").setDesc("Library type for Web API writes.").addDropdown(g=>g.addOption("user","user").addOption("group","group").setValue(this.plugin.settings.webApiLibraryType).onChange(async _=>{this.plugin.settings.webApiLibraryType=_,await this.plugin.saveSettings()})),new E.Setting(h).setName("Web API library ID").setDesc("Numeric Zotero user/group ID for Web API writes.").addText(g=>g.setPlaceholder("15218").setValue(this.plugin.settings.webApiLibraryId).onChange(async _=>{this.plugin.settings.webApiLibraryId=_.trim(),await this.plugin.saveSettings()})),new E.Setting(h).setName("Web API key").setDesc("Zotero API key for write fallback (from zotero.org).").addText(g=>{t(g),g.setPlaceholder("your-api-key").setValue(this.plugin.settings.webApiKey).onChange(async _=>{this.plugin.settings.webApiKey=_.trim(),await this.plugin.saveSettings()})}),h.createEl("h2",{text:"Output"}),new E.Setting(h).setName("PDF folder").addText(g=>g.setPlaceholder("zotero/pdfs").setValue(this.plugin.settings.outputPdfDir).onChange(async _=>{this.plugin.settings.outputPdfDir=_.trim(),await this.plugin.saveSettings()})),new E.Setting(h).setName("Notes folder").addText(g=>g.setPlaceholder("zotero/notes").setValue(this.plugin.settings.outputNoteDir).onChange(async _=>{this.plugin.settings.outputNoteDir=_.trim(),await this.plugin.saveSettings()})),new E.Setting(h).setName("Frontmatter template").setDesc("Template for note YAML frontmatter. Use {{var}} placeholders; leave blank to omit.").addTextArea(g=>{g.setValue(this.plugin.settings.frontmatterTemplate).onChange(async _=>{this.plugin.settings.frontmatterTemplate=_,await this.plugin.saveSettings()}),g.inputEl.rows=10,g.inputEl.style.width="100%"}),new E.Setting(h).setName("Tag sanitization").setDesc("Normalize Zotero tags for Obsidian (no spaces, punctuation trimmed).").addDropdown(g=>g.addOption("none","No change").addOption("camel","camelCase").addOption("pascal","PascalCase").addOption("snake","snake_case").addOption("kebab","kebab-case").setValue(this.plugin.settings.tagSanitizeMode==="replace"?"kebab":this.plugin.settings.tagSanitizeMode).onChange(async _=>{this.plugin.settings.tagSanitizeMode=_,await this.plugin.saveSettings()})),new E.Setting(h).setName("Note body template").setDesc("Template for the note body after frontmatter. Use {{pdf_block}} and {{docling_markdown}} placeholders.").addTextArea(g=>{g.setValue(this.plugin.settings.noteBodyTemplate).onChange(async _=>{this.plugin.settings.noteBodyTemplate=_,await this.plugin.saveSettings()}),g.inputEl.rows=8,g.inputEl.style.width="100%"}),h.createEl("h2",{text:"LLM Provider Profiles"});let o=h.createDiv({cls:"zrr-llm-profiles"}),l=()=>{o.empty();let g=e();g.length||o.createEl("p",{text:"No profiles yet. Add one below."});for(let _ of g){let P=o.createEl("details",{cls:"zrr-profile"});g.length===1&&(P.open=!0);let M=P.createEl("summary",{text:_.name||_.id||"Profile"});M.addClass("zrr-profile-title");let te=P.createDiv({cls:"zrr-profile-body"});new E.Setting(te).setName("Profile name").addText(X=>X.setPlaceholder("My provider").setValue(_.name||"").onChange(async ee=>{_.name=ee.trim(),M.textContent=_.name||_.id||"Profile",await n(e())})),new E.Setting(te).setName("Base URL").addText(X=>X.setPlaceholder("http://localhost:1234/v1").setValue(_.baseUrl||"").onChange(async ee=>{_.baseUrl=ee.trim(),await n(e())})),new E.Setting(te).setName("API key").setDesc("Stored in settings (not encrypted).").addText(X=>{t(X),X.setPlaceholder("sk-...").setValue(_.apiKey||"").onChange(async ee=>{_.apiKey=ee.trim(),await n(e())})}),new E.Setting(te).setName("Remove profile").setDesc("Deletes this saved profile.").addButton(X=>X.setButtonText("Delete profile").onClick(async()=>{let ee=e().filter(le=>le.id!==_.id);this.plugin.settings.embedProviderProfileId=this.plugin.settings.embedProviderProfileId===_.id?"":this.plugin.settings.embedProviderProfileId,this.plugin.settings.chatProviderProfileId=this.plugin.settings.chatProviderProfileId===_.id?"":this.plugin.settings.chatProviderProfileId,this.plugin.settings.llmCleanupProviderProfileId=this.plugin.settings.llmCleanupProviderProfileId===_.id?"":this.plugin.settings.llmCleanupProviderProfileId,await n(ee),l()}))}new E.Setting(o).addButton(_=>_.setButtonText("Add profile").onClick(async()=>{let P=`profile-${Date.now().toString(36)}`,M=e().concat([{id:P,name:"Custom",baseUrl:"",apiKey:""}]);await n(M),l()}))};l(),new E.Setting(h).setName("Saved chats folder").setDesc("Where exported chat notes are stored (vault-relative).").addText(g=>g.setPlaceholder("zotero/chats").setValue(this.plugin.settings.chatOutputDir).onChange(async _=>{this.plugin.settings.chatOutputDir=_.trim()||"zotero/chats",await this.plugin.saveSettings()})),new E.Setting(h).setName("Copy PDFs into vault").setDesc("Disable to use Zotero storage paths directly. If a local file path is unavailable, the plugin temporarily copies the PDF into the vault for processing.").addToggle(g=>g.setValue(this.plugin.settings.copyPdfToVault).onChange(async _=>{this.plugin.settings.copyPdfToVault=_,!_&&this.plugin.settings.createOcrLayeredPdf&&(this.plugin.settings.createOcrLayeredPdf=!1),await this.plugin.saveSettings(),this.display()})),new E.Setting(h).setName("Create OCR-layered PDF copy").setDesc("When OCR is used, replace the vault PDF with a Tesseract text layer (requires Copy PDFs into vault).").addToggle(g=>{let _=this.plugin.settings.copyPdfToVault;g.setValue(_?this.plugin.settings.createOcrLayeredPdf:!1).setDisabled(!_).onChange(async P=>{if(!this.plugin.settings.copyPdfToVault){this.plugin.settings.createOcrLayeredPdf=!1,await this.plugin.saveSettings();return}this.plugin.settings.createOcrLayeredPdf=P,await this.plugin.saveSettings()})}),new E.Setting(h).setName("Prefer Obsidian note for citations").setDesc("Link citations to the Obsidian note when available; otherwise use Zotero deep links.").addToggle(g=>g.setValue(this.plugin.settings.preferObsidianNoteForCitations).onChange(async _=>{this.plugin.settings.preferObsidianNoteForCitations=_,await this.plugin.saveSettings()})),h.createEl("h2",{text:"Docling"}),new E.Setting(h).setName("OCR mode").setDesc("auto: skip OCR when text is readable; force if bad: OCR only when text looks poor; force: always OCR.").addDropdown(g=>g.addOption("auto","auto").addOption("force_low_quality","force if quality is bad").addOption("force","force").setValue(this.plugin.settings.ocrMode).onChange(async _=>{this.plugin.settings.ocrMode=_,await this.plugin.saveSettings()})),new E.Setting(h).setName("Text quality threshold").setDesc("Lower values are stricter; below this threshold the text is treated as low quality.").addSlider(g=>{g.setLimits(0,1,.05).setValue(this.plugin.settings.ocrQualityThreshold).setDynamicTooltip().onChange(async _=>{this.plugin.settings.ocrQualityThreshold=_,await this.plugin.saveSettings()})}),new E.Setting(h).setName("Chunking").setDesc("page or section").addDropdown(g=>g.addOption("page","page").addOption("section","section").setValue(this.plugin.settings.chunkingMode).onChange(async _=>{this.plugin.settings.chunkingMode=_,await this.plugin.saveSettings()})),h.createEl("h2",{text:"OCR cleanup"}),new E.Setting(h).setName("LLM cleanup for low-quality chunks").setDesc("Automatic AI cleanup for poor OCR at import. Can be slow/costly.").addToggle(g=>g.setValue(this.plugin.settings.enableLlmCleanup).onChange(async _=>{this.plugin.settings.enableLlmCleanup=_,await this.plugin.saveSettings()}));let d=null,c=null,u=null,f=async()=>{},m=async(g,_=!0)=>{let P=g.trim();this.plugin.settings.llmCleanupBaseUrl=P,_&&(this.plugin.settings.llmCleanupProviderProfileId="",d&&d.setValue("custom")),c&&c.setValue(P),await this.plugin.saveSettings()},y=async g=>{let _=e().find(P=>P.id===g);this.plugin.settings.llmCleanupProviderProfileId=g,_&&(this.plugin.settings.llmCleanupBaseUrl=_.baseUrl,this.plugin.settings.llmCleanupApiKey=_.apiKey,c==null||c.setValue(_.baseUrl),u==null||u.setValue(_.apiKey)),await this.plugin.saveSettings(),await f()};new E.Setting(h).setName("LLM cleanup provider profile").setDesc("Select a profile to populate base URL and API key.").addDropdown(g=>{d=g,g.addOption("custom","Custom (manual)");for(let P of e())g.addOption(P.id,P.name||P.id);let _=this.plugin.settings.llmCleanupProviderProfileId;g.setValue(_&&e().some(P=>P.id===_)?_:"custom"),g.onChange(async P=>{if(P==="custom"){this.plugin.settings.llmCleanupProviderProfileId="",await this.plugin.saveSettings();return}await y(P)})}),new E.Setting(h).setName("LLM cleanup base URL").setDesc("OpenAI-compatible endpoint, e.g. http://127.0.0.1:1234/v1").addText(g=>{c=g,g.setPlaceholder("http://127.0.0.1:1234/v1").setValue(this.plugin.settings.llmCleanupBaseUrl).onChange(async _=>{await m(_)})}),new E.Setting(h).setName("LLM cleanup API key").setDesc("Optional API key for the cleanup endpoint.").addText(g=>{u=g,t(g),g.setPlaceholder("sk-...").setValue(this.plugin.settings.llmCleanupApiKey).onChange(async _=>{this.plugin.settings.llmCleanupApiKey=_.trim(),this.plugin.settings.llmCleanupProviderProfileId="",d&&d.setValue("custom"),await this.plugin.saveSettings()})});let k=new E.Setting(h).setName("LLM cleanup model").setDesc("Select a cleanup-capable model from the provider."),b=null,x=g=>{if(!b)return;let _=(this.plugin.settings.llmCleanupModel||"").trim(),P=new Set(g.map(M=>M.value));_&&!P.has(_)&&(g=g.concat([{value:_,label:`Custom (${_})`}])),b.selectEl.options.length=0;for(let M of g)b.addOption(M.value,M.label);_&&b.setValue(_)};f=async()=>{if(b){b.setDisabled(!0);try{let g=await this.plugin.fetchCleanupModelOptions();x(g)}finally{b.setDisabled(!1)}}},k.addDropdown(g=>{b=g;let _=(this.plugin.settings.llmCleanupModel||"").trim();g.addOption(_||"loading","Loading..."),g.setValue(_||"loading"),g.onChange(async P=>{this.plugin.settings.llmCleanupModel=P.trim(),await this.plugin.saveSettings()})}),k.addButton(g=>{g.setButtonText("Refresh").onClick(async()=>{await f()})}),f(),new E.Setting(h).setName("LLM cleanup temperature").setDesc("Lower is more conservative.").addText(g=>g.setPlaceholder("0.0").setValue(String(this.plugin.settings.llmCleanupTemperature)).onChange(async _=>{let P=Number.parseFloat(_);this.plugin.settings.llmCleanupTemperature=Number.isFinite(P)?P:0,await this.plugin.saveSettings()})),new E.Setting(h).setName("LLM cleanup min quality").setDesc("Only run cleanup when chunk quality is below this threshold (0-1).").addSlider(g=>g.setLimits(0,1,.05).setValue(this.plugin.settings.llmCleanupMinQuality).setDynamicTooltip().onChange(async _=>{this.plugin.settings.llmCleanupMinQuality=_,await this.plugin.saveSettings()})),new E.Setting(h).setName("LLM cleanup max chars").setDesc("Skip cleanup for chunks longer than this limit.").addText(g=>g.setPlaceholder("2000").setValue(String(this.plugin.settings.llmCleanupMaxChars)).onChange(async _=>{let P=Number.parseInt(_,10);this.plugin.settings.llmCleanupMaxChars=Number.isFinite(P)?P:2e3,await this.plugin.saveSettings()})),h.createEl("h2",{text:"Text Embedding"});let v=null,S=null,A=null,T=async()=>{},D=async(g,_=!0)=>{let P=g.trim();this.plugin.settings.embedBaseUrl=P,_&&(this.plugin.settings.embedProviderProfileId="",v&&v.setValue("custom")),S&&S.setValue(P),await this.plugin.saveSettings()},F=async g=>{let _=e().find(P=>P.id===g);this.plugin.settings.embedProviderProfileId=g,_&&(this.plugin.settings.embedBaseUrl=_.baseUrl,this.plugin.settings.embedApiKey=_.apiKey,S==null||S.setValue(_.baseUrl),A==null||A.setValue(_.apiKey)),await this.plugin.saveSettings(),await T()};new E.Setting(h).setName("Embeddings provider profile").setDesc("Select a profile to populate base URL and API key.").addDropdown(g=>{v=g,g.addOption("custom","Custom (manual)");for(let P of e())g.addOption(P.id,P.name||P.id);let _=this.plugin.settings.embedProviderProfileId;g.setValue(_&&e().some(P=>P.id===_)?_:"custom"),g.onChange(async P=>{if(P==="custom"){this.plugin.settings.embedProviderProfileId="",await this.plugin.saveSettings();return}await F(P)})}),new E.Setting(h).setName("Embeddings base URL").addText(g=>{S=g,g.setPlaceholder("http://localhost:1234/v1").setValue(this.plugin.settings.embedBaseUrl).onChange(async _=>{await D(_)})}),new E.Setting(h).setName("Embeddings API key").addText(g=>{A=g,t(g),g.setPlaceholder("lm-studio").setValue(this.plugin.settings.embedApiKey).onChange(async _=>{this.plugin.settings.embedApiKey=_.trim(),this.plugin.settings.embedProviderProfileId="",v&&v.setValue("custom"),await this.plugin.saveSettings()})});let I=new E.Setting(h).setName("Embeddings model").setDesc("Select an embeddings model from the provider."),q=null,L=g=>{if(!q)return;let _=(this.plugin.settings.embedModel||"").trim(),P=new Set(g.map(M=>M.value));_&&!P.has(_)&&(g=g.concat([{value:_,label:`Custom (${_})`}])),q.selectEl.options.length=0;for(let M of g)q.addOption(M.value,M.label);_&&q.setValue(_)};T=async()=>{if(q){q.setDisabled(!0);try{let g=await this.plugin.fetchEmbeddingModelOptions();L(g)}finally{q.setDisabled(!1)}}},I.addDropdown(g=>{q=g;let _=(this.plugin.settings.embedModel||"").trim();g.addOption(_||"loading","Loading..."),g.setValue(_||"loading"),g.onChange(async P=>{this.plugin.settings.embedModel=P.trim(),await this.plugin.saveSettings()})}),I.addButton(g=>{g.setButtonText("Refresh").onClick(async()=>{await T()})}),T(),new E.Setting(h).setName("Include metadata in embeddings").setDesc("Prepend title/authors/tags/section info before embedding chunks.").addToggle(g=>g.setValue(this.plugin.settings.embedIncludeMetadata).onChange(async _=>{this.plugin.settings.embedIncludeMetadata=_,await this.plugin.saveSettings()})),new E.Setting(h).setName("Embedding subchunk size (chars)").setDesc("Split long chunks into smaller subchunks for embedding only (0 disables).").addText(g=>g.setPlaceholder("1800").setValue(String(this.plugin.settings.embedSubchunkChars)).onChange(async _=>{let P=Number.parseInt(_,10);this.plugin.settings.embedSubchunkChars=Number.isFinite(P)?Math.max(0,P):3500,await this.plugin.saveSettings()})),new E.Setting(h).setName("Embedding subchunk overlap (chars)").setDesc("Overlap between embedding subchunks to keep context intact.").addText(g=>g.setPlaceholder("200").setValue(String(this.plugin.settings.embedSubchunkOverlap)).onChange(async _=>{let P=Number.parseInt(_,10);this.plugin.settings.embedSubchunkOverlap=Number.isFinite(P)?Math.max(0,P):200,await this.plugin.saveSettings()})),new E.Setting(h).setName("Generate LLM tags for chunks").setDesc("Use the OCR cleanup model to tag chunks before indexing.").addToggle(g=>g.setValue(this.plugin.settings.enableChunkTagging).onChange(async _=>{this.plugin.settings.enableChunkTagging=_,await this.plugin.saveSettings()})),h.createEl("h2",{text:"Chat LLM"});let O=null,N=null,G=null,W=async()=>{},R=async(g,_=!0)=>{let P=g.trim();this.plugin.settings.chatBaseUrl=P,_&&(this.plugin.settings.chatProviderProfileId="",O&&O.setValue("custom")),N&&N.setValue(P),await this.plugin.saveSettings()},z=async g=>{let _=e().find(P=>P.id===g);this.plugin.settings.chatProviderProfileId=g,_&&(this.plugin.settings.chatBaseUrl=_.baseUrl,this.plugin.settings.chatApiKey=_.apiKey,N==null||N.setValue(_.baseUrl),G==null||G.setValue(_.apiKey)),await this.plugin.saveSettings(),await W()};new E.Setting(h).setName("Chat provider profile").setDesc("Select a profile to populate base URL and API key.").addDropdown(g=>{O=g,g.addOption("custom","Custom (manual)");for(let P of e())g.addOption(P.id,P.name||P.id);let _=this.plugin.settings.chatProviderProfileId;g.setValue(_&&e().some(P=>P.id===_)?_:"custom"),g.onChange(async P=>{if(P==="custom"){this.plugin.settings.chatProviderProfileId="",await this.plugin.saveSettings();return}await z(P)})}),new E.Setting(h).setName("Chat base URL").setDesc("OpenAI-compatible base URL for chat requests.").addText(g=>{N=g,g.setPlaceholder("http://localhost:1234/v1").setValue(this.plugin.settings.chatBaseUrl).onChange(async _=>{await R(_)})}),new E.Setting(h).setName("Chat API key").addText(g=>{G=g,t(g),g.setPlaceholder("lm-studio").setValue(this.plugin.settings.chatApiKey).onChange(async _=>{this.plugin.settings.chatApiKey=_.trim(),this.plugin.settings.chatProviderProfileId="",O&&O.setValue("custom"),await this.plugin.saveSettings()})});let Z=new E.Setting(h).setName("Chat model").setDesc("Select a chat-capable model from the provider."),V=null,oe=g=>{if(!V)return;let _=(this.plugin.settings.chatModel||"").trim(),P=new Set(g.map(M=>M.value));_&&!P.has(_)&&(g=g.concat([{value:_,label:`Custom (${_})`}])),V.selectEl.options.length=0;for(let M of g)V.addOption(M.value,M.label);_&&V.setValue(_)};W=async()=>{if(V){V.setDisabled(!0);try{let g=await this.plugin.fetchChatModelOptions();oe(g)}finally{V.setDisabled(!1)}}},Z.addDropdown(g=>{V=g;let _=(this.plugin.settings.chatModel||"").trim();g.addOption(_||"loading","Loading..."),g.setValue(_||"loading"),g.onChange(async P=>{this.plugin.settings.chatModel=P.trim(),await this.plugin.saveSettings()})}),Z.addButton(g=>{g.setButtonText("Refresh").onClick(async()=>{await W()})}),W(),new E.Setting(h).setName("Temperature").addText(g=>g.setPlaceholder("0.2").setValue(String(this.plugin.settings.chatTemperature)).onChange(async _=>{let P=Number.parseFloat(_);this.plugin.settings.chatTemperature=Number.isFinite(P)?P:.2,await this.plugin.saveSettings()})),new E.Setting(h).setName("Chat history messages").setDesc("Number of recent messages to include for conversational continuity (0 disables).").addText(g=>g.setPlaceholder("6").setValue(String(this.plugin.settings.chatHistoryMessages)).onChange(async _=>{let P=Number.parseInt(_,10);this.plugin.settings.chatHistoryMessages=Number.isFinite(P)?Math.max(0,P):6,await this.plugin.saveSettings()})),new E.Setting(h).setName("Chat panel location").setDesc("Where to open the chat view by default.").addDropdown(g=>g.addOption("right","Right sidebar").addOption("main","Main window").setValue(this.plugin.settings.chatPaneLocation).onChange(async _=>{this.plugin.settings.chatPaneLocation=_,await this.plugin.saveSettings()})),h.createEl("h2",{text:"Logging"}),new E.Setting(h).setName("Enable logging to file").setDesc("Write plugin logs to a file.").addToggle(g=>g.setValue(this.plugin.settings.enableFileLogging).onChange(async _=>{this.plugin.settings.enableFileLogging=_,await this.plugin.saveSettings()})),new E.Setting(h).setName("Log file path (vault-relative)").setDesc("Where to write the log file. Keep inside the vault.").addText(g=>g.setPlaceholder(`${J}/logs/docling_extract.log`).setValue(this.plugin.settings.logFilePath).onChange(async _=>{this.plugin.settings.logFilePath=_.trim()||`${J}/logs/docling_extract.log`,await this.plugin.saveSettings()})),new E.Setting(h).setName("View or clear log").setDesc("Open the log file or clear it.").addButton(g=>g.setButtonText("Open log").onClick(async()=>{var _,P;await((P=(_=this.plugin).openLogFile)==null?void 0:P.call(_))})).addButton(g=>g.setButtonText("Clear log").onClick(async()=>{var _,P;await((P=(_=this.plugin).clearLogFile)==null?void 0:P.call(_))})),h.createEl("h2",{text:"Maintenance"}),new E.Setting(h).setName("Reindex Redis from cached chunks").setDesc("Rebuild the Redis index from cached chunk JSON files.").addButton(g=>g.setButtonText("Reindex").onClick(async()=>{await this.plugin.reindexRedisFromCache()})),new E.Setting(h).setName("Recreate missing notes from cache").setDesc("Rebuild missing notes using cached Zotero items and chunks.").addButton(g=>g.setButtonText("Recreate").onClick(async()=>{await this.plugin.recreateMissingNotesFromCache()})).addButton(g=>g.setButtonText("Cancel").onClick(()=>{this.plugin.cancelRecreateMissingNotesFromCache()}))}};var we={"zrr-picker":`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">

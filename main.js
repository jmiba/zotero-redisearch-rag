...
item_json: {{item_json_yaml}}`,tagSanitizeMode:"kebab",noteBodyTemplate:"{{annotation_block}}{{docling_markdown}}",annotationPageLabel:"Seite",annotationColorMap:{yellow:{heading:"Questions",callout:"question"},red:{heading:"Problems/Critique",callout:"bug"},green:{heading:"Main Ideas",callout:"idea"},blue:{heading:"Facts",callout:"fact"},purple:{heading:"Arguments/Solutions",callout:"argument"},magenta:{heading:"Opinions",callout:"opinion"},orange:{heading:"Follow Up",callout:"pursue"},gray:{heading:"Citable Passages",callout:"cite"}},includeAnnotationImages:!0,zoteroCompanionEnabled:!1,zoteroCompanionBaseUrl:"http://127.0.0.1:23120",zoteroCompanionToken:"",llmProviderProfiles:[{id:"lm-studio",name:"LM Studio",baseUrl:"http://localhost:1234/v1",apiKey:"lm-studio"},{id:"ollama",name:"Ollama",baseUrl:"http://localhost:11434/v1",apiKey:""},{id:"openrouter",name:"OpenRouter",baseUrl:"https://openrouter.ai/api/v1",apiKey:""},{id:"openai",name:"OpenAI",baseUrl:"https://api.openai.com/v1",apiKey:""}],chatOutputDir:"Zotero/Chats",copyPdfToVault:!0,createOcrLayeredPdf:!1,preferObsidianNoteForCitations:!0,ocrMode:"auto",ocrQualityThreshold:.5,chunkingMode:"page",ocrEngine:"auto",forcePerPageOcr:!1,paddleApiKey:"",paddleVlApiUrl:"",paddleStructureApiUrl:"",enableLlmCleanup:!1,llmCleanupProviderProfileId:"lm-studio",llmCleanupBaseUrl:"http://127.0.0.1:1234/v1",llmCleanupApiKey:"",llmCleanupModel:"openai/gpt-oss-20b",llmCleanupTemperature:0,llmCleanupMinQuality:.35,llmCleanupMaxChars:2e3,embedProviderProfileId:"lm-studio",embedBaseUrl:"http://localhost:1234/v1",embedApiKey:"lm-studio",embedModel:"google/embedding-gemma-300m",embedIncludeMetadata:!0,embedSubchunkChars:3500,embedSubchunkOverlap:200,embedContextWindow:1,embedContextChars:220,enableChunkTagging:!1,chatProviderProfileId:"lm-studio",chatBaseUrl:"http://127.0.0.1:1234/v1",chatApiKey:"",chatModel:"openai/gpt-oss-20b",chatTemperature:.2,chatHistoryMessages:6,chatPaneLocation:"right",enableQueryExpansion:!1,queryExpansionCount:3,enableCrossEncoderRerank:!1,rerankModel:"BAAI/bge-reranker-v2-m3",rerankCandidateMultiplier:4,rrfK:60,rrfLogTop:0,maxChunksPerDoc:0,enableFileLogging:!1,logFilePath:`${Y}/logs/docling_extract.log`,redisIndex:"idx:zotero",redisPrefix:"zotero:chunk:"},Ie=class extends D.PluginSettingTab{constructor(e,n){super(e,n);this.activeTab="prerequisites";this.companionTokenInput=null;this.plugin=n}display(){let{containerEl:e}=this;e.empty();let n=()=>Array.isArray(this.plugin.settings.llmProviderProfiles)?this.plugin.settings.llmProviderProfiles:[],t=async f=>{this.plugin.settings.llmProviderProfiles=f,await this.plugin.saveSettings()},i=f=>{f.inputEl.type="password",f.inputEl.autocomplete="off",f.inputEl.spellcheck=!1},d=[{id:"prerequisites",label:"Prerequisites",render:f=>{f.createEl("h2",{text:"Prerequisites"});let k=null,x=null,w=null,R=null,P=null,S=null,L=null,z=(N,q)=>{if(!N)return;N.settingEl.classList.toggle("is-disabled",q),N.settingEl.style.opacity=q?"0.55":"";let B=N.settingEl.querySelector(".setting-item-description");B&&(B.style.color=q?"var(--text-faint)":"")},O=()=>{let N=this.plugin.settings.pythonRuntime==="local";R==null||R.setDisabled(!N),P==null||P.setDisabled(!N),L==null||L.setDisabled(!N),S&&(S.disabled=!N),z(k,!N),z(x,!N),z(w,!N)};new D.Setting(f).setName("Python runtime").setDesc("Recommended: Worker container (Docker/Podman Compose). Local interpreter mode is optional.").addDropdown(N=>{N.addOption("worker","Python worker container (recommended)"),N.addOption("local","Local interpreter/venv"),N.setValue(this.plugin.settings.pythonRuntime).onChange(async q=>{q!=="worker"&&q!=="local"||(this.plugin.settings.pythonRuntime=q,await this.plugin.saveSettings(),O())})}),k=new D.Setting(f).setName("Python path").setDesc("Local mode only: optional path to the Python interpreter used to create or run the plugin env. Leave blank to auto-detect (python3.13/3.12/3.11/3.10/python3/python, or py on Windows). Supports ~, $HOME, and %USERPROFILE%. Relative paths with separators resolve from your home dir.").addText(N=>(R=N,N.setPlaceholder("auto-detect").setValue(this.plugin.settings.pythonPath).onChange(async q=>{this.plugin.settings.pythonPath=q.trim(),await this.plugin.saveSettings()}))),x=new D.Setting(f).setName("Python environment").setDesc("Local mode: create or update the plugin's Python env. Worker mode: managed by Docker startup.").addButton(N=>{L=N,S=N.buttonEl,N.setButtonText("Create/Update").setCta(),N.onClick(async()=>{N.setDisabled(!0);try{await this.plugin.setupPythonEnv()}finally{N.setDisabled(this.plugin.settings.pythonRuntime!=="local")}})}),w=new D.Setting(f).setName("Python env location").setDesc("Local mode only: shared user cache can be reused across vaults; plugin folder keeps a per-vault env.").addDropdown(N=>{P=N,N.addOption("shared","Shared user cache"),N.addOption("plugin","Plugin folder (.venv)"),N.setValue(this.plugin.settings.pythonEnvLocation).onChange(async q=>{q!=="shared"&&q!=="plugin"||(this.plugin.settings.pythonEnvLocation=q,await this.plugin.saveSettings())})}),new D.Setting(f).setName("Docker/Podman path").setDesc("CLI path for Docker or Podman (used to start Redis Stack and the Python worker). Leave as 'docker'/'podman' to auto-detect via PATH and common locations without saving an absolute path (keeps cross-OS sync). Supports ~, $HOME, and %USERPROFILE%. Relative paths with separators resolve from your home dir.").addText(N=>N.setPlaceholder("docker").setValue(this.plugin.settings.dockerPath).onChange(async q=>{this.plugin.settings.dockerPath=q.trim()||"docker",await this.plugin.saveSettings()})),O(),new D.Setting(f).setName("Redis URL").addText(N=>N.setPlaceholder("redis://127.0.0.1:6379").setValue(this.plugin.settings.redisUrl).onChange(async q=>{this.plugin.settings.redisUrl=q.trim(),await this.plugin.saveSettings()}));let j=null,F=null,M=null,$=null,I=()=>{let N=this.plugin.settings.autoAssignRedisPort;M==null||M.setDisabled(N),$==null||$.setDisabled(N),j==null||j.settingEl.classList.toggle("is-disabled",N),F==null||F.settingEl.classList.toggle("is-disabled",N)};new D.Setting(f).setName("Auto-assign Redis port").setDesc("When starting Redis stack, pick a free local port and update the Redis URL.").addToggle(N=>N.setValue(this.plugin.settings.autoAssignRedisPort).onChange(async q=>{this.plugin.settings.autoAssignRedisPort=q,await this.plugin.saveSettings(),I()})),j=new D.Setting(f).setName("Redis data directory override").setDesc("Optional path to store Redis persistence when auto-assign is off. Env var ZRR_DATA_DIR overrides this setting. Supports ~, $HOME, and %USERPROFILE%. Relative paths with separators resolve from your home dir; use ./ to keep it vault-relative.").addText(N=>(M=N,N.setPlaceholder("~/Redis/zrr-data").setValue(this.plugin.settings.redisDataDirOverride).onChange(async q=>{this.plugin.settings.redisDataDirOverride=q.trim(),await this.plugin.saveSettings()}))),j.settingEl.addClass("zrr-redis-override-setting"),F=new D.Setting(f).setName("Redis project name override").setDesc("Optional Docker/Podman Compose project name when auto-assign is off. Env var ZRR_PROJECT_NAME overrides this setting.").addText(N=>($=N,N.setPlaceholder("zrr-shared").setValue(this.plugin.settings.redisProjectName).onChange(async q=>{this.plugin.settings.redisProjectName=q.trim(),await this.plugin.saveSettings()}))),F.settingEl.addClass("zrr-redis-override-setting"),I(),new D.Setting(f).setName("Auto-start Redis stack (Docker/Podman Compose)").setDesc("Requires Docker Desktop running and your vault path shared with Docker. Starts Redis and, in worker mode, the Python worker container.").addToggle(N=>N.setValue(this.plugin.settings.autoStartRedis).onChange(async q=>{this.plugin.settings.autoStartRedis=q,await this.plugin.saveSettings()})),new D.Setting(f).setName("Start Redis stack now").setDesc("Starts or restarts Redis stack (and Python worker in worker mode).").addButton(N=>N.setButtonText("Start").onClick(async()=>{await this.plugin.startRedisStack()}))}},{id:"zotero-import",label:"Zotero import",render:f=>{f.createEl("h2",{text:"Zotero Local API"}),new D.Setting(f).setName("Zotero base URL").setDesc("Local Zotero API base URL, e.g. http://127.0.0.1:23119/api").addText(P=>P.setPlaceholder("http://127.0.0.1:23119/api").setValue(this.plugin.settings.zoteroBaseUrl).onChange(async S=>{this.plugin.settings.zoteroBaseUrl=S.trim(),await this.plugin.saveSettings()}));let k=new D.Setting(f).setName("Zotero library").setDesc("Select your local library or a Zotero group library."),x=null,w=P=>{if(!x)return;let S=(this.plugin.settings.zoteroUserId||"0").trim()||"0";new Set(P.map(z=>z.value)).has(S)||(P=P.concat([{value:S,label:`Custom (${S})`}])),x.selectEl.options.length=0;for(let z of P)x.addOption(z.value,z.label);x.setValue(S)},R=async()=>{if(x){x.setDisabled(!0);try{let P=await this.plugin.fetchZoteroLibraryOptions();w(P)}finally{x.setDisabled(!1)}}};k.addDropdown(P=>{x=P;let S=(this.plugin.settings.zoteroUserId||"0").trim()||"0";P.addOption(S,"Loading..."),P.setValue(S),P.onChange(async L=>{this.plugin.settings.zoteroUserId=L.trim(),await this.plugin.saveSettings()})}),k.addButton(P=>{P.setButtonText("Refresh").onClick(async()=>{await R()})}),R(),f.createEl("h2",{text:"Zotero Web API"}),new D.Setting(f).setName("Web API base URL").setDesc("Zotero Web API base URL for write fallback, e.g. https://api.zotero.org").addText(P=>P.setPlaceholder("https://api.zotero.org").setValue(this.plugin.settings.webApiBaseUrl).onChange(async S=>{this.plugin.settings.webApiBaseUrl=S.trim(),await this.plugin.saveSettings()})),new D.Setting(f).setName("Web API library type").setDesc("Library type for Web API writes.").addDropdown(P=>P.addOption("user","user").addOption("group","group").setValue(this.plugin.settings.webApiLibraryType).onChange(async S=>{this.plugin.settings.webApiLibraryType=S,await this.plugin.saveSettings()})),new D.Setting(f).setName("Web API library ID").setDesc("Numeric Zotero user/group ID for Web API writes.").addText(P=>P.setPlaceholder("15218").setValue(this.plugin.settings.webApiLibraryId).onChange(async S=>{this.plugin.settings.webApiLibraryId=S.trim(),await this.plugin.saveSettings()})),new D.Setting(f).setName("Web API key").setDesc("Zotero API key for write fallback (from zotero.org).").addText(P=>{i(P),P.setPlaceholder("your-api-key").setValue(this.plugin.settings.webApiKey).onChange(async S=>{this.plugin.settings.webApiKey=S.trim(),await this.plugin.saveSettings()})}),f.createEl("h2",{text:"Output"}),new D.Setting(f).setName("PDF folder").addText(P=>P.setPlaceholder("zotero/pdfs").setValue(this.plugin.settings.outputPdfDir).onChange(async S=>{this.plugin.settings.outputPdfDir=S.trim(),await this.plugin.saveSettings()})),new D.Setting(f).setName("Notes folder").addText(P=>P.setPlaceholder("zotero/notes").setValue(this.plugin.settings.outputNoteDir).onChange(async S=>{this.plugin.settings.outputNoteDir=S.trim(),await this.plugin.saveSettings()})),new D.Setting(f).setName("Frontmatter template").setDesc("Template for note YAML frontmatter. Use {{var}} placeholders; leave blank to omit.").addTextArea(P=>{P.setValue(this.plugin.settings.frontmatterTemplate).onChange(async S=>{this.plugin.settings.frontmatterTemplate=S,await this.plugin.saveSettings()}),P.inputEl.rows=10,P.inputEl.style.width="100%"}),new D.Setting(f).setName("Tag sanitization").setDesc("Normalize Zotero tags for Obsidian (no spaces, punctuation trimmed).").addDropdown(P=>P.addOption("none","No change").addOption("camel","camelCase").addOption("pascal","PascalCase").addOption("snake","snake_case").addOption("kebab","kebab-case").setValue(this.plugin.settings.tagSanitizeMode==="replace"?"kebab":this.plugin.settings.tagSanitizeMode).onChange(async S=>{this.plugin.settings.tagSanitizeMode=S,await this.plugin.saveSettings()})),new D.Setting(f).setName("Note body template").setDesc("Template for the note body after frontmatter. Use {{pdf_block}}, {{annotation_block}}, and {{docling_markdown}} placeholders.").addTextArea(P=>{P.setValue(this.plugin.settings.noteBodyTemplate).onChange(async S=>{this.plugin.settings.noteBodyTemplate=S,await this.plugin.saveSettings()}),P.inputEl.rows=8,P.inputEl.style.width="100%"}),new D.Setting(f).setName("Saved chats folder").setDesc("Where exported chat notes are stored (vault-relative).").addText(P=>P.setPlaceholder("zotero/chats").setValue(this.plugin.settings.chatOutputDir).onChange(async S=>{this.plugin.settings.chatOutputDir=S.trim()||"zotero/chats",await this.plugin.saveSettings()})),new D.Setting(f).setName("Copy PDFs into vault").setDesc("Disable to use Zotero storage paths directly. If a local file path is unavailable, the plugin temporarily copies the PDF into the vault for processing.").addToggle(P=>P.setValue(this.plugin.settings.copyPdfToVault).onChange(async S=>{this.plugin.settings.copyPdfToVault=S,!S&&this.plugin.settings.createOcrLayeredPdf&&(this.plugin.settings.createOcrLayeredPdf=!1),await this.plugin.saveSettings(),this.display()})),new D.Setting(f).setName("Create OCR-layered PDF copy").setDesc("When OCR is used, replace the vault PDF with a Tesseract text layer (requires Copy PDFs into vault).").addToggle(P=>{let S=this.plugin.settings.copyPdfToVault;P.setValue(S?this.plugin.settings.createOcrLayeredPdf:!1).setDisabled(!S).onChange(async L=>{if(!this.plugin.settings.copyPdfToVault){this.plugin.settings.createOcrLayeredPdf=!1,await this.plugin.saveSettings();return}this.plugin.settings.createOcrLayeredPdf=L,await this.plugin.saveSettings()})}),new D.Setting(f).setName("Prefer Obsidian note for citations").setDesc("Link citations to the Obsidian note when available; otherwise use Zotero deep links.").addToggle(P=>P.setValue(this.plugin.settings.preferObsidianNoteForCitations).onChange(async S=>{this.plugin.settings.preferObsidianNoteForCitations=S,await this.plugin.saveSettings()}))}},{id:"annotations",label:"Annotations",render:f=>{f.createEl("h2",{text:"Annotations"}),new D.Setting(f).setName("Annotation page label").setDesc("Label shown before the page link in annotation callouts.").addText(R=>R.setPlaceholder("Page").setValue(this.plugin.settings.annotationPageLabel).onChange(async P=>{this.plugin.settings.annotationPageLabel=P.trim()||"Page",await this.plugin.saveSettings()})),new D.Setting(f).setName("Include annotation images").setDesc("Embed image/rect annotations as images in callouts when available.").addToggle(R=>R.setValue(this.plugin.settings.includeAnnotationImages).onChange(async P=>{this.plugin.settings.includeAnnotationImages=P,await this.plugin.saveSettings()})),f.createEl("h3",{text:"Zotero companion"}),f.createEl("p",{text:"Install the Zotero companion add-on to enable cached image/rect annotations. See tab Maintenance for instructions."}),new D.Setting(f).setName("Use Zotero companion for annotation images").setDesc("Fetch cached annotation images from a local Zotero companion plugin.").addToggle(R=>R.setValue(this.plugin.settings.zoteroCompanionEnabled).onChange(async P=>{this.plugin.settings.zoteroCompanionEnabled=P,await this.plugin.saveSettings()})),new D.Setting(f).setName("Zotero companion base URL").setDesc("Local URL for the Zotero companion plugin (loopback only).").addText(R=>R.setPlaceholder("http://127.0.0.1:23120").setValue(this.plugin.settings.zoteroCompanionBaseUrl).onChange(async P=>{this.plugin.settings.zoteroCompanionBaseUrl=P.trim(),await this.plugin.saveSettings()})),new D.Setting(f).setName("Zotero companion token").setDesc("Optional shared token for the companion endpoint.").addText(R=>{i(R),this.companionTokenInput=R,R.setPlaceholder("optional-token").setValue(this.plugin.settings.zoteroCompanionToken).onChange(async P=>{this.plugin.settings.zoteroCompanionToken=P.trim(),await this.plugin.saveSettings()})}),new D.Setting(f).setName("Generate companion token").setDesc("Create and paste a secure token. The generated token is copied to your clipboard. In Zotero, set your Zotero Companion Plugin token to this value.").addButton(R=>R.setButtonText("Generate token").onClick(async()=>{let P=(0,An.randomBytes)(32).toString("base64url");this.plugin.settings.zoteroCompanionToken=P,await this.plugin.saveSettings(),this.companionTokenInput&&this.companionTokenInput.setValue(P);try{await navigator.clipboard.writeText(P),new D.Notice("Generated and copied companion token.")}catch(S){new D.Notice("Generated token, but failed to copy to clipboard."),console.warn("Failed to copy generated companion token",S)}})),f.createEl("h3",{text:"Annotation color map"}),f.createEl("p",{text:"Map Zotero highlight colors to section headings and callout types."});let k=f.createDiv({cls:"zrr-annotation-map"}),x=async R=>{this.plugin.settings.annotationColorMap=R,await this.plugin.saveSettings()},w=()=>{var O,j;k.empty();let R=this.plugin.settings.annotationColorMap||{},P=Object.entries(R);if(P.length||k.createEl("p",{text:"No color mappings configured."}),P.length){let F=k.createDiv({cls:"zrr-annotation-map-row-header"});F.createEl("span",{text:"Color"}),F.createEl("span",{text:"Heading"}),F.createEl("span",{text:"Callout"}),F.createEl("span")}for(let[F,M]of P){let I=k.createDiv({cls:"zrr-annotation-map-row"}).createDiv({cls:"zrr-annotation-map-row-body"}),N=I.createEl("input",{type:"text",value:F,cls:"zrr-annotation-map-input"}),q=I.createEl("input",{type:"text",value:(O=M.heading)!=null?O:"",cls:"zrr-annotation-map-input"}),B=I.createEl("input",{type:"text",value:(j=M.callout)!=null?j:"",cls:"zrr-annotation-map-input"}),J=I.createEl("button",{text:"Delete",cls:"zrr-annotation-map-delete"});J.type="button",N.addEventListener("change",async()=>{let Z=N.value.trim().toLowerCase();if(!Z){N.value=F;return}let Q={...this.plugin.settings.annotationColorMap||{}};if(Z!==F&&Q[Z]){new D.Notice(`Annotation color '${Z}' already exists.`),N.value=F;return}let X=Q[F];delete Q[F],Q[Z]=X!=null?X:{heading:"",callout:""},await x(Q),w()}),q.addEventListener("change",async()=>{var X;let Z={...this.plugin.settings.annotationColorMap||{}},Q=(X=Z[F])!=null?X:{heading:"",callout:""};Q.heading=q.value.trim(),Z[F]=Q,await x(Z)}),B.addEventListener("change",async()=>{var X;let Z={...this.plugin.settings.annotationColorMap||{}},Q=(X=Z[F])!=null?X:{heading:"",callout:""};Q.callout=B.value.trim(),Z[F]=Q,await x(Z)}),J.addEventListener("click",async()=>{let Z={...this.plugin.settings.annotationColorMap||{}};delete Z[F],await x(Z),w()})}let S=k.createDiv({cls:"zrr-annotation-map-actions"}),L=S.createEl("button",{text:"Add mapping"});L.type="button",L.addEventListener("click",async()=>{let F={...this.plugin.settings.annotationColorMap||{}},M="new-color",$=1;for(;F[M];)M=`new-color-${$}`,$+=1;F[M]={heading:"",callout:""},await x(F),w()});let z=S.createEl("button",{text:"Reset to defaults"});z.type="button",z.addEventListener("click",async()=>{await x({...Fe.annotationColorMap}),w()})};w()}},{id:"ocr",label:"OCR",render:f=>{f.createEl("h2",{text:"Docling"});let k=null,x=S=>{if(!k)return;let L=this.plugin.settings.ocrEngine;new Set(S.map(O=>O.value)).has(L)||(S=S.concat([{value:L,label:`Current (unavailable): ${L}`}])),k.selectEl.options.length=0;for(let O of S)k.addOption(O.value,O.label);k.setValue(L)},w=async()=>{if(!k)return;k.setDisabled(!0);let S={tesseract:!1,paddleStructureLocal:!1,paddleVlLocal:!1};if(this.plugin.detectOcrEngines)try{S=await this.plugin.detectOcrEngines()}catch(O){S={tesseract:!1,paddleStructureLocal:!1,paddleVlLocal:!1}}let L=[{value:"auto",label:"Auto (default)"}];S.tesseract&&L.push({value:"tesseract",label:"Tesseract (local)"}),S.paddleStructureLocal&&L.push({value:"paddle_structure_local",label:"Paddle PP-StructureV3 (local)"}),S.paddleVlLocal&&L.push({value:"paddle_vl_local",label:"PaddleOCR-VL (local)"}),(this.plugin.settings.paddleApiKey||"").trim()&&(L.push({value:"paddle_structure_api",label:"PP-StructureV3 API"}),L.push({value:"paddle_vl_api",label:"PaddleOCR-VL API"})),x(L),k.setDisabled(!1)},R=new D.Setting(f).setName("Paddle OCR API key").setDesc("API token for PaddleOCR-VL / PP-StructureV3 endpoints. Get a free API key at "),P=document.createElement("a");P.href="https://aistudio.baidu.com/paddleocr",P.textContent="https://aistudio.baidu.com/paddleocr",P.target="_blank",P.rel="noopener noreferrer",R.descEl.appendChild(P),R.descEl.append("."),R.addText(S=>{i(S),S.setPlaceholder("your-api-token").setValue(this.plugin.settings.paddleApiKey).onChange(async L=>{this.plugin.settings.paddleApiKey=L.trim(),await this.plugin.saveSettings(),await w()})}),new D.Setting(f).setName("PaddleOCR-VL API URL").setDesc("Optional override for the PaddleOCR-VL API endpoint.").addText(S=>S.setPlaceholder("https://.../layout-parsing").setValue(this.plugin.settings.paddleVlApiUrl).onChange(async L=>{this.plugin.settings.paddleVlApiUrl=L.trim(),await this.plugin.saveSettings()})),new D.Setting(f).setName("PP-StructureV3 API URL").setDesc("API endpoint for PP-StructureV3 (see Baidu AI Studio docs).").addText(S=>S.setPlaceholder("https://.../pp-structure").setValue(this.plugin.settings.paddleStructureApiUrl).onChange(async L=>{this.plugin.settings.paddleStructureApiUrl=L.trim(),await this.plugin.saveSettings(),await w()})),new D.Setting(f).setName("OCR engine").setDesc("Select the OCR engine to use when OCR is required.").addDropdown(S=>{k=S,S.addOption("auto","Auto (default)"),S.setValue(this.plugin.settings.ocrEngine),S.onChange(async L=>{this.plugin.settings.ocrEngine=L,await this.plugin.saveSettings()})}),new D.Setting(f).setName("OCR decision (when to OCR)").setDesc("Controls when OCR runs; per-page behavior is configured separately below.").addDropdown(S=>S.addOption("auto","Auto: use text layer when reliable").addOption("force_low_quality","OCR only if text is poor").addOption("force","Prefer OCR for full document").setValue(this.plugin.settings.ocrMode).onChange(async L=>{this.plugin.settings.ocrMode=L,await this.plugin.saveSettings()})),new D.Setting(f).setName("OCR layout override (per-page)").setDesc("Force per-page OCR when OCR runs, bypassing layout heuristics; can be slower for multi-column PDFs.").addToggle(S=>S.setValue(this.plugin.settings.forcePerPageOcr).onChange(async L=>{this.plugin.settings.forcePerPageOcr=L,await this.plugin.saveSettings()})),new D.Setting(f).setName("Text quality threshold").setDesc("Lower values are stricter; below this threshold the text is treated as low quality.").addSlider(S=>{S.setLimits(0,1,.05).setValue(this.plugin.settings.ocrQualityThreshold).setDynamicTooltip().onChange(async L=>{this.plugin.settings.ocrQualityThreshold=L,await this.plugin.saveSettings()})}),new D.Setting(f).setName("Chunking").setDesc("page or section").addDropdown(S=>S.addOption("page","page").addOption("section","section").setValue(this.plugin.settings.chunkingMode).onChange(async L=>{this.plugin.settings.chunkingMode=L,await this.plugin.saveSettings()})),w()}},{id:"llms",label:"LLMs",render:f=>{f.createEl("h2",{text:"LLM Provider Profiles"});let k=f.createDiv({cls:"zrr-llm-profiles"}),x=()=>{k.empty();let v=n();v.length||k.createEl("p",{text:"No profiles yet. Add one below."});for(let C of v){let T=k.createEl("details",{cls:"zrr-profile"});v.length===1&&(T.open=!0);let W=T.createEl("summary",{text:C.name||C.id||"Profile"});W.addClass("zrr-profile-title");let Re=T.createDiv({cls:"zrr-profile-body"});new D.Setting(Re).setName("Profile name").addText(le=>le.setPlaceholder("My provider").setValue(C.name||"").onChange(async de=>{C.name=de.trim(),W.textContent=C.name||C.id||"Profile",await t(n())})),new D.Setting(Re).setName("Base URL").addText(le=>le.setPlaceholder("http://localhost:1234/v1").setValue(C.baseUrl||"").onChange(async de=>{C.baseUrl=de.trim(),await t(n())})),new D.Setting(Re).setName("API key").setDesc("Stored in settings (not encrypted).").addText(le=>{i(le),le.setPlaceholder("sk-...").setValue(C.apiKey||"").onChange(async de=>{C.apiKey=de.trim(),await t(n())})}),new D.Setting(Re).setName("Remove profile").setDesc("Deletes this saved profile.").addButton(le=>le.setButtonText("Delete profile").onClick(async()=>{let de=n().filter(nt=>nt.id!==C.id);this.plugin.settings.embedProviderProfileId=this.plugin.settings.embedProviderProfileId===C.id?"":this.plugin.settings.embedProviderProfileId,this.plugin.settings.chatProviderProfileId=this.plugin.settings.chatProviderProfileId===C.id?"":this.plugin.settings.chatProviderProfileId,this.plugin.settings.llmCleanupProviderProfileId=this.plugin.settings.llmCleanupProviderProfileId===C.id?"":this.plugin.settings.llmCleanupProviderProfileId,await t(de),x()}))}new D.Setting(k).addButton(C=>C.setButtonText("Add profile").onClick(async()=>{let T=`profile-${Date.now().toString(36)}`,W=n().concat([{id:T,name:"Custom",baseUrl:"",apiKey:""}]);await t(W),x()}))};x(),f.createEl("h2",{text:"OCR cleanup"}),new D.Setting(f).setName("LLM cleanup for low-quality chunks").setDesc("Automatic AI cleanup for poor OCR at import. Can be slow/costly.").addToggle(v=>v.setValue(this.plugin.settings.enableLlmCleanup).onChange(async C=>{this.plugin.settings.enableLlmCleanup=C,await this.plugin.saveSettings()}));let w=null,R=null,P=null,S=async()=>{},L=async(v,C=!0)=>{let T=v.trim();this.plugin.settings.llmCleanupBaseUrl=T,C&&(this.plugin.settings.llmCleanupProviderProfileId="",w&&w.setValue("custom")),R&&R.setValue(T),await this.plugin.saveSettings()},z=async v=>{let C=n().find(T=>T.id===v);this.plugin.settings.llmCleanupProviderProfileId=v,C&&(this.plugin.settings.llmCleanupBaseUrl=C.baseUrl,this.plugin.settings.llmCleanupApiKey=C.apiKey,R==null||R.setValue(C.baseUrl),P==null||P.setValue(C.apiKey)),await this.plugin.saveSettings(),await S()};new D.Setting(f).setName("LLM cleanup provider profile").setDesc("Select a profile to populate base URL and API key.").addDropdown(v=>{w=v,v.addOption("custom","Custom (manual)");for(let T of n())v.addOption(T.id,T.name||T.id);let C=this.plugin.settings.llmCleanupProviderProfileId;v.setValue(C&&n().some(T=>T.id===C)?C:"custom"),v.onChange(async T=>{if(T==="custom"){this.plugin.settings.llmCleanupProviderProfileId="",await this.plugin.saveSettings();return}await z(T)})}),new D.Setting(f).setName("LLM cleanup base URL").setDesc("OpenAI-compatible endpoint, e.g. http://127.0.0.1:1234/v1").addText(v=>{R=v,v.setPlaceholder("http://127.0.0.1:1234/v1").setValue(this.plugin.settings.llmCleanupBaseUrl).onChange(async C=>{await L(C)})}),new D.Setting(f).setName("LLM cleanup API key").setDesc("Optional API key for the cleanup endpoint.").addText(v=>{P=v,i(v),v.setPlaceholder("sk-...").setValue(this.plugin.settings.llmCleanupApiKey).onChange(async C=>{this.plugin.settings.llmCleanupApiKey=C.trim(),this.plugin.settings.llmCleanupProviderProfileId="",w&&w.setValue("custom"),await this.plugin.saveSettings()})});let O=new D.Setting(f).setName("LLM cleanup model").setDesc("Select a cleanup-capable model from the provider."),j=null,F=v=>{if(!j)return;let C=(this.plugin.settings.llmCleanupModel||"").trim(),T=new Set(v.map(W=>W.value));C&&!T.has(C)&&(v=v.concat([{value:C,label:`Custom (${C})`}])),j.selectEl.options.length=0;for(let W of v)j.addOption(W.value,W.label);C&&j.setValue(C)};S=async()=>{if(j){j.setDisabled(!0);try{let v=await this.plugin.fetchCleanupModelOptions();F(v)}finally{j.setDisabled(!1)}}},O.addDropdown(v=>{j=v;let C=(this.plugin.settings.llmCleanupModel||"").trim();v.addOption(C||"loading","Loading..."),v.setValue(C||"loading"),v.onChange(async T=>{this.plugin.settings.llmCleanupModel=T.trim(),await this.plugin.saveSettings()})}),O.addButton(v=>{v.setButtonText("Refresh").onClick(async()=>{await S()})}),S(),new D.Setting(f).setName("LLM cleanup temperature").setDesc("Lower is more conservative.").addText(v=>v.setPlaceholder("0.0").setValue(String(this.plugin.settings.llmCleanupTemperature)).onChange(async C=>{let T=Number.parseFloat(C);this.plugin.settings.llmCleanupTemperature=Number.isFinite(T)?T:0,await this.plugin.saveSettings()})),new D.Setting(f).setName("LLM cleanup min quality").setDesc("Only run cleanup when chunk quality is below this threshold (0-1).").addSlider(v=>v.setLimits(0,1,.05).setValue(this.plugin.settings.llmCleanupMinQuality).setDynamicTooltip().onChange(async C=>{this.plugin.settings.llmCleanupMinQuality=C,await this.plugin.saveSettings()})),new D.Setting(f).setName("LLM cleanup max chars").setDesc("Skip cleanup for chunks longer than this limit.").addText(v=>v.setPlaceholder("2000").setValue(String(this.plugin.settings.llmCleanupMaxChars)).onChange(async C=>{let T=Number.parseInt(C,10);this.plugin.settings.llmCleanupMaxChars=Number.isFinite(T)?T:2e3,await this.plugin.saveSettings()})),f.createEl("h2",{text:"Text Embedding"});let M=null,$=null,I=null,N=async()=>{},q=async(v,C=!0)=>{let T=v.trim();this.plugin.settings.embedBaseUrl=T,C&&(this.plugin.settings.embedProviderProfileId="",M&&M.setValue("custom")),$&&$.setValue(T),await this.plugin.saveSettings()},B=async v=>{let C=n().find(T=>T.id===v);this.plugin.settings.embedProviderProfileId=v,C&&(this.plugin.settings.embedBaseUrl=C.baseUrl,this.plugin.settings.embedApiKey=C.apiKey,$==null||$.setValue(C.baseUrl),I==null||I.setValue(C.apiKey)),await this.plugin.saveSettings(),await N()};new D.Setting(f).setName("Embeddings provider profile").setDesc("Select a profile to populate base URL and API key.").addDropdown(v=>{M=v,v.addOption("custom","Custom (manual)");for(let T of n())v.addOption(T.id,T.name||T.id);let C=this.plugin.settings.embedProviderProfileId;v.setValue(C&&n().some(T=>T.id===C)?C:"custom"),v.onChange(async T=>{if(T==="custom"){this.plugin.settings.embedProviderProfileId="",await this.plugin.saveSettings();return}await B(T)})}),new D.Setting(f).setName("Embeddings base URL").addText(v=>{$=v,v.setPlaceholder("http://localhost:1234/v1").setValue(this.plugin.settings.embedBaseUrl).onChange(async C=>{await q(C)})}),new D.Setting(f).setName("Embeddings API key").addText(v=>{I=v,i(v),v.setPlaceholder("lm-studio").setValue(this.plugin.settings.embedApiKey).onChange(async C=>{this.plugin.settings.embedApiKey=C.trim(),this.plugin.settings.embedProviderProfileId="",M&&M.setValue("custom"),await this.plugin.saveSettings()})});let J=new D.Setting(f).setName("Embeddings model").setDesc("Select an embeddings model from the provider."),Z=null,Q=v=>{if(!Z)return;let C=(this.plugin.settings.embedModel||"").trim(),T=new Set(v.map(W=>W.value));C&&!T.has(C)&&(v=v.concat([{value:C,label:`Custom (${C})`}])),Z.selectEl.options.length=0;for(let W of v)Z.addOption(W.value,W.label);C&&Z.setValue(C)};N=async()=>{if(Z){Z.setDisabled(!0);try{let v=await this.plugin.fetchEmbeddingModelOptions();Q(v)}finally{Z.setDisabled(!1)}}},J.addDropdown(v=>{Z=v;let C=(this.plugin.settings.embedModel||"").trim();v.addOption(C||"loading","Loading..."),v.setValue(C||"loading"),v.onChange(async T=>{this.plugin.settings.embedModel=T.trim(),await this.plugin.saveSettings()})}),J.addButton(v=>{v.setButtonText("Refresh").onClick(async()=>{await N()})}),N(),new D.Setting(f).setName("Include metadata in embeddings").setDesc("Prepend title/authors/tags/section info before embedding chunks.").addToggle(v=>v.setValue(this.plugin.settings.embedIncludeMetadata).onChange(async C=>{this.plugin.settings.embedIncludeMetadata=C,await this.plugin.saveSettings()})),new D.Setting(f).setName("Embedding context window (chunks)").setDesc("Include neighboring chunk text around each chunk when embedding (0 disables).").addText(v=>v.setPlaceholder("1").setValue(String(this.plugin.settings.embedContextWindow)).onChange(async C=>{let T=Number.parseInt(C,10);this.plugin.settings.embedContextWindow=Number.isFinite(T)?Math.max(0,T):1,await this.plugin.saveSettings()})),new D.Setting(f).setName("Embedding context snippet size (chars)").setDesc("Max chars per neighboring chunk included in embeddings.").addText(v=>v.setPlaceholder("220").setValue(String(this.plugin.settings.embedContextChars)).onChange(async C=>{let T=Number.parseInt(C,10);this.plugin.settings.embedContextChars=Number.isFinite(T)?Math.max(0,T):220,await this.plugin.saveSettings()})),new D.Setting(f).setName("Embedding subchunk size (chars)").setDesc("Split long chunks into smaller subchunks for embedding only (0 disables).").addText(v=>v.setPlaceholder("1800").setValue(String(this.plugin.settings.embedSubchunkChars)).onChange(async C=>{let T=Number.parseInt(C,10);this.plugin.settings.embedSubchunkChars=Number.isFinite(T)?Math.max(0,T):3500,await this.plugin.saveSettings()})),new D.Setting(f).setName("Embedding subchunk overlap (chars)").setDesc("Overlap between embedding subchunks to keep context intact.").addText(v=>v.setPlaceholder("200").setValue(String(this.plugin.settings.embedSubchunkOverlap)).onChange(async C=>{let T=Number.parseInt(C,10);this.plugin.settings.embedSubchunkOverlap=Number.isFinite(T)?Math.max(0,T):200,await this.plugin.saveSettings()})),new D.Setting(f).setName("Generate LLM tags for chunks").setDesc("Use the OCR cleanup model to tag chunks before indexing.").addToggle(v=>v.setValue(this.plugin.settings.enableChunkTagging).onChange(async C=>{this.plugin.settings.enableChunkTagging=C,await this.plugin.saveSettings()})),f.createEl("h2",{text:"Chat LLM"});let X=null,ie=null,se=null,ge=async()=>{},en=async(v,C=!0)=>{let T=v.trim();this.plugin.settings.chatBaseUrl=T,C&&(this.plugin.settings.chatProviderProfileId="",X&&X.setValue("custom")),ie&&ie.setValue(T),await this.plugin.saveSettings()},nn=async v=>{let C=n().find(T=>T.id===v);this.plugin.settings.chatProviderProfileId=v,C&&(this.plugin.settings.chatBaseUrl=C.baseUrl,this.plugin.settings.chatApiKey=C.apiKey,ie==null||ie.setValue(C.baseUrl),se==null||se.setValue(C.apiKey)),await this.plugin.saveSettings(),await ge()};new D.Setting(f).setName("Chat provider profile").setDesc("Select a profile to populate base URL and API key.").addDropdown(v=>{X=v,v.addOption("custom","Custom (manual)");for(let T of n())v.addOption(T.id,T.name||T.id);let C=this.plugin.settings.chatProviderProfileId;v.setValue(C&&n().some(T=>T.id===C)?C:"custom"),v.onChange(async T=>{if(T==="custom"){this.plugin.settings.chatProviderProfileId="",await this.plugin.saveSettings();return}await nn(T)})}),new D.Setting(f).setName("Chat base URL").setDesc("OpenAI-compatible base URL for chat requests.").addText(v=>{ie=v,v.setPlaceholder("http://localhost:1234/v1").setValue(this.plugin.settings.chatBaseUrl).onChange(async C=>{await en(C)})}),new D.Setting(f).setName("Chat API key").addText(v=>{se=v,i(v),v.setPlaceholder("lm-studio").setValue(this.plugin.settings.chatApiKey).onChange(async C=>{this.plugin.settings.chatApiKey=C.trim(),this.plugin.settings.chatProviderProfileId="",X&&X.setValue("custom"),await this.plugin.saveSettings()})});let Ee=new D.Setting(f).setName("Chat model").setDesc("Select a chat-capable model from the provider."),re=null,tn=v=>{if(!re)return;let C=(this.plugin.settings.chatModel||"").trim(),T=new Set(v.map(W=>W.value));C&&!T.has(C)&&(v=v.concat([{value:C,label:`Custom (${C})`}])),re.selectEl.options.length=0;for(let W of v)re.addOption(W.value,W.label);C&&re.setValue(C)};ge=async()=>{if(re){re.setDisabled(!0);try{let v=await this.plugin.fetchChatModelOptions();tn(v)}finally{re.setDisabled(!1)}}},Ee.addDropdown(v=>{re=v;let C=(this.plugin.settings.chatModel||"").trim();v.addOption(C||"loading","Loading..."),v.setValue(C||"loading"),v.onChange(async T=>{this.plugin.settings.chatModel=T.trim(),await this.plugin.saveSettings()})}),Ee.addButton(v=>{v.setButtonText("Refresh").onClick(async()=>{await ge()})}),ge(),new D.Setting(f).setName("Temperature").addText(v=>v.setPlaceholder("0.2").setValue(String(this.plugin.settings.chatTemperature)).onChange(async C=>{let T=Number.parseFloat(C);this.plugin.settings.chatTemperature=Number.isFinite(T)?T:.2,await this.plugin.saveSettings()})),new D.Setting(f).setName("Chat history messages").setDesc("Number of recent messages to include for conversational continuity (0 disables).").addText(v=>v.setPlaceholder("6").setValue(String(this.plugin.settings.chatHistoryMessages)).onChange(async C=>{let T=Number.parseInt(C,10);this.plugin.settings.chatHistoryMessages=Number.isFinite(T)?Math.max(0,T):6,await this.plugin.saveSettings()})),new D.Setting(f).setName("Chat panel location").setDesc("Where to open the chat view by default.").addDropdown(v=>v.addOption("right","Right sidebar").addOption("main","Main window").setValue(this.plugin.settings.chatPaneLocation).onChange(async C=>{this.plugin.settings.chatPaneLocation=C,await this.plugin.saveSettings()})),f.createEl("h2",{text:"Retrieval"}),new D.Setting(f).setName("Enable query expansion").setDesc("Use the chat model to expand queries before retrieval.").addToggle(v=>v.setValue(this.plugin.settings.enableQueryExpansion).onChange(async C=>{this.plugin.settings.enableQueryExpansion=C,await this.plugin.saveSettings()})),new D.Setting(f).setName("Query expansion count").setDesc("Number of expansion variants to request.").addText(v=>v.setPlaceholder("3").setValue(String(this.plugin.settings.queryExpansionCount)).onChange(async C=>{let T=Number.parseInt(C,10);this.plugin.settings.queryExpansionCount=Number.isFinite(T)?Math.max(1,T):3,await this.plugin.saveSettings()})),new D.Setting(f).setName("Enable cross-encoder reranking").setDesc("Rerank candidates locally with sentence-transformers (downloads model on first use).").addToggle(v=>v.setValue(this.plugin.settings.enableCrossEncoderRerank).onChange(async C=>{this.plugin.settings.enableCrossEncoderRerank=C,await this.plugin.saveSettings()})),new D.Setting(f).setName("Cross-encoder model").setDesc("Local reranker model name or path.").addText(v=>v.setPlaceholder("BAAI/bge-reranker-v2-m3").setValue(this.plugin.settings.rerankModel).onChange(async C=>{this.plugin.settings.rerankModel=C.trim()||"BAAI/bge-reranker-v2-m3",await this.plugin.saveSettings()})),new D.Setting(f).setName("Rerank candidate multiplier").setDesc("Retrieve k \xD7 N candidates before reranking.").addText(v=>v.setPlaceholder("4").setValue(String(this.plugin.settings.rerankCandidateMultiplier)).onChange(async C=>{let T=Number.parseInt(C,10);this.plugin.settings.rerankCandidateMultiplier=Number.isFinite(T)?Math.max(1,T):4,await this.plugin.saveSettings()})),new D.Setting(f).setName("RRF k").setDesc("Rank fusion constant for blending lexical and vector results.").addText(v=>v.setPlaceholder("60").setValue(String(this.plugin.settings.rrfK)).onChange(async C=>{let T=Number.parseInt(C,10);this.plugin.settings.rrfK=Number.isFinite(T)?Math.max(1,T):60,await this.plugin.saveSettings()})),new D.Setting(f).setName("RRF log top N").setDesc("Log the top N RRF-ranked chunks to stderr (0 disables).").addText(v=>v.setPlaceholder("0").setValue(String(this.plugin.settings.rrfLogTop)).onChange(async C=>{let T=Number.parseInt(C,10);this.plugin.settings.rrfLogTop=Number.isFinite(T)?Math.max(0,T):0,await this.plugin.saveSettings()})),new D.Setting(f).setName("Max chunks per document").setDesc("Limit how many chunks from a single document can appear in retrieval (0 disables).").addText(v=>v.setPlaceholder("0").setValue(String(this.plugin.settings.maxChunksPerDoc)).onChange(async C=>{let T=Number.parseInt(C,10);this.plugin.settings.maxChunksPerDoc=Number.isFinite(T)?Math.max(0,T):0,await this.plugin.saveSettings()}))}},{id:"maintenance",label:"Maintenance",render:f=>{f.createEl("h2",{text:"Logging"}),new D.Setting(f).setName("Enable logging to file").setDesc("Write plugin logs to a file.").addToggle(k=>k.setValue(this.plugin.settings.enableFileLogging).onChange(async x=>{this.plugin.settings.enableFileLogging=x,await this.plugin.saveSettings()})),new D.Setting(f).setName("Log file path (vault-relative)").setDesc("Where to write the log file. Keep inside the vault.").addText(k=>k.setPlaceholder(`${Y}/logs/docling_extract.log`).setValue(this.plugin.settings.logFilePath).onChange(async x=>{this.plugin.settings.logFilePath=x.trim()||`${Y}/logs/docling_extract.log`,await this.plugin.saveSettings()})),new D.Setting(f).setName("View or clear log").setDesc("Open the log file or clear it.").addButton(k=>k.setButtonText("Open log").onClick(async()=>{var x,w;await((w=(x=this.plugin).openLogFile)==null?void 0:w.call(x))})).addButton(k=>k.setButtonText("Clear log").onClick(async()=>{var x,w;await((w=(x=this.plugin).clearLogFile)==null?void 0:w.call(x))})),f.createEl("h2",{text:"Zotero Companion Plugin"}),f.createEl("p",{text:"Download the XPI, then in Zotero go to Tools \u2192 Add-ons \u2192 Install from File and restart."}),new D.Setting(f).setName("Download companion add-on").setDesc("Downloads the companion XPI to your system Downloads folder.").addButton(k=>k.setButtonText("Download XPI").onClick(async()=>{let x=this.getDefaultDownloadDir();if(!x){new D.Notice("Unable to resolve the system Downloads folder.");return}let w=Oe.default.join(x,"zrr-companion.xpi");if(await this.companionXpiExists(w)){new D.Notice(`Companion XPI already exists: ${w}`);return}new D.Notice("Downloading companion XPI..."),await this.downloadCompanionXpi(w)})),new D.Setting(f).setName("Open Zotero").setDesc("Launch Zotero and open the Add-ons window (Tools \u2192 Add-ons).").addButton(k=>k.setButtonText("Open Zotero").onClick(async()=>{await this.plugin.openZoteroAddons()})),new D.Setting(f).setName("Check companion status").setDesc("Ping the companion /health endpoint.").addButton(k=>k.setButtonText("Check status").onClick(async()=>{await this.plugin.checkZoteroCompanionHealth()})),f.createEl("h2",{text:"Redis Indexing"}),new D.Setting(f).setName("Reindex Redis from cached chunks").setDesc("Rebuild the Redis index from cached chunk JSON files.").addButton(k=>k.setButtonText("Reindex").onClick(async()=>{await this.plugin.reindexRedisFromCache()})),new D.Setting(f).setName("Recreate missing notes from cache").setDesc("Rebuild missing notes using cached Zotero items and chunks.").addButton(k=>k.setButtonText("Recreate").onClick(async()=>{await this.plugin.recreateMissingNotesFromCache()})).addButton(k=>k.setButtonText("Cancel").onClick(()=>{this.plugin.cancelRecreateMissingNotesFromCache()}))}}],_=new Map,u=new Map,m=f=>{this.activeTab=f;for(let[k,x]of _){let w=k===f;x.classList.toggle("is-active",w),x.setAttribute("aria-selected",w?"true":"false"),x.setAttribute("tabindex",w?"0":"-1")}for(let[k,x]of u){let w=k===f;x.classList.toggle("is-active",w),x.hidden=!w}},y=e.createDiv({cls:"zrr-settings-tabs"});y.setAttribute("role","tablist");let b=e.createDiv({cls:"zrr-settings-tabs-panels"});for(let f of d){let k=y.createEl("button",{text:f.label,cls:"zrr-settings-tab-button"}),x=`zrr-settings-tab-${f.id}`,w=`zrr-settings-panel-${f.id}`;k.type="button",k.id=x,k.setAttribute("role","tab"),k.setAttribute("aria-controls",w),k.setAttribute("aria-selected","false"),k.addEventListener("click",()=>m(f.id));let R=b.createDiv({cls:"zrr-settings-tab-panel"});R.id=w,R.setAttribute("role","tabpanel"),R.setAttribute("aria-labelledby",x),R.hidden=!0,f.render(R),_.set(f.id,k),u.set(f.id,R)}let A=d.some(f=>f.id===this.activeTab)?this.activeTab:d[0].id;m(A)}getDefaultDownloadDir(){let e=process.env.HOME||process.env.USERPROFILE;return e?Oe.default.join(e,"Downloads"):null}async companionXpiExists(e){try{return await Te.promises.access(e),!0}catch(n){return!1}}async downloadCompanionXpi(e){try{await Te.promises.mkdir(Oe.default.dirname(e),{recursive:!0});let n=await(0,D.requestUrl)({url:ut,method:"GET"});if(n.status!==200){new D.Notice(`Failed to download companion XPI (HTTP ${n.status}).`);return}let t=Buffer.from(n.arrayBuffer);await Te.promises.writeFile(e,t),new D.Notice(`Downloaded companion XPI: ${e}`)}catch(n){new D.Notice("Failed to download companion XPI. See console for details."),console.warn("Failed to download companion XPI",n)}}};var Rn=require("@codemirror/state"),Ln=require("@codemirror/view"),oe=require("obsidian");var _t=g=>{let p=g.scrollDOM.getBoundingClientRect(),e=g.posAtCoords({x:p.left+8,y:+p.height*.25});return e===null?null:g.state.doc.lineAt(e).number},ze=class{constructor(p,e){this.pdfSidebarLeaf=null;this.pdfSidebarDocId=null;this.pdfSidebarPdfPath=null;this.pdfSidebarPage=null;this.pendingPdfSync=null;this.chunkPageCache=new Map;this.previewScrollEl=null;this.previewScrollHandler=null;this.previewScrollFrame=null;this.pdfSyncInFlight=!1;this.queuedPdfSync=null;this.deps=p,this.helpers=e}createSyncExtension(){let p=this,e=this.helpers;return Ln.ViewPlugin.fromClass(class{constructor(n){this.docId=null;this.lastPage=null;this.lastChunkId=null;this.scrollFrame=null;this.view=n,this.docId=e.extractDocIdFromDoc(n.state.doc),this.onScroll=()=>this.scheduleSync(!1),n.scrollDOM.addEventListener("scroll",this.onScroll,{passive:!0}),this.scheduleSync(!0)}update(n){n.docChanged&&(this.docId=e.extractDocIdFromDoc(n.view.state.doc),this.lastPage=null,this.lastChunkId=null),(n.docChanged||n.viewportChanged)&&this.scheduleSync(!1)}destroy(){this.view.scrollDOM.removeEventListener("scroll",this.onScroll),this.scrollFrame!==null&&(window.cancelAnimationFrame(this.scrollFrame),this.scrollFrame=null)}scheduleSync(n){this.scrollFrame===null&&(this.scrollFrame=window.requestAnimationFrame(()=>{this.scrollFrame=null,this.syncPdfSidebar(this.view,n)}))}syncPdfSidebar(n,t){var _,u;let i=this.docId;if(!i)return;let r=_t(n);if(r===null)return;let a=e.findChunkStartLineInDoc(n.state.doc,r);if(!a)return;let s=e.parseChunkMarkerLine(a.text),o=(_=s==null?void 0:s.pageNumber)!=null?_:null,l=(u=s==null?void 0:s.chunkId)!=null?u:null;if(!o&&!l)return;let c=o!==null&&this.lastPage===o,d=o===null&&l!==null&&this.lastChunkId===l;!t&&(c||d)||(this.lastPage=o,this.lastChunkId=l,p.syncPdfSidebarForDoc(i,o!=null?o:void 0,l!=null?l:void 0))}})}async maybeSyncPendingPdf(){var e,n;if(!this.pendingPdfSync||(!this.pdfSidebarLeaf||((n=(e=this.pdfSidebarLeaf.view)==null?void 0:e.getViewType)==null?void 0:n.call(e))!=="pdf")&&!await this.getPdfSidebarLeaf()||!this.pdfSidebarLeaf||!this.isLeafTabActive(this.pdfSidebarLeaf))return;let p=this.pendingPdfSync;this.pendingPdfSync=null,await this.syncPdfSidebarForDoc(p.docId,p.pageNumber,p.chunkId)}updatePreviewScrollHandler(){var t;let p=this.deps.app.workspace.getActiveViewOfType(oe.MarkdownView);if(!p||p.getMode()!=="preview"){this.detachPreviewScrollHandler();return}let e=(t=p.previewMode)==null?void 0:t.containerEl;if(!e){this.detachPreviewScrollHandler();return}if(this.previewScrollEl===e)return;this.detachPreviewScrollHandler();let n=()=>this.schedulePreviewSync(p);e.addEventListener("scroll",n,{passive:!0}),this.previewScrollEl=e,this.previewScrollHandler=n,this.schedulePreviewSync(p)}async syncPdfSidebarForFile(p){try{let e=await this.deps.app.vault.read(p),n=await this.deps.resolveDocIdForNote(p,e);if(!n)return;let t=this.helpers.extractFirstChunkMarkerFromContent(e);await this.syncPdfSidebarForDoc(n,t==null?void 0:t.pageNumber,t==null?void 0:t.chunkId)}catch(e){console.warn("Failed to sync PDF sidebar for opened note",e)}}async syncPdfSidebarForDoc(p,e,n){if(p){if(this.pdfSyncInFlight){this.queuedPdfSync={docId:p,pageNumber:e,chunkId:n};return}this.pdfSyncInFlight=!0;try{let t=await this.deps.getDocIndexEntry(p);t||(t=await this.deps.hydrateDocIndexFromCache(p));let i=t!=null&&t.pdf_path?String(t.pdf_path):"";if(!i)return;let r=this.deps.toVaultRelativePath(i);if(!r){let d=(0,oe.normalizePath)(i);this.deps.app.vault.getAbstractFileByPath(d)instanceof oe.TFile&&(r=d)}if(!r)return;let a=this.deps.app.vault.getAbstractFileByPath(r);if(!(a instanceof oe.TFile)||a.extension.toLowerCase()!=="pdf")return;let s=await this.getPdfSidebarLeaf();if(!s){this.pendingPdfSync={docId:p,pageNumber:e,chunkId:n};return}if(this.updatePdfSidebarIcon(s),!this.isLeafTabActive(s)){this.pendingPdfSync={docId:p,pageNumber:e,chunkId:n};return}let o=Number.isFinite(e!=null?e:NaN)?Number(e):null;if(o===null&&n&&(o=await this.resolvePageNumberForChunk(p,n)),o===null||this.pdfSidebarDocId===p&&this.pdfSidebarPdfPath===a.path&&this.pdfSidebarPage===o)return;this.pdfSidebarDocId=p,this.pdfSidebarPdfPath=a.path,this.pdfSidebarPage=o;let l=r,c=o!==null?`${l}#page=${o}`:l;await this.openPdfLinkInLeaf(s,c),this.updatePdfSidebarIcon(s)}finally{this.pdfSyncInFlight=!1;let t=this.queuedPdfSync;this.queuedPdfSync=null,t&&this.syncPdfSidebarForDoc(t.docId,t.pageNumber,t.chunkId)}}}detachPreviewScrollHandler(){this.previewScrollEl&&this.previewScrollHandler&&this.previewScrollEl.removeEventListener("scroll",this.previewScrollHandler),this.previewScrollEl=null,this.previewScrollHandler=null,this.previewScrollFrame!==null&&(window.cancelAnimationFrame(this.previewScrollFrame),this.previewScrollFrame=null)}schedulePreviewSync(p){this.previewScrollFrame===null&&(this.previewScrollFrame=window.requestAnimationFrame(()=>{this.previewScrollFrame=null,this.syncPdfSidebarForPreview(p)}))}syncPdfSidebarForPreview(p){let e=this.previewScrollEl;if(!e)return;let n=p.getViewData();if(!n)return;let t=Rn.Text.of(n.split(/\r?\n/)),i=this.helpers.extractDocIdFromDoc(t);if(!i)return;let r=this.getPreviewTopLineNumber(e);if(r===null)return;let a=this.helpers.findChunkStartLineInDoc(t,r+1);if(!a)return;let s=this.helpers.parseChunkMarkerLine(a.text);!(s!=null&&s.pageNumber)&&!(s!=null&&s.chunkId)||this.syncPdfSidebarForDoc(i,s==null?void 0:s.pageNumber,s==null?void 0:s.chunkId)}getPreviewTopLineNumber(p){var t;let e=p.getBoundingClientRect().top,n=p.querySelectorAll("[data-line]");for(let i of Array.from(n))if(i.getBoundingClientRect().bottom>=e+2){let a=(t=i.getAttribute("data-line"))!=null?t:"",s=Number.parseInt(a,10);if(Number.isFinite(s))return s}return null}async getPdfSidebarLeaf(){var t,i;if(this.pdfSidebarLeaf&&((i=(t=this.pdfSidebarLeaf.view)==null?void 0:t.getViewType)==null?void 0:i.call(t))==="pdf")return this.pdfSidebarLeaf;let p=this.deps.app.workspace;if(typeof p.ensureSideLeaf=="function")try{let r=await p.ensureSideLeaf("pdf","right",{active:!1,split:!1,reveal:!0});return this.isRightSidebarLeaf(r)?(this.pdfSidebarLeaf=r,this.updatePdfSidebarIcon(r),r):null}catch(r){console.warn("Failed to ensure PDF side leaf",r)}let e=this.deps.app.workspace.getRightLeaf(!1);if(e&&this.isRightSidebarLeaf(e))return this.pdfSidebarLeaf=e,this.updatePdfSidebarIcon(e),e;let n=this.deps.app.workspace.getRightLeaf(!0);return n&&this.isRightSidebarLeaf(n)?(this.pdfSidebarLeaf=n,this.updatePdfSidebarIcon(n),n):null}isRightSidebarLeaf(p){if(!p)return!1;let n=p.containerEl;return!!(n!=null&&n.closest(".workspace-split.mod-right-split, .mod-right-split"))}updatePdfSidebarIcon(p){if(!p||!this.isRightSidebarLeaf(p))return;let n=p.containerEl,t=[];n&&t.push(...Array.from(n.querySelectorAll(".view-header-icon")));let i=p;if(i.tabHeaderEl&&t.push(...Array.from(i.tabHeaderEl.querySelectorAll(".workspace-tab-header-inner-icon, .view-header-icon"))),i.tabHeaderInnerIconEl&&t.push(i.tabHeaderInnerIconEl),t.length===0)return;let r=new Set;for(let s of t)r.has(s)||(r.add(s),s.innerHTML="",(0,oe.setIcon)(s,"zrr-pdf"),!s.querySelector("svg")&&this.deps.iconSvg&&(s.innerHTML=this.deps.iconSvg),s.dataset&&(s.dataset.zrrIcon="zrr-pdf"));let a=p.view;a.icon="zrr-pdf",typeof a.getIcon=="function"&&(a.getIcon=()=>"zrr-pdf")}async openPdfLinkInLeaf(p,e){var r,a,s;let n=(r=this.getPluginsRegistry())==null?void 0:r["pdf-plus"],t=(s=(a=n==null?void 0:n.lib)==null?void 0:a.workspace)==null?void 0:s.openPDFLinkTextInLeaf;if(typeof t=="function")try{await t.call(n.lib.workspace,p,e,"",{active:!1});return}catch(o){if(!this.isPdfAnnotationRaceError(o))throw o;await this.delay(150)}let i=p;if(typeof i.openLinkText=="function"){try{await i.openLinkText(e,"",{active:!1})}catch(o){if(!this.isPdfAnnotationRaceError(o))throw o;await this.delay(150),await i.openLinkText(e,"",{active:!1})}return}await this.deps.app.workspace.openLinkText(e,"",!1)}isPdfAnnotationRaceError(p){let e=p instanceof Error?p.message:String(p!=null?p:"");return e?/injectLinkAnnotations/i.test(e)||/render method must be called before/i.test(e):!1}delay(p){return new Promise(e=>setTimeout(e,p))}getPluginsRegistry(){var p;return(p=this.deps.app.plugins)==null?void 0:p.plugins}isLeafTabActive(p){var i;let e=p;if((i=e.parent)!=null&&i.activeLeaf)return e.parent.activeLeaf===p;let n=e.containerEl;if(n!=null&&n.classList.contains("is-active")||n!=null&&n.classList.contains("mod-active"))return!0;let t=e.tabHeaderEl;return!!(t!=null&&t.classList.contains("is-active")||t!=null&&t.classList.contains("mod-active"))}async resolvePageNumberForChunk(p,e){var l,c,d,_,u;if(!p||!e)return null;let n=this.deps.normalizeChunkIdForNote(e,p)||e,t=this.chunkPageCache.get(p);if(t&&t.has(n))return(l=t.get(n))!=null?l:null;let i=(0,oe.normalizePath)(`${H}/${p}.json`);if(!await this.deps.app.vault.adapter.exists(i))return null;let a=await this.deps.readChunkPayload(i),s=Array.isArray(a==null?void 0:a.chunks)?a==null?void 0:a.chunks:[],o=new Map;for(let m of s){let y=typeof(m==null?void 0:m.chunk_id)=="string"?m.chunk_id.trim():"";if(!y)continue;let b=Number.isFinite((c=m==null?void 0:m.page_start)!=null?c:NaN)?Number(m.page_start):null,A=Number.isFinite((d=m==null?void 0:m.page_end)!=null?d:NaN)?Number(m.page_end):null,f=b!=null?b:A;f!==null&&(o.set(y,f),o.set(`${p}:${y}`,f))}return this.chunkPageCache.set(p,o),(u=(_=o.get(n))!=null?_:o.get(e))!=null?u:null}};var Me={"zrr-picker":`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
...
`,"ocr_wordlist.txt":`# zotero-redisearch-rag tool version: 0.7.0
aai
aam
abb
abge
...
\xF6pnv
\xFCberarb
`,"requirements.txt":`# zotero-redisearch-rag tool version: 0.7.0
docling
langcodes[data]
markdown
markdown-it-py
...
spylls
`,"docker-compose.yml":`# zotero-redisearch-rag tool version: 0.7.0
services:
  redis-stack:
...
`,"python-worker.Dockerfile":`# zotero-redisearch-rag tool version: 0.7.0
...
`)}async renderFrontmatter(e,n,t,i,r,a){var d;let s=(d=this.settings.frontmatterTemplate)!=null?d:"";if(!s.trim())return"";let o=a!=null?a:await this.buildTemplateVars(e,n,t,i,r),l=this.renderTemplate(s,o,"",{appendDocling:!1}).trim(),c=this.stripEmptyFrontmatterFields(l);return this.normalizeFrontmatterKeySpacing(c)}stripEmptyFrontmatterFields(e){if(!e.trim())return"";let n=e.split(/\r?\n/),t=[],i=/^([A-Za-z0-9][A-Za-z0-9 _-]*)\s*:\s*(.*)$/,r=/^[ \t]+-\s*(.*)$/,a=new Set(["abstract"]),s=l=>l===""||l==='""'||l==="''",o=0;for(;o<n.length;){let l=n[o],c=l.match(i);if(!c){t.push(l),o+=1;continue}let d=c[1].trim(),_=c[2].trim();if(a.has(d)){t.push(l),o+=1;continue}if(!s(_)){t.push(l),o+=1;continue}let u=o+1,m=[];for(;u<n.length&&n[u].match(r);)m.push(n[u]),u+=1;if(m.length===0){for(o=u;o<n.length&&n[o].trim()==="";)o+=1;continue}let y=m.filter(b=>{let A=b.match(r);if(!A)return!1;let f=A[1].trim();return!s(f)});if(y.length>0&&t.push(l,...y),o=u,y.length===0)for(;o<n.length&&n[o].trim()==="";)o+=1}for(;t.length>0&&t[t.length-1].trim()==="";)t.pop();return t.join(`

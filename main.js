"use strict";var Ye=Object.create;var _e=Object.defineProperty;var Qe=Object.getOwnPropertyDescriptor;var en=Object.getOwnPropertyNames;var nn=Object.getPrototypeOf,tn=Object.prototype.hasOwnProperty;var rn=(x,_)=>{for(var e in _)_e(x,e,{get:_[e],enumerable:!0})},ze=(x,_,e,n)=>{if(_&&typeof _=="object"||typeof _=="function")for(let t of en(_))!tn.call(x,t)&&t!==e&&_e(x,t,{get:()=>_[t],enumerable:!(n=Qe(_,t))||n.enumerable});return x};var ie=(x,_,e)=>(e=x!=null?Ye(nn(x)):{},ze(_||!x||!x.__esModule?_e(e,"default",{value:x,enumerable:!0}):e,x)),sn=x=>ze(_e({},"__esModule",{value:!0}),x);var yn={};rn(yn,{default:()=>be});module.exports=sn(yn);var p=require("obsidian"),ne=require("@codemirror/state"),$=require("@codemirror/view"),Y=require("child_process"),B=require("fs"),Ue=ie(require("http")),Ze=ie(require("https")),de=ie(require("net")),Ve=ie(require("os")),He=ie(require("tls")),C=ie(require("path")),ae=require("url"),he=require("crypto");var S=require("obsidian"),J=".zotero-redisearch-rag",X=`${J}/items`,H=`${J}/chunks`,Fe={pythonPath:"",pythonEnvLocation:"shared",dockerPath:"docker",redisUrl:"redis://127.0.0.1:6379",autoAssignRedisPort:!1,autoStartRedis:!0,zoteroBaseUrl:"http://127.0.0.1:23119/api",zoteroUserId:"0",webApiBaseUrl:"https://api.zotero.org",webApiLibraryType:"user",webApiLibraryId:"",webApiKey:"",outputPdfDir:"Zotero/PDFs",outputNoteDir:"Zotero/Notes",frontmatterTemplate:`doc_id: {{doc_id}}
...
item_json: {{item_json_yaml}}`,tagSanitizeMode:"kebab",noteBodyTemplate:"{{pdf_block}}{{docling_markdown}}",llmProviderProfiles:[{id:"lm-studio",name:"LM Studio",baseUrl:"http://localhost:1234/v1",apiKey:"lm-studio"},{id:"ollama",name:"Ollama",baseUrl:"http://localhost:11434/v1",apiKey:""},{id:"openrouter",name:"OpenRouter",baseUrl:"https://openrouter.ai/api/v1",apiKey:""},{id:"openai",name:"OpenAI",baseUrl:"https://api.openai.com/v1",apiKey:""}],chatOutputDir:"Zotero/Chats",copyPdfToVault:!0,createOcrLayeredPdf:!1,preferObsidianNoteForCitations:!0,ocrMode:"auto",ocrQualityThreshold:.5,chunkingMode:"page",enableLlmCleanup:!1,llmCleanupProviderProfileId:"lm-studio",llmCleanupBaseUrl:"http://127.0.0.1:1234/v1",llmCleanupApiKey:"",llmCleanupModel:"openai/gpt-oss-20b",llmCleanupTemperature:0,llmCleanupMinQuality:.35,llmCleanupMaxChars:2e3,embedProviderProfileId:"lm-studio",embedBaseUrl:"http://localhost:1234/v1",embedApiKey:"lm-studio",embedModel:"google/embedding-gemma-300m",embedIncludeMetadata:!0,embedSubchunkChars:3500,embedSubchunkOverlap:200,enableChunkTagging:!1,chatProviderProfileId:"lm-studio",chatBaseUrl:"http://127.0.0.1:1234/v1",chatApiKey:"",chatModel:"openai/gpt-oss-20b",chatTemperature:.2,chatHistoryMessages:6,chatPaneLocation:"right",enableFileLogging:!1,logFilePath:`${J}/logs/docling_extract.log`,redisIndex:"idx:zotero",redisPrefix:"zotero:chunk:"},fe=class extends S.PluginSettingTab{constructor(_,e){super(_,e),this.plugin=e}display(){let{containerEl:_}=this;_.empty();let e=()=>Array.isArray(this.plugin.settings.llmProviderProfiles)?this.plugin.settings.llmProviderProfiles:[],n=async g=>{this.plugin.settings.llmProviderProfiles=g,await this.plugin.saveSettings()},t=g=>{g.inputEl.type="password",g.inputEl.autocomplete="off",g.inputEl.spellcheck=!1};_.createEl("h2",{text:"Prerequisites"}),new S.Setting(_).setName("Python path").setDesc("Optional path to the Python interpreter used to create or run the plugin env. Leave blank to auto-detect (python3.13/3.12/3.11/3.10/python3/python, or py on Windows).").addText(g=>g.setPlaceholder("auto-detect").setValue(this.plugin.settings.pythonPath).onChange(async f=>{this.plugin.settings.pythonPath=f.trim(),await this.plugin.saveSettings()})),new S.Setting(_).setName("Python environment").setDesc("Create or update the plugin's Python env (location configured below).").addButton(g=>{g.setButtonText("Create/Update").setCta(),g.onClick(async()=>{g.setDisabled(!0);try{await this.plugin.setupPythonEnv()}finally{g.setDisabled(!1)}})}),new S.Setting(_).setName("Python env location").setDesc("Shared user cache can be reused across vaults; plugin folder keeps a per-vault env.").addDropdown(g=>{g.addOption("shared","Shared user cache"),g.addOption("plugin","Plugin folder (.venv)"),g.setValue(this.plugin.settings.pythonEnvLocation).onChange(async f=>{f!=="shared"&&f!=="plugin"||(this.plugin.settings.pythonEnvLocation=f,await this.plugin.saveSettings())})}),new S.Setting(_).setName("Docker/Podman path").setDesc("CLI path for Docker or Podman (used to start Redis Stack).").addText(g=>g.setPlaceholder("docker").setValue(this.plugin.settings.dockerPath).onChange(async f=>{this.plugin.settings.dockerPath=f.trim()||"docker",await this.plugin.saveSettings()})),new S.Setting(_).setName("Redis URL").addText(g=>g.setPlaceholder("redis://127.0.0.1:6379").setValue(this.plugin.settings.redisUrl).onChange(async f=>{this.plugin.settings.redisUrl=f.trim(),await this.plugin.saveSettings()})),new S.Setting(_).setName("Auto-assign Redis port").setDesc("When starting Redis stack, pick a free local port and update the Redis URL.").addToggle(g=>g.setValue(this.plugin.settings.autoAssignRedisPort).onChange(async f=>{this.plugin.settings.autoAssignRedisPort=f,await this.plugin.saveSettings()})),new S.Setting(_).setName("Auto-start Redis stack (Docker/Podman Compose)").setDesc("Requires Docker Desktop running and your vault path shared with Docker. Uses a vault-specific data dir at .obsidian/zotero-redisearch-rag/redis-data.").addToggle(g=>g.setValue(this.plugin.settings.autoStartRedis).onChange(async f=>{this.plugin.settings.autoStartRedis=f,await this.plugin.saveSettings()})),new S.Setting(_).setName("Start Redis stack now").setDesc("Restarts Docker/Podman Compose with the vault data directory.").addButton(g=>g.setButtonText("Start").onClick(async()=>{await this.plugin.startRedisStack()})),_.createEl("h2",{text:"Zotero Local API"}),new S.Setting(_).setName("Zotero base URL").setDesc("Local Zotero API base URL, e.g. http://127.0.0.1:23119/api").addText(g=>g.setPlaceholder("http://127.0.0.1:23119/api").setValue(this.plugin.settings.zoteroBaseUrl).onChange(async f=>{this.plugin.settings.zoteroBaseUrl=f.trim(),await this.plugin.saveSettings()}));let i=new S.Setting(_).setName("Zotero library").setDesc("Select your local library or a Zotero group library."),r=null,s=g=>{if(!r)return;let f=(this.plugin.settings.zoteroUserId||"0").trim()||"0";new Set(g.map(M=>M.value)).has(f)||(g=g.concat([{value:f,label:`Custom (${f})`}])),r.selectEl.options.length=0;for(let M of g)r.addOption(M.value,M.label);r.setValue(f)},a=async()=>{if(r){r.setDisabled(!0);try{let g=await this.plugin.fetchZoteroLibraryOptions();s(g)}finally{r.setDisabled(!1)}}};i.addDropdown(g=>{r=g;let f=(this.plugin.settings.zoteroUserId||"0").trim()||"0";g.addOption(f,"Loading..."),g.setValue(f),g.onChange(async P=>{this.plugin.settings.zoteroUserId=P.trim(),await this.plugin.saveSettings()})}),i.addButton(g=>{g.setButtonText("Refresh").onClick(async()=>{await a()})}),a(),_.createEl("h2",{text:"Zotero Web API"}),new S.Setting(_).setName("Web API base URL").setDesc("Zotero Web API base URL for write fallback, e.g. https://api.zotero.org").addText(g=>g.setPlaceholder("https://api.zotero.org").setValue(this.plugin.settings.webApiBaseUrl).onChange(async f=>{this.plugin.settings.webApiBaseUrl=f.trim(),await this.plugin.saveSettings()})),new S.Setting(_).setName("Web API library type").setDesc("Library type for Web API writes.").addDropdown(g=>g.addOption("user","user").addOption("group","group").setValue(this.plugin.settings.webApiLibraryType).onChange(async f=>{this.plugin.settings.webApiLibraryType=f,await this.plugin.saveSettings()})),new S.Setting(_).setName("Web API library ID").setDesc("Numeric Zotero user/group ID for Web API writes.").addText(g=>g.setPlaceholder("15218").setValue(this.plugin.settings.webApiLibraryId).onChange(async f=>{this.plugin.settings.webApiLibraryId=f.trim(),await this.plugin.saveSettings()})),new S.Setting(_).setName("Web API key").setDesc("Zotero API key for write fallback (from zotero.org).").addText(g=>{t(g),g.setPlaceholder("your-api-key").setValue(this.plugin.settings.webApiKey).onChange(async f=>{this.plugin.settings.webApiKey=f.trim(),await this.plugin.saveSettings()})}),_.createEl("h2",{text:"Output"}),new S.Setting(_).setName("PDF folder").addText(g=>g.setPlaceholder("zotero/pdfs").setValue(this.plugin.settings.outputPdfDir).onChange(async f=>{this.plugin.settings.outputPdfDir=f.trim(),await this.plugin.saveSettings()})),new S.Setting(_).setName("Notes folder").addText(g=>g.setPlaceholder("zotero/notes").setValue(this.plugin.settings.outputNoteDir).onChange(async f=>{this.plugin.settings.outputNoteDir=f.trim(),await this.plugin.saveSettings()})),new S.Setting(_).setName("Frontmatter template").setDesc("Template for note YAML frontmatter. Use {{var}} placeholders; leave blank to omit.").addTextArea(g=>{g.setValue(this.plugin.settings.frontmatterTemplate).onChange(async f=>{this.plugin.settings.frontmatterTemplate=f,await this.plugin.saveSettings()}),g.inputEl.rows=10,g.inputEl.style.width="100%"}),new S.Setting(_).setName("Tag sanitization").setDesc("Normalize Zotero tags for Obsidian (no spaces, punctuation trimmed).").addDropdown(g=>g.addOption("none","No change").addOption("camel","camelCase").addOption("pascal","PascalCase").addOption("snake","snake_case").addOption("kebab","kebab-case").setValue(this.plugin.settings.tagSanitizeMode==="replace"?"kebab":this.plugin.settings.tagSanitizeMode).onChange(async f=>{this.plugin.settings.tagSanitizeMode=f,await this.plugin.saveSettings()})),new S.Setting(_).setName("Note body template").setDesc("Template for the note body after frontmatter. Use {{pdf_block}} and {{docling_markdown}} placeholders.").addTextArea(g=>{g.setValue(this.plugin.settings.noteBodyTemplate).onChange(async f=>{this.plugin.settings.noteBodyTemplate=f,await this.plugin.saveSettings()}),g.inputEl.rows=8,g.inputEl.style.width="100%"}),_.createEl("h2",{text:"LLM Provider Profiles"});let o=_.createDiv({cls:"zrr-llm-profiles"}),l=()=>{o.empty();let g=e();g.length||o.createEl("p",{text:"No profiles yet. Add one below."});for(let f of g){let P=o.createEl("details",{cls:"zrr-profile"});g.length===1&&(P.open=!0);let M=P.createEl("summary",{text:f.name||f.id||"Profile"});M.addClass("zrr-profile-title");let te=P.createDiv({cls:"zrr-profile-body"});new S.Setting(te).setName("Profile name").addText(Q=>Q.setPlaceholder("My provider").setValue(f.name||"").onChange(async ee=>{f.name=ee.trim(),M.textContent=f.name||f.id||"Profile",await n(e())})),new S.Setting(te).setName("Base URL").addText(Q=>Q.setPlaceholder("http://localhost:1234/v1").setValue(f.baseUrl||"").onChange(async ee=>{f.baseUrl=ee.trim(),await n(e())})),new S.Setting(te).setName("API key").setDesc("Stored in settings (not encrypted).").addText(Q=>{t(Q),Q.setPlaceholder("sk-...").setValue(f.apiKey||"").onChange(async ee=>{f.apiKey=ee.trim(),await n(e())})}),new S.Setting(te).setName("Remove profile").setDesc("Deletes this saved profile.").addButton(Q=>Q.setButtonText("Delete profile").onClick(async()=>{let ee=e().filter(le=>le.id!==f.id);this.plugin.settings.embedProviderProfileId=this.plugin.settings.embedProviderProfileId===f.id?"":this.plugin.settings.embedProviderProfileId,this.plugin.settings.chatProviderProfileId=this.plugin.settings.chatProviderProfileId===f.id?"":this.plugin.settings.chatProviderProfileId,this.plugin.settings.llmCleanupProviderProfileId=this.plugin.settings.llmCleanupProviderProfileId===f.id?"":this.plugin.settings.llmCleanupProviderProfileId,await n(ee),l()}))}new S.Setting(o).addButton(f=>f.setButtonText("Add profile").onClick(async()=>{let P=`profile-${Date.now().toString(36)}`,M=e().concat([{id:P,name:"Custom",baseUrl:"",apiKey:""}]);await n(M),l()}))};l(),new S.Setting(_).setName("Saved chats folder").setDesc("Where exported chat notes are stored (vault-relative).").addText(g=>g.setPlaceholder("zotero/chats").setValue(this.plugin.settings.chatOutputDir).onChange(async f=>{this.plugin.settings.chatOutputDir=f.trim()||"zotero/chats",await this.plugin.saveSettings()})),new S.Setting(_).setName("Copy PDFs into vault").setDesc("Disable to use Zotero storage paths directly. If a local file path is unavailable, the plugin temporarily copies the PDF into the vault for processing.").addToggle(g=>g.setValue(this.plugin.settings.copyPdfToVault).onChange(async f=>{this.plugin.settings.copyPdfToVault=f,!f&&this.plugin.settings.createOcrLayeredPdf&&(this.plugin.settings.createOcrLayeredPdf=!1),await this.plugin.saveSettings(),this.display()})),new S.Setting(_).setName("Create OCR-layered PDF copy").setDesc("When OCR is used, replace the vault PDF with a Tesseract text layer (requires Copy PDFs into vault).").addToggle(g=>{let f=this.plugin.settings.copyPdfToVault;g.setValue(f?this.plugin.settings.createOcrLayeredPdf:!1).setDisabled(!f).onChange(async P=>{if(!this.plugin.settings.copyPdfToVault){this.plugin.settings.createOcrLayeredPdf=!1,await this.plugin.saveSettings();return}this.plugin.settings.createOcrLayeredPdf=P,await this.plugin.saveSettings()})}),new S.Setting(_).setName("Prefer Obsidian note for citations").setDesc("Link citations to the Obsidian note when available; otherwise use Zotero deep links.").addToggle(g=>g.setValue(this.plugin.settings.preferObsidianNoteForCitations).onChange(async f=>{this.plugin.settings.preferObsidianNoteForCitations=f,await this.plugin.saveSettings()})),_.createEl("h2",{text:"Docling"}),new S.Setting(_).setName("OCR mode").setDesc("auto: skip OCR when text is readable; force if bad: OCR only when text looks poor; force: always OCR.").addDropdown(g=>g.addOption("auto","auto").addOption("force_low_quality","force if quality is bad").addOption("force","force").setValue(this.plugin.settings.ocrMode).onChange(async f=>{this.plugin.settings.ocrMode=f,await this.plugin.saveSettings()})),new S.Setting(_).setName("Text quality threshold").setDesc("Lower values are stricter; below this threshold the text is treated as low quality.").addSlider(g=>{g.setLimits(0,1,.05).setValue(this.plugin.settings.ocrQualityThreshold).setDynamicTooltip().onChange(async f=>{this.plugin.settings.ocrQualityThreshold=f,await this.plugin.saveSettings()})}),new S.Setting(_).setName("Chunking").setDesc("page or section").addDropdown(g=>g.addOption("page","page").addOption("section","section").setValue(this.plugin.settings.chunkingMode).onChange(async f=>{this.plugin.settings.chunkingMode=f,await this.plugin.saveSettings()})),_.createEl("h2",{text:"OCR cleanup"}),new S.Setting(_).setName("LLM cleanup for low-quality chunks").setDesc("Automatic AI cleanup for poor OCR at import. Can be slow/costly.").addToggle(g=>g.setValue(this.plugin.settings.enableLlmCleanup).onChange(async f=>{this.plugin.settings.enableLlmCleanup=f,await this.plugin.saveSettings()}));let c=null,d=null,u=null,h=async()=>{},m=async(g,f=!0)=>{let P=g.trim();this.plugin.settings.llmCleanupBaseUrl=P,f&&(this.plugin.settings.llmCleanupProviderProfileId="",c&&c.setValue("custom")),d&&d.setValue(P),await this.plugin.saveSettings()},y=async g=>{let f=e().find(P=>P.id===g);this.plugin.settings.llmCleanupProviderProfileId=g,f&&(this.plugin.settings.llmCleanupBaseUrl=f.baseUrl,this.plugin.settings.llmCleanupApiKey=f.apiKey,d==null||d.setValue(f.baseUrl),u==null||u.setValue(f.apiKey)),await this.plugin.saveSettings(),await h()};new S.Setting(_).setName("LLM cleanup provider profile").setDesc("Select a profile to populate base URL and API key.").addDropdown(g=>{c=g,g.addOption("custom","Custom (manual)");for(let P of e())g.addOption(P.id,P.name||P.id);let f=this.plugin.settings.llmCleanupProviderProfileId;g.setValue(f&&e().some(P=>P.id===f)?f:"custom"),g.onChange(async P=>{if(P==="custom"){this.plugin.settings.llmCleanupProviderProfileId="",await this.plugin.saveSettings();return}await y(P)})}),new S.Setting(_).setName("LLM cleanup base URL").setDesc("OpenAI-compatible endpoint, e.g. http://127.0.0.1:1234/v1").addText(g=>{d=g,g.setPlaceholder("http://127.0.0.1:1234/v1").setValue(this.plugin.settings.llmCleanupBaseUrl).onChange(async f=>{await m(f)})}),new S.Setting(_).setName("LLM cleanup API key").setDesc("Optional API key for the cleanup endpoint.").addText(g=>{u=g,t(g),g.setPlaceholder("sk-...").setValue(this.plugin.settings.llmCleanupApiKey).onChange(async f=>{this.plugin.settings.llmCleanupApiKey=f.trim(),this.plugin.settings.llmCleanupProviderProfileId="",c&&c.setValue("custom"),await this.plugin.saveSettings()})});let k=new S.Setting(_).setName("LLM cleanup model").setDesc("Select a cleanup-capable model from the provider."),b=null,w=g=>{if(!b)return;let f=(this.plugin.settings.llmCleanupModel||"").trim(),P=new Set(g.map(M=>M.value));f&&!P.has(f)&&(g=g.concat([{value:f,label:`Custom (${f})`}])),b.selectEl.options.length=0;for(let M of g)b.addOption(M.value,M.label);f&&b.setValue(f)};h=async()=>{if(b){b.setDisabled(!0);try{let g=await this.plugin.fetchCleanupModelOptions();w(g)}finally{b.setDisabled(!1)}}},k.addDropdown(g=>{b=g;let f=(this.plugin.settings.llmCleanupModel||"").trim();g.addOption(f||"loading","Loading..."),g.setValue(f||"loading"),g.onChange(async P=>{this.plugin.settings.llmCleanupModel=P.trim(),await this.plugin.saveSettings()})}),k.addButton(g=>{g.setButtonText("Refresh").onClick(async()=>{await h()})}),h(),new S.Setting(_).setName("LLM cleanup temperature").setDesc("Lower is more conservative.").addText(g=>g.setPlaceholder("0.0").setValue(String(this.plugin.settings.llmCleanupTemperature)).onChange(async f=>{let P=Number.parseFloat(f);this.plugin.settings.llmCleanupTemperature=Number.isFinite(P)?P:0,await this.plugin.saveSettings()})),new S.Setting(_).setName("LLM cleanup min quality").setDesc("Only run cleanup when chunk quality is below this threshold (0-1).").addSlider(g=>g.setLimits(0,1,.05).setValue(this.plugin.settings.llmCleanupMinQuality).setDynamicTooltip().onChange(async f=>{this.plugin.settings.llmCleanupMinQuality=f,await this.plugin.saveSettings()})),new S.Setting(_).setName("LLM cleanup max chars").setDesc("Skip cleanup for chunks longer than this limit.").addText(g=>g.setPlaceholder("2000").setValue(String(this.plugin.settings.llmCleanupMaxChars)).onChange(async f=>{let P=Number.parseInt(f,10);this.plugin.settings.llmCleanupMaxChars=Number.isFinite(P)?P:2e3,await this.plugin.saveSettings()})),_.createEl("h2",{text:"Text Embedding"});let v=null,E=null,R=null,N=async()=>{},D=async(g,f=!0)=>{let P=g.trim();this.plugin.settings.embedBaseUrl=P,f&&(this.plugin.settings.embedProviderProfileId="",v&&v.setValue("custom")),E&&E.setValue(P),await this.plugin.saveSettings()},F=async g=>{let f=e().find(P=>P.id===g);this.plugin.settings.embedProviderProfileId=g,f&&(this.plugin.settings.embedBaseUrl=f.baseUrl,this.plugin.settings.embedApiKey=f.apiKey,E==null||E.setValue(f.baseUrl),R==null||R.setValue(f.apiKey)),await this.plugin.saveSettings(),await N()};new S.Setting(_).setName("Embeddings provider profile").setDesc("Select a profile to populate base URL and API key.").addDropdown(g=>{v=g,g.addOption("custom","Custom (manual)");for(let P of e())g.addOption(P.id,P.name||P.id);let f=this.plugin.settings.embedProviderProfileId;g.setValue(f&&e().some(P=>P.id===f)?f:"custom"),g.onChange(async P=>{if(P==="custom"){this.plugin.settings.embedProviderProfileId="",await this.plugin.saveSettings();return}await F(P)})}),new S.Setting(_).setName("Embeddings base URL").addText(g=>{E=g,g.setPlaceholder("http://localhost:1234/v1").setValue(this.plugin.settings.embedBaseUrl).onChange(async f=>{await D(f)})}),new S.Setting(_).setName("Embeddings API key").addText(g=>{R=g,t(g),g.setPlaceholder("lm-studio").setValue(this.plugin.settings.embedApiKey).onChange(async f=>{this.plugin.settings.embedApiKey=f.trim(),this.plugin.settings.embedProviderProfileId="",v&&v.setValue("custom"),await this.plugin.saveSettings()})});let I=new S.Setting(_).setName("Embeddings model").setDesc("Select an embeddings model from the provider."),q=null,L=g=>{if(!q)return;let f=(this.plugin.settings.embedModel||"").trim(),P=new Set(g.map(M=>M.value));f&&!P.has(f)&&(g=g.concat([{value:f,label:`Custom (${f})`}])),q.selectEl.options.length=0;for(let M of g)q.addOption(M.value,M.label);f&&q.setValue(f)};N=async()=>{if(q){q.setDisabled(!0);try{let g=await this.plugin.fetchEmbeddingModelOptions();L(g)}finally{q.setDisabled(!1)}}},I.addDropdown(g=>{q=g;let f=(this.plugin.settings.embedModel||"").trim();g.addOption(f||"loading","Loading..."),g.setValue(f||"loading"),g.onChange(async P=>{this.plugin.settings.embedModel=P.trim(),await this.plugin.saveSettings()})}),I.addButton(g=>{g.setButtonText("Refresh").onClick(async()=>{await N()})}),N(),new S.Setting(_).setName("Include metadata in embeddings").setDesc("Prepend title/authors/tags/section info before embedding chunks.").addToggle(g=>g.setValue(this.plugin.settings.embedIncludeMetadata).onChange(async f=>{this.plugin.settings.embedIncludeMetadata=f,await this.plugin.saveSettings()})),new S.Setting(_).setName("Embedding subchunk size (chars)").setDesc("Split long chunks into smaller subchunks for embedding only (0 disables).").addText(g=>g.setPlaceholder("1800").setValue(String(this.plugin.settings.embedSubchunkChars)).onChange(async f=>{let P=Number.parseInt(f,10);this.plugin.settings.embedSubchunkChars=Number.isFinite(P)?Math.max(0,P):3500,await this.plugin.saveSettings()})),new S.Setting(_).setName("Embedding subchunk overlap (chars)").setDesc("Overlap between embedding subchunks to keep context intact.").addText(g=>g.setPlaceholder("200").setValue(String(this.plugin.settings.embedSubchunkOverlap)).onChange(async f=>{let P=Number.parseInt(f,10);this.plugin.settings.embedSubchunkOverlap=Number.isFinite(P)?Math.max(0,P):200,await this.plugin.saveSettings()})),new S.Setting(_).setName("Generate LLM tags for chunks").setDesc("Use the OCR cleanup model to tag chunks before indexing.").addToggle(g=>g.setValue(this.plugin.settings.enableChunkTagging).onChange(async f=>{this.plugin.settings.enableChunkTagging=f,await this.plugin.saveSettings()})),_.createEl("h2",{text:"Chat LLM"});let O=null,T=null,U=null,W=async()=>{},A=async(g,f=!0)=>{let P=g.trim();this.plugin.settings.chatBaseUrl=P,f&&(this.plugin.settings.chatProviderProfileId="",O&&O.setValue("custom")),T&&T.setValue(P),await this.plugin.saveSettings()},z=async g=>{let f=e().find(P=>P.id===g);this.plugin.settings.chatProviderProfileId=g,f&&(this.plugin.settings.chatBaseUrl=f.baseUrl,this.plugin.settings.chatApiKey=f.apiKey,T==null||T.setValue(f.baseUrl),U==null||U.setValue(f.apiKey)),await this.plugin.saveSettings(),await W()};new S.Setting(_).setName("Chat provider profile").setDesc("Select a profile to populate base URL and API key.").addDropdown(g=>{O=g,g.addOption("custom","Custom (manual)");for(let P of e())g.addOption(P.id,P.name||P.id);let f=this.plugin.settings.chatProviderProfileId;g.setValue(f&&e().some(P=>P.id===f)?f:"custom"),g.onChange(async P=>{if(P==="custom"){this.plugin.settings.chatProviderProfileId="",await this.plugin.saveSettings();return}await z(P)})}),new S.Setting(_).setName("Chat base URL").setDesc("OpenAI-compatible base URL for chat requests.").addText(g=>{T=g,g.setPlaceholder("http://localhost:1234/v1").setValue(this.plugin.settings.chatBaseUrl).onChange(async f=>{await A(f)})}),new S.Setting(_).setName("Chat API key").addText(g=>{U=g,t(g),g.setPlaceholder("lm-studio").setValue(this.plugin.settings.chatApiKey).onChange(async f=>{this.plugin.settings.chatApiKey=f.trim(),this.plugin.settings.chatProviderProfileId="",O&&O.setValue("custom"),await this.plugin.saveSettings()})});let Z=new S.Setting(_).setName("Chat model").setDesc("Select a chat-capable model from the provider."),V=null,oe=g=>{if(!V)return;let f=(this.plugin.settings.chatModel||"").trim(),P=new Set(g.map(M=>M.value));f&&!P.has(f)&&(g=g.concat([{value:f,label:`Custom (${f})`}])),V.selectEl.options.length=0;for(let M of g)V.addOption(M.value,M.label);f&&V.setValue(f)};W=async()=>{if(V){V.setDisabled(!0);try{let g=await this.plugin.fetchChatModelOptions();oe(g)}finally{V.setDisabled(!1)}}},Z.addDropdown(g=>{V=g;let f=(this.plugin.settings.chatModel||"").trim();g.addOption(f||"loading","Loading..."),g.setValue(f||"loading"),g.onChange(async P=>{this.plugin.settings.chatModel=P.trim(),await this.plugin.saveSettings()})}),Z.addButton(g=>{g.setButtonText("Refresh").onClick(async()=>{await W()})}),W(),new S.Setting(_).setName("Temperature").addText(g=>g.setPlaceholder("0.2").setValue(String(this.plugin.settings.chatTemperature)).onChange(async f=>{let P=Number.parseFloat(f);this.plugin.settings.chatTemperature=Number.isFinite(P)?P:.2,await this.plugin.saveSettings()})),new S.Setting(_).setName("Chat history messages").setDesc("Number of recent messages to include for conversational continuity (0 disables).").addText(g=>g.setPlaceholder("6").setValue(String(this.plugin.settings.chatHistoryMessages)).onChange(async f=>{let P=Number.parseInt(f,10);this.plugin.settings.chatHistoryMessages=Number.isFinite(P)?Math.max(0,P):6,await this.plugin.saveSettings()})),new S.Setting(_).setName("Chat panel location").setDesc("Where to open the chat view by default.").addDropdown(g=>g.addOption("right","Right sidebar").addOption("main","Main window").setValue(this.plugin.settings.chatPaneLocation).onChange(async f=>{this.plugin.settings.chatPaneLocation=f,await this.plugin.saveSettings()})),_.createEl("h2",{text:"Logging"}),new S.Setting(_).setName("Enable logging to file").setDesc("Write plugin logs to a file.").addToggle(g=>g.setValue(this.plugin.settings.enableFileLogging).onChange(async f=>{this.plugin.settings.enableFileLogging=f,await this.plugin.saveSettings()})),new S.Setting(_).setName("Log file path (vault-relative)").setDesc("Where to write the log file. Keep inside the vault.").addText(g=>g.setPlaceholder(`${J}/logs/docling_extract.log`).setValue(this.plugin.settings.logFilePath).onChange(async f=>{this.plugin.settings.logFilePath=f.trim()||`${J}/logs/docling_extract.log`,await this.plugin.saveSettings()})),new S.Setting(_).setName("View or clear log").setDesc("Open the log file or clear it.").addButton(g=>g.setButtonText("Open log").onClick(async()=>{var f,P;await((P=(f=this.plugin).openLogFile)==null?void 0:P.call(f))})).addButton(g=>g.setButtonText("Clear log").onClick(async()=>{var f,P;await((P=(f=this.plugin).clearLogFile)==null?void 0:P.call(f))})),_.createEl("h2",{text:"Maintenance"}),new S.Setting(_).setName("Reindex Redis from cached chunks").setDesc("Rebuild the Redis index from cached chunk JSON files.").addButton(g=>g.setButtonText("Reindex").onClick(async()=>{await this.plugin.reindexRedisFromCache()})),new S.Setting(_).setName("Recreate missing notes from cache").setDesc("Rebuild missing notes using cached Zotero items and chunks.").addButton(g=>g.setButtonText("Recreate").onClick(async()=>{await this.plugin.recreateMissingNotesFromCache()})).addButton(g=>g.setButtonText("Cancel").onClick(()=>{this.plugin.cancelRecreateMissingNotesFromCache()}))}};var we={"zrr-picker":`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">

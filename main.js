...
`,"ocr_wordlist.txt":`# zotero-redisearch-rag tool version: 0.9.17
ackley
afton
alonso
altmetric
...
zaiser
zenodo
`,"requirements.txt":`# zotero-redisearch-rag tool version: 0.9.17
docling
langcodes[data]
markdown
markdown-it-py
...
spylls
`,"docker-compose.yml":`# zotero-redisearch-rag tool version: 0.9.17
services:
  redis-stack:
...
`,"python-worker.Dockerfile":`# zotero-redisearch-rag tool version: 0.9.17
...
`);for(let g of d){let y=String(g.doc_id||"").trim(),x=String(g.chunk_id||"").trim(),k=String(g.page_start||"").trim(),h=String(g.page_end||"").trim(),b=String(g.title||"").trim(),w=String(g.section||"").trim(),E=String(g.score||"").trim(),R=String(g.authors||"").trim(),v=String(g.item_type||"").trim(),A=String(g.year||"").trim(),T=String(g.tags||"").trim(),I=String(g.chunk_tags||"").trim(),L=String(g.attachment_key||"").trim(),q=String(g.source_pdf||"").trim(),M=String(g.text||"").replace(/\s+/g," ").trim(),z=M.length>220?`${M.slice(0,220)}\u2026`:M,$=[y];x&&$.push(x),(k||h)&&$.push(`p.${k||"?"}-${h||"?"}`),p.push($.filter(Boolean).join(" \u2022 ")),E&&p.push(`  score: ${E}`),b&&p.push(`  title: ${b}`),R&&p.push(`  authors: ${R}`),A&&p.push(`  year: ${A}`),v&&p.push(`  item_type: ${v}`),T&&p.push(`  tags: ${T}`),I&&p.push(`  chunk_tags: ${I}`),L&&p.push(`  attachment_key: ${L}`),w&&p.push(`  section: ${w}`),q&&p.push(`  source_pdf: ${q}`),z&&p.push(`  ${z}`),p.push("")}return p.join(`

"use strict";var dn=Object.create;var be=Object.defineProperty;var cn=Object.getOwnPropertyDescriptor;var pn=Object.getOwnPropertyNames;var un=Object.getPrototypeOf,_n=Object.prototype.hasOwnProperty;var gn=(v,h)=>{for(var e in h)be(v,e,{get:h[e],enumerable:!0})},Ze=(v,h,e,n)=>{if(h&&typeof h=="object"||typeof h=="function")for(let t of pn(h))!_n.call(v,t)&&t!==e&&be(v,t,{get:()=>h[t],enumerable:!(n=cn(h,t))||n.enumerable});return v};var le=(v,h,e)=>(e=v!=null?dn(un(v)):{},Ze(h||!v||!v.__esModule?be(e,"default",{value:v,enumerable:!0}):e,v)),fn=v=>Ze(be({},"__esModule",{value:!0}),v);var Rn={};gn(Rn,{default:()=>Le});module.exports=fn(Rn);var p=require("obsidian"),ee=require("@codemirror/state"),j=require("@codemirror/view"),X=require("child_process"),V=require("fs"),nn=le(require("http")),tn=le(require("https")),ge=le(require("net")),rn=le(require("os")),sn=le(require("tls")),L=le(require("path")),ue=require("url"),ve=require("crypto");var N=require("obsidian"),K=".zotero-redisearch-rag",J=`${K}/items`,G=`${K}/chunks`,He={pythonPath:"",pythonEnvLocation:"shared",dockerPath:"docker",redisUrl:"redis://127.0.0.1:6379",autoAssignRedisPort:!1,redisDataDirOverride:"",redisProjectName:"",autoStartRedis:!0,zoteroBaseUrl:"http://127.0.0.1:23119/api",zoteroUserId:"0",webApiBaseUrl:"https://api.zotero.org",webApiLibraryType:"user",webApiLibraryId:"",webApiKey:"",outputPdfDir:"Zotero/PDFs",outputNoteDir:"Zotero/Notes",frontmatterTemplate:`doc_id: {{doc_id}}
...
item_json: {{item_json_yaml}}`,tagSanitizeMode:"kebab",noteBodyTemplate:"{{pdf_block}}{{docling_markdown}}",llmProviderProfiles:[{id:"lm-studio",name:"LM Studio",baseUrl:"http://localhost:1234/v1",apiKey:"lm-studio"},{id:"ollama",name:"Ollama",baseUrl:"http://localhost:11434/v1",apiKey:""},{id:"openrouter",name:"OpenRouter",baseUrl:"https://openrouter.ai/api/v1",apiKey:""},{id:"openai",name:"OpenAI",baseUrl:"https://api.openai.com/v1",apiKey:""}],chatOutputDir:"Zotero/Chats",copyPdfToVault:!0,createOcrLayeredPdf:!1,preferObsidianNoteForCitations:!0,ocrMode:"auto",ocrQualityThreshold:.5,chunkingMode:"page",ocrEngine:"auto",paddleApiKey:"",paddleVlApiUrl:"",paddleStructureApiUrl:"",enableLlmCleanup:!1,llmCleanupProviderProfileId:"lm-studio",llmCleanupBaseUrl:"http://127.0.0.1:1234/v1",llmCleanupApiKey:"",llmCleanupModel:"openai/gpt-oss-20b",llmCleanupTemperature:0,llmCleanupMinQuality:.35,llmCleanupMaxChars:2e3,embedProviderProfileId:"lm-studio",embedBaseUrl:"http://localhost:1234/v1",embedApiKey:"lm-studio",embedModel:"google/embedding-gemma-300m",embedIncludeMetadata:!0,embedSubchunkChars:3500,embedSubchunkOverlap:200,embedContextWindow:1,embedContextChars:220,enableChunkTagging:!1,chatProviderProfileId:"lm-studio",chatBaseUrl:"http://127.0.0.1:1234/v1",chatApiKey:"",chatModel:"openai/gpt-oss-20b",chatTemperature:.2,chatHistoryMessages:6,chatPaneLocation:"right",enableQueryExpansion:!1,queryExpansionCount:3,enableCrossEncoderRerank:!1,rerankModel:"cross-encoder/ms-marco-MiniLM-L-6-v2",rerankCandidateMultiplier:4,rrfK:60,rrfLogTop:0,maxChunksPerDoc:0,enableFileLogging:!1,logFilePath:`${K}/logs/docling_extract.log`,redisIndex:"idx:zotero",redisPrefix:"zotero:chunk:"},xe=class extends N.PluginSettingTab{constructor(e,n){super(e,n);this.activeTab="prerequisites";this.plugin=n}display(){let{containerEl:e}=this;e.empty();let n=()=>Array.isArray(this.plugin.settings.llmProviderProfiles)?this.plugin.settings.llmProviderProfiles:[],t=async _=>{this.plugin.settings.llmProviderProfiles=_,await this.plugin.saveSettings()},i=_=>{_.inputEl.type="password",_.inputEl.autocomplete="off",_.inputEl.spellcheck=!1},d=[{id:"prerequisites",label:"Prerequisites",render:_=>{_.createEl("h2",{text:"Prerequisites"}),new N.Setting(_).setName("Python path").setDesc("Optional path to the Python interpreter used to create or run the plugin env. Leave blank to auto-detect (python3.13/3.12/3.11/3.10/python3/python, or py on Windows).").addText(f=>f.setPlaceholder("auto-detect").setValue(this.plugin.settings.pythonPath).onChange(async x=>{this.plugin.settings.pythonPath=x.trim(),await this.plugin.saveSettings()})),new N.Setting(_).setName("Python environment").setDesc("Create or update the plugin's Python env (location configured below).").addButton(f=>{f.setButtonText("Create/Update").setCta(),f.onClick(async()=>{f.setDisabled(!0);try{await this.plugin.setupPythonEnv()}finally{f.setDisabled(!1)}})}),new N.Setting(_).setName("Python env location").setDesc("Shared user cache can be reused across vaults; plugin folder keeps a per-vault env.").addDropdown(f=>{f.addOption("shared","Shared user cache"),f.addOption("plugin","Plugin folder (.venv)"),f.setValue(this.plugin.settings.pythonEnvLocation).onChange(async x=>{x!=="shared"&&x!=="plugin"||(this.plugin.settings.pythonEnvLocation=x,await this.plugin.saveSettings())})}),new N.Setting(_).setName("Docker/Podman path").setDesc("CLI path for Docker or Podman (used to start Redis Stack).").addText(f=>f.setPlaceholder("docker").setValue(this.plugin.settings.dockerPath).onChange(async x=>{this.plugin.settings.dockerPath=x.trim()||"docker",await this.plugin.saveSettings()})),new N.Setting(_).setName("Redis URL").addText(f=>f.setPlaceholder("redis://127.0.0.1:6379").setValue(this.plugin.settings.redisUrl).onChange(async x=>{this.plugin.settings.redisUrl=x.trim(),await this.plugin.saveSettings()})),new N.Setting(_).setName("Redis data directory override").setDesc("Optional absolute path to store Redis persistence when auto-assign is off. Env var ZRR_DATA_DIR overrides this setting.").addText(f=>f.setPlaceholder("/Users/you/Redis/zrr-data").setValue(this.plugin.settings.redisDataDirOverride).onChange(async x=>{this.plugin.settings.redisDataDirOverride=x.trim(),await this.plugin.saveSettings()})),new N.Setting(_).setName("Redis project name override").setDesc("Optional Docker/Podman Compose project name when auto-assign is off. Env var ZRR_PROJECT_NAME overrides this setting.").addText(f=>f.setPlaceholder("zrr-shared").setValue(this.plugin.settings.redisProjectName).onChange(async x=>{this.plugin.settings.redisProjectName=x.trim(),await this.plugin.saveSettings()})),new N.Setting(_).setName("Auto-assign Redis port").setDesc("When starting Redis stack, pick a free local port and update the Redis URL.").addToggle(f=>f.setValue(this.plugin.settings.autoAssignRedisPort).onChange(async x=>{this.plugin.settings.autoAssignRedisPort=x,await this.plugin.saveSettings()})),new N.Setting(_).setName("Auto-start Redis stack (Docker/Podman Compose)").setDesc("Requires Docker Desktop running and your vault path shared with Docker. Uses a vault-specific data dir at .obsidian/zotero-redisearch-rag/redis-data unless overridden.").addToggle(f=>f.setValue(this.plugin.settings.autoStartRedis).onChange(async x=>{this.plugin.settings.autoStartRedis=x,await this.plugin.saveSettings()})),new N.Setting(_).setName("Start Redis stack now").setDesc("Restarts Docker/Podman Compose with the vault data directory.").addButton(f=>f.setButtonText("Start").onClick(async()=>{await this.plugin.startRedisStack()}))}},{id:"zotero-import",label:"Zotero import",render:_=>{_.createEl("h2",{text:"Zotero Local API"}),new N.Setting(_).setName("Zotero base URL").setDesc("Local Zotero API base URL, e.g. http://127.0.0.1:23119/api").addText(A=>A.setPlaceholder("http://127.0.0.1:23119/api").setValue(this.plugin.settings.zoteroBaseUrl).onChange(async w=>{this.plugin.settings.zoteroBaseUrl=w.trim(),await this.plugin.saveSettings()}));let f=new N.Setting(_).setName("Zotero library").setDesc("Select your local library or a Zotero group library."),x=null,P=A=>{if(!x)return;let w=(this.plugin.settings.zoteroUserId||"0").trim()||"0";new Set(A.map(I=>I.value)).has(w)||(A=A.concat([{value:w,label:`Custom (${w})`}])),x.selectEl.options.length=0;for(let I of A)x.addOption(I.value,I.label);x.setValue(w)},R=async()=>{if(x){x.setDisabled(!0);try{let A=await this.plugin.fetchZoteroLibraryOptions();P(A)}finally{x.setDisabled(!1)}}};f.addDropdown(A=>{x=A;let w=(this.plugin.settings.zoteroUserId||"0").trim()||"0";A.addOption(w,"Loading..."),A.setValue(w),A.onChange(async D=>{this.plugin.settings.zoteroUserId=D.trim(),await this.plugin.saveSettings()})}),f.addButton(A=>{A.setButtonText("Refresh").onClick(async()=>{await R()})}),R(),_.createEl("h2",{text:"Zotero Web API"}),new N.Setting(_).setName("Web API base URL").setDesc("Zotero Web API base URL for write fallback, e.g. https://api.zotero.org").addText(A=>A.setPlaceholder("https://api.zotero.org").setValue(this.plugin.settings.webApiBaseUrl).onChange(async w=>{this.plugin.settings.webApiBaseUrl=w.trim(),await this.plugin.saveSettings()})),new N.Setting(_).setName("Web API library type").setDesc("Library type for Web API writes.").addDropdown(A=>A.addOption("user","user").addOption("group","group").setValue(this.plugin.settings.webApiLibraryType).onChange(async w=>{this.plugin.settings.webApiLibraryType=w,await this.plugin.saveSettings()})),new N.Setting(_).setName("Web API library ID").setDesc("Numeric Zotero user/group ID for Web API writes.").addText(A=>A.setPlaceholder("15218").setValue(this.plugin.settings.webApiLibraryId).onChange(async w=>{this.plugin.settings.webApiLibraryId=w.trim(),await this.plugin.saveSettings()})),new N.Setting(_).setName("Web API key").setDesc("Zotero API key for write fallback (from zotero.org).").addText(A=>{i(A),A.setPlaceholder("your-api-key").setValue(this.plugin.settings.webApiKey).onChange(async w=>{this.plugin.settings.webApiKey=w.trim(),await this.plugin.saveSettings()})}),_.createEl("h2",{text:"Output"}),new N.Setting(_).setName("PDF folder").addText(A=>A.setPlaceholder("zotero/pdfs").setValue(this.plugin.settings.outputPdfDir).onChange(async w=>{this.plugin.settings.outputPdfDir=w.trim(),await this.plugin.saveSettings()})),new N.Setting(_).setName("Notes folder").addText(A=>A.setPlaceholder("zotero/notes").setValue(this.plugin.settings.outputNoteDir).onChange(async w=>{this.plugin.settings.outputNoteDir=w.trim(),await this.plugin.saveSettings()})),new N.Setting(_).setName("Frontmatter template").setDesc("Template for note YAML frontmatter. Use {{var}} placeholders; leave blank to omit.").addTextArea(A=>{A.setValue(this.plugin.settings.frontmatterTemplate).onChange(async w=>{this.plugin.settings.frontmatterTemplate=w,await this.plugin.saveSettings()}),A.inputEl.rows=10,A.inputEl.style.width="100%"}),new N.Setting(_).setName("Tag sanitization").setDesc("Normalize Zotero tags for Obsidian (no spaces, punctuation trimmed).").addDropdown(A=>A.addOption("none","No change").addOption("camel","camelCase").addOption("pascal","PascalCase").addOption("snake","snake_case").addOption("kebab","kebab-case").setValue(this.plugin.settings.tagSanitizeMode==="replace"?"kebab":this.plugin.settings.tagSanitizeMode).onChange(async w=>{this.plugin.settings.tagSanitizeMode=w,await this.plugin.saveSettings()})),new N.Setting(_).setName("Note body template").setDesc("Template for the note body after frontmatter. Use {{pdf_block}} and {{docling_markdown}} placeholders.").addTextArea(A=>{A.setValue(this.plugin.settings.noteBodyTemplate).onChange(async w=>{this.plugin.settings.noteBodyTemplate=w,await this.plugin.saveSettings()}),A.inputEl.rows=8,A.inputEl.style.width="100%"}),new N.Setting(_).setName("Saved chats folder").setDesc("Where exported chat notes are stored (vault-relative).").addText(A=>A.setPlaceholder("zotero/chats").setValue(this.plugin.settings.chatOutputDir).onChange(async w=>{this.plugin.settings.chatOutputDir=w.trim()||"zotero/chats",await this.plugin.saveSettings()})),new N.Setting(_).setName("Copy PDFs into vault").setDesc("Disable to use Zotero storage paths directly. If a local file path is unavailable, the plugin temporarily copies the PDF into the vault for processing.").addToggle(A=>A.setValue(this.plugin.settings.copyPdfToVault).onChange(async w=>{this.plugin.settings.copyPdfToVault=w,!w&&this.plugin.settings.createOcrLayeredPdf&&(this.plugin.settings.createOcrLayeredPdf=!1),await this.plugin.saveSettings(),this.display()})),new N.Setting(_).setName("Create OCR-layered PDF copy").setDesc("When OCR is used, replace the vault PDF with a Tesseract text layer (requires Copy PDFs into vault).").addToggle(A=>{let w=this.plugin.settings.copyPdfToVault;A.setValue(w?this.plugin.settings.createOcrLayeredPdf:!1).setDisabled(!w).onChange(async D=>{if(!this.plugin.settings.copyPdfToVault){this.plugin.settings.createOcrLayeredPdf=!1,await this.plugin.saveSettings();return}this.plugin.settings.createOcrLayeredPdf=D,await this.plugin.saveSettings()})}),new N.Setting(_).setName("Prefer Obsidian note for citations").setDesc("Link citations to the Obsidian note when available; otherwise use Zotero deep links.").addToggle(A=>A.setValue(this.plugin.settings.preferObsidianNoteForCitations).onChange(async w=>{this.plugin.settings.preferObsidianNoteForCitations=w,await this.plugin.saveSettings()}))}},{id:"ocr",label:"OCR",render:_=>{_.createEl("h2",{text:"Docling"});let f=null,x=w=>{if(!f)return;let D=this.plugin.settings.ocrEngine;new Set(w.map(F=>F.value)).has(D)||(w=w.concat([{value:D,label:`Current (unavailable): ${D}`}])),f.selectEl.options.length=0;for(let F of w)f.addOption(F.value,F.label);f.setValue(D)},P=async()=>{if(!f)return;f.setDisabled(!0);let w={tesseract:!1,paddleStructureLocal:!1,paddleVlLocal:!1};if(this.plugin.detectOcrEngines)try{w=await this.plugin.detectOcrEngines()}catch(F){w={tesseract:!1,paddleStructureLocal:!1,paddleVlLocal:!1}}let D=[{value:"auto",label:"Auto (default)"}];w.tesseract&&D.push({value:"tesseract",label:"Tesseract (local)"}),w.paddleStructureLocal&&D.push({value:"paddle_structure_local",label:"Paddle PP-StructureV3 (local)"}),w.paddleVlLocal&&D.push({value:"paddle_vl_local",label:"PaddleOCR-VL (local)"}),(this.plugin.settings.paddleApiKey||"").trim()&&(D.push({value:"paddle_structure_api",label:"PP-StructureV3 API"}),D.push({value:"paddle_vl_api",label:"PaddleOCR-VL API"})),x(D),f.setDisabled(!1)},R=new N.Setting(_).setName("Paddle OCR API key").setDesc("API token for PaddleOCR-VL / PP-StructureV3 endpoints. Get a free API key at "),A=document.createElement("a");A.href="https://aistudio.baidu.com/paddleocr",A.textContent="https://aistudio.baidu.com/paddleocr",A.target="_blank",A.rel="noopener noreferrer",R.descEl.appendChild(A),R.descEl.append("."),R.addText(w=>{i(w),w.setPlaceholder("your-api-token").setValue(this.plugin.settings.paddleApiKey).onChange(async D=>{this.plugin.settings.paddleApiKey=D.trim(),await this.plugin.saveSettings(),await P()})}),new N.Setting(_).setName("PaddleOCR-VL API URL").setDesc("Optional override for the PaddleOCR-VL API endpoint.").addText(w=>w.setPlaceholder("https://.../layout-parsing").setValue(this.plugin.settings.paddleVlApiUrl).onChange(async D=>{this.plugin.settings.paddleVlApiUrl=D.trim(),await this.plugin.saveSettings()})),new N.Setting(_).setName("PP-StructureV3 API URL").setDesc("API endpoint for PP-StructureV3 (see Baidu AI Studio docs).").addText(w=>w.setPlaceholder("https://.../pp-structure").setValue(this.plugin.settings.paddleStructureApiUrl).onChange(async D=>{this.plugin.settings.paddleStructureApiUrl=D.trim(),await this.plugin.saveSettings(),await P()})),new N.Setting(_).setName("OCR engine").setDesc("Select the OCR engine to use when OCR is required.").addDropdown(w=>{f=w,w.addOption("auto","Auto (default)"),w.setValue(this.plugin.settings.ocrEngine),w.onChange(async D=>{this.plugin.settings.ocrEngine=D,await this.plugin.saveSettings()})}),new N.Setting(_).setName("OCR mode").setDesc("auto: skip OCR when text is readable; force if bad: OCR only when text looks poor; force: always OCR.").addDropdown(w=>w.addOption("auto","auto").addOption("force_low_quality","force if quality is bad").addOption("force","force").setValue(this.plugin.settings.ocrMode).onChange(async D=>{this.plugin.settings.ocrMode=D,await this.plugin.saveSettings()})),new N.Setting(_).setName("Text quality threshold").setDesc("Lower values are stricter; below this threshold the text is treated as low quality.").addSlider(w=>{w.setLimits(0,1,.05).setValue(this.plugin.settings.ocrQualityThreshold).setDynamicTooltip().onChange(async D=>{this.plugin.settings.ocrQualityThreshold=D,await this.plugin.saveSettings()})}),new N.Setting(_).setName("Chunking").setDesc("page or section").addDropdown(w=>w.addOption("page","page").addOption("section","section").setValue(this.plugin.settings.chunkingMode).onChange(async D=>{this.plugin.settings.chunkingMode=D,await this.plugin.saveSettings()})),P()}},{id:"llms",label:"LLMs",render:_=>{_.createEl("h2",{text:"LLM Provider Profiles"});let f=_.createDiv({cls:"zrr-llm-profiles"}),x=()=>{f.empty();let m=n();m.length||f.createEl("p",{text:"No profiles yet. Add one below."});for(let k of m){let C=f.createEl("details",{cls:"zrr-profile"});m.length===1&&(C.open=!0);let B=C.createEl("summary",{text:k.name||k.id||"Profile"});B.addClass("zrr-profile-title");let ye=C.createDiv({cls:"zrr-profile-body"});new N.Setting(ye).setName("Profile name").addText(te=>te.setPlaceholder("My provider").setValue(k.name||"").onChange(async re=>{k.name=re.trim(),B.textContent=k.name||k.id||"Profile",await t(n())})),new N.Setting(ye).setName("Base URL").addText(te=>te.setPlaceholder("http://localhost:1234/v1").setValue(k.baseUrl||"").onChange(async re=>{k.baseUrl=re.trim(),await t(n())})),new N.Setting(ye).setName("API key").setDesc("Stored in settings (not encrypted).").addText(te=>{i(te),te.setPlaceholder("sk-...").setValue(k.apiKey||"").onChange(async re=>{k.apiKey=re.trim(),await t(n())})}),new N.Setting(ye).setName("Remove profile").setDesc("Deletes this saved profile.").addButton(te=>te.setButtonText("Delete profile").onClick(async()=>{let re=n().filter(ln=>ln.id!==k.id);this.plugin.settings.embedProviderProfileId=this.plugin.settings.embedProviderProfileId===k.id?"":this.plugin.settings.embedProviderProfileId,this.plugin.settings.chatProviderProfileId=this.plugin.settings.chatProviderProfileId===k.id?"":this.plugin.settings.chatProviderProfileId,this.plugin.settings.llmCleanupProviderProfileId=this.plugin.settings.llmCleanupProviderProfileId===k.id?"":this.plugin.settings.llmCleanupProviderProfileId,await t(re),x()}))}new N.Setting(f).addButton(k=>k.setButtonText("Add profile").onClick(async()=>{let C=`profile-${Date.now().toString(36)}`,B=n().concat([{id:C,name:"Custom",baseUrl:"",apiKey:""}]);await t(B),x()}))};x(),_.createEl("h2",{text:"OCR cleanup"}),new N.Setting(_).setName("LLM cleanup for low-quality chunks").setDesc("Automatic AI cleanup for poor OCR at import. Can be slow/costly.").addToggle(m=>m.setValue(this.plugin.settings.enableLlmCleanup).onChange(async k=>{this.plugin.settings.enableLlmCleanup=k,await this.plugin.saveSettings()}));let P=null,R=null,A=null,w=async()=>{},D=async(m,k=!0)=>{let C=m.trim();this.plugin.settings.llmCleanupBaseUrl=C,k&&(this.plugin.settings.llmCleanupProviderProfileId="",P&&P.setValue("custom")),R&&R.setValue(C),await this.plugin.saveSettings()},I=async m=>{let k=n().find(C=>C.id===m);this.plugin.settings.llmCleanupProviderProfileId=m,k&&(this.plugin.settings.llmCleanupBaseUrl=k.baseUrl,this.plugin.settings.llmCleanupApiKey=k.apiKey,R==null||R.setValue(k.baseUrl),A==null||A.setValue(k.apiKey)),await this.plugin.saveSettings(),await w()};new N.Setting(_).setName("LLM cleanup provider profile").setDesc("Select a profile to populate base URL and API key.").addDropdown(m=>{P=m,m.addOption("custom","Custom (manual)");for(let C of n())m.addOption(C.id,C.name||C.id);let k=this.plugin.settings.llmCleanupProviderProfileId;m.setValue(k&&n().some(C=>C.id===k)?k:"custom"),m.onChange(async C=>{if(C==="custom"){this.plugin.settings.llmCleanupProviderProfileId="",await this.plugin.saveSettings();return}await I(C)})}),new N.Setting(_).setName("LLM cleanup base URL").setDesc("OpenAI-compatible endpoint, e.g. http://127.0.0.1:1234/v1").addText(m=>{R=m,m.setPlaceholder("http://127.0.0.1:1234/v1").setValue(this.plugin.settings.llmCleanupBaseUrl).onChange(async k=>{await D(k)})}),new N.Setting(_).setName("LLM cleanup API key").setDesc("Optional API key for the cleanup endpoint.").addText(m=>{A=m,i(m),m.setPlaceholder("sk-...").setValue(this.plugin.settings.llmCleanupApiKey).onChange(async k=>{this.plugin.settings.llmCleanupApiKey=k.trim(),this.plugin.settings.llmCleanupProviderProfileId="",P&&P.setValue("custom"),await this.plugin.saveSettings()})});let F=new N.Setting(_).setName("LLM cleanup model").setDesc("Select a cleanup-capable model from the provider."),S=null,q=m=>{if(!S)return;let k=(this.plugin.settings.llmCleanupModel||"").trim(),C=new Set(m.map(B=>B.value));k&&!C.has(k)&&(m=m.concat([{value:k,label:`Custom (${k})`}])),S.selectEl.options.length=0;for(let B of m)S.addOption(B.value,B.label);k&&S.setValue(k)};w=async()=>{if(S){S.setDisabled(!0);try{let m=await this.plugin.fetchCleanupModelOptions();q(m)}finally{S.setDisabled(!1)}}},F.addDropdown(m=>{S=m;let k=(this.plugin.settings.llmCleanupModel||"").trim();m.addOption(k||"loading","Loading..."),m.setValue(k||"loading"),m.onChange(async C=>{this.plugin.settings.llmCleanupModel=C.trim(),await this.plugin.saveSettings()})}),F.addButton(m=>{m.setButtonText("Refresh").onClick(async()=>{await w()})}),w(),new N.Setting(_).setName("LLM cleanup temperature").setDesc("Lower is more conservative.").addText(m=>m.setPlaceholder("0.0").setValue(String(this.plugin.settings.llmCleanupTemperature)).onChange(async k=>{let C=Number.parseFloat(k);this.plugin.settings.llmCleanupTemperature=Number.isFinite(C)?C:0,await this.plugin.saveSettings()})),new N.Setting(_).setName("LLM cleanup min quality").setDesc("Only run cleanup when chunk quality is below this threshold (0-1).").addSlider(m=>m.setLimits(0,1,.05).setValue(this.plugin.settings.llmCleanupMinQuality).setDynamicTooltip().onChange(async k=>{this.plugin.settings.llmCleanupMinQuality=k,await this.plugin.saveSettings()})),new N.Setting(_).setName("LLM cleanup max chars").setDesc("Skip cleanup for chunks longer than this limit.").addText(m=>m.setPlaceholder("2000").setValue(String(this.plugin.settings.llmCleanupMaxChars)).onChange(async k=>{let C=Number.parseInt(k,10);this.plugin.settings.llmCleanupMaxChars=Number.isFinite(C)?C:2e3,await this.plugin.saveSettings()})),_.createEl("h2",{text:"Text Embedding"});let T=null,$=null,Z=null,O=async()=>{},z=async(m,k=!0)=>{let C=m.trim();this.plugin.settings.embedBaseUrl=C,k&&(this.plugin.settings.embedProviderProfileId="",T&&T.setValue("custom")),$&&$.setValue(C),await this.plugin.saveSettings()},U=async m=>{let k=n().find(C=>C.id===m);this.plugin.settings.embedProviderProfileId=m,k&&(this.plugin.settings.embedBaseUrl=k.baseUrl,this.plugin.settings.embedApiKey=k.apiKey,$==null||$.setValue(k.baseUrl),Z==null||Z.setValue(k.apiKey)),await this.plugin.saveSettings(),await O()};new N.Setting(_).setName("Embeddings provider profile").setDesc("Select a profile to populate base URL and API key.").addDropdown(m=>{T=m,m.addOption("custom","Custom (manual)");for(let C of n())m.addOption(C.id,C.name||C.id);let k=this.plugin.settings.embedProviderProfileId;m.setValue(k&&n().some(C=>C.id===k)?k:"custom"),m.onChange(async C=>{if(C==="custom"){this.plugin.settings.embedProviderProfileId="",await this.plugin.saveSettings();return}await U(C)})}),new N.Setting(_).setName("Embeddings base URL").addText(m=>{$=m,m.setPlaceholder("http://localhost:1234/v1").setValue(this.plugin.settings.embedBaseUrl).onChange(async k=>{await z(k)})}),new N.Setting(_).setName("Embeddings API key").addText(m=>{Z=m,i(m),m.setPlaceholder("lm-studio").setValue(this.plugin.settings.embedApiKey).onChange(async k=>{this.plugin.settings.embedApiKey=k.trim(),this.plugin.settings.embedProviderProfileId="",T&&T.setValue("custom"),await this.plugin.saveSettings()})});let W=new N.Setting(_).setName("Embeddings model").setDesc("Select an embeddings model from the provider."),Q=null,Ee=m=>{if(!Q)return;let k=(this.plugin.settings.embedModel||"").trim(),C=new Set(m.map(B=>B.value));k&&!C.has(k)&&(m=m.concat([{value:k,label:`Custom (${k})`}])),Q.selectEl.options.length=0;for(let B of m)Q.addOption(B.value,B.label);k&&Q.setValue(k)};O=async()=>{if(Q){Q.setDisabled(!0);try{let m=await this.plugin.fetchEmbeddingModelOptions();Ee(m)}finally{Q.setDisabled(!1)}}},W.addDropdown(m=>{Q=m;let k=(this.plugin.settings.embedModel||"").trim();m.addOption(k||"loading","Loading..."),m.setValue(k||"loading"),m.onChange(async C=>{this.plugin.settings.embedModel=C.trim(),await this.plugin.saveSettings()})}),W.addButton(m=>{m.setButtonText("Refresh").onClick(async()=>{await O()})}),O(),new N.Setting(_).setName("Include metadata in embeddings").setDesc("Prepend title/authors/tags/section info before embedding chunks.").addToggle(m=>m.setValue(this.plugin.settings.embedIncludeMetadata).onChange(async k=>{this.plugin.settings.embedIncludeMetadata=k,await this.plugin.saveSettings()})),new N.Setting(_).setName("Embedding context window (chunks)").setDesc("Include neighboring chunk text around each chunk when embedding (0 disables).").addText(m=>m.setPlaceholder("1").setValue(String(this.plugin.settings.embedContextWindow)).onChange(async k=>{let C=Number.parseInt(k,10);this.plugin.settings.embedContextWindow=Number.isFinite(C)?Math.max(0,C):1,await this.plugin.saveSettings()})),new N.Setting(_).setName("Embedding context snippet size (chars)").setDesc("Max chars per neighboring chunk included in embeddings.").addText(m=>m.setPlaceholder("220").setValue(String(this.plugin.settings.embedContextChars)).onChange(async k=>{let C=Number.parseInt(k,10);this.plugin.settings.embedContextChars=Number.isFinite(C)?Math.max(0,C):220,await this.plugin.saveSettings()})),new N.Setting(_).setName("Embedding subchunk size (chars)").setDesc("Split long chunks into smaller subchunks for embedding only (0 disables).").addText(m=>m.setPlaceholder("1800").setValue(String(this.plugin.settings.embedSubchunkChars)).onChange(async k=>{let C=Number.parseInt(k,10);this.plugin.settings.embedSubchunkChars=Number.isFinite(C)?Math.max(0,C):3500,await this.plugin.saveSettings()})),new N.Setting(_).setName("Embedding subchunk overlap (chars)").setDesc("Overlap between embedding subchunks to keep context intact.").addText(m=>m.setPlaceholder("200").setValue(String(this.plugin.settings.embedSubchunkOverlap)).onChange(async k=>{let C=Number.parseInt(k,10);this.plugin.settings.embedSubchunkOverlap=Number.isFinite(C)?Math.max(0,C):200,await this.plugin.saveSettings()})),new N.Setting(_).setName("Generate LLM tags for chunks").setDesc("Use the OCR cleanup model to tag chunks before indexing.").addToggle(m=>m.setValue(this.plugin.settings.enableChunkTagging).onChange(async k=>{this.plugin.settings.enableChunkTagging=k,await this.plugin.saveSettings()})),_.createEl("h2",{text:"Chat LLM"});let ie=null,ne=null,se=null,ae=async()=>{},he=async(m,k=!0)=>{let C=m.trim();this.plugin.settings.chatBaseUrl=C,k&&(this.plugin.settings.chatProviderProfileId="",ie&&ie.setValue("custom")),ne&&ne.setValue(C),await this.plugin.saveSettings()},Ae=async m=>{let k=n().find(C=>C.id===m);this.plugin.settings.chatProviderProfileId=m,k&&(this.plugin.settings.chatBaseUrl=k.baseUrl,this.plugin.settings.chatApiKey=k.apiKey,ne==null||ne.setValue(k.baseUrl),se==null||se.setValue(k.apiKey)),await this.plugin.saveSettings(),await ae()};new N.Setting(_).setName("Chat provider profile").setDesc("Select a profile to populate base URL and API key.").addDropdown(m=>{ie=m,m.addOption("custom","Custom (manual)");for(let C of n())m.addOption(C.id,C.name||C.id);let k=this.plugin.settings.chatProviderProfileId;m.setValue(k&&n().some(C=>C.id===k)?k:"custom"),m.onChange(async C=>{if(C==="custom"){this.plugin.settings.chatProviderProfileId="",await this.plugin.saveSettings();return}await Ae(C)})}),new N.Setting(_).setName("Chat base URL").setDesc("OpenAI-compatible base URL for chat requests.").addText(m=>{ne=m,m.setPlaceholder("http://localhost:1234/v1").setValue(this.plugin.settings.chatBaseUrl).onChange(async k=>{await he(k)})}),new N.Setting(_).setName("Chat API key").addText(m=>{se=m,i(m),m.setPlaceholder("lm-studio").setValue(this.plugin.settings.chatApiKey).onChange(async k=>{this.plugin.settings.chatApiKey=k.trim(),this.plugin.settings.chatProviderProfileId="",ie&&ie.setValue("custom"),await this.plugin.saveSettings()})});let oe=new N.Setting(_).setName("Chat model").setDesc("Select a chat-capable model from the provider."),Y=null,M=m=>{if(!Y)return;let k=(this.plugin.settings.chatModel||"").trim(),C=new Set(m.map(B=>B.value));k&&!C.has(k)&&(m=m.concat([{value:k,label:`Custom (${k})`}])),Y.selectEl.options.length=0;for(let B of m)Y.addOption(B.value,B.label);k&&Y.setValue(k)};ae=async()=>{if(Y){Y.setDisabled(!0);try{let m=await this.plugin.fetchChatModelOptions();M(m)}finally{Y.setDisabled(!1)}}},oe.addDropdown(m=>{Y=m;let k=(this.plugin.settings.chatModel||"").trim();m.addOption(k||"loading","Loading..."),m.setValue(k||"loading"),m.onChange(async C=>{this.plugin.settings.chatModel=C.trim(),await this.plugin.saveSettings()})}),oe.addButton(m=>{m.setButtonText("Refresh").onClick(async()=>{await ae()})}),ae(),new N.Setting(_).setName("Temperature").addText(m=>m.setPlaceholder("0.2").setValue(String(this.plugin.settings.chatTemperature)).onChange(async k=>{let C=Number.parseFloat(k);this.plugin.settings.chatTemperature=Number.isFinite(C)?C:.2,await this.plugin.saveSettings()})),new N.Setting(_).setName("Chat history messages").setDesc("Number of recent messages to include for conversational continuity (0 disables).").addText(m=>m.setPlaceholder("6").setValue(String(this.plugin.settings.chatHistoryMessages)).onChange(async k=>{let C=Number.parseInt(k,10);this.plugin.settings.chatHistoryMessages=Number.isFinite(C)?Math.max(0,C):6,await this.plugin.saveSettings()})),new N.Setting(_).setName("Chat panel location").setDesc("Where to open the chat view by default.").addDropdown(m=>m.addOption("right","Right sidebar").addOption("main","Main window").setValue(this.plugin.settings.chatPaneLocation).onChange(async k=>{this.plugin.settings.chatPaneLocation=k,await this.plugin.saveSettings()})),_.createEl("h2",{text:"Retrieval"}),new N.Setting(_).setName("Enable query expansion").setDesc("Use the chat model to expand queries before retrieval.").addToggle(m=>m.setValue(this.plugin.settings.enableQueryExpansion).onChange(async k=>{this.plugin.settings.enableQueryExpansion=k,await this.plugin.saveSettings()})),new N.Setting(_).setName("Query expansion count").setDesc("Number of expansion variants to request.").addText(m=>m.setPlaceholder("3").setValue(String(this.plugin.settings.queryExpansionCount)).onChange(async k=>{let C=Number.parseInt(k,10);this.plugin.settings.queryExpansionCount=Number.isFinite(C)?Math.max(1,C):3,await this.plugin.saveSettings()})),new N.Setting(_).setName("Enable cross-encoder reranking").setDesc("Rerank candidates locally with sentence-transformers (downloads model on first use).").addToggle(m=>m.setValue(this.plugin.settings.enableCrossEncoderRerank).onChange(async k=>{this.plugin.settings.enableCrossEncoderRerank=k,await this.plugin.saveSettings()})),new N.Setting(_).setName("Cross-encoder model").setDesc("Local reranker model name or path.").addText(m=>m.setPlaceholder("cross-encoder/ms-marco-MiniLM-L-6-v2").setValue(this.plugin.settings.rerankModel).onChange(async k=>{this.plugin.settings.rerankModel=k.trim()||"cross-encoder/ms-marco-MiniLM-L-6-v2",await this.plugin.saveSettings()})),new N.Setting(_).setName("Rerank candidate multiplier").setDesc("Retrieve k \xD7 N candidates before reranking.").addText(m=>m.setPlaceholder("4").setValue(String(this.plugin.settings.rerankCandidateMultiplier)).onChange(async k=>{let C=Number.parseInt(k,10);this.plugin.settings.rerankCandidateMultiplier=Number.isFinite(C)?Math.max(1,C):4,await this.plugin.saveSettings()})),new N.Setting(_).setName("RRF k").setDesc("Rank fusion constant for blending lexical and vector results.").addText(m=>m.setPlaceholder("60").setValue(String(this.plugin.settings.rrfK)).onChange(async k=>{let C=Number.parseInt(k,10);this.plugin.settings.rrfK=Number.isFinite(C)?Math.max(1,C):60,await this.plugin.saveSettings()})),new N.Setting(_).setName("RRF log top N").setDesc("Log the top N RRF-ranked chunks to stderr (0 disables).").addText(m=>m.setPlaceholder("0").setValue(String(this.plugin.settings.rrfLogTop)).onChange(async k=>{let C=Number.parseInt(k,10);this.plugin.settings.rrfLogTop=Number.isFinite(C)?Math.max(0,C):0,await this.plugin.saveSettings()})),new N.Setting(_).setName("Max chunks per document").setDesc("Limit how many chunks from a single document can appear in retrieval (0 disables).").addText(m=>m.setPlaceholder("0").setValue(String(this.plugin.settings.maxChunksPerDoc)).onChange(async k=>{let C=Number.parseInt(k,10);this.plugin.settings.maxChunksPerDoc=Number.isFinite(C)?Math.max(0,C):0,await this.plugin.saveSettings()}))}},{id:"maintenance",label:"Maintenance",render:_=>{_.createEl("h2",{text:"Logging"}),new N.Setting(_).setName("Enable logging to file").setDesc("Write plugin logs to a file.").addToggle(f=>f.setValue(this.plugin.settings.enableFileLogging).onChange(async x=>{this.plugin.settings.enableFileLogging=x,await this.plugin.saveSettings()})),new N.Setting(_).setName("Log file path (vault-relative)").setDesc("Where to write the log file. Keep inside the vault.").addText(f=>f.setPlaceholder(`${K}/logs/docling_extract.log`).setValue(this.plugin.settings.logFilePath).onChange(async x=>{this.plugin.settings.logFilePath=x.trim()||`${K}/logs/docling_extract.log`,await this.plugin.saveSettings()})),new N.Setting(_).setName("View or clear log").setDesc("Open the log file or clear it.").addButton(f=>f.setButtonText("Open log").onClick(async()=>{var x,P;await((P=(x=this.plugin).openLogFile)==null?void 0:P.call(x))})).addButton(f=>f.setButtonText("Clear log").onClick(async()=>{var x,P;await((P=(x=this.plugin).clearLogFile)==null?void 0:P.call(x))})),_.createEl("h2",{text:"Maintenance"}),new N.Setting(_).setName("Reindex Redis from cached chunks").setDesc("Rebuild the Redis index from cached chunk JSON files.").addButton(f=>f.setButtonText("Reindex").onClick(async()=>{await this.plugin.reindexRedisFromCache()})),new N.Setting(_).setName("Recreate missing notes from cache").setDesc("Rebuild missing notes using cached Zotero items and chunks.").addButton(f=>f.setButtonText("Recreate").onClick(async()=>{await this.plugin.recreateMissingNotesFromCache()})).addButton(f=>f.setButtonText("Cancel").onClick(()=>{this.plugin.cancelRecreateMissingNotesFromCache()}))}}],c=new Map,u=new Map,g=_=>{this.activeTab=_;for(let[f,x]of c){let P=f===_;x.classList.toggle("is-active",P),x.setAttribute("aria-selected",P?"true":"false"),x.setAttribute("tabindex",P?"0":"-1")}for(let[f,x]of u){let P=f===_;x.classList.toggle("is-active",P),x.hidden=!P}},b=e.createDiv({cls:"zrr-settings-tabs"});b.setAttribute("role","tablist");let y=e.createDiv({cls:"zrr-settings-tabs-panels"});for(let _ of d){let f=b.createEl("button",{text:_.label,cls:"zrr-settings-tab-button"}),x=`zrr-settings-tab-${_.id}`,P=`zrr-settings-panel-${_.id}`;f.type="button",f.id=x,f.setAttribute("role","tab"),f.setAttribute("aria-controls",P),f.setAttribute("aria-selected","false"),f.addEventListener("click",()=>g(_.id));let R=y.createDiv({cls:"zrr-settings-tab-panel"});R.id=P,R.setAttribute("role","tabpanel"),R.setAttribute("aria-labelledby",x),R.hidden=!0,_.render(R),c.set(_.id,f),u.set(_.id,R)}let E=d.some(_=>_.id===this.activeTab)?this.activeTab:d[0].id;g(E)}};var ke={"zrr-picker":`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">

...
${n}`}renderFrontmatter(e,t,r,s,i){var d;let n=(d=this.settings.frontmatterTemplate)!=null?d:"";if(!n.trim())return"";let a=this.buildTemplateVars(e,t,r,s,i);return n.replace(/{{\s*([a-z0-9_]+)\s*}}/gi,(l,p)=>{var m;return(m=a[p])!=null?m:""}).trim()}buildTemplateVars(e,t,r,s,i){let n=typeof e.title=="string"?e.title:"",a=typeof e.shortTitle=="string"?e.shortTitle:"",d=typeof e.date=="string"?e.date:"",l=typeof(t==null?void 0:t.parsedDate)=="string"?t.parsedDate:"",p=this.extractYear(l||d),w=(Array.isArray(e.creators)?e.creators:[]).filter(u=>u.creatorType==="author").map(u=>this.formatCreatorName(u)),y=w.join("; "),f=Array.isArray(e.tags)?e.tags.map(u=>typeof u=="string"?u:u==null?void 0:u.tag).filter(Boolean):[],k=f.join("; "),b=typeof e.itemType=="string"?e.itemType:"",P=typeof(t==null?void 0:t.creatorSummary)=="string"?t.creatorSummary:"",h={doc_id:r,zotero_key:typeof e.key=="string"?e.key:r,title:n,short_title:a,date:d,year:p,authors:y,tags:k,item_type:b,creator_summary:P,pdf_link:this.escapeYamlString(s),item_json:this.escapeYamlString(i)};for(let[u,A]of Object.entries(h))h[`${u}_yaml`]=this.escapeYamlString(A);return h.authors_yaml=this.toYamlList(w),h.tags_yaml=this.toYamlList(f),h}extractYear(e){if(!e)return"";let t=e.match(/\b(\d{4})\b/);return t?t[1]:""}formatCreatorName(e){if(!e||typeof e!="object")return"";if(e.name)return String(e.name);let t=e.firstName?String(e.firstName):"",r=e.lastName?String(e.lastName):"";return[r,t].filter(Boolean).join(", ")||`${t} ${r}`.trim()}escapeYamlString(e){return`"${String(e).replace(/\\/g,"\\\\").replace(/"/g,'\\"')}"`}toYamlList(e){return e.length?e.map(t=>`  - ${this.escapeYamlString(t)}`).join(`
`):'  - ""'}getVaultBasePath(){var r;let e=this.app.vault.adapter;if(e instanceof o.FileSystemAdapter)return e.getBasePath();let t=(r=e.getBasePath)==null?void 0:r.call(e);if(t)return t;throw new Error("Vault base path is unavailable.")}getPluginDir(){var s;let e=this.getVaultBasePath(),t=(s=this.manifest.dir)!=null?s:this.manifest.id;if(!t)throw new Error("Plugin directory is unavailable.");let r=g.default.isAbsolute(t)?t:g.default.join(e,t);return g.default.normalize(r)}async ensureBundledTools(){let e=this.getPluginDir(),t=g.default.join(e,"tools");await S.promises.mkdir(t,{recursive:!0});for(let[r,s]of Object.entries(q)){let i=g.default.join(t,r),n=!0;try{await S.promises.readFile(i,"utf8")===s&&(n=!1)}catch(a){}n&&await S.promises.writeFile(i,s,"utf8")}}async migrateCachePaths(){let e="zotero/items",t="zotero/chunks",r=D,s=C,i=this.app.vault.adapter,n=(0,o.normalizePath)(e),a=(0,o.normalizePath)(t),d=(0,o.normalizePath)(r),l=(0,o.normalizePath)(s),p=d.split("/").slice(0,-1).join("/"),m=l.split("/").slice(0,-1).join("/");p&&await this.ensureFolder(p),m&&await this.ensureFolder(m);let w=await i.exists(n),y=await i.exists(a),f=await i.exists(d),k=await i.exists(l);w&&!f&&await i.rename(n,d),y&&!k&&await i.rename(a,l)}getAbsoluteVaultPath(e){let t=this.getVaultBasePath(),r=g.default.isAbsolute(e)?e:g.default.join(t,e);return g.default.normalize(r)}runPython(e,t){return new Promise((r,s)=>{let i=(0,$.spawn)(this.settings.pythonPath,[e,...t],{cwd:g.default.dirname(e)}),n="";i.stderr.on("data",a=>{n+=a.toString()}),i.on("close",a=>{a===0?r():s(new Error(n||`Process exited with code ${a}`))})})}runPythonStreaming(e,t,r,s){return new Promise((i,n)=>{let a=(0,$.spawn)(this.settings.pythonPath,[e,...t],{cwd:g.default.dirname(e)}),d="",l="",p=null,m=!1,w=y=>{if(y.trim())try{let f=JSON.parse(y);p=f,((f==null?void 0:f.type)==="final"||f!=null&&f.answer)&&(m=!0),r(f)}catch(f){}};a.stdout.on("data",y=>{var k;d+=y.toString();let f=d.split(/\r?\n/);d=(k=f.pop())!=null?k:"";for(let b of f)w(b)}),a.stderr.on("data",y=>{l+=y.toString()}),a.on("close",y=>{d.trim()&&w(d),!m&&p&&s(p),y===0?i():n(new Error(l||`Process exited with code ${y}`))})})}runPythonWithOutput(e,t){return new Promise((r,s)=>{let i=(0,$.spawn)(this.settings.pythonPath,[e,...t],{cwd:g.default.dirname(e)}),n="",a="";i.stdout.on("data",d=>{n+=d.toString()}),i.stderr.on("data",d=>{a+=d.toString()}),i.on("close",d=>{d===0?r(n.trim()):s(new Error(a||`Process exited with code ${d}`))})})}},j=class extends o.SuggestModal{constructor(e,t,r){super(e);this.lastError=null;this.plugin=t,this.resolveSelection=r,this.setPlaceholder("Search Zotero items...")}async getSuggestions(e){try{return await this.plugin.searchZoteroItems(e)}catch(t){let r=t instanceof Error?t.message:String(t);return this.lastError!==r&&(this.lastError=r,new o.Notice(r)),console.error("Zotero search failed",t),[]}}renderSuggestion(e,t){var i,n;let r=(n=(i=e.data)==null?void 0:i.title)!=null?n:"[No title]",s=this.extractYear(e);t.createEl("div",{text:r}),t.createEl("small",{text:s?`${s}`:""}),t.addEventListener("click",()=>{this.resolveSelection&&(this.resolveSelection(e),this.resolveSelection=null),this.close()})}onChooseSuggestion(e,t){this.resolveSelection&&(this.resolveSelection(e),this.resolveSelection=null),this.close()}onClose(){this.resolveSelection&&(this.resolveSelection(null),this.resolveSelection=null)}extractYear(e){var s,i,n,a;let t=(a=(n=(s=e.meta)==null?void 0:s.parsedDate)!=null?n:(i=e.data)==null?void 0:i.date)!=null?a:"";if(typeof t!="string")return"";let r=t.match(/\b(\d{4})\b/);return r?r[1]:""}};

"use strict";var Ze=Object.create;var le=Object.defineProperty;var Ue=Object.getOwnPropertyDescriptor;var Ve=Object.getOwnPropertyNames;var He=Object.getPrototypeOf,We=Object.prototype.hasOwnProperty;var Ke=(b,p)=>{for(var e in p)le(b,e,{get:p[e],enumerable:!0})},De=(b,p,e,n)=>{if(p&&typeof p=="object"||typeof p=="function")for(let t of Ve(p))!We.call(b,t)&&t!==e&&le(b,t,{get:()=>p[t],enumerable:!(n=Ue(p,t))||n.enumerable});return b};var ce=(b,p,e)=>(e=b!=null?Ze(He(b)):{},De(p||!b||!b.__esModule?le(e,"default",{value:b,enumerable:!0}):e,b)),Je=b=>De(le({},"__esModule",{value:!0}),b);var dn={};Ke(dn,{default:()=>he});module.exports=Je(dn);var u=require("obsidian"),X=require("@codemirror/state"),M=require("@codemirror/view"),W=require("child_process"),B=require("fs"),qe=ce(require("http")),Me=ce(require("https")),xe=ce(require("net")),v=ce(require("path")),ee=require("url"),pe=require("crypto");var L=require("obsidian"),H=".zotero-redisearch-rag",J=`${H}/items`,U=`${H}/chunks`,Re={zoteroBaseUrl:"http://127.0.0.1:23119/api",zoteroUserId:"0",webApiBaseUrl:"https://api.zotero.org",webApiLibraryType:"user",webApiLibraryId:"",webApiKey:"",pythonPath:"python3",dockerPath:"docker",autoStartRedis:!1,copyPdfToVault:!0,frontmatterTemplate:`doc_id: {{doc_id}}
...
`;t.replaceRange(l,{line:r.startLine+1,ch:0},{line:r.endLine,ch:0}),this.showStatusProgress("Chunk cleaned.",100),window.setTimeout(()=>this.clearStatusProgress(),1200),new u.Notice("Chunk cleaned.")}async requestOcrCleanup(e){var l,c,g,_,f,y,m,x,k;let n=(this.settings.llmCleanupBaseUrl||"").trim().replace(/\/$/,""),t=(this.settings.llmCleanupModel||"").trim();if(!n||!t)return new u.Notice("OCR cleanup model is not configured."),this.openPluginSettings(),null;let i=Number(this.settings.llmCleanupMaxChars||0);if(i>0&&e.length>i)return new u.Notice("Chunk exceeds OCR cleanup max length. Adjust settings to clean it."),this.openPluginSettings(),null;let r=`${n}/chat/completions`,s={"Content-Type":"application/json"},a=(this.settings.llmCleanupApiKey||"").trim();a&&(s.Authorization=`Bearer ${a}`);let o={model:t,temperature:Number((l=this.settings.llmCleanupTemperature)!=null?l:0),messages:[{role:"system",content:"You are an OCR cleanup assistant. Fix OCR errors without changing meaning. Do not add content. Return corrected text only."},{role:"user",content:e}]};try{let C=await this.requestLocalApiRaw(r,{method:"POST",headers:s,body:JSON.stringify(o)});if(C.statusCode>=400){let R=C.body.toString("utf8");throw new Error(`Cleanup request failed (${C.statusCode}): ${R||"no response body"}`)}let S=JSON.parse(C.body.toString("utf8")),P=(k=(x=(m=(_=(g=(c=S==null?void 0:S.choices)==null?void 0:c[0])==null?void 0:g.message)==null?void 0:_.content)!=null?m:(y=(f=S==null?void 0:S.choices)==null?void 0:f[0])==null?void 0:y.text)!=null?x:S==null?void 0:S.output_text)!=null?k:"",A=String(P||"").trim();return A||(new u.Notice("Cleanup returned empty text."),null)}catch(C){return console.error("OCR cleanup failed",C),new u.Notice("OCR cleanup failed. Check the cleanup model settings."),null}}parseChunkTags(e,n){if(!e)return[];let t=e.trim(),i=[];if(t.startsWith("[")&&t.endsWith("]"))try{let a=JSON.parse(t);Array.isArray(a)&&(i=a.map(o=>String(o)))}catch(a){i=[]}i.length===0&&(i=t.split(/[,;\n]+/));let r=new Set,s=[];for(let a of i){let o=a.trim();if(o=o.replace(/^[-•\d.\)\s]+/,""),o=o.replace(/\s+/g," ").trim(),!o||o.length<2)continue;let l=o.toLowerCase();if(!r.has(l)&&(r.add(l),s.push(o),s.length>=n))break}return s}async requestChunkTags(e){var g,_,f,y,m,x,k,C,S;let n=(this.settings.llmCleanupBaseUrl||"").trim().replace(/\/$/,""),t=(this.settings.llmCleanupModel||"").trim();if(!n||!t)return new u.Notice("OCR cleanup model is not configured."),this.openPluginSettings(),null;let i=e.trim().slice(0,2e3);if(!i)return[];let r=`${n}/chat/completions`,s={"Content-Type":"application/json"},a=(this.settings.llmCleanupApiKey||"").trim();a&&(s.Authorization=`Bearer ${a}`);let o=5,c={model:t,temperature:Number((g=this.settings.llmCleanupTemperature)!=null?g:0),messages:[{role:"system",content:"Return 3 to 5 high-signal, concrete noun-phrase tags. Avoid generic terms (study, paper, method), verbs, and filler. Prefer specific entities, methods, datasets, and named concepts. Output comma-separated tags only. No extra text."},{role:"user",content:i}]};this.showStatusProgress("Generating tags...",null);try{let P=await this.requestLocalApiRaw(r,{method:"POST",headers:s,body:JSON.stringify(c)});if(P.statusCode>=400){let F=P.body.toString("utf8");throw new Error(`Tag request failed (${P.statusCode}): ${F||"no response body"}`)}let A=JSON.parse(P.body.toString("utf8")),R=(S=(C=(k=(y=(f=(_=A==null?void 0:A.choices)==null?void 0:_[0])==null?void 0:f.message)==null?void 0:y.content)!=null?k:(x=(m=A==null?void 0:A.choices)==null?void 0:m[0])==null?void 0:x.text)!=null?C:A==null?void 0:A.output_text)!=null?S:"",T=this.parseChunkTags(String(R||""),o);return T.length||new u.Notice("Tag generation returned no tags."),T}catch(P){return console.error("Tag generation failed",P),new u.Notice("Tag generation failed. Check the cleanup model settings."),null}finally{this.clearStatusProgress()}}async renderMarkdownToIndexText(e){if(!e)return"";let n=document.createElement("div");try{await u.MarkdownRenderer.renderMarkdown(e,n,"",this)}catch(o){return console.warn("Failed to render markdown for index preview",o),this.normalizeIndexPreviewText(e)}let r=(n.innerHTML||"").replace(/<br\s*\/?>/gi,`
...
`).replace(/\s+/g," ").trim()}scheduleNoteSync(e){let n=this.noteSyncTimers.get(e.path);n!==void 0&&window.clearTimeout(n);let t=window.setTimeout(()=>{this.noteSyncTimers.delete(e.path),this.syncNoteToRedis(e)},1200);this.noteSyncTimers.set(e.path,t)}escapeRegExp(e){return e.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")}formatCitationPageLabel(e){let n=e.page_start?String(e.page_start):"",t=e.page_end?String(e.page_end):"";if(n&&(!t||n===t))return n;if(n&&t)return`${n} - ${t}`;let i=(e.pages||"").trim();if(!i)return"?";let r=i.match(/^(\d+)\s*-\s*(\d+)$/);return r?r[1]===r[2]?r[1]:`${r[1]} - ${r[2]}`:i.replace("-"," - ")}normalizeChunkIdForNote(e,n){if(!e)return null;let t=String(e);if(n&&t.startsWith(`${n}:`))return t.slice(n.length+1);if(t.includes(":")){let i=t.split(":");if(i.length>1&&n&&i[0]===n)return i.slice(1).join(":")}return t}async syncNoteToRedis(e){var n,t,i,r;if(!this.noteSyncInFlight.has(e.path)&&!this.noteSyncSuppressed.has(e.path)){this.noteSyncInFlight.add(e.path);try{let s=await this.app.vault.read(e),a=this.extractSyncSection(s);if(!a)return;let o=(n=this.extractDocIdFromFrontmatter(s))!=null?n:this.extractDocIdFromSyncMarker(s);if(!o)return;let l=this.parseSyncedChunkBlocks(a);if(!l.length)return;let c=(0,u.normalizePath)(`${U}/${o}.json`),g=this.app.vault.adapter;if(!await g.exists(c))return;let _=await this.readChunkPayload(c);if(!_)return;let f=Array.isArray(_.chunks)?_.chunks:[],y=new Map;for(let P of f){let A=typeof(P==null?void 0:P.chunk_id)=="string"?P.chunk_id:String((t=P==null?void 0:P.chunk_id)!=null?t:"");A&&y.set(A,P)}let m=new Set,x=new Set,k=new Set,C=new Set,S=!1;for(let P of l){let A=P.chunkId;if(!A)continue;m.add(A);let R=y.get(A);if(!R){console.warn(`Sync note: chunk id not found in cache (${A})`);continue}if(P.excludeFlag){R.excluded!==!0&&(R.excluded=!0,S=!0);let E=this.normalizeChunkText(P.text);E&&E!==String((i=R.text)!=null?i:"")&&(R.text=E,R.char_count=E.length,S=!0),k.add(A);continue}if(R.excluded&&(R.excluded=!1,S=!0,x.add(A)),!P.text.trim()){k.add(A),C.add(A);continue}let T=this.normalizeChunkText(P.text);if(!T){k.add(A),C.add(A);continue}let F=typeof R.text=="string"?R.text:String((r=R.text)!=null?r:"");T!==F&&(R.text=T,R.char_count=T.length,x.add(A),S=!0)}for(let P of y.keys())m.has(P)||(k.add(P),C.add(P));if(!x.size&&!k.size&&!C.size&&!S)return;C.size&&(_.chunks=f.filter(P=>{var R;let A=typeof(P==null?void 0:P.chunk_id)=="string"?P.chunk_id:String((R=P==null?void 0:P.chunk_id)!=null?R:"");return A&&!C.has(A)}),S=!0),(S||C.size)&&await g.write(c,JSON.stringify(_,null,2)),await this.reindexChunkUpdates(o,c,Array.from(x),Array.from(k))}catch(s){console.warn("Failed to sync note edits to Redis",s)}finally{this.noteSyncInFlight.delete(e.path)}}}extractSyncSection(e){let n=re.exec(e);if(!n)return null;let t=e.slice(n.index+n[0].length),i=se.exec(t);return i?t.slice(0,i.index):null}extractDocIdFromSyncMarker(e){var r;let n=re.exec(e);if(!n)return null;let i=((r=n[0])!=null?r:"").match(/doc_id=([\"']?)([^\"'\s]+)\1/i);return i?i[2].trim():null}parseSyncedChunkBlocks(e){var o;let n=e.split(/\r?\n/),t=[],i="",r=!1,s=[],a=()=>{i&&(t.push({chunkId:i,text:s.join(`
`).trim(),excludeFlag:r}),i="",r=!1,s=[])};for(let l of n){let c=l.match(ae);if(c){a();let g=(o=c[1])!=null?o:"",_=g.match(/id=([\"']?)([^\"'\s]+)\1/i),f=_?_[2].trim():"";if(!f)continue;i=f,r=/\bexclude\b/i.test(g)||/\bdelete\b/i.test(g),s=[];continue}if(we.test(l)){a();continue}if(i){if(ue.test(l)){r=!0;continue}s.push(l)}}return a(),t}normalizeChunkText(e){return e.split(/\r?\n/).map(n=>n.replace(/\s+/g," ").trim()).filter((n,t,i)=>!(n===""&&i[t-1]==="")).join(`
`).trim()}registerSyncCommentBadges(){this.registerMarkdownPostProcessor(e=>{var l,c;let n=document.createNodeIterator(e,NodeFilter.SHOW_COMMENT),t=[],i=n.nextNode();for(;i;)i instanceof Comment&&t.push(i),i=n.nextNode();if(!t.length)return;let r=t.map(g=>({comment:g,info:$e(g.data||"")})).filter(g=>g.info!==null);if(!r.length)return;let s=r.filter(g=>{var _;return(_=g.info)==null?void 0:_.pageNumber}).map(g=>{var _;return((_=g.info)==null?void 0:_.pageNumber)||0}),a=s.length?Math.max(...s):0,o=null;for(let g of r){let _=g.info;if(!_)continue;_.type==="chunk-start"?o=(l=_.chunkKind)!=null?l:_.pageNumber?"page":"section":_.type==="chunk-end"&&(_.chunkKind=o!=null?o:"section",o=null);let f=je(_,a);f&&((c=g.comment.parentNode)==null||c.insertBefore(f,g.comment),g.comment.remove())}})}buildSyncedDoclingContent(e,n,t){let i=Array.isArray(n==null?void 0:n.chunks)?n==null?void 0:n.chunks:[];if(!i.length)return`<!-- zrr:sync-start doc_id=${e} -->
${t}

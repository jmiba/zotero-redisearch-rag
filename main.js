...
item_json: {{item_json_yaml}}`,tagSanitizeMode:"kebab",noteBodyTemplate:"{{annotation_block}}{{docling_markdown}}",annotationPageLabel:"Seite",annotationColorMap:{yellow:{heading:"Questions",callout:"question"},red:{heading:"Problems/Critique",callout:"bug"},green:{heading:"Main Ideas",callout:"idea"},blue:{heading:"Facts",callout:"fact"},purple:{heading:"Arguments/Solutions",callout:"argument"},magenta:{heading:"Opinions",callout:"opinion"},orange:{heading:"Follow Up",callout:"pursue"},gray:{heading:"Citable Passages",callout:"cite"}},includeAnnotationImages:!0,zoteroCompanionEnabled:!1,zoteroCompanionBaseUrl:"http://127.0.0.1:23120",zoteroCompanionToken:"",llmProviderProfiles:[{id:"lm-studio",name:"LM Studio",baseUrl:"http://localhost:1234/v1",apiKey:"lm-studio"},{id:"ollama",name:"Ollama",baseUrl:"http://localhost:11434/v1",apiKey:""},{id:"openrouter",name:"OpenRouter",baseUrl:"https://openrouter.ai/api/v1",apiKey:""},{id:"openai",name:"OpenAI",baseUrl:"https://api.openai.com/v1",apiKey:""}],chatOutputDir:"Zotero/Chats",copyPdfToVault:!0,createOcrLayeredPdf:!1,preferObsidianNoteForCitations:!0,ocrMode:"auto",ocrQualityThreshold:.5,chunkingMode:"page",ocrEngine:"auto",forcePerPageOcr:!1,paddleApiKey:"",paddleVlApiUrl:"",paddleStructureApiUrl:"",enableLlmCleanup:!1,llmCleanupProviderProfileId:"lm-studio",llmCleanupBaseUrl:"http://127.0.0.1:1234/v1",llmCleanupApiKey:"",llmCleanupModel:"openai/gpt-oss-20b",llmCleanupTemperature:0,llmCleanupMinQuality:.35,llmCleanupMaxChars:2e3,embedProviderProfileId:"lm-studio",embedBaseUrl:"http://localhost:1234/v1",embedApiKey:"lm-studio",embedModel:"google/embedding-gemma-300m",embedIncludeMetadata:!0,embedSubchunkChars:3500,embedSubchunkOverlap:200,embedContextWindow:1,embedContextChars:220,enableChunkTagging:!1,chatProviderProfileId:"lm-studio",chatBaseUrl:"http://127.0.0.1:1234/v1",chatApiKey:"",chatModel:"openai/gpt-oss-20b",chatTemperature:.2,chatHistoryMessages:6,lastSeenReleaseNotesVersion:"",chatPaneLocation:"right",enableAgenticRag:!1,agenticMaxIters:2,enableQueryExpansion:!1,queryExpansionCount:3,enableCrossEncoderRerank:!1,rerankModel:Ae,rerankCandidateMultiplier:4,rrfK:60,rrfLogTop:0,maxChunksPerDoc:0,enableFileLogging:!1,logFilePath:`${ae}/logs/docling_extract.log`,redisIndex:"idx:zotero",redisPrefix:"zotero:chunk:"},He=class extends L.PluginSettingTab{constructor(e,t){super(e,t);this.activeTab="prerequisites";this.companionTokenInput=null;this.plugin=t}display(){let{containerEl:e}=this;e.empty();let t=()=>Array.isArray(this.plugin.settings.llmProviderProfiles)?this.plugin.settings.llmProviderProfiles:[],n=async h=>{this.plugin.settings.llmProviderProfiles=h,await this.plugin.saveSettings()},i=h=>{h.inputEl.type="password",h.inputEl.autocomplete="off",h.inputEl.spellcheck=!1},d=[{id:"prerequisites",label:"Prerequisites",render:h=>{new L.Setting(h).setName("Prerequisites").setHeading();let b=null,w=null,v=null,S=null,P=null,E=null,D=null,N=null,q=null,F=(I,j)=>{I&&(I.settingEl.classList.toggle("is-disabled",j),I.settingEl.classList.toggle("zrr-setting-disabled",j))},W=(I,j)=>{I&&I.settingEl.classList.toggle("zrr-setting-hidden",!j)},M=()=>{let I=!!this.plugin.settings.showAdvancedPythonRuntimeOptions,j=this.plugin.settings.pythonRuntime==="local",$=I&&j;w==null||w.setDisabled(!I),F(b,!I),W(v,I),W(S,I),W(P,I),E==null||E.setDisabled(!$),D==null||D.setDisabled(!$),q==null||q.setDisabled(!$),N&&(N.disabled=!$),F(v,!$),F(S,!$),F(P,!$)};new L.Setting(h).setName("Docker/podman path").setDesc("CLI path for Docker or Podman (used to start Redis Stack and the Python worker). Leave as 'docker'/'podman' to auto-detect via PATH and common locations without saving an absolute path (keeps cross-OS sync). Supports ~, $HOME, and %USERPROFILE%. Relative paths with separators resolve from your home dir.").addText(I=>I.setPlaceholder("Docker").setValue(this.plugin.settings.dockerPath).onChange(async j=>{this.plugin.settings.dockerPath=j.trim()||"docker",await this.plugin.saveSettings()})),new L.Setting(h).setName("Advanced python runtime options").setDesc("Show legacy local interpreter/venv controls. Recommended to keep this disabled.").addToggle(I=>I.setValue(this.plugin.settings.showAdvancedPythonRuntimeOptions).onChange(async j=>{this.plugin.settings.showAdvancedPythonRuntimeOptions=j,!j&&this.plugin.settings.pythonRuntime==="local"&&(this.plugin.settings.pythonRuntime="worker",new L.Notice("Switched python runtime to worker mode.")),await this.plugin.saveSettings(),M()})),b=new L.Setting(h).setName("Python runtime").setDesc("Worker runtime is recommended. Enable advanced options to expose local interpreter/venv mode.").addDropdown(I=>{w=I,I.addOption("worker","Python worker container (recommended)"),I.addOption("local","Local interpreter/venv"),I.setValue(this.plugin.settings.pythonRuntime).onChange(async j=>{j!=="worker"&&j!=="local"||(this.plugin.settings.pythonRuntime=j,await this.plugin.saveSettings(),M())})}),v=new L.Setting(h).setName("Python path").setDesc("Local mode only: optional path to the Python interpreter used to create or run the plugin env. Leave blank to auto-detect (python3.13/3.12/3.11/3.10/python3/python, or py on Windows). Supports ~, $HOME, and %USERPROFILE%. Relative paths with separators resolve from your home dir.").addText(I=>(E=I,I.setPlaceholder("Auto-detect").setValue(this.plugin.settings.pythonPath).onChange(async j=>{this.plugin.settings.pythonPath=j.trim(),await this.plugin.saveSettings()}))),S=new L.Setting(h).setName("Python environment").setDesc("Local mode: create or update the plugin's python env. Worker mode: managed by docker startup.").addButton(I=>{q=I,N=I.buttonEl,I.setButtonText("Create/update").setCta(),I.onClick(async()=>{I.setDisabled(!0);try{await this.plugin.setupPythonEnv()}finally{I.setDisabled(this.plugin.settings.pythonRuntime!=="local")}})}),P=new L.Setting(h).setName("Python env location").setDesc("Local mode only: shared user cache can be reused across vaults; plugin folder keeps a per-vault env.").addDropdown(I=>{D=I,I.addOption("shared","Shared user cache"),I.addOption("plugin","Plugin folder (.venv)"),I.setValue(this.plugin.settings.pythonEnvLocation).onChange(async j=>{j!=="shared"&&j!=="plugin"||(this.plugin.settings.pythonEnvLocation=j,await this.plugin.saveSettings())})}),M(),new L.Setting(h).setName("Redis URL").addText(I=>I.setPlaceholder("Redis://127.0.0.1:6379").setValue(this.plugin.settings.redisUrl).onChange(async j=>{this.plugin.settings.redisUrl=j.trim(),await this.plugin.saveSettings()}));let G=null,O=null,U=null,B=null,Z=()=>{let I=this.plugin.settings.autoAssignRedisPort;U==null||U.setDisabled(I),B==null||B.setDisabled(I),G==null||G.settingEl.classList.toggle("is-disabled",I),O==null||O.settingEl.classList.toggle("is-disabled",I)};new L.Setting(h).setName("Auto-assign redis port").setDesc("When starting redis stack, pick a free local port and update the redis URL.").addToggle(I=>I.setValue(this.plugin.settings.autoAssignRedisPort).onChange(async j=>{this.plugin.settings.autoAssignRedisPort=j,await this.plugin.saveSettings(),Z()})),G=new L.Setting(h).setName("Redis data directory override").setDesc("Optional path to store Redis persistence when auto-assign is off. Env var ZRR_DATA_DIR overrides this setting. Supports ~, $HOME, and %USERPROFILE%. Relative paths with separators resolve from your home dir; use ./ to keep it vault-relative.").addText(I=>(U=I,I.setPlaceholder("~/redis/zrr-data").setValue(this.plugin.settings.redisDataDirOverride).onChange(async j=>{this.plugin.settings.redisDataDirOverride=j.trim(),await this.plugin.saveSettings()}))),G.settingEl.addClass("zrr-redis-override-setting"),O=new L.Setting(h).setName("Redis project name override").setDesc("Optional Docker/Podman Compose project name when auto-assign is off. Env var ZRR_PROJECT_NAME overrides this setting.").addText(I=>(B=I,I.setPlaceholder("Zrr-shared").setValue(this.plugin.settings.redisProjectName).onChange(async j=>{this.plugin.settings.redisProjectName=j.trim(),await this.plugin.saveSettings()}))),O.settingEl.addClass("zrr-redis-override-setting"),Z(),new L.Setting(h).setName("Auto-start redis stack (docker/podman compose)").setDesc("Requires Docker Desktop running and your vault path shared with Docker. Starts Redis and, in worker mode, the Python worker container.").addToggle(I=>I.setValue(this.plugin.settings.autoStartRedis).onChange(async j=>{this.plugin.settings.autoStartRedis=j,await this.plugin.saveSettings()})),new L.Setting(h).setName("Start redis stack now").setDesc("Starts or restarts redis stack (and python worker in worker mode).").addButton(I=>I.setButtonText("Start").onClick(async()=>{await this.plugin.startRedisStack()}))}},{id:"zotero-import",label:"Zotero import",render:h=>{new L.Setting(h).setName("Zotero local API").setHeading(),new L.Setting(h).setName("Zotero base URL").setDesc("Local Zotero API base URL, e.g. HTTP://127.0.0.1:23119/API").addText(P=>P.setPlaceholder("HTTP://127.0.0.1:23119/API").setValue(this.plugin.settings.zoteroBaseUrl).onChange(async E=>{this.plugin.settings.zoteroBaseUrl=E.trim(),await this.plugin.saveSettings()}));let b=new L.Setting(h).setName("Zotero library").setDesc("Select your local library or a Zotero group library."),w=null,v=P=>{if(!w)return;let E=(this.plugin.settings.zoteroUserId||"0").trim()||"0";new Set(P.map(N=>N.value)).has(E)||(P=P.concat([{value:E,label:`Custom (${E})`}])),w.selectEl.options.length=0;for(let N of P)w.addOption(N.value,N.label);w.setValue(E)},S=async()=>{if(w){w.setDisabled(!0);try{let P=await this.plugin.fetchZoteroLibraryOptions();v(P)}finally{w.setDisabled(!1)}}};b.addDropdown(P=>{w=P;let E=(this.plugin.settings.zoteroUserId||"0").trim()||"0";P.addOption(E,"Loading..."),P.setValue(E),P.onChange(async D=>{this.plugin.settings.zoteroUserId=D.trim(),await this.plugin.saveSettings()})}),b.addButton(P=>{P.setButtonText("Refresh").onClick(async()=>{await S()})}),S(),new L.Setting(h).setName("Zotero web API").setHeading(),new L.Setting(h).setName("Web API base URL").setDesc("Zotero Web API base URL for write fallback, e.g. https://api.zotero.org").addText(P=>P.setPlaceholder("https://api.zotero.org").setValue(this.plugin.settings.webApiBaseUrl).onChange(async E=>{this.plugin.settings.webApiBaseUrl=E.trim(),await this.plugin.saveSettings()})),new L.Setting(h).setName("Web API library type").setDesc("Library type for web API writes.").addDropdown(P=>P.addOption("user","User").addOption("group","Group").setValue(this.plugin.settings.webApiLibraryType).onChange(async E=>{this.plugin.settings.webApiLibraryType=E,await this.plugin.saveSettings()})),new L.Setting(h).setName("Web API library ID").setDesc("Numeric Zotero user/group ID for web API writes.").addText(P=>P.setPlaceholder("15218").setValue(this.plugin.settings.webApiLibraryId).onChange(async E=>{this.plugin.settings.webApiLibraryId=E.trim(),await this.plugin.saveSettings()})),new L.Setting(h).setName("Web API key").setDesc("Zotero API key for write fallback (from Zotero.org).").addText(P=>{i(P),P.setPlaceholder("Your-api-key").setValue(this.plugin.settings.webApiKey).onChange(async E=>{this.plugin.settings.webApiKey=E.trim(),await this.plugin.saveSettings()})}),new L.Setting(h).setName("Output").setHeading(),new L.Setting(h).setName("PDF folder").addText(P=>P.setPlaceholder("Zotero/pdfs").setValue(this.plugin.settings.outputPdfDir).onChange(async E=>{this.plugin.settings.outputPdfDir=E.trim(),await this.plugin.saveSettings()})),new L.Setting(h).setName("Notes folder").addText(P=>P.setPlaceholder("Zotero/notes").setValue(this.plugin.settings.outputNoteDir).onChange(async E=>{this.plugin.settings.outputNoteDir=E.trim(),await this.plugin.saveSettings()})),new L.Setting(h).setName("Frontmatter template").setDesc("Template for note YAML frontmatter. Use {{var}} placeholders; leave blank to omit.").addTextArea(P=>{P.setValue(this.plugin.settings.frontmatterTemplate).onChange(async E=>{this.plugin.settings.frontmatterTemplate=E,await this.plugin.saveSettings()}),P.inputEl.rows=10,P.inputEl.addClass("zrr-u-width-100")}),new L.Setting(h).setName("Tag sanitization").setDesc("Normalize Zotero tags for Obsidian (no spaces, punctuation trimmed).").addDropdown(P=>P.addOption("none","No change").addOption("camel","Camelcase").addOption("pascal","Pascalcase").addOption("snake","Snake_case").addOption("kebab","Kebab-case").setValue(this.plugin.settings.tagSanitizeMode==="replace"?"kebab":this.plugin.settings.tagSanitizeMode).onChange(async E=>{this.plugin.settings.tagSanitizeMode=E,await this.plugin.saveSettings()})),new L.Setting(h).setName("Note body template").setDesc("Template for the note body after frontmatter. Use {{pdf_block}}, {{annotation_block}}, and {{docling_markdown}} placeholders.").addTextArea(P=>{P.setValue(this.plugin.settings.noteBodyTemplate).onChange(async E=>{this.plugin.settings.noteBodyTemplate=E,await this.plugin.saveSettings()}),P.inputEl.rows=8,P.inputEl.addClass("zrr-u-width-100")}),new L.Setting(h).setName("Saved chats folder").setDesc("Where exported chat notes are stored (vault-relative).").addText(P=>P.setPlaceholder("Zotero/chats").setValue(this.plugin.settings.chatOutputDir).onChange(async E=>{this.plugin.settings.chatOutputDir=E.trim()||"zotero/chats",await this.plugin.saveSettings()})),new L.Setting(h).setName("Copy pdfs into vault").setDesc("Disable to use Zotero storage paths directly. If a local file path is unavailable, the plugin temporarily copies the PDF into the vault for processing.").addToggle(P=>P.setValue(this.plugin.settings.copyPdfToVault).onChange(async E=>{this.plugin.settings.copyPdfToVault=E,!E&&this.plugin.settings.createOcrLayeredPdf&&(this.plugin.settings.createOcrLayeredPdf=!1),await this.plugin.saveSettings(),this.display()})),new L.Setting(h).setName("Create ocr-layered PDF copy").setDesc("When ocr is used, replace the vault PDF with a tesseract text layer (requires copy pdfs into vault).").addToggle(P=>{let E=this.plugin.settings.copyPdfToVault;P.setValue(E?this.plugin.settings.createOcrLayeredPdf:!1).setDisabled(!E).onChange(async D=>{if(!this.plugin.settings.copyPdfToVault){this.plugin.settings.createOcrLayeredPdf=!1,await this.plugin.saveSettings();return}this.plugin.settings.createOcrLayeredPdf=D,await this.plugin.saveSettings()})}),new L.Setting(h).setName("Prefer Obsidian note for citations").setDesc("Link citations to the Obsidian note when available; otherwise use Zotero deep links.").addToggle(P=>P.setValue(this.plugin.settings.preferObsidianNoteForCitations).onChange(async E=>{this.plugin.settings.preferObsidianNoteForCitations=E,await this.plugin.saveSettings()}))}},{id:"annotations",label:"Annotations",render:h=>{new L.Setting(h).setName("Annotations").setHeading(),new L.Setting(h).setName("Annotation page label").setDesc("Label shown before the page link in annotation callouts.").addText(S=>S.setPlaceholder("Page").setValue(this.plugin.settings.annotationPageLabel).onChange(async P=>{this.plugin.settings.annotationPageLabel=P.trim()||"Page",await this.plugin.saveSettings()})),new L.Setting(h).setName("Include annotation images").setDesc("Embed image/rect annotations as images in callouts when available.").addToggle(S=>S.setValue(this.plugin.settings.includeAnnotationImages).onChange(async P=>{this.plugin.settings.includeAnnotationImages=P,await this.plugin.saveSettings()})),new L.Setting(h).setName("Zotero companion").setHeading(),h.createEl("p",{text:"Install the Zotero companion add-on to enable cached image/rect annotations. See tab Maintenance for instructions."}),new L.Setting(h).setName("Use Zotero companion for annotation images").setDesc("Fetch cached annotation images from a local Zotero companion plugin.").addToggle(S=>S.setValue(this.plugin.settings.zoteroCompanionEnabled).onChange(async P=>{this.plugin.settings.zoteroCompanionEnabled=P,await this.plugin.saveSettings()})),new L.Setting(h).setName("Zotero companion base URL").setDesc("Local URL for the Zotero companion plugin (loopback only).").addText(S=>S.setPlaceholder("HTTP://127.0.0.1:23120").setValue(this.plugin.settings.zoteroCompanionBaseUrl).onChange(async P=>{this.plugin.settings.zoteroCompanionBaseUrl=P.trim(),await this.plugin.saveSettings()})),new L.Setting(h).setName("Zotero companion token").setDesc("Optional shared token for the companion endpoint.").addText(S=>{i(S),this.companionTokenInput=S,S.setPlaceholder("Optional-token").setValue(this.plugin.settings.zoteroCompanionToken).onChange(async P=>{this.plugin.settings.zoteroCompanionToken=P.trim(),await this.plugin.saveSettings()})}),new L.Setting(h).setName("Generate companion token").setDesc("Create and paste a secure token. The generated token is copied to your clipboard. In Zotero, set your Zotero companion plugin token to this value.").addButton(S=>S.setButtonText("Generate token").onClick(async()=>{let P=(0,Vt.randomBytes)(32).toString("base64url");this.plugin.settings.zoteroCompanionToken=P,await this.plugin.saveSettings(),this.companionTokenInput&&this.companionTokenInput.setValue(P);try{await navigator.clipboard.writeText(P),new L.Notice("Generated and copied companion token.")}catch(E){new L.Notice("Generated token, but failed to copy to clipboard."),console.warn("Failed to copy generated companion token",E)}})),new L.Setting(h).setName("Annotation color map").setHeading(),h.createEl("p",{text:"Map Zotero highlight colors to section headings and callout types."});let b=h.createDiv({cls:"zrr-annotation-map"}),w=async S=>{this.plugin.settings.annotationColorMap=S,await this.plugin.saveSettings()},v=()=>{var q,F;b.empty();let S=this.plugin.settings.annotationColorMap||{},P=Object.entries(S);if(P.length||b.createEl("p",{text:"No color mappings configured."}),P.length){let W=b.createDiv({cls:"zrr-annotation-map-row-header"});W.createEl("span",{text:"Color"}),W.createEl("span",{text:"Heading"}),W.createEl("span",{text:"Callout"}),W.createEl("span")}for(let[W,M]of P){let O=b.createDiv({cls:"zrr-annotation-map-row"}).createDiv({cls:"zrr-annotation-map-row-body"}),U=O.createEl("input",{type:"text",value:W,cls:"zrr-annotation-map-input"}),B=O.createEl("input",{type:"text",value:(q=M.heading)!=null?q:"",cls:"zrr-annotation-map-input"}),Z=O.createEl("input",{type:"text",value:(F=M.callout)!=null?F:"",cls:"zrr-annotation-map-input"}),I=O.createEl("button",{text:"Delete",cls:"zrr-annotation-map-delete"});I.type="button",U.addEventListener("change",()=>{(async()=>{let j=U.value.trim().toLowerCase();if(!j){U.value=W;return}let $={...this.plugin.settings.annotationColorMap||{}};if(j!==W&&$[j]){new L.Notice(`Annotation color '${j}' already exists.`),U.value=W;return}let z=$[W];delete $[W],$[j]=z!=null?z:{heading:"",callout:""},await w($),v()})()}),B.addEventListener("change",()=>{(async()=>{var z;let j={...this.plugin.settings.annotationColorMap||{}},$=(z=j[W])!=null?z:{heading:"",callout:""};$.heading=B.value.trim(),j[W]=$,await w(j)})()}),Z.addEventListener("change",()=>{(async()=>{var z;let j={...this.plugin.settings.annotationColorMap||{}},$=(z=j[W])!=null?z:{heading:"",callout:""};$.callout=Z.value.trim(),j[W]=$,await w(j)})()}),I.addEventListener("click",()=>{(async()=>{let j={...this.plugin.settings.annotationColorMap||{}};delete j[W],await w(j),v()})()})}let E=b.createDiv({cls:"zrr-annotation-map-actions"}),D=E.createEl("button",{text:"Add mapping"});D.type="button",D.addEventListener("click",()=>{(async()=>{let W={...this.plugin.settings.annotationColorMap||{}},M="new-color",G=1;for(;W[M];)M=`new-color-${G}`,G+=1;W[M]={heading:"",callout:""},await w(W),v()})()});let N=E.createEl("button",{text:"Reset to defaults"});N.type="button",N.addEventListener("click",()=>{(async()=>(await w({...Ke.annotationColorMap}),v()))()})};v()}},{id:"ocr",label:"OCR",render:h=>{new L.Setting(h).setName("Docling").setHeading();let b=null,w=E=>{if(!b)return;let D=this.plugin.settings.ocrEngine;new Set(E.map(q=>q.value)).has(D)||(E=E.concat([{value:D,label:`Current (unavailable): ${D}`}])),b.selectEl.options.length=0;for(let q of E)b.addOption(q.value,q.label);b.setValue(D)},v=async()=>{if(!b)return;b.setDisabled(!0);let E={tesseract:!1,paddleStructureLocal:!1,paddleVlLocal:!1};if(this.plugin.detectOcrEngines)try{E=await this.plugin.detectOcrEngines()}catch(q){E={tesseract:!1,paddleStructureLocal:!1,paddleVlLocal:!1}}let D=[{value:"auto",label:"Auto (default)"}];E.tesseract&&D.push({value:"tesseract",label:"Tesseract (local)"}),E.paddleStructureLocal&&D.push({value:"paddle_structure_local",label:"Paddle PP-StructureV3 (local)"}),E.paddleVlLocal&&D.push({value:"paddle_vl_local",label:"PaddleOCR-VL (local)"}),(this.plugin.settings.paddleApiKey||"").trim()&&(D.push({value:"paddle_structure_api",label:"PP-StructureV3 API"}),D.push({value:"paddle_vl_api",label:"PaddleOCR-VL API"})),w(D),b.setDisabled(!1)},S=new L.Setting(h).setName("Paddle ocr API key").setDesc("API token for paddleocr-vl / pp-structurev3 endpoints. Get a free API key at "),P=document.createElement("a");P.href="https://aistudio.baidu.com/paddleocr",P.textContent="HTTPS://aistudio.baidu.com/paddleocr",P.target="_blank",P.rel="noopener noreferrer",S.descEl.appendChild(P),S.descEl.append("."),S.addText(E=>{i(E),E.setPlaceholder("Your-api-token").setValue(this.plugin.settings.paddleApiKey).onChange(async D=>{this.plugin.settings.paddleApiKey=D.trim(),await this.plugin.saveSettings(),await v()})}),new L.Setting(h).setName("Paddleocr-vl API URL").setDesc("Optional override for the paddleocr-vl API endpoint.").addText(E=>E.setPlaceholder("HTTPS://.../layout-parsing").setValue(this.plugin.settings.paddleVlApiUrl).onChange(async D=>{this.plugin.settings.paddleVlApiUrl=D.trim(),await this.plugin.saveSettings()})),new L.Setting(h).setName("Pp-structurev3 API URL").setDesc("API endpoint for pp-structurev3 (see baidu AI studio docs).").addText(E=>E.setPlaceholder("HTTPS://.../pp-structure").setValue(this.plugin.settings.paddleStructureApiUrl).onChange(async D=>{this.plugin.settings.paddleStructureApiUrl=D.trim(),await this.plugin.saveSettings(),await v()})),new L.Setting(h).setName("Ocr engine").setDesc("Select the ocr engine to use when ocr is required.").addDropdown(E=>{b=E,E.addOption("auto","Auto (default)"),E.setValue(this.plugin.settings.ocrEngine),E.onChange(async D=>{this.plugin.settings.ocrEngine=D,await this.plugin.saveSettings()})}),new L.Setting(h).setName("Ocr decision (when to ocr)").setDesc("Controls when ocr runs; per-page behavior is configured separately below.").addDropdown(E=>E.addOption("auto","Auto: use text layer when reliable").addOption("force_low_quality","Ocr only if text is poor").addOption("force","Prefer ocr for full document").setValue(this.plugin.settings.ocrMode).onChange(async D=>{this.plugin.settings.ocrMode=D,await this.plugin.saveSettings()})),new L.Setting(h).setName("Ocr layout override (per-page)").setDesc("Force per-page ocr when ocr runs, bypassing layout heuristics; can be slower for multi-column pdfs.").addToggle(E=>E.setValue(this.plugin.settings.forcePerPageOcr).onChange(async D=>{this.plugin.settings.forcePerPageOcr=D,await this.plugin.saveSettings()})),new L.Setting(h).setName("Text quality threshold").setDesc("Lower values are stricter; below this threshold the text is treated as low quality.").addSlider(E=>{E.setLimits(0,1,.05).setValue(this.plugin.settings.ocrQualityThreshold).setDynamicTooltip().onChange(async D=>{this.plugin.settings.ocrQualityThreshold=D,await this.plugin.saveSettings()})}),new L.Setting(h).setName("Chunking").setDesc("Page or section").addDropdown(E=>E.addOption("page","Page").addOption("section","Section").setValue(this.plugin.settings.chunkingMode).onChange(async D=>{this.plugin.settings.chunkingMode=D,await this.plugin.saveSettings()})),v()}},{id:"llms",label:"LLMs",render:h=>{new L.Setting(h).setName("Model provider profiles").setHeading();let b=h.createDiv({cls:"zrr-llm-profiles"}),w=()=>{b.empty();let C=t();C.length||b.createEl("p",{text:"No profiles yet. Add one below."});for(let A of C){let T=b.createEl("details",{cls:"zrr-profile"});C.length===1&&(T.open=!0);let ie=T.createEl("summary",{text:A.name||A.id||"Profile"});ie.addClass("zrr-profile-title");let Ze=T.createDiv({cls:"zrr-profile-body"});new L.Setting(Ze).setName("Profile name").addText(me=>me.setPlaceholder("My provider").setValue(A.name||"").onChange(async ye=>{A.name=ye.trim(),ie.textContent=A.name||A.id||"Profile",await n(t())})),new L.Setting(Ze).setName("Base URL").addText(me=>me.setPlaceholder("HTTP://localhost:1234/v1").setValue(A.baseUrl||"").onChange(async ye=>{A.baseUrl=ye.trim(),await n(t())})),new L.Setting(Ze).setName("API key").setDesc("Stored in settings (not encrypted).").addText(me=>{i(me),me.setPlaceholder("Sk-...").setValue(A.apiKey||"").onChange(async ye=>{A.apiKey=ye.trim(),await n(t())})}),new L.Setting(Ze).setName("Remove profile").setDesc("Deletes this saved profile.").addButton(me=>me.setButtonText("Delete profile").onClick(async()=>{let ye=t().filter(yn=>yn.id!==A.id);this.plugin.settings.embedProviderProfileId=this.plugin.settings.embedProviderProfileId===A.id?"":this.plugin.settings.embedProviderProfileId,this.plugin.settings.chatProviderProfileId=this.plugin.settings.chatProviderProfileId===A.id?"":this.plugin.settings.chatProviderProfileId,this.plugin.settings.llmCleanupProviderProfileId=this.plugin.settings.llmCleanupProviderProfileId===A.id?"":this.plugin.settings.llmCleanupProviderProfileId,await n(ye),w()}))}new L.Setting(b).addButton(A=>A.setButtonText("Add profile").onClick(async()=>{let T=`profile-${Date.now().toString(36)}`,ie=t().concat([{id:T,name:"Custom",baseUrl:"",apiKey:""}]);await n(ie),w()}))};w(),new L.Setting(h).setName("Ocr cleanup").setHeading(),new L.Setting(h).setName("Model cleanup for low-quality chunks").setDesc("Automatic AI cleanup for poor ocr at import. Can be slow/costly.").addToggle(C=>C.setValue(this.plugin.settings.enableLlmCleanup).onChange(async A=>{this.plugin.settings.enableLlmCleanup=A,await this.plugin.saveSettings()}));let v=null,S=null,P=null,E=()=>Promise.resolve(),D=async(C,A=!0)=>{let T=C.trim();this.plugin.settings.llmCleanupBaseUrl=T,A&&(this.plugin.settings.llmCleanupProviderProfileId="",v&&v.setValue("custom")),S&&S.setValue(T),await this.plugin.saveSettings()},N=async C=>{let A=t().find(T=>T.id===C);this.plugin.settings.llmCleanupProviderProfileId=C,A&&(this.plugin.settings.llmCleanupBaseUrl=A.baseUrl,this.plugin.settings.llmCleanupApiKey=A.apiKey,S==null||S.setValue(A.baseUrl),P==null||P.setValue(A.apiKey)),await this.plugin.saveSettings(),await E()};new L.Setting(h).setName("Cleanup provider profile").setDesc("Select a profile to populate base URL and API key.").addDropdown(C=>{v=C,C.addOption("custom","Custom (manual)");for(let T of t())C.addOption(T.id,T.name||T.id);let A=this.plugin.settings.llmCleanupProviderProfileId;C.setValue(A&&t().some(T=>T.id===A)?A:"custom"),C.onChange(async T=>{if(T==="custom"){this.plugin.settings.llmCleanupProviderProfileId="",await this.plugin.saveSettings();return}await N(T)})}),new L.Setting(h).setName("Cleanup base URL").setDesc("OpenAI-compatible endpoint, e.g. HTTP://127.0.0.1:1234/v1").addText(C=>{S=C,C.setPlaceholder("HTTP://127.0.0.1:1234/v1").setValue(this.plugin.settings.llmCleanupBaseUrl).onChange(async A=>{await D(A)})}),new L.Setting(h).setName("Cleanup API key").setDesc("Optional API key for the cleanup endpoint.").addText(C=>{P=C,i(C),C.setPlaceholder("Sk-...").setValue(this.plugin.settings.llmCleanupApiKey).onChange(async A=>{this.plugin.settings.llmCleanupApiKey=A.trim(),this.plugin.settings.llmCleanupProviderProfileId="",v&&v.setValue("custom"),await this.plugin.saveSettings()})});let q=new L.Setting(h).setName("Cleanup model").setDesc("Select a cleanup-capable model from the provider."),F=null,W=C=>{if(!F)return;let A=(this.plugin.settings.llmCleanupModel||"").trim(),T=new Set(C.map(ie=>ie.value));A&&!T.has(A)&&(C=C.concat([{value:A,label:`Custom (${A})`}])),F.selectEl.options.length=0;for(let ie of C)F.addOption(ie.value,ie.label);A&&F.setValue(A)};E=async()=>{if(F){F.setDisabled(!0);try{let C=await this.plugin.fetchCleanupModelOptions();W(C)}finally{F.setDisabled(!1)}}},q.addDropdown(C=>{F=C;let A=(this.plugin.settings.llmCleanupModel||"").trim();C.addOption(A||"loading","Loading..."),C.setValue(A||"loading"),C.onChange(async T=>{this.plugin.settings.llmCleanupModel=T.trim(),await this.plugin.saveSettings()})}),q.addButton(C=>{C.setButtonText("Refresh").onClick(async()=>{await E()})}),E(),new L.Setting(h).setName("Cleanup temperature").setDesc("Lower is more conservative.").addText(C=>C.setPlaceholder("0.0").setValue(String(this.plugin.settings.llmCleanupTemperature)).onChange(async A=>{let T=Number.parseFloat(A);this.plugin.settings.llmCleanupTemperature=Number.isFinite(T)?T:0,await this.plugin.saveSettings()})),new L.Setting(h).setName("Cleanup minimum quality").setDesc("Only run cleanup when chunk quality is below this threshold (0-1).").addSlider(C=>C.setLimits(0,1,.05).setValue(this.plugin.settings.llmCleanupMinQuality).setDynamicTooltip().onChange(async A=>{this.plugin.settings.llmCleanupMinQuality=A,await this.plugin.saveSettings()})),new L.Setting(h).setName("Cleanup maximum chars").setDesc("Skip cleanup for chunks longer than this limit.").addText(C=>C.setPlaceholder("2000").setValue(String(this.plugin.settings.llmCleanupMaxChars)).onChange(async A=>{let T=Number.parseInt(A,10);this.plugin.settings.llmCleanupMaxChars=Number.isFinite(T)?T:2e3,await this.plugin.saveSettings()})),new L.Setting(h).setName("Text embedding").setHeading();let M=null,G=null,O=null,U=()=>Promise.resolve(),B=async(C,A=!0)=>{let T=C.trim();this.plugin.settings.embedBaseUrl=T,A&&(this.plugin.settings.embedProviderProfileId="",M&&M.setValue("custom")),G&&G.setValue(T),await this.plugin.saveSettings()},Z=async C=>{let A=t().find(T=>T.id===C);this.plugin.settings.embedProviderProfileId=C,A&&(this.plugin.settings.embedBaseUrl=A.baseUrl,this.plugin.settings.embedApiKey=A.apiKey,G==null||G.setValue(A.baseUrl),O==null||O.setValue(A.apiKey)),await this.plugin.saveSettings(),await U()};new L.Setting(h).setName("Embeddings provider profile").setDesc("Select a profile to populate base URL and API key.").addDropdown(C=>{M=C,C.addOption("custom","Custom (manual)");for(let T of t())C.addOption(T.id,T.name||T.id);let A=this.plugin.settings.embedProviderProfileId;C.setValue(A&&t().some(T=>T.id===A)?A:"custom"),C.onChange(async T=>{if(T==="custom"){this.plugin.settings.embedProviderProfileId="",await this.plugin.saveSettings();return}await Z(T)})}),new L.Setting(h).setName("Embeddings base URL").addText(C=>{G=C,C.setPlaceholder("HTTP://localhost:1234/v1").setValue(this.plugin.settings.embedBaseUrl).onChange(async A=>{await B(A)})}),new L.Setting(h).setName("Embeddings API key").addText(C=>{O=C,i(C),C.setPlaceholder("Lm-studio").setValue(this.plugin.settings.embedApiKey).onChange(async A=>{this.plugin.settings.embedApiKey=A.trim(),this.plugin.settings.embedProviderProfileId="",M&&M.setValue("custom"),await this.plugin.saveSettings()})});let I=new L.Setting(h).setName("Embeddings model").setDesc("Select an embeddings model from the provider."),j=null,$=C=>{if(!j)return;let A=(this.plugin.settings.embedModel||"").trim(),T=new Set(C.map(ie=>ie.value));A&&!T.has(A)&&(C=C.concat([{value:A,label:`Custom (${A})`}])),j.selectEl.options.length=0;for(let ie of C)j.addOption(ie.value,ie.label);A&&j.setValue(A)};U=async()=>{if(j){j.setDisabled(!0);try{let C=await this.plugin.fetchEmbeddingModelOptions();$(C)}finally{j.setDisabled(!1)}}},I.addDropdown(C=>{j=C;let A=(this.plugin.settings.embedModel||"").trim();C.addOption(A||"loading","Loading..."),C.setValue(A||"loading"),C.onChange(async T=>{this.plugin.settings.embedModel=T.trim(),await this.plugin.saveSettings()})}),I.addButton(C=>{C.setButtonText("Refresh").onClick(async()=>{await U()})}),U(),new L.Setting(h).setName("Include metadata in embeddings").setDesc("Prepend title/authors/tags/section info before embedding chunks.").addToggle(C=>C.setValue(this.plugin.settings.embedIncludeMetadata).onChange(async A=>{this.plugin.settings.embedIncludeMetadata=A,await this.plugin.saveSettings()})),new L.Setting(h).setName("Embedding context window (chunks)").setDesc("Include neighboring chunk text around each chunk when embedding (0 disables).").addText(C=>C.setPlaceholder("1").setValue(String(this.plugin.settings.embedContextWindow)).onChange(async A=>{let T=Number.parseInt(A,10);this.plugin.settings.embedContextWindow=Number.isFinite(T)?Math.max(0,T):1,await this.plugin.saveSettings()})),new L.Setting(h).setName("Embedding context snippet size (chars)").setDesc("Max chars per neighboring chunk included in embeddings.").addText(C=>C.setPlaceholder("220").setValue(String(this.plugin.settings.embedContextChars)).onChange(async A=>{let T=Number.parseInt(A,10);this.plugin.settings.embedContextChars=Number.isFinite(T)?Math.max(0,T):220,await this.plugin.saveSettings()})),new L.Setting(h).setName("Embedding subchunk size (chars)").setDesc("Split long chunks into smaller subchunks for embedding only (0 disables).").addText(C=>C.setPlaceholder("1800").setValue(String(this.plugin.settings.embedSubchunkChars)).onChange(async A=>{let T=Number.parseInt(A,10);this.plugin.settings.embedSubchunkChars=Number.isFinite(T)?Math.max(0,T):3500,await this.plugin.saveSettings()})),new L.Setting(h).setName("Embedding subchunk overlap (chars)").setDesc("Overlap between embedding subchunks to keep context intact.").addText(C=>C.setPlaceholder("200").setValue(String(this.plugin.settings.embedSubchunkOverlap)).onChange(async A=>{let T=Number.parseInt(A,10);this.plugin.settings.embedSubchunkOverlap=Number.isFinite(T)?Math.max(0,T):200,await this.plugin.saveSettings()})),new L.Setting(h).setName("Generate model tags for chunks").setDesc("Use the ocr cleanup model to tag chunks before indexing.").addToggle(C=>C.setValue(this.plugin.settings.enableChunkTagging).onChange(async A=>{this.plugin.settings.enableChunkTagging=A,await this.plugin.saveSettings()})),new L.Setting(h).setName("Chat model").setHeading();let z=null,V=null,K=null,ne=()=>Promise.resolve(),pe=async(C,A=!0)=>{let T=C.trim();this.plugin.settings.chatBaseUrl=T,A&&(this.plugin.settings.chatProviderProfileId="",z&&z.setValue("custom")),V&&V.setValue(T),await this.plugin.saveSettings()},fe=async C=>{let A=t().find(T=>T.id===C);this.plugin.settings.chatProviderProfileId=C,A&&(this.plugin.settings.chatBaseUrl=A.baseUrl,this.plugin.settings.chatApiKey=A.apiKey,V==null||V.setValue(A.baseUrl),K==null||K.setValue(A.apiKey)),await this.plugin.saveSettings(),await ne()};new L.Setting(h).setName("Chat provider profile").setDesc("Select a profile to populate base URL and API key.").addDropdown(C=>{z=C,C.addOption("custom","Custom (manual)");for(let T of t())C.addOption(T.id,T.name||T.id);let A=this.plugin.settings.chatProviderProfileId;C.setValue(A&&t().some(T=>T.id===A)?A:"custom"),C.onChange(async T=>{if(T==="custom"){this.plugin.settings.chatProviderProfileId="",await this.plugin.saveSettings();return}await fe(T)})}),new L.Setting(h).setName("Chat base URL").setDesc("Compatible endpoint for chat requests.").addText(C=>{V=C,C.setPlaceholder("HTTP://localhost:1234/v1").setValue(this.plugin.settings.chatBaseUrl).onChange(async A=>{await pe(A)})}),new L.Setting(h).setName("Chat API key").addText(C=>{K=C,i(C),C.setPlaceholder("Lm-studio").setValue(this.plugin.settings.chatApiKey).onChange(async A=>{this.plugin.settings.chatApiKey=A.trim(),this.plugin.settings.chatProviderProfileId="",z&&z.setValue("custom"),await this.plugin.saveSettings()})});let Pe=new L.Setting(h).setName("Chat model").setDesc("Select a chat-capable model from the provider."),ce=null,_t=C=>{if(!ce)return;let A=(this.plugin.settings.chatModel||"").trim(),T=new Set(C.map(ie=>ie.value));A&&!T.has(A)&&(C=C.concat([{value:A,label:`Custom (${A})`}])),ce.selectEl.options.length=0;for(let ie of C)ce.addOption(ie.value,ie.label);A&&ce.setValue(A)};ne=async()=>{if(ce){ce.setDisabled(!0);try{let C=await this.plugin.fetchChatModelOptions();_t(C)}finally{ce.setDisabled(!1)}}},Pe.addDropdown(C=>{ce=C;let A=(this.plugin.settings.chatModel||"").trim();C.addOption(A||"loading","Loading..."),C.setValue(A||"loading"),C.onChange(async T=>{this.plugin.settings.chatModel=T.trim(),await this.plugin.saveSettings()})}),Pe.addButton(C=>{C.setButtonText("Refresh").onClick(async()=>{await ne()})}),ne(),new L.Setting(h).setName("Temperature").addText(C=>C.setPlaceholder("0.2").setValue(String(this.plugin.settings.chatTemperature)).onChange(async A=>{let T=Number.parseFloat(A);this.plugin.settings.chatTemperature=Number.isFinite(T)?T:.2,await this.plugin.saveSettings()})),new L.Setting(h).setName("Chat history messages").setDesc("Number of recent messages to include for conversational continuity (0 disables).").addText(C=>C.setPlaceholder("6").setValue(String(this.plugin.settings.chatHistoryMessages)).onChange(async A=>{let T=Number.parseInt(A,10);this.plugin.settings.chatHistoryMessages=Number.isFinite(T)?Math.max(0,T):6,await this.plugin.saveSettings()})),new L.Setting(h).setName("Chat panel location").setDesc("Where to open the chat view by default.").addDropdown(C=>C.addOption("right","Right sidebar").addOption("main","Main window").setValue(this.plugin.settings.chatPaneLocation).onChange(async A=>{this.plugin.settings.chatPaneLocation=A,await this.plugin.saveSettings()})),new L.Setting(h).setName("Retrieval").setHeading(),new L.Setting(h).setName("Enable agentic retrieval").setDesc("Use a small planner step that can trigger expansion retry or full-document retrieval before answering.").addToggle(C=>C.setValue(this.plugin.settings.enableAgenticRag).onChange(async A=>{this.plugin.settings.enableAgenticRag=A,await this.plugin.saveSettings()})),new L.Setting(h).setName("Agentic max iterations").setDesc("Maximum number of planner steps per query.").addText(C=>C.setPlaceholder("2").setValue(String(this.plugin.settings.agenticMaxIters)).onChange(async A=>{let T=Number.parseInt(A,10);this.plugin.settings.agenticMaxIters=Number.isFinite(T)?Math.max(1,T):2,await this.plugin.saveSettings()})),new L.Setting(h).setName("Enable query expansion").setDesc("Use the chat model to expand queries before retrieval.").addToggle(C=>C.setValue(this.plugin.settings.enableQueryExpansion).onChange(async A=>{this.plugin.settings.enableQueryExpansion=A,await this.plugin.saveSettings()})),new L.Setting(h).setName("Query expansion count").setDesc("Number of expansion variants to request.").addText(C=>C.setPlaceholder("3").setValue(String(this.plugin.settings.queryExpansionCount)).onChange(async A=>{let T=Number.parseInt(A,10);this.plugin.settings.queryExpansionCount=Number.isFinite(T)?Math.max(1,T):3,await this.plugin.saveSettings()})),new L.Setting(h).setName("Enable cross-encoder reranking").setDesc("Rerank candidates locally with sentence-transformers (downloads model on first use).").addToggle(C=>C.setValue(this.plugin.settings.enableCrossEncoderRerank).onChange(async A=>{this.plugin.settings.enableCrossEncoderRerank=A,await this.plugin.saveSettings()}));let X=new Set($t.map(C=>C.value)),Q=(this.plugin.settings.rerankModel||"").trim()||Ae,_e=X.has(Q)?Q:De,se=null,Et=()=>{if(!se)return;let A=!(_e===De);se.setDisabled(A),se.inputEl.disabled=A,se.inputEl.readOnly=A,se.inputEl.classList.toggle("is-disabled",A),se.inputEl.setAttribute("aria-disabled",String(A))},St=new L.Setting(h).setName("Cross-encoder model").setDesc("Choose a multilingual preset, or select custom to edit the model ID.");St.addDropdown(C=>{for(let A of $t)C.addOption(A.value,A.label);C.addOption(De,"Custom"),C.setValue(_e),C.onChange(async A=>{_e=A,A!==De?this.plugin.settings.rerankModel=A:(this.plugin.settings.rerankModel||"").trim()||(this.plugin.settings.rerankModel=Ae),await this.plugin.saveSettings(),se==null||se.setValue(this.plugin.settings.rerankModel||Ae),Et()})}),St.addText(C=>{se=C,C.setPlaceholder(Ae).setValue(Q).onChange(async A=>{_e===De&&(this.plugin.settings.rerankModel=A.trim()||Ae,await this.plugin.saveSettings())}),Et()}),new L.Setting(h).setName("Rerank candidate multiplier").setDesc("Retrieve k \xD7 n candidates before reranking.").addText(C=>C.setPlaceholder("4").setValue(String(this.plugin.settings.rerankCandidateMultiplier)).onChange(async A=>{let T=Number.parseInt(A,10);this.plugin.settings.rerankCandidateMultiplier=Number.isFinite(T)?Math.max(1,T):4,await this.plugin.saveSettings()})),new L.Setting(h).setName("Rrf k").setDesc("Rank fusion constant for blending lexical and vector results.").addText(C=>C.setPlaceholder("60").setValue(String(this.plugin.settings.rrfK)).onChange(async A=>{let T=Number.parseInt(A,10);this.plugin.settings.rrfK=Number.isFinite(T)?Math.max(1,T):60,await this.plugin.saveSettings()})),new L.Setting(h).setName("Rrf log top n").setDesc("Log the top n rrf-ranked chunks to stderr (0 disables).").addText(C=>C.setPlaceholder("0").setValue(String(this.plugin.settings.rrfLogTop)).onChange(async A=>{let T=Number.parseInt(A,10);this.plugin.settings.rrfLogTop=Number.isFinite(T)?Math.max(0,T):0,await this.plugin.saveSettings()})),new L.Setting(h).setName("Max chunks per document").setDesc("Limit how many chunks from a single document can appear in retrieval (0 disables).").addText(C=>C.setPlaceholder("0").setValue(String(this.plugin.settings.maxChunksPerDoc)).onChange(async A=>{let T=Number.parseInt(A,10);this.plugin.settings.maxChunksPerDoc=Number.isFinite(T)?Math.max(0,T):0,await this.plugin.saveSettings()}))}},{id:"maintenance",label:"Maintenance",render:h=>{new L.Setting(h).setName("Python runtime").setHeading(),new L.Setting(h).setName("Use local runtime (legacy)").setDesc("Switch to local interpreter/venv mode and enable advanced runtime settings.").addButton(b=>b.setButtonText("Switch").onClick(async()=>{await this.plugin.switchPythonRuntimeToLocalLegacy()})),new L.Setting(h).setName("Logging").setHeading(),new L.Setting(h).setName("Enable logging to file").setDesc("Write plugin logs to a file.").addToggle(b=>b.setValue(this.plugin.settings.enableFileLogging).onChange(async w=>{this.plugin.settings.enableFileLogging=w,await this.plugin.saveSettings()})),new L.Setting(h).setName("Log file path (vault-relative)").setDesc("Where to write the log file. Keep inside the vault.").addText(b=>b.setPlaceholder(`${ae}/logs/docling_extract.log`).setValue(this.plugin.settings.logFilePath).onChange(async w=>{this.plugin.settings.logFilePath=w.trim()||`${ae}/logs/docling_extract.log`,await this.plugin.saveSettings()})),new L.Setting(h).setName("View or clear log").setDesc("Open the log file or clear it.").addButton(b=>b.setButtonText("Open log").onClick(async()=>{var w,v;await((v=(w=this.plugin).openLogFile)==null?void 0:v.call(w))})).addButton(b=>b.setButtonText("Clear log").onClick(async()=>{var w,v;await((v=(w=this.plugin).clearLogFile)==null?void 0:v.call(w))})),new L.Setting(h).setName("Zotero companion plugin").setHeading(),h.createEl("p",{text:"Download the xpi, then in Zotero go to tools \u2192 add-ons \u2192 install from file and restart."}),new L.Setting(h).setName("Download companion add-on").setDesc("Downloads the companion xpi to your system downloads folder.").addButton(b=>b.setButtonText("Download xpi").onClick(async()=>{let w=this.getDefaultDownloadDir();if(!w){new L.Notice("Unable to resolve the system downloads folder.");return}let v=We.default.join(w,"zrr-companion.xpi");if(await this.companionXpiExists(v)){new L.Notice(`Companion XPI already exists: ${v}`);return}new L.Notice("Downloading companion xpi..."),await this.downloadCompanionXpi(v)})),new L.Setting(h).setName("Open Zotero").setDesc("Launch Zotero and open the add-ons window (tools \u2192 add-ons).").addButton(b=>b.setButtonText("Open Zotero").onClick(async()=>{await this.plugin.openZoteroAddons()})),new L.Setting(h).setName("Check companion status").setDesc("Ping the companion /health endpoint.").addButton(b=>b.setButtonText("Check status").onClick(async()=>{await this.plugin.checkZoteroCompanionHealth()})),new L.Setting(h).setName("Redis indexing").setHeading(),new L.Setting(h).setName("Reindex redis from cached chunks").setDesc("Rebuild the redis index from cached chunk JSON files.").addButton(b=>b.setButtonText("Reindex").onClick(async()=>{await this.plugin.reindexRedisFromCache()})),new L.Setting(h).setName("Recreate redis stack").setDesc("Pulls the configured redis image and force-recreates the redis-stack service.").addButton(b=>b.setButtonText("Recreate").onClick(async()=>{await this.plugin.recreateRedisStack()})),new L.Setting(h).setName("Recreate missing notes from cache").setDesc("Rebuild missing notes using cached Zotero items and chunks.").addButton(b=>b.setButtonText("Recreate").onClick(async()=>{await this.plugin.recreateMissingNotesFromCache()})).addButton(b=>b.setButtonText("Cancel").onClick(()=>{this.plugin.cancelRecreateMissingNotesFromCache()})),new L.Setting(h).setName("Release notes").setHeading(),new L.Setting(h).setName("Show release notes").setDesc("Open the current version splash screen.").addButton(b=>b.setButtonText("Show").onClick(()=>{this.plugin.openReleaseNotesModal()}))}}],p=new Map,u=new Map,g=h=>{this.activeTab=h;for(let[b,w]of p){let v=b===h;w.classList.toggle("is-active",v),w.setAttribute("aria-selected",v?"true":"false"),w.setAttribute("tabindex",v?"0":"-1")}for(let[b,w]of u){let v=b===h;w.classList.toggle("is-active",v),w.hidden=!v}},y=e.createDiv({cls:"zrr-settings-tabs"});y.setAttribute("role","tablist");let x=e.createDiv({cls:"zrr-settings-tabs-panels"});for(let h of d){let b=y.createEl("button",{text:h.label,cls:"zrr-settings-tab-button"}),w=`zrr-settings-tab-${h.id}`,v=`zrr-settings-panel-${h.id}`;b.type="button",b.id=w,b.setAttribute("role","tab"),b.setAttribute("aria-controls",v),b.setAttribute("aria-selected","false"),b.addEventListener("click",()=>g(h.id));let S=x.createDiv({cls:"zrr-settings-tab-panel"});S.id=v,S.setAttribute("role","tabpanel"),S.setAttribute("aria-labelledby",w),S.hidden=!0,h.render(S),p.set(h.id,b),u.set(h.id,S)}let k=d.some(h=>h.id===this.activeTab)?this.activeTab:d[0].id;g(k)}getDefaultDownloadDir(){let e=process.env.HOME||process.env.USERPROFILE;return e?We.default.join(e,"Downloads"):null}async companionXpiExists(e){try{return await Ge.promises.access(e),!0}catch(t){return!1}}async downloadCompanionXpi(e){try{await Ge.promises.mkdir(We.default.dirname(e),{recursive:!0});let t=await(0,L.requestUrl)({url:Rn,method:"GET"});if(t.status!==200){new L.Notice(`Failed to download companion XPI (HTTP ${t.status}).`);return}let n=Buffer.from(t.arrayBuffer);await Ge.promises.writeFile(e,n),new L.Notice(`Downloaded companion XPI: ${e}`)}catch(t){new L.Notice("Failed to download companion xpi. See console for details."),console.warn("Failed to download companion XPI",t)}}};var Bt=require("@codemirror/state"),Gt=require("@codemirror/view"),ue=require("obsidian");var Ln=f=>{let _=f.scrollDOM.getBoundingClientRect(),e=f.posAtCoords({x:_.left+8,y:+_.height*.25});return e===null?null:f.state.doc.lineAt(e).number},Oe=f=>!f||typeof f!="object"?null:f,Je=class{constructor(_,e){this.pdfSidebarLeaf=null;this.pdfSidebarDocId=null;this.pdfSidebarPdfPath=null;this.pdfSidebarPage=null;this.pendingPdfSync=null;this.chunkPageCache=new Map;this.previewScrollEl=null;this.previewScrollHandler=null;this.previewScrollFrame=null;this.pdfSyncInFlight=!1;this.queuedPdfSync=null;this.deps=_,this.helpers=e}createSyncExtension(){let _=this.helpers,e=this.syncPdfSidebarForDoc.bind(this);return Gt.ViewPlugin.fromClass(class{constructor(t){this.docId=null;this.lastPage=null;this.lastChunkId=null;this.scrollFrame=null;this.view=t,this.docId=_.extractDocIdFromDoc(t.state.doc),this.onScroll=()=>this.scheduleSync(!1),t.scrollDOM.addEventListener("scroll",this.onScroll,{passive:!0}),this.scheduleSync(!0)}update(t){t.docChanged&&(this.docId=_.extractDocIdFromDoc(t.view.state.doc),this.lastPage=null,this.lastChunkId=null),(t.docChanged||t.viewportChanged)&&this.scheduleSync(!1)}destroy(){this.view.scrollDOM.removeEventListener("scroll",this.onScroll),this.scrollFrame!==null&&(window.cancelAnimationFrame(this.scrollFrame),this.scrollFrame=null)}scheduleSync(t){this.scrollFrame===null&&(this.scrollFrame=window.requestAnimationFrame(()=>{this.scrollFrame=null,this.syncPdfSidebar(this.view,t)}))}syncPdfSidebar(t,n){var p,u;let i=this.docId;if(!i)return;let r=Ln(t);if(r===null)return;let a=_.findChunkStartLineInDoc(t.state.doc,r);if(!a)return;let s=_.parseChunkMarkerLine(a.text),o=(p=s==null?void 0:s.pageNumber)!=null?p:null,l=(u=s==null?void 0:s.chunkId)!=null?u:null;if(!o&&!l)return;let c=o!==null&&this.lastPage===o,d=o===null&&l!==null&&this.lastChunkId===l;!n&&(c||d)||(this.lastPage=o,this.lastChunkId=l,e(i,o!=null?o:void 0,l!=null?l:void 0))}})}async maybeSyncPendingPdf(_={}){if(!this.pendingPdfSync||!this.isPdfSidebarLeaf(this.pdfSidebarLeaf)&&!await this.getPdfSidebarLeaf(!!_.allowCreateLeaf)||!this.pdfSidebarLeaf||!this.isLeafTabActive(this.pdfSidebarLeaf))return;let e=this.pendingPdfSync;this.pendingPdfSync=null,await this.syncPdfSidebarForDoc(e.docId,e.pageNumber,e.chunkId)}async revealPdfSidebar(_={}){let e=await this.getPdfSidebarLeaf(!!_.allowCreateLeaf);if(e)try{await this.deps.app.workspace.revealLeaf(e)}catch(t){console.warn("Failed to reveal PDF sidebar leaf",t)}}updatePreviewScrollHandler(){var n;let _=this.deps.app.workspace.getActiveViewOfType(ue.MarkdownView);if(!_||_.getMode()!=="preview"){this.detachPreviewScrollHandler();return}let e=(n=_.previewMode)==null?void 0:n.containerEl;if(!e){this.detachPreviewScrollHandler();return}if(this.previewScrollEl===e)return;this.detachPreviewScrollHandler();let t=()=>this.schedulePreviewSync(_);e.addEventListener("scroll",t,{passive:!0}),this.previewScrollEl=e,this.previewScrollHandler=t,this.schedulePreviewSync(_)}async syncPdfSidebarForFile(_,e={}){try{let t=await this.deps.app.vault.read(_),n=await this.deps.resolveDocIdForNote(_,t);if(!n)return;let i=this.helpers.extractFirstChunkMarkerFromContent(t);await this.syncPdfSidebarForDoc(n,i==null?void 0:i.pageNumber,i==null?void 0:i.chunkId,e)}catch(t){console.warn("Failed to sync PDF sidebar for opened note",t)}}async syncPdfSidebarForDoc(_,e,t,n={}){if(_){if(this.pdfSyncInFlight){this.queuedPdfSync={docId:_,pageNumber:e,chunkId:t};return}this.pdfSyncInFlight=!0;try{let i=await this.deps.getDocIndexEntry(_);i||(i=await this.deps.hydrateDocIndexFromCache(_));let r=i!=null&&i.pdf_path?String(i.pdf_path):"";if(!r)return;let a=this.deps.toVaultRelativePath(r);if(!a){let p=(0,ue.normalizePath)(r);this.deps.app.vault.getAbstractFileByPath(p)instanceof ue.TFile&&(a=p)}if(!a)return;let s=this.deps.app.vault.getAbstractFileByPath(a);if(!(s instanceof ue.TFile)||s.extension.toLowerCase()!=="pdf")return;let o=await this.getPdfSidebarLeaf(!!n.allowCreateLeaf);if(!o){this.pendingPdfSync={docId:_,pageNumber:e,chunkId:t};return}if(!this.isLeafTabActive(o)&&!n.allowCreateLeaf){this.pendingPdfSync={docId:_,pageNumber:e,chunkId:t};return}let l=Number.isFinite(e!=null?e:NaN)?Number(e):null;if(l===null&&t&&(l=await this.resolvePageNumberForChunk(_,t)),l===null||this.isPdfSidebarLeaf(o)&&this.pdfSidebarLeaf===o&&this.pdfSidebarDocId===_&&this.pdfSidebarPdfPath===s.path&&this.pdfSidebarPage===l)return;let c=a,d=l!==null?`${c}#page=${l}`:c;if(await this.openPdfLinkInLeaf(o,s,d),this.pdfSidebarLeaf=o,this.pdfSidebarDocId=_,this.pdfSidebarPdfPath=s.path,this.pdfSidebarPage=l,this.updatePdfSidebarIcon(o),n.allowCreateLeaf&&!this.isLeafTabActive(o))try{await this.deps.app.workspace.revealLeaf(o)}catch(p){console.warn("Failed to reveal PDF sidebar leaf",p)}}finally{this.pdfSyncInFlight=!1;let i=this.queuedPdfSync;this.queuedPdfSync=null,i&&this.syncPdfSidebarForDoc(i.docId,i.pageNumber,i.chunkId)}}}detachPreviewScrollHandler(){this.previewScrollEl&&this.previewScrollHandler&&this.previewScrollEl.removeEventListener("scroll",this.previewScrollHandler),this.previewScrollEl=null,this.previewScrollHandler=null,this.previewScrollFrame!==null&&(window.cancelAnimationFrame(this.previewScrollFrame),this.previewScrollFrame=null)}schedulePreviewSync(_){this.previewScrollFrame===null&&(this.previewScrollFrame=window.requestAnimationFrame(()=>{this.previewScrollFrame=null,this.syncPdfSidebarForPreview(_)}))}syncPdfSidebarForPreview(_){let e=this.previewScrollEl;if(!e)return;let t=_.getViewData();if(!t)return;let n=Bt.Text.of(t.split(/\r?\n/)),i=this.helpers.extractDocIdFromDoc(n);if(!i)return;let r=this.getPreviewTopLineNumber(e);if(r===null)return;let a=this.helpers.findChunkStartLineInDoc(n,r+1);if(!a)return;let s=this.helpers.parseChunkMarkerLine(a.text);!(s!=null&&s.pageNumber)&&!(s!=null&&s.chunkId)||this.syncPdfSidebarForDoc(i,s==null?void 0:s.pageNumber,s==null?void 0:s.chunkId)}getPreviewTopLineNumber(_){var n;let e=_.getBoundingClientRect().top,t=_.querySelectorAll("[data-line]");for(let i of Array.from(t))if(i.getBoundingClientRect().bottom>=e+2){let a=(n=i.getAttribute("data-line"))!=null?n:"",s=Number.parseInt(a,10);if(Number.isFinite(s))return s}return null}async getPdfSidebarLeaf(_=!1){if(this.pdfSidebarLeaf&&this.isUsableRightSidebarLeaf(this.pdfSidebarLeaf)&&this.isPdfSidebarLeaf(this.pdfSidebarLeaf))return this.pdfSidebarLeaf;this.resetPdfSidebarLeaf();let e=this.findRightSidebarPdfLeaf();if(e)return this.pdfSidebarLeaf=e,this.updatePdfSidebarIcon(e),e;if(_){let t=this.deps.app.workspace;if(typeof t.ensureSideLeaf=="function")try{let i=await t.ensureSideLeaf("pdf","right",{active:!1,split:!1,reveal:!1});if(this.isUsableRightSidebarLeaf(i))return this.pdfSidebarLeaf=i,i.isDeferred&&await i.loadIfDeferred(),this.updatePdfSidebarIcon(i),i}catch(i){console.warn("Failed to ensure PDF side leaf",i)}let n=this.deps.app.workspace.getRightLeaf(!1);if(this.isUsableRightSidebarLeaf(n))return this.pdfSidebarLeaf=n,n}return null}findRightSidebarPdfLeaf(){let _=this.deps.app.workspace;if(typeof _.iterateAllLeaves!="function")return null;let e=null;return _.iterateAllLeaves(t=>{if(!(!this.isUsableRightSidebarLeaf(t)||!this.isPdfSidebarLeaf(t))){if(this.isLeafTabActive(t)){e=t;return}e||(e=t)}}),e}resetPdfSidebarLeaf(){this.pdfSidebarLeaf=null,this.pdfSidebarDocId=null,this.pdfSidebarPdfPath=null,this.pdfSidebarPage=null}isUsableRightSidebarLeaf(_){var t;return this.isRightSidebarLeaf(_)?!!((t=_.containerEl)!=null&&t.isConnected):!1}getLeafViewType(_){var t,n;if(!_)return"";try{let i=_.getViewState();if(typeof(i==null?void 0:i.type)=="string"&&i.type)return i.type}catch(i){console.debug("Failed to read leaf view state",i)}let e=(n=(t=_.view)==null?void 0:t.getViewType)==null?void 0:n.call(t);return typeof e=="string"?e:""}isPdfSidebarLeaf(_){return this.getLeafViewType(_)==="pdf"}isRightSidebarLeaf(_){if(!_)return!1;let t=_.containerEl;return t?!!t.closest(".workspace-split.mod-right-split, .mod-right-split"):!1}updatePdfSidebarIcon(_){if(!_||!this.isRightSidebarLeaf(_)||!this.isPdfSidebarLeaf(_))return;let t=_.containerEl,n=[];t&&n.push(...Array.from(t.querySelectorAll(".view-header-icon")));let i=_;if(i.tabHeaderEl&&n.push(...Array.from(i.tabHeaderEl.querySelectorAll(".workspace-tab-header-inner-icon, .view-header-icon"))),i.tabHeaderInnerIconEl&&n.push(i.tabHeaderInnerIconEl),n.length===0)return;let r=new Set;for(let s of n)if(!r.has(s)){if(r.add(s),s.replaceChildren(),(0,ue.setIcon)(s,"zrr-pdf"),!s.querySelector("svg")&&this.deps.iconSvg){let o=document.createRange().createContextualFragment(this.deps.iconSvg);s.replaceChildren(o)}s.dataset&&(s.dataset.zrrIcon="zrr-pdf")}let a=_.view;a.icon="zrr-pdf",typeof a.getIcon=="function"&&(a.getIcon=()=>"zrr-pdf")}async openPdfLinkInLeaf(_,e,t){var l;if(!await this.ensurePdfLeafReady(_,e)){await _.openFile(e,{active:!1});return}let i=Oe((l=this.getPluginsRegistry())==null?void 0:l["pdf-plus"]),r=Oe(i==null?void 0:i.lib),a=Oe(r==null?void 0:r.workspace),s=typeof(a==null?void 0:a.openPDFLinkTextInLeaf)=="function"?a.openPDFLinkTextInLeaf:null;if(typeof s=="function")try{await s.call(a,_,t,"",{active:!1});return}catch(c){if(!this.isPdfAnnotationRaceError(c))throw c;await this.delay(150)}let o=_;if(typeof o.openLinkText=="function"){try{await o.openLinkText(t,"",{active:!1})}catch(c){if(!this.isPdfAnnotationRaceError(c))throw c;await this.delay(150),await o.openLinkText(t,"",{active:!1})}return}await _.openFile(e,{active:!1})}async ensurePdfLeafReady(_,e){if(this.isPdfSidebarLeaf(_))return!0;try{await _.setViewState({type:"pdf",active:!1})}catch(t){console.debug("Failed to set PDF view state on sidebar leaf",t)}if(_.isDeferred)try{await _.loadIfDeferred()}catch(t){console.debug("Failed to load deferred PDF sidebar leaf",t)}if(this.isPdfSidebarLeaf(_))return!0;try{await _.openFile(e,{active:!1})}catch(t){return console.warn("Failed to open PDF file in sidebar leaf",t),!1}return this.isPdfSidebarLeaf(_)}isPdfAnnotationRaceError(_){let e=_ instanceof Error?_.message:typeof _=="string"?_:"";return e?/injectLinkAnnotations/i.test(e)||/render method must be called before/i.test(e):!1}delay(_){return new Promise(e=>setTimeout(e,_))}getPluginsRegistry(){var e;return(e=this.deps.app.plugins)==null?void 0:e.plugins}isLeafTabActive(_){var i;let e=_;if((i=e.parent)!=null&&i.activeLeaf)return e.parent.activeLeaf===_;let t=e.containerEl;if(t!=null&&t.classList.contains("is-active")||t!=null&&t.classList.contains("mod-active"))return!0;let n=e.tabHeaderEl;return!!(n!=null&&n.classList.contains("is-active")||n!=null&&n.classList.contains("mod-active"))}async resolvePageNumberForChunk(_,e){var l,c,d,p,u;if(!_||!e)return null;let t=this.deps.normalizeChunkIdForNote(e,_)||e,n=this.chunkPageCache.get(_);if(n&&n.has(t))return(l=n.get(t))!=null?l:null;let i=(0,ue.normalizePath)(`${Y}/${_}.json`);if(!await this.deps.app.vault.adapter.exists(i))return null;let a=Oe(await this.deps.readChunkPayload(i)),s=Array.isArray(a==null?void 0:a.chunks)?a.chunks:[],o=new Map;for(let g of s){let y=Oe(g),x=typeof(y==null?void 0:y.chunk_id)=="string"?y.chunk_id.trim():"";if(!x)continue;let k=Number.isFinite((c=y==null?void 0:y.page_start)!=null?c:NaN)?Number(y==null?void 0:y.page_start):null,h=Number.isFinite((d=y==null?void 0:y.page_end)!=null?d:NaN)?Number(y==null?void 0:y.page_end):null,b=k!=null?k:h;b!==null&&(o.set(x,b),o.set(`${_}:${x}`,b))}return this.chunkPageCache.set(_,o),(u=(p=o.get(t))!=null?p:o.get(e))!=null?u:null}};var Xe={"zrr-picker":`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
...
`).trim(),excludeFlag:r,pageStart:Number.isFinite(a!=null?a:NaN)?Number(a):void 0,sectionChunk:s}),i="",r=!1,a=void 0,s=!1,o=[])};for(let p of t){let u=p.match(ge);if(u){l();let g=(c=u[1])!=null?c:"",y=g.match(/id=(["']?)([^"'\s]+)\1/i),x=y?y[2].trim():"";if(!x)continue;let k=Te(p);i=x,r=/\bexclude\b/i.test(g)||/\bdelete\b/i.test(g),a=Number.isFinite((d=k==null?void 0:k.pageNumber)!=null?d:NaN)?Number(k==null?void 0:k.pageNumber):void 0,s=/\bsection\b/i.test(g),o=[];continue}if(Le.test(p)){l();continue}if(i){if(Ne.test(p)){r=!0;continue}this.isChunkBlockIdLine(p)||o.push(p)}}return l(),n}inferSectionTitleFromChunkText(e){if(!e)return"";let t=e.split(/\r?\n/);for(let n of t){let i=n.trim();if(!i)continue;let r=i.match(/^#{1,6}\s+(.+)$/);if(r!=null&&r[1])return r[1].trim();break}return""}extractAttachmentKeyFromAnnotationMarker(e){var i,r;let t=this.findAnnotationBlockRange(e);return t&&(r=(i=this.parseAnnotationBlockMarker(t.startMarker).attachmentKey)==null?void 0:i.trim())!=null?r:""}async restoreMissingChunkCacheFromNote(e,t,n,i){var x,k,h,b,w,v;let r=this.app.vault.adapter,a=(0,m.normalizePath)(`${Y}/${n}.json`);if(await r.exists(a))return a;let s=this.extractSyncSection(t);if(!s)return i&&new m.Notice(`Chunks cache missing for ${n}, and no sync section was found in the note.`),null;let o=this.parseSyncedChunkBlocks(s);if(!o.length)return i&&new m.Notice(`Chunks cache missing for ${n}, and no chunk markers were found in the sync section.`),null;let l=[];for(let S of o){let P=String(S.chunkId||"").trim();if(!P)continue;let E=(x=this.normalizeChunkIdForNote(P,n))!=null?x:P,D=this.normalizeChunkText(String(S.text||"")),N=Number.isFinite((k=S.pageStart)!=null?k:NaN)?Number(S.pageStart):(h=ht(E))!=null?h:void 0,q={chunk_id:E,text:D,char_count:D.length};if(S.excludeFlag&&(q.excluded=!0),Number.isFinite(N!=null?N:NaN)&&(q.page_start=Number(N),q.page_end=Number(N)),S.sectionChunk){let F=this.inferSectionTitleFromChunkText(D);F&&(q.section=F)}l.push(q)}if(!l.length)return i&&new m.Notice(`Chunks cache missing for ${n}, and chunk markers contained no usable chunks.`),null;await this.ensureFolder(Y);let c=await this.getDocIndexEntry(n);c||(c=await this.hydrateDocIndexFromCache(n));let d=(w=(b=this.app.metadataCache.getFileCache(e))==null?void 0:b.frontmatter)!=null?w:{},p=this.normalizeDocIndexPdfPath((v=c==null?void 0:c.pdf_path)!=null?v:""),u=(c==null?void 0:c.attachment_key)||this.extractAttachmentKeyFromAnnotationMarker(t)||await this.resolveAttachmentKeyForDocId(n,d),g={doc_id:n,chunks:l};p&&(g.source_pdf=p),u&&(g.metadata={attachment_key:u}),await r.write(a,JSON.stringify(g,null,2));let y={doc_id:n,note_path:e.path,note_title:R.default.basename(e.path,".md")};return p&&(y.pdf_path=p),u&&(y.attachment_key=u),await this.updateDocIndex(y),i&&new m.Notice(`Restored chunk cache from note markers for ${n}.`),a}normalizeChunkText(e){return e.split(/\r?\n/).filter(t=>!this.isChunkBlockIdLine(t)).map(t=>t.replace(/\s+/g," ").trim()).filter((t,n,i)=>!(t===""&&i[n-1]==="")).join(`
...
`);for(let g of d){let y=String(g.doc_id||"").trim(),x=String(g.chunk_id||"").trim(),k=String(g.page_start||"").trim(),h=String(g.page_end||"").trim(),b=String(g.title||"").trim(),w=String(g.section||"").trim(),v=String(g.score||"").trim(),S=String(g.authors||"").trim(),P=String(g.item_type||"").trim(),E=String(g.year||"").trim(),D=String(g.tags||"").trim(),N=String(g.chunk_tags||"").trim(),q=String(g.attachment_key||"").trim(),F=String(g.source_pdf||"").trim(),W=String(g.text||"").replace(/\s+/g," ").trim(),M=W.length>220?`${W.slice(0,220)}\u2026`:W,G=[y];x&&G.push(x),(k||h)&&G.push(`p.${k||"?"}-${h||"?"}`),p.push(G.filter(Boolean).join(" \u2022 ")),v&&p.push(`  score: ${v}`),b&&p.push(`  title: ${b}`),S&&p.push(`  authors: ${S}`),E&&p.push(`  year: ${E}`),P&&p.push(`  item_type: ${P}`),D&&p.push(`  tags: ${D}`),N&&p.push(`  chunk_tags: ${N}`),q&&p.push(`  attachment_key: ${q}`),w&&p.push(`  section: ${w}`),F&&p.push(`  source_pdf: ${F}`),M&&p.push(`  ${M}`),p.push("")}return p.join(`

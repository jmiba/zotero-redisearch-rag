...
Release assets must be reproducible from the tagged commit. Put the current version at the top of `CHANGELOG.md`, matching `manifest.json`; `npm run package-release` derives the release body, regenerates `src/releaseNotes.ts`, builds, and packages the supported assets. Commit `src/releaseNotes.ts`, `main.js`, `manifest.json`, `styles.css`, and `src/toolAssets.ts` before tagging. The GitHub workflow intentionally does not rewrite source. Release tags use the bare version number, for example `1.0.9`, without a leading `v`.
